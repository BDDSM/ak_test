...
Процедура ПроводкиПоРасхождениямМеждуОтправленнымСкладомИПолученнымНаТТ_Старая(ДатаОбработки)
	...
	|	И РасходныйОрдерСклад.Получатель.ТипСтруктурнойЕдиницы <> ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.СторонняяРозница)
	//---АК Susk (Суслин К.В.) 
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	...
	Если Дата > '20170101' Тогда
		...
		Если Дата >= '20181101' Тогда
			Запрос.Текст = "ВЫБРАТЬ
			               ...
			               |				И Субконто1.ТипТовара = ЗНАЧЕНИЕ(Перечисление.ТипыТоваров.Упаковка)
			...
			Пока Выборка.Следующий() Цикл
				...
				МассивСтрок = ТЗ.Скопировать(Новый Структура("Номенклатура", Выборка.Номенклатура));
				Если МассивСтрок.Итог("Количество") <> ТекКол Тогда
					Сообщить("По номенклатуре " + СокрЛП(Выборка.Номенклатура) + " остаток по количеству на счете 41.2 не совпадает с остатком по регистру ""Товары на складах""!");
				    Продолжить;
				КонецЕсли;
