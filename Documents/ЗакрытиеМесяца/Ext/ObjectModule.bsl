...
Процедура ПроводкиПоРасхождениямМеждуОтправленнымСкладомИПолученнымНаТТ()
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |ПОМЕСТИТЬ ВТ_Данные
	                      |ИЗ
	                      |	РегистрНакопления.ДвиженияТоваровПоЛистамУчета.Обороты(&ДатаНач, &ДатаКон, Регистратор, ВидДвиженияТовара = ЗНАЧЕНИЕ(Перечисление.ВидДвиженияТовараПоЛистуУчета.Поступление)) КАК ДвиженияТоваровПоЛистамУчетаОбороты
	                      |ГДЕ
	                      |	НЕ ДвиженияТоваровПоЛистамУчетаОбороты.Номенклатура В
	                      |				(ВЫБРАТЬ
	                      |					Номенклатура.Ссылка
	                      ...
	                      |				ГДЕ
	                      |					(Номенклатура.СлужебныйТовар
	                      |						ИЛИ Номенклатура.НеУчитыватьВЗакрытииМесяца)) И НЕ ДвиженияТоваровПоЛистамУчетаОбороты.Регистратор ССЫЛКА Документ.КорректировкаЗаписейРегистровНакопления
	                      |
	                      |
	                      |ОБЪЕДИНИТЬ ВСЕ
	                      |
	                      ...
	                      |	И ПриходныйОрдерСклад.Проведен
	                      |	И ПриходныйОрдерСклад.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходСкладскойУчет.ИзТорговойТочки)
						  |	И НЕ ПриходныйОрдерСклад.Поставщик.ТипСтруктурнойЕдиницы В (ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.СторонняяРозница),ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.СкладСтороннейРозницы))
	                      |	И НЕ ПриходныйОрдерСкладТовары.Номенклатура В
	                      |				(ВЫБРАТЬ
	                      |					Номенклатура.Ссылка
	                      |				ИЗ
...
Процедура СписатьПогрешностиРасчетаСебестоимости()
	...
	Если Дата > '20170101' Тогда
		...
		Если Дата >= '20181101' Тогда
			...
			Запрос.Текст = "ВЫБРАТЬ
			               |	Обороты.Организация,
			               |	Обороты.СубконтоДт2 КАК ТоварДт,
			               |	Обороты.СубконтоКт1 КАК МатериалКт,
			               |	СУММА(Обороты.КоличествоОборотКт) КАК Количество,
			               |	СУММА(Обороты.СуммаОборот) КАК Сумма
			               |ИЗ
			               |	(ВЫБРАТЬ
			               |		ФинансовыйОборотыДтКт.Организация КАК Организация,
			               |		ФинансовыйОборотыДтКт.СубконтоДт1 КАК СубконтоДт1,
			               |		ФинансовыйОборотыДтКт.СубконтоДт2 КАК СубконтоДт2,
			               |		ФинансовыйОборотыДтКт.СубконтоКт1 КАК СубконтоКт1,
			               |		ФинансовыйОборотыДтКт.СубконтоКт2 КАК СубконтоКт2,
			               |		СУММА(ФинансовыйОборотыДтКт.КоличествоОборотКт) КАК КоличествоОборотКт,
			               ...
			               |				,
			               |				СчетКт = ЗНАЧЕНИЕ(ПланСчетов.Финансовый.МатералыДляВыпуска),
			               |				,
			               |				СубконтоКт1.ТипТовара = ЗНАЧЕНИЕ(Перечисление.ТипыТоваров.Упаковка)
			               |					И НЕ СубконтоКт1.ДробныйОстаток) КАК ФинансовыйОборотыДтКт
			               |	
			               |	СГРУППИРОВАТЬ ПО
			               ...
			               |		ФинансовыйОборотыДтКт.СубконтоКт2) КАК Обороты
			               |ГДЕ
			               |	Обороты.КоличествоОборотКт > 0
			               |	И Обороты.КоличествоОборотКт <> (ВЫРАЗИТЬ(Обороты.КоличествоОборотКт КАК ЧИСЛО(15, 0)))
			               |
			               |СГРУППИРОВАТЬ ПО
			               |	Обороты.Организация,
			               |	Обороты.СубконтоДт2,
			               ...
			               |	СУММА(ФинансовыйОстатки.КоличествоОстаток) КАК КоличествоИсх
			               |ПОМЕСТИТЬ Ост
			               |ИЗ
			               ...
			               |	ОстРег.Номенклатура,
			               |	ОстРег.Склад,
			               |	ОстРег.Характеристика,
			               |	ОстРег.ЕдиницаИзмерения,
			               |	ОстРег.Количество
			               |ИЗ
			               |	ОстРег КАК ОстРег
			               |;
			               ...
			               |	СУММА(ВложенныйЗапрос.Количество) КАК КоличествоИсх,
			               |	СУММА(ВложенныйЗапрос.Количество) КАК Количество,
			               |	СУММА(ВложенныйЗапрос.КоличествоБух) КАК КоличествоБух
			               |ИЗ
			               |	(ВЫБРАТЬ
			               |		ОстРег.Номенклатура КАК Номенклатура,
			               |		СУММА(ОстРег.Количество) КАК КоличествоИсх,
			               |		СУММА(ОстРег.Количество) КАК Количество,
			               |		0 КАК КоличествоБух
			               |	ИЗ
			               |		ОстРег КАК ОстРег
			               |	
			               ...
			               |		Ост.Количество
			               |	ИЗ
			               |		Ост КАК Ост) КАК ВложенныйЗапрос
			               |
			               |СГРУППИРОВАТЬ ПО
			               |	ВложенныйЗапрос.Номенклатура";
			Запрос.УстановитьПараметр("Дата1", НачалоМесяца(Дата));
			Запрос.УстановитьПараметр("Дата2", Новый Граница(КонецМесяца(Дата), ВидГраницы.Включая));
			Результаты = Запрос.ВыполнитьПакет();
			ТЗ_Ост = Результаты[2].Выгрузить();
			ТЗ_Рег1 = Результаты[4].Выгрузить();
			ТЗ_Рег2 = Результаты[5].Выгрузить();
			...
			Выборка = Результаты[0].Выбрать();
			Пока Выборка.Следующий() Цикл
				Если Выборка.Количество = Окр(Выборка.Количество) или Выборка.Количество <= 0 Тогда
					Продолжить;
				КонецЕсли;
				ТекРазница = Выборка.Количество - Окр(Выборка.Количество, 0, 1);
				...
				Проводка.СубконтоДт.Товары 		= Выборка.ТоварДт;
				
				Проводка.СчетКт 				= ПланыСчетов.Финансовый.МатералыДляВыпуска;
				Проводка.СубконтоКт.Материалы	= Выборка.МатериалКт;
				
				Проводка.КоличествоКт 			= -ТекРазница;
				Проводка.Сумма 					= Окр(-ТекРазница * Выборка.Сумма / Выборка.Количество, 2, 1);
				
				Если Проводка.Сумма < 0 Тогда
					ИнвертироватьПроводку(Проводка);				
				КонецЕсли; 
				
				Отбор = ТЗ_Ост.НайтиСтроки(Новый Структура("Организация, Номенклатура", Выборка.Организация, Выборка.МатериалКт));
				Если Отбор.Количество() > 0 Тогда
					Отбор[0].Количество = Отбор[0].Количество - Проводка.КоличествоКт;
					Отбор[0].Сумма = Отбор[0].Сумма - Проводка.Сумма;
				КонецЕсли;
			КонецЦикла;
			...
			Для каждого стр из ТЗ_Ост Цикл
				Если стр.Количество = Окр(стр.Количество) или стр.Количество = 0 Тогда
					Продолжить;
				КонецЕсли;
				Если стр.Количество < 0 и стр.КоличествоИсх <= 0 Тогда     
					Продолжить;
				КонецЕсли;
				
				Если стр.Количество < 0 и стр.КоличествоИсх > 0 Тогда     
					ТекРазница = стр.Количество;
					ТекСумма = Окр(ТекРазница * стр.Сумма / стр.КоличествоИсх, 2, 1); 
				Иначе
					ТекРазница = стр.Количество - Окр(стр.Количество, 0, 1);
					ТекСумма = Окр(ТекРазница * стр.Сумма / стр.КоличествоИсх, 2, 1); 
				КонецЕсли;
				ТТ = ?(стр.Организация.ИНН = "7734410589", ТТ_ТилсиОбщий, ТТВсе);
				
				Отбор = ТЗ_Рег2.НайтиСтроки(Новый Структура("Номенклатура", стр.Номенклатура));
				Если Отбор.Количество() > 0 Тогда
					Отбор[0].Количество = Отбор[0].Количество - стр.КоличествоИсх + стр.Количество - ТекРазница;
				КонецЕсли;
				...
				Проводка.СубконтоДт.ТорговыеТочки 			= ТТ;
				Проводка.СубконтоДт.СтатьиДоходовРасходов	= СтатьяДР;
				Проводка.СубконтоДт.ЦФО						= ЦФОУК;
				
				Проводка.СчетКт 				= ПланыСчетов.Финансовый.МатералыДляВыпуска;
				Проводка.СубконтоКт.Материалы	= стр.Номенклатура;
				
				Проводка.КоличествоКт 			= ТекРазница;
				Проводка.Сумма 					= ТекСумма;
			...
			Для каждого стр из ТЗ_Рег2 Цикл
				Если стр.КоличествоИсх = стр.КоличествоБух и стр.КоличествоИсх = стр.Количество Тогда
					Продолжить;
				ИначеЕсли стр.КоличествоИсх <> стр.КоличествоБух Тогда
					Продолжить;
				Иначе
					ТЗ = ТЗ_Рег1.Скопировать(Новый Структура("Номенклатура", стр.Номенклатура));
					
					ВсегоКол = 0;
					Для каждого стр1 из ТЗ Цикл
						//Сообщить("" + стр.Номенклатура + " " + стр.КоличествоИсх + " " + стр.Количество);
						ТекКол = Окр(стр1.Количество, 0 , 1);
						
						ВсегоКол = ВсегоКол + ТекКол;
						Если стр1.Количество <> ТекКол или 
							ТЗ.Количество() = ТЗ.Индекс(стр1) + 1 и ВсегоКол <> стр.Количество Тогда
							Движение = Движения.ТоварыНаСкладах.ДобавитьРасход();
							Движение.Период = КонецМесяца(Дата);
							Движение.Склад = стр1.Склад;
							Движение.Номенклатура = стр1.Номенклатура;
							Движение.Характеристика = стр1.Характеристика;
							Движение.ЕдиницаИзмерения = стр1.ЕдиницаИзмерения;
							Движение.Количество = стр1.Количество - ТекКол;
						КонецЕсли;
						Если ТЗ.Количество() = ТЗ.Индекс(стр1) + 1 Тогда
							Если ВсегоКол <> стр.Количество Тогда
								Движение.Количество = Движение.Количество + стр.Количество - ВсегоКол;
							КонецЕсли;
						КонецЕсли;
					КонецЦикла;
				КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ИначеЕсли ВидЗакрытия = 6 Тогда
		...
		//+++АК Shek 2018.12.18 ИП-00020732  
		ПроводкиПоРасхождениямМеждуОтправленнымСкладомИПолученнымНаТТ_ПриходныеОрдераПоНедостаче();
		//---АК Shek  2018.12.18 
...
//+++АК Shek 2018.12.18 ИП-00020732
Процедура ПроводкиПоРасхождениямМеждуОтправленнымСкладомИПолученнымНаТТ_ПриходныеОрдераПоНедостаче()
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ПриходныйОрдерСкладТовары.Ссылка,
	|	КОНЕЦПЕРИОДА(ПриходныйОрдерСкладТовары.Ссылка.Дата, ДЕНЬ) КАК Дата,
	|	ПриходныйОрдерСкладТовары.Номенклатура,
	|	СУММА(ПриходныйОрдерСкладТовары.Количество) КАК Количество,
	|	ВЫРАЗИТЬ(ПриходныйОрдерСкладТовары.Ссылка.Основание КАК Документ.РасходныйОрдерСклад) КАК РасходныйОрдер
	|ПОМЕСТИТЬ ТоварыПриходныйОрдер
	|ИЗ
	|	Документ.ПриходныйОрдерСклад.Товары КАК ПриходныйОрдерСкладТовары
	|ГДЕ
	|	ПриходныйОрдерСкладТовары.Ссылка.Дата МЕЖДУ &ДатаНач И &ДатаКон
	|	И ПриходныйОрдерСкладТовары.Ссылка.Проведен
	|	И ПриходныйОрдерСкладТовары.Ссылка.Организация В(&Организации)
	|	И ПриходныйОрдерСкладТовары.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходСкладскойУчет.ИзТорговойТочки)
	|
	|СГРУППИРОВАТЬ ПО
	|	ПриходныйОрдерСкладТовары.Номенклатура,
	|	ПриходныйОрдерСкладТовары.Ссылка,
	|	ВЫРАЗИТЬ(ПриходныйОрдерСкладТовары.Ссылка.Основание КАК Документ.РасходныйОрдерСклад),
	|	КОНЕЦПЕРИОДА(ПриходныйОрдерСкладТовары.Ссылка.Дата, ДЕНЬ)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	И РеализацияТоваровУслугРасходныеОрдера.Ссылка.Дата МЕЖДУ &ДатаНач И &ДатаКон
	|	И РеализацияТоваровУслугРасходныеОрдера.Ссылка.Организация В(&Организации)
	|	И РеализацияТоваровУслугРасходныеОрдера.РасходныйОрдер В
	|			(ВЫБРАТЬ
	|				ВТ.РасходныйОрдер
	|			ИЗ
	|				ТоварыПриходныйОрдер КАК ВТ)
	|
	|СГРУППИРОВАТЬ ПО
	|	РеализацияТоваровУслугРасходныеОрдера.РасходныйОрдер
	|;
	...
	|	ФинансовыйДвиженияССубконто.СчетДт КАК СчетКт,
	|	ФинансовыйДвиженияССубконто.СубконтоДт1 КАК СубконтоКт1,
	|	ФинансовыйДвиженияССубконто.СубконтоДт2 КАК СубконтоКт2,
	|	ФинансовыйДвиженияССубконто.СубконтоДт3 КАК СубконтоКт3,
	|	ФинансовыйДвиженияССубконто.СчетКт КАК СчетДт,
	|	ФинансовыйДвиженияССубконто.СубконтоКт1 КАК СубконтоДт1,
	|	ФинансовыйДвиженияССубконто.СубконтоКт2 КАК СубконтоДт2,
	|	ФинансовыйДвиженияССубконто.СубконтоКт3 КАК СубконтоДт3,
	|	ФинансовыйДвиженияССубконто.Организация,
	|	ФинансовыйДвиженияССубконто.ВалютаДт КАК ВалютаКт,
	|	ФинансовыйДвиженияССубконто.ВалютаКт КАК ВалютаДт,
	|	ФинансовыйДвиженияССубконто.Сумма,
	|	ФинансовыйДвиженияССубконто.КоличествоКт КАК Количество,
	|	ФинансовыйДвиженияССубконто.ВидСубконтоДт1 КАК ВидСубконтоКт1,
	|	ФинансовыйДвиженияССубконто.ВидСубконтоДт2 КАК ВидСубконтоКт2,
	|	ФинансовыйДвиженияССубконто.ВидСубконтоДт3 КАК ВидСубконтоКт3,
	|	ФинансовыйДвиженияССубконто.ВидСубконтоКт1 КАК ВидСубконтоДт1,
	|	ФинансовыйДвиженияССубконто.ВидСубконтоКт2 КАК ВидСубконтоДт2,
	|	ФинансовыйДвиженияССубконто.ВидСубконтоКт3 КАК ВидСубконтоДт3
	|ИЗ
	...
	|					ИЗ
	|						РТИУОтбор КАК ВТ)
	|				И СчетДт = ЗНАЧЕНИЕ(ПланСчетов.Финансовый.ЗатратыТоварныеДляРаспределения)
	|				И СчетКт = ЗНАЧЕНИЕ(ПланСчетов.Финансовый.Товары)
	|				И Содержание ПОДОБНО ""Недостача товара%"",
	|			,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТоварыПриходныйОрдер.Дата,
	|	ТоварыПриходныйОрдер.Номенклатура,
	|	СУММА(ТоварыПриходныйОрдер.Количество) КАК Количество,
	|	РТИУОтбор.Ссылка КАК РТИУ
	|ИЗ
	|	ТоварыПриходныйОрдер КАК ТоварыПриходныйОрдер
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РТИУОтбор КАК РТИУОтбор
	|		ПО ТоварыПриходныйОрдер.РасходныйОрдер = РТИУОтбор.РасходныйОрдер
	|
	|СГРУППИРОВАТЬ ПО
	|	ТоварыПриходныйОрдер.Номенклатура,
	|	РТИУОтбор.Ссылка,
	...
	МассивОрганизаций.Добавить(Справочники.Организации.ПолучитьСсылку(Новый УникальныйИдентификатор("aec7bdbc-0fdd-11e8-8b52-005056a714c6")));//Тилси
	Запрос.УстановитьПараметр("Организации", МассивОрганизаций);
	
	РезультатЗапросаМассив = Запрос.ВыполнитьПакет();
	ТаблицаПроводокНедостача = РезультатЗапросаМассив[2].Выгрузить();
	
	ВыборкаДанныеПриходногоОрдера = РезультатЗапросаМассив[3].Выбрать();
	Пока ВыборкаДанныеПриходногоОрдера.Следующий() Цикл
		
		КоличествоРаспределить = ВыборкаДанныеПриходногоОрдера.Количество;
		НайдСтрНедостача = ТаблицаПроводокНедостача.НайтиСтроки(Новый Структура("Регистратор, СубконтоДт2", ВыборкаДанныеПриходногоОрдера.РТИУ, ВыборкаДанныеПриходногоОрдера.Номенклатура));
		Для Каждого СтрНедостача Из НайдСтрНедостача Цикл
			
			ОприходоватьНаШаге = Мин(СтрНедостача.Количество, КоличествоРаспределить);
			Если ОприходоватьНаШаге <= 0 Тогда 
				Продолжить;
			КонецЕсли;
			
			Движение = Движения.Финансовый.Добавить();
			ЗаполнитьЗначенияСвойств(Движение, СтрНедостача);
			Движение.Период = ВыборкаДанныеПриходногоОрдера.Дата;
			
			Движение.СубконтоДт[СтрНедостача.ВидСубконтоДт1] = СтрНедостача.СубконтоДт1;
			Движение.СубконтоДт[СтрНедостача.ВидСубконтоДт2] = СтрНедостача.СубконтоДт2;
			Движение.СубконтоДт[СтрНедостача.ВидСубконтоДт3] = СтрНедостача.СубконтоДт3;
			Движение.СубконтоКт[СтрНедостача.ВидСубконтоКт1] = СтрНедостача.СубконтоКт1;
			Движение.СубконтоКт[СтрНедостача.ВидСубконтоКт2] = СтрНедостача.СубконтоКт2;
			Движение.СубконтоКт[СтрНедостача.ВидСубконтоКт3] = СтрНедостача.СубконтоКт3;
			
			Движение.Сумма = СтрНедостача.Сумма * ОприходоватьНаШаге / СтрНедостача.Количество;
			Движение.КоличествоДт = ОприходоватьНаШаге;
			
			Движение.Содержание = "Возврат недостач";
			
			КоличествоРаспределить = КоличествоРаспределить - ОприходоватьНаШаге;
			СтрНедостача.Количество = СтрНедостача.Количество - ОприходоватьНаШаге;
