	...
	Если Дата > '20170101' Тогда
		...
		Если Дата >= '20181101' Тогда
			...
			Запрос.Текст = "ВЫБРАТЬ
			               ...
			               |	Обороты.СубконтоКт2,
			               |	Обороты.КоличествоОборотКт КАК Количество,
			               |	Обороты.СуммаОборот КАК Сумма,
			               |	ЕСТЬNULL(ЕдИзм.ЕдиницаИзмерения, ЗНАЧЕНИЕ(Справочник.ЕдиницыИзмерения.ПустаяСсылка)) КАК ЕдиницаИзмерения
			               |ИЗ
			               |	(ВЫБРАТЬ
			               ...
			               |		ГДЕ
			               |			ЕдиницыИзмерения.ЕдиницаПоКлассификатору = ЕдиницыИзмерения.Владелец.БазоваяЕдиницаИзмерения
			               |			И НЕ ЕдиницыИзмерения.ПометкаУдаления
			               |		
			               |		СГРУППИРОВАТЬ ПО
			               |			ЕдиницыИзмерения.Владелец) КАК ЕдИзм
			               |		ПО Обороты.СубконтоКт1 = ЕдИзм.Номенклатура
			               |ГДЕ
			...
			Пока Выборка.Следующий() Цикл
				...
				Движение.Период = КонецМесяца(Дата);
				Движение.Склад = Выборка.СубконтоКт2;
				Движение.Номенклатура = Выборка.СубконтоКт1;
				...
				Проводка.СубконтоДт.МестаХранения 					= Выборка.СубконтоДт1;
				Проводка.СубконтоДт.Товары 							= Выборка.СубконтоДт2;
				
				Проводка.СчетКт = ПланыСчетов.Финансовый.МатералыДляВыпуска;
				Проводка.СубконтоКт.Материалы 						= Выборка.СубконтоКт1;
				Проводка.СубконтоКт.Склады 							= Выборка.СубконтоКт2;
