...
Функция ПолучитьТаблицуИзмененийВДвижениях()
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ЗаданияНаФасовку.Номенклатура,
	|	ЗаданияНаФасовку.Характеристика,
	|	ЗаданияНаФасовку.ДатаПроизводства,
	|	СУММА(ЗаданияНаФасовку.Количество * ВЫБОР
	|			КОГДА ЗаданияНаФасовку.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|				ТОГДА 1
	|			ИНАЧЕ -1
	|		КОНЕЦ) КАК Количество
	|ПОМЕСТИТЬ ВТ_Движения
	|ИЗ
	|	РегистрНакопления.ЗаданияНаФасовку КАК ЗаданияНаФасовку
	|ГДЕ
	|	ЗаданияНаФасовку.Регистратор = &Регистратор
	|	И ЗаданияНаФасовку.Активность = ИСТИНА
	|	И ЗаданияНаФасовку.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|
	|СГРУППИРОВАТЬ ПО
	|	ЗаданияНаФасовку.Номенклатура,
	|	ЗаданияНаФасовку.Характеристика,
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не указана номенклатура в строке " + Строка(СтрокаТЧ.НомерСтроки));
	//		Отказ = Истина;
	//	КонецЕсли;
	//КонецЦикла;
	...
	мДвижения = ЭтотОбъект.Движения.ЗаданияНаФасовку;
	...
	Если ЭтотОбъект.Закрыто Тогда //Надо остатки по заданию в регистре закрыть
		...
		Запрос.Текст =
		"ВЫБРАТЬ
		|	ЗаданияНаФасовкуОстатки.Задание,
		|	ЗаданияНаФасовкуОстатки.Номенклатура,
		|	ЗаданияНаФасовкуОстатки.Характеристика,
		|	ЗаданияНаФасовкуОстатки.ДатаПроизводства,
		|	ЗаданияНаФасовкуОстатки.КоличествоОстаток КАК Количество
		|ИЗ
		|	РегистрНакопления.ЗаданияНаФасовку.Остатки(, Задание = &ТекущийДокумент) КАК ЗаданияНаФасовкуОстатки";
