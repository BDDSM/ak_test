...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//+++AK GREK 17.12.2018 ИП-00019188
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		ПроверитьПривязкуКРасходнымОрдерам(Отказ);
	КонецЕсли;
	//---AK
КонецПроцедуры

//+++AK GREK 17.12.2018 ИП-00019188
//К документу Расходный ордер(склад) должен "привязываться" только один документ Возврат товаров поставщику.
Процедура ПроверитьПривязкуКРасходнымОрдерам(Отказ)
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВозвратТоваровПоставщикуРасходныеОрдера.Ссылка) КАК Ссылка,
	               |	ВозвратТоваровПоставщикуРасходныеОрдера.РасходныйОрдер КАК РасходныйОрдер
	               |ИЗ
	               |	Документ.ВозвратТоваровПоставщику.РасходныеОрдера КАК ВозвратТоваровПоставщикуРасходныеОрдера
	               |ГДЕ
	               |	ВозвратТоваровПоставщикуРасходныеОрдера.РасходныйОрдер В(&РасходныйОрдерСписок)
	               |	И ВозвратТоваровПоставщикуРасходныеОрдера.Ссылка <> &Ссылка
	               |	И ВозвратТоваровПоставщикуРасходныеОрдера.Ссылка.Проведен = ИСТИНА
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	ВозвратТоваровПоставщикуРасходныеОрдера.РасходныйОрдер";
	Запрос.УстановитьПараметр("Ссылка", ЭтотОбъект.Ссылка);
	Запрос.УстановитьПараметр("РасходныйОрдерСписок", ЭтотОбъект.РасходныеОрдера.ВыгрузитьКолонку("РасходныйОрдер"));
	Рез = Запрос.Выполнить();
	Если НЕ Рез.Пустой() Тогда
		Отказ = Истина;
		Выборка = Рез.Выбрать();
		Пока Выборка.Следующий() Цикл
			Сообщить("Расходный ордер " + Выборка.РасходныйОрдер + " уже был ранее привязан к другому возврату поставщику!");
		КонецЦикла;
