...
Процедура ОбработкаКоманды(ПараметрКоманды, ПараметрыВыполненияКоманды)
	...
	Если НЕ ПроверитьРасходныйОрдер(ПараметрКоманды) Тогда
		ОбщегоНазначения.СообщитьИнформациюПользователю("Документ не проведен! Не задан ""Расходный ордер (склад)"" - основание документа!");
		Возврат;
	КонецЕсли;
	
	Если НЕ ПроверитьЗаданиеТехнолога(ПараметрКоманды) Тогда
		ОбщегоНазначения.СообщитьИнформациюПользователю("Документ не проведен! У ""Расходного ордера"" основание является не ""Задание технолога магазинам""!");
		Возврат;
	КонецЕсли;
	
	ТабДок = ПечатьАктОбнаружения(ПараметрКоманды);
...
&НаСервере
Функция ПечатьАктОбнаружения(СсылкаНаОбъект)
	
	Возврат Документы.ВозвратТоваровПоставщику.ПечатьАктОбнаружения(СсылкаНаОбъект);
