...
//KL ITseboev + 14.12.2018 ИП-00020699.01
&НаКлиенте
Процедура ДобавитьТТМосква(Команда)
	
	Если Объект.ТорговыеТочки.Количество() > 0 Тогда 
		Ответ = Вопрос("Табличную часть документа не пустая. Очистить?", РежимДиалогаВопрос.ДаНет, 60, КодВозвратаДиалога.Да);
		Если Ответ = КодВозвратаДиалога.Да Тогда
			Объект.ТорговыеТочки.Очистить();
		КонецЕсли;
	КонецЕсли;
	...
	Регион = "Москва";
	ДобавитьТТПоРегионамНаСервере(Регион);	
	
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьТТМО(Команда)
...
&НаКлиенте
Процедура ДобавитьТТПитер(Команда)
...
&НаКлиенте
Процедура ДобавитьТТДрРегионы(Команда)
...
&НаСервере
Процедура ДобавитьТТПоРегионамНаСервере(Регион)
	
	Если Регион = "Москва" Тогда
		Регион = Справочники.Регионы.НайтиПоКоду("77");
	ИначеЕсли Регион = "МО" Тогда 
		Регион = Справочники.Регионы.НайтиПоКоду("50");
	ИначеЕсли Регион = "Питер" Тогда 
		Регион = Справочники.Регионы.НайтиПоКоду("82");
	КонецЕсли;
	
	МассивИсклРегионы = Новый Массив;
	МассивИсклРегионы.Добавить(Справочники.Регионы.НайтиПоКоду("77"));
	МассивИсклРегионы.Добавить(Справочники.Регионы.НайтиПоКоду("50"));
	МассивИсклРегионы.Добавить(Справочники.Регионы.НайтиПоКоду("82"));
	...
	ТекстЗапроса = 
		...
		|	И СтруктурныеЕдиницы.СтатусТорговойТочки = &СтатусТорговойТочки
		|	И СтруктурныеЕдиницы.ТипРозничнойТочки = &ТипРозничнойТочки";
	Если Регион <> "ДрРегионы" Тогда
		ТекстЗапроса = ТекстЗапроса + "
		|	И СтруктурныеЕдиницы.Регион = &Регион";
	Иначе
		ТекстЗапроса = ТекстЗапроса + "
		|	И НЕ СтруктурныеЕдиницы.Регион В(&ИсклРегионы)";
	КонецЕсли;
	ТекстЗапроса = ТекстЗапроса + "
		|
		|УПОРЯДОЧИТЬ ПО
	...
	Запрос.УстановитьПараметр("ИсклРегионы",МассивИсклРегионы);
	Результат = Запрос.Выполнить();
