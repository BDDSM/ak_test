...
Процедура ПроводкиПоИнвенаризации2()
	...
	// комментирую в рамках задачи, чтобы не тратить время на получение лишних данных. Себестоимость в проводку должна идти
	// из регистра, а не из ПТУ
	...
	//|			ТОГДА (ПоступлениеТоваровУслугТовары.СуммаНДС + ПоступлениеТоваровУслугТовары.Сумма) / ПоступлениеТоваровУслугТовары.Количество
	//|		ИНАЧЕ 0
	//|	КОНЕЦ КАК ЦенаСНДС
	//|ИЗ
	//|	Документ.ПоступлениеТоваровУслуг.Товары КАК ПоступлениеТоваровУслугТовары
...
//+++ AK BARA ИП-00015673.000.00000001
Процедура ПроводкиПоИнвенаризации()
	//+++АК POZM 2018.08.29 ИП-00019521 
	Если ЭтотОбъект.ПродажиКурьеров Тогда
		Возврат;
	КонецЕсли;	
	...
	Если Дата >= '20131201' Тогда
		...
		//ТабСебестоимость = ОбщегоНазначенияСервер.ПолучитьСебестоимостьДляМассиваНоменклатур(Товары.ВыгрузитьКолонку("Номенклатура"), Дата);
		ТипыСтруктурныхЕдиницРозница = Перечисления.ТипыСтруктурныхЕдиниц.Розница;
		//+++АК SHEP 2018.05.06 ИП-00018453
		//СтруктурныеЕдиницыСторонняяПереработка = Справочники.СтруктурныеЕдиницы.СторонняяПереработка;
		...
		ТипСтруктурнойЕдиницы = Склад.Владелец.ТипСтруктурнойЕдиницы;
		...
		//|ИЗ
		//|	Документ.РеализацияТоваровУслуг.Товары КАК РеализацияТоваровУслугТовары
		//|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_Ном_Дата КАК ВТ_Ном_Дата
		//|		ПО РеализацияТоваровУслугТовары.Номенклатура = ВТ_Ном_Дата.Номенклатура
		//|			И РеализацияТоваровУслугТовары.Ссылка.Дата = ВТ_Ном_Дата.Дата
		//|ГДЕ
		...
		Для каждого ТекСтрокаТовары Из Товары Цикл
			...
			Если ЗначениеЗаполнено(Склад) Тогда
				Если (Склад = СтруктурнаяЕдиница.СкладДегустация 
					ИЛИ Склад = СтруктурнаяЕдиница.СкладСписаниеПоСроку 
					ИЛИ Склад = СтруктурнаяЕдиница.СкладСписаниеПоКачеству)  Тогда
					Продолжить;
				КонецЕсли;
			КонецЕсли;	
						
			Если ТипСтруктурнойЕдиницы = ТипыСтруктурныхЕдиницРозница Тогда
				
				СтруктураНастроек = ТабНастройки.Найти(Перечисления.ВидыОперацийВУчете.ЛистыУчета_ИнвентаризацияВкусвилл, "ВидОперации");
				
			Иначе	
				
				Если СтруктурнаяЕдиница = СтруктурныеЕдиницыСторонняяПереработка Тогда
					...
					КонецЕсли;
				Иначе
					Если ТекСтрокаТовары.Номенклатура.ТипТовара = ТипыТоваровУпаковка Тогда
			...
			Если ЗначениеЗаполнено(СтруктураНастроек.Счет) Тогда
				...
				ПроводкаНедостачи.Период 		= Дата;
				ПроводкаНедостачи.НомерЖурнала	= "ТД";
				ПроводкаНедостачи.Организация = ОрганизацияВкусВилл;
				
				// Кт
				ПроводкаНедостачи.СчетКт 							= СтруктураНастроек.Счет;
				ПроводкаНедостачи.СубконтоКт.ТорговыеТочки  		= СтруктураНастроек.СтруктурнаяЕдиница;
				ПроводкаНедостачи.СубконтоКт.СтатьиДоходовРасходов	= СтруктураНастроек.СтатьяДоходовРасходов;
				ПроводкаНедостачи.СубконтоКт.ЦФО					= ?(ЗначениеЗаполнено(СтруктураНастроек.ЦФО), СтруктураНастроек.ЦФО, ТекСтрокаТовары.ЦФО);
				
				// Дт
				Если ТекСтрокаТовары.Номенклатура.ТипТовара = ТипыТоваровУпаковка Тогда
					
					ПроводкаНедостачи.СчетДт 							= ПланыСчетов.Финансовый.МатералыДляВыпуска;
					ПроводкаНедостачи.КоличествоДт 						= ТекСтрокаТовары.Количество;
					ПроводкаНедостачи.СубконтоДт.Материалы  			= ТекСтрокаТовары.Номенклатура;
					ПроводкаНедостачи.СубконтоДт.Склады					= ТекСтрокаТовары.Склад;
					ПроводкаНедостачи.СубконтоДт.СтатьиТовародвижения 	= ?(ЗначениеЗаполнено(СтруктураНастроек.ДопСтатьяДвиженияТоваров), СтруктураНастроек.ДопСтатьяДвиженияТоваров, СтруктураНастроек.СтатьяДвиженияТоваров);
					ПроводкаНедостачи.Содержание	= "Недостача упаковки";
					
				Иначе
					
					ПроводкаНедостачи.СчетДт 							= ПланыСчетов.Финансовый.Товары;
					ПроводкаНедостачи.КоличествоДт 						= ТекСтрокаТовары.Количество;
					ПроводкаНедостачи.СубконтоДт.МестаХранения  		= СтруктурнаяЕдиница;
					//+++АК POZM 2018.08.28 ИП-00019521 
					Если ЗначениеЗаполнено(ТекСтрокаТовары.СтруктурнаяЕдиница) Тогда
						ПроводкаНедостачи.СубконтоДт.МестаХранения  		= ТекСтрокаТовары.СтруктурнаяЕдиница;
					КонецЕсли;	
					//---АК POZM 
					ПроводкаНедостачи.СубконтоДт.Товары					= ТекСтрокаТовары.Номенклатура;
					ПроводкаНедостачи.СубконтоДт.СтатьиТовародвижения 	= СтруктураНастроек.СтатьяДвиженияТоваров;
					ПроводкаНедостачи.Содержание	= "Недостача товара";
					
				КонецЕсли;
				
				//СтрСебестоимость = ТабСебестоимость.Найти(ТекСтрокаТовары.Номенклатура, "Номенклатура");
				//СтрЦены = ТаблицаЦен.Найти(ТекСтрокаТовары.Номенклатура, "Номенклатура");

				//ПроводкаНедостачи.Сумма = ПроводкаНедостачи.КоличествоКт * СтрСебестоимость.Себестоимость * Наценка / 100;
				ПроводкаНедостачи.Сумма = ТекСтрокаТовары.Количество * ТекСтрокаТовары.Цена * (Наценка+100) / 100;

				ПроводкаНедостачи.СторонаПроводкиДляПересчетаПоСебестоимости = "ДТ";
