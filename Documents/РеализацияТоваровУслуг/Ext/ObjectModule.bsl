...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//+++АК SHEP 2018.10.04 ИП-00020060
	//Если ЭтоТилси ТОгда
	//+++АК sils 17.12.2018 ИП-00020568
	ЦФО_Вендинг = Справочники.СтруктурныеЕдиницы.НайтиПоКоду("000001538");
	//---АК
	...
	//---АК SHEP 2018.10.04
		Если ЗначениеЗаполнено(СтруктураНастроекВыручка_вн) и ЗначениеЗаполнено(СтруктураНастроек_вн) Тогда
			...
			//+++АК sils 17.12.2018 ИП-00020568
			Если ЭтоВендинг() и ЦФО_Вендинг <> Справочники.СтруктурныеЕдиницы.ПустаяСсылка() Тогда
				Проводка.СубконтоКт.ЦФО						= ЦФО_Вендинг;
			Иначе	
			//---АК
...
//+++АК sils 17.12.2018 ИП-00020568
Функция ЭтоВендинг()
	СвойствоВендинг = ПланыВидовХарактеристик.СвойстваОбъектов.НайтиПоНаименованию("Оплата возвратов в случае вендинга");
	Если СвойствоВендинг = ПланыВидовХарактеристик.СвойстваОбъектов.ПустаяСсылка() Тогда
		Возврат Ложь;
	КонецЕсли;
	Запрос = Новый Запрос;			 
	Запрос.Текст = "ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("Свойство", СвойствоВендинг);
	Запрос.УстановитьПараметр("Контрагент", Контрагент);
