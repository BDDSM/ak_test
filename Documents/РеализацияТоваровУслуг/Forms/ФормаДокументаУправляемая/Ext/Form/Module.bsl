...
//+++АК sils 05.12.2018 ИП-00020438
&НаКлиенте
Процедура РасходныеОрдераПриИзменении(Элемент)
	Если Объект.Организация = ПредопределенноеЗначение("Справочник.Организации.ПустаяСсылка") или 
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Организация, "ИНН") <> "7734675810" Тогда
		Возврат;
	КонецЕсли;

	ТекСтр = Элементы.РасходныеОрдера.ТекущиеДанные;
	Если ТекСтр <> Неопределено и ТекСтр.РасходныйОрдер <> ПредопределенноеЗначение("Документ.РасходныйОрдерСклад.ПустаяСсылка") Тогда
		Реализация = НайтиРеализациюПоРасходномуОрдеруНаСервере(ТекСтр.РасходныйОрдер);
		Если Реализация <> Неопределено и (Объект.Ссылка.Пустая() или Реализация <> Объект.Ссылка) Тогда
			Сообщить("У ""Расходного ордера"" № " + ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекСтр.РасходныйОрдер, "Номер") + 
			" уже есть привязка к документу ""Реализация товаров и услуг"" № " + 
				ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Реализация, "Номер") + ". Выбор этого ""Расходного ордера"" невозможен!");
			ТекСтр.РасходныйОрдер = ПредопределенноеЗначение("Документ.РасходныйОрдерСклад.ПустаяСсылка");
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
//+++АК sils 05.12.2018 ИП-00020438
&НаСервере
Функция НайтиРеализациюПоРасходномуОрдеруНаСервере(РО)
	Возврат Документы.РеализацияТоваровУслуг.НайтиРеализациюПоРасходнику(РО);
КонецФункции
