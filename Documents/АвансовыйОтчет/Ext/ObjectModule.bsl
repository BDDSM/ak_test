...
Функция ПолучитьНастройкиОборудование_ВводВЭксплуатациюНаСтаройТТ()
	...
	|ИЗ
	|	РегистрСведений.НастройкаОтраженияОперацийВУчете.СрезПоследних(&Дата, ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВУчете.Оборудование_ВводВЭксплуатациюНаСтаройТТ)) КАК Срез";
...
//	КонецЕсли;
//	
//	Если ЭтотОбъект.Склад.Пустая() Тогда
...
Процедура ОтразитьДвиженияПоИзлишкамТоваров(СтрокаТЧ, СуммаВсего)
	...
	СтруктураНастроек = ОбщегоНазначенияСервер.ПолучитьНастройкиОтраженияОперацийВУчете(Перечисления.ВидыОперацийВУчете.Поступление_Излишки, Дата);
...
Процедура ОтразитьДвиженияПоНедостачеТоваров(СтрокаТЧ, СуммаВсего)
	...
	СтруктураНастроек = ОбщегоНазначенияСервер.ПолучитьНастройкиОтраженияОперацийВУчете(Перечисления.ВидыОперацийВУчете.Поступление_Недостачи, Дата);
