...
&НаКлиенте
Процедура ВыделитьКнопкуВидаСписка(ИмяКнопки)
	
	Для каждого Элемент Из Элементы Цикл
		Если Элемент.Родитель = Элементы.ВидыСписка Тогда
			Если Элемент.Имя = ИмяКнопки Тогда
				Элемент.Шрифт = Новый Шрифт(Элемент.Шрифт,,, Истина);
				ИмяАктивнойКнопкиВидаСписка = ИмяКнопки;
			Иначе
				Элемент.Шрифт = Новый Шрифт(Элемент.Шрифт,,, Ложь);
			КонецЕсли;
		КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	ВыделитьКнопкуВидаСписка("ВидСпискаОбщая");
...
Процедура ОбновитьТаблицуНаСервере(ИмяАктивнойКнопки)
	...
	Если ИмяТаблицы = "ТЗРез" ИЛИ ИмяТаблицы = "ТЗРезУС" ИЛИ ИмяТаблицы = "ДЗРез" Тогда
		Запрос1.Текст = 
		"ВЫБРАТЬ
		...
		|	СообщениеМОССтруктурныеЕдиницы.Ссылка.ТипСообщения2
		|ИЗ
	...
	Если Выб1.Количество() = КоличествоПредыдущегоСчитывания Тогда
		Возврат
	КонецЕсли;
	
	КоличествоПредыдущегоСчитывания = Выб1.Количество();
	
	Если ТекСтраница <> "СтраницаДС" И ТекСтраница <> "СтраницаДСУС" Тогда
	...
	Если ИмяТаблицы = "ДС" ИЛИ ИмяТаблицы = "ДСУС" Тогда
		Данные1 = Новый ТаблицаЗначений;
		Данные1.Колонки.Добавить("Ссылка");
		Данные1.Колонки.Добавить("Дата");
	...
	Если НачалоДня(ДатаПоследнегоСчитывания) < ТекДата Тогда
		Данные1.Очистить();
	КонецЕсли;
	
	ДатаПоследнегоСчитывания = ТекущаяДатаСеанса();
	
	ПоследняяДатаВСписке = '00010101';
	Если Данные1.Количество() > 0 Тогда
		ПоследняяДатаВСписке = Данные1[0].Дата;
	КонецЕсли;
	
	КолВоСтрокСтар = Данные1.Количество();
	...
	Пока Выб1.Следующий() Цикл
		...
		Если Выб1.ТипСообщения2 = "Уведомление с сервера" Тогда
			Если ИмяАктивнойКнопкиВидаСписка <> "ВидСпискаУС" И ИмяТаблицы <> "ТЗРезУС" Тогда
				ДобавлятьСтроку = Ложь;
			КонецЕсли;
		Иначе
			Если ИмяАктивнойКнопкиВидаСписка = "ВидСпискаУС" ИЛИ ИмяТаблицы = "ТЗРезУС" Тогда
				ДобавлятьСтроку = Ложь;
			КонецЕсли;
		КонецЕсли;
		...
		Если Выб1.Дата < ПоследняяДатаВСписке Тогда
			Возврат;
		ИначеЕсли Выб1.Дата = ПоследняяДатаВСписке Тогда
			// тогда надо поискать
			ПисьмоВСпискеЕсть = Ложь;
			Для каждого СтрокаКолл1 Из Данные1 Цикл
				Если СтрокаКолл1.Дата < Выб1.Дата Тогда
					Прервать;
				КонецЕсли;
				Если СтрокаКолл1.Ссылка = Выб1.Ссылка Тогда
					ПисьмоВСпискеЕсть = Истина;
					Прервать;
				КонецЕсли;
			КонецЦикла;
			
			Если ПисьмоВСпискеЕсть Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		...
		Если ИмяТаблицы = "ТЗРез" ИЛИ ИмяТаблицы = "ТЗРезУС" ИЛИ ИмяТаблицы = "ДС" ИЛИ ИмяТаблицы = "ДСУС" Тогда
			СтрокаТаблицы = Данные1.Добавить();
					
		ИначеЕсли ИмяТаблицы = "ДЗРез" Тогда
		
			Тема2 = ПолучитьНормализованнуюТему(Выб1.Тема);
			
			РодительНайден = Ложь;
			Для каждого СтрокаДЗ1 Из Данные1 Цикл
				Тема1 = ПолучитьНормализованнуюТему(СтрокаДЗ1.Тема);
				Если СокрЛП(ВРег(Тема1)) = СокрЛП(ВРег(Тема2)) Тогда
		...
		Если ИмяТаблицы = "ДС" ИЛИ ИмяТаблицы = "ДСУС" Тогда
			ЗаполнитьЗначенияСвойств(СтрокаТаблицы, Выб1);
			Продолжить;
		Иначе
	...
	Если ИмяТаблицы = "ДС" ИЛИ ИмяТаблицы = "ДСУС" Тогда
		МСДокументов = Данные1.ВыгрузитьКолонку("Ссылка");
		Список.Параметры.УстановитьЗначениеПараметра("ВыбранныеДокументы", МСДокументов);
		Элементы.ДС.Обновить();
		Возврат;
	ИначеЕсли ИмяТаблицы = "ДЗРез" Тогда
		ДЗРезОбъект = РеквизитФормыВЗначение("ДЗРез");
		СтрокиДЗРез = ДЗРезОбъект.Строки;
		СтрокиДЗРез.Сортировать("Дата Убыв", Истина);
	Иначе
		Данные1.Сортировать("Дата Убыв");
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Список.Параметры.УстановитьЗначениеПараметра("ВыбранныеДокументы", Новый  Массив);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ИзменениеСообщенияМОС" Тогда
		КоличествоПредыдущегоСчитывания = 0;
		ОбновитьТаблицуНаСервере(ИмяАктивнойКнопки);
	КонецЕсли;
...
Процедура СписокПриАктивизацииСтроки(Элемент)
	...
	Если ТД1 <> Неопределено Тогда
		Если ЗначениеЗаполнено(ТД1.Прочитал) Тогда
			...
			ТекстСообщения = ПолучитьТекстСообщения(ТД1.Ссылка);
		Иначе
...
Функция ПолучитьТекстСообщения(Ссылка)

	Возврат Ссылка.ТекстСообщения;
...
Функция ПолучитьНормализованнуюТему(Тема)

	Тема2 = СокрЛП(СтрЗаменить(Тема, "Re:", ""));
	Тема2 = СокрЛП(СтрЗаменить(Тема2, "Ответ на", ""));
	Тема2 = СокрЛП(СтрЗаменить(Тема2, "Fwd:", ""));
	
	Пока Найти(Тема2, "  ") > 0 Цикл
		Тема2 = СтрЗаменить(Тема2, "  ", " ");
	КонецЦикла;
...
&НаКлиенте
Процедура ВидСпискаОбщая(Команда)
	
	ВыделитьКнопкуВидаСписка(Команда.Имя);
	УстановитьВидСписка(Команда.Имя);
	
КонецПроцедуры

&НаКлиенте
Процедура ВидСпискаУС(Команда)
...
&НаКлиенте
Процедура ВидСпискаГруппировка(Команда)
...
Процедура УстановитьВидСписка(ИмяКоманды)
	
	Если ИмяКоманды = "ВидСпискаОбщая" ИЛИ ИмяКоманды = "ВидСпискаУС" Тогда
		Элементы.Панель.ТекущаяСтраница = Элементы.СтраницаДС;
	ИначеЕсли ИмяКоманды = "ВидСпискаГруппировка" Тогда
		Элементы.Панель.ТекущаяСтраница = Элементы.СтраницаДЗРез;
	Иначе
		Элементы.Панель.ТекущаяСтраница = Элементы.ПустаяСтраница;
	КонецЕсли;
