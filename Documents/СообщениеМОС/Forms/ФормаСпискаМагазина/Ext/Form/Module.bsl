...
Процедура ПриОткрытии(Отказ)
	...
	//Отказ = Истина;
	//ОткрытьФорму("Документ.СообщениеМОС.Форма.ФормаСпискаМагазина20170217");
	//ВОЗВРАТ;
	...
	Если (ТекДата - НачалоДня(ТекДата)) > 3600*20 Тогда
		...
		ОбновитьТЗРезНаСервере("ФильтрОбязательноВыполнить");
...
Процедура ОбновитьДанныеНаСервере(ИмяАктивнойКнопки)
	...
	ОбновитьТЗРезНаСервере(ИмяАктивнойКнопки);
	
КонецПроцедуры


Процедура ОбновитьТЗРезНаСервере(ИмяАктивнойКнопки)
	
	ТекСсылка = Неопределено;
	
	ТекСтрока = Элементы.ТЗРез.ТекущаяСтрока;
	Если ТекСтрока <> Неопределено Тогда
		ТекСтрокаТЗРез = ТЗРез.НайтиПоИдентификатору(ТекСтрока);
	...
	Пока Выб1.Следующий() Цикл
		...
		ПолныйКаталогФВ =  СокрЛП(Константы.КаталогХраненияФайлов.Получить()) + "MessAtt\" + Папка1;
		МСФВ = НайтиФайлы(ПолныйКаталогФВ, "*.*");
		Если МСФВ.Количество() > 0 Тогда
			СтрокаТЗРез.Вложения = 1;
		Иначе
			СтрокаТЗРез.Вложения = 0;
		КонецЕсли;
