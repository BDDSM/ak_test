...
Процедура Заполнить(Команда)
	...
	Для каждого ТекСтрока Из Объект.СписокРассылки Цикл
		...
		Если ЗначениеЗаполнено(ТекСтрока.Отправлено) или ЗначениеЗаполнено(ТекСтрока.ОтправленоСФ) Тогда
...
Процедура ОтправитьПисьма(Команда)                                                     
	...
	ОтправитьПисьмаНаСервере("АктСверкиПДФ");
...
&НаСервере
Процедура ОтправитьПисьмаНаСервере(Реквизит)
	...
	Для каждого ПодчиненнаяСтрока Из ДокОбъект.СписокРассылки Цикл
		...
		Если Реквизит="СчетФактураПДФ" и ЗначениеЗаполнено(ПодчиненнаяСтрока.ОтправленоСФ) или Реквизит="АктСверкиПДФ" и ЗначениеЗаполнено(ПодчиненнаяСтрока.Отправлено) Тогда
...
Процедура СоздатьАктыВБухгалтерииНаСервере(Номер = Неопределено)
	...
	Для каждого ТекСтрока Из ДокОбъект.СписокРассылки Цикл
		...
		Если Бухгалтерия.ЗначениеЗаполнено(ДокБух) Тогда
			...
			ФаксимилеБух=Справочники.Факсимиле.НайтиПоРеквизиту("ФизЛицо",ФИОБух);
			...
			//	КонецЕсли; 
			//КонецЕсли;
			ДобавитьФаксимиле(Факсимиле,ТабДок,8,30,6,5);	
			ДобавитьФаксимиле(Факсимиле,ТабДок,8,30,8,5);
			...
			Если Бухгалтерия.ЗначениеЗаполнено(СчетФакт) Тогда
				...
				ДобавитьФаксимиле(Факсимиле,ТабДокСчФ,10,30,5,3);
				ДобавитьФаксимиле(ФаксимилеБух,ТабДокСчФ,10,30,5,11);
...
Процедура СписокРассылкиПередНачаломИзменения(Элемент, Отказ)
	...
	Отказ = ЗначениеЗаполнено(ТекСтрока.Отправлено) или ЗначениеЗаполнено(ТекСтрока.ОтправленоСФ);
...
Функция  ОткрытьВложенныйАктСверкиНаСервере(НомерСтроки)
	...
	Хранилище=Новый ДвоичныеДанные(ДокОбъект.СписокРассылки[НомерСтроки-1].АктСверкиПДФИмяФайла);
...
&НаСервере
Функция  ОткрытьВложенныйСчетФактуруНаСервере(НомерСтроки)
	
	ДокОбъект = РеквизитФормыВЗначение("Объект");
	
	ЗаписатьАктСверкиВХранилище(ДокОбъект.СписокРассылки[НомерСтроки-1]);
	
	//Хранилище = ДокОбъект.СписокРассылки[НомерСтроки-1].СчетФактураПДФ.Получить();
	
	Хранилище=Новый ДвоичныеДанные(ДокОбъект.СписокРассылки[НомерСтроки-1].СчетФактураПДФИмяФайла);
	//ВремФайл=КаталогВременныхФайлов()+"АктСверки.pdf";
	Если Не ЗначениеЗаполнено(Хранилище) Тогда
...
Процедура ОткрытьАктСверки(Команда)
	...
	Если ТекСтрока <> Неопределено Тогда
		...
		ВложенныйАкт = ОткрытьВложенныйСчетФактуруНаСервере(ТекСтрока.НомерСтроки);
...
&НаСервере
Процедура ДобавитьФаксимиле(Факсимиле,ТабДок,Высота,Ширина,ОтступНиз,Колонка)
	Если ЗначениеЗаполнено(Факсимиле) Тогда
		ДД=Факсимиле.Подпись.Получить();
		Если ТипЗнч(ДД)=Тип("Картинка") Тогда
...
Процедура ОтправитьСчетаФактуры(Команда)
	ОтправитьПисьмаНаСервере("СчетФактураПДФ");
