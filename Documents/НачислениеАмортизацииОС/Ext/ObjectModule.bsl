...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	ПринятыеКУчетуОССрезПоследних.НачислятьАмортизацию = ИСТИНА
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
	...
	//ДвиженияАмортизация = РезультатЗапроса[1].Выгрузить();	
	//ЭтотОбъект.Движения.АК_НачислениеАмортизации.Записывать = Истина;
	//ЭтотОбъект.Движения.АК_НачислениеАмортизации.Загрузить(ДвиженияАмортизация);
	//
	//ДвиженияФинансовый  = РезультатЗапроса[1].Выгрузить();
	...
	//Если ТипОперации = Перечисления.АК_ТипыРегламентныхОпераций.НачислениеАмортизации Тогда
	//	РасчетАмортизации();
	//КонецЕсли;
	//
КонецПроцедуры
...
Функция ПолучитьНастройкуСписания(ВидСписания)
	...
	Результат = Новый Структура("Счет,СтатьяДоходовРасходов,ТорговаяТочка,ЦФО");
	...
	|ИЗ
	|	РегистрСведений.НастройкаОтраженияОперацийВУчете.СрезПоследних(&ДатаОкончания, ВидОперации = &ВидСписания) КАК Срез";
...
// Запрос для тестирования НМА
Процедура НачислениеАмортизацииНМА()

	//Тк ввод делается операцией за начальную стоимость будем брать оборот по дебету.
	...
	Запрос.УстановитьПараметр("ДатаОкончания", ЭтотОбъект.Дата);
...
Функция ПолучитьПараметрыСписанияСоСчета01()
	...
	Результат = Новый Структура("Счет,СтатьяДоходовРасходов");
	
	Запрос = Новый Запрос;
	Запрос.Параметры.Вставить("Дата", КонецМесяца(ЭтотОбъект.Дата));
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Срез.Счет,
	|	Срез.СтатьяДоходовРасходов
	|ИЗ
	|	РегистрСведений.НастройкаОтраженияОперацийВУчете.СрезПоследних(&Дата, ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВУчете.СписаниеОССоСчета01ПриВводеВЭксплуатацию)) КАК Срез";
