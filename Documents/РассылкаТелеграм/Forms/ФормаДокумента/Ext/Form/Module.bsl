//
&НаСервере
Процедура РазослатьВБотНаСервере(ИмяКартинки="") Экспорт 
	...
	Документы.РассылкаТелеграм.РазослатьВБотНаСервере(Объект, ИмяКартинки);
	//---АК mika
...
//
&НаСервере
Процедура РазослатьВМПНаСервере(ИмяКартинки = "") Экспорт 
	...
	|	,'" + ИмяКартинки + "'
	|FROM
	|	(SELECT _IDRRef,_Fld13431RRef, _Fld13151 as title, _Fld11702 as template,
	|	_Fld11719 as keyboard_id,_Fld11720 as keyboard_template, _Fld16362 as picture
...
Процедура Разосласть(Команда)
	...
	Если ЗначениеЗаполнено(Объект.Картинка) ТОгда
		ИмяКартинки = ПолучитьИмяКартинки();
		Если ИмяКартинки = "" Тогда
			Если НЕ Вопрос("Файл картинки не найден. Продолжить?", РежимДиалогаВопрос.ДаНетОтмена) = КодВозвратаДиалога.Да Тогда
				Возврат;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Попытка
		РазослатьВМПНаСервере(ИмяКартинки);
	Исключение
	КонецПопытки;
	РазослатьВБотНаСервере(?(ЗначениеЗаполнено(ИмяКартинки), Объект.Картинка, ""));
...
//
&НаСервере
Функция ПолучитьИмяКартинки()
	
	Попытка
		Файл = Новый Файл(Объект.Картинка);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.Картинка) Тогда
		Попытка
			ДД = Новый ДвоичныеДанные(Объект.Картинка);
			КартинкаСтрока = ПоместитьВоВременноеХранилище(ДД, УникальныйИдентификатор);
		Исключение
		КонецПопытки;
	КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ЗаписатьКартинку Тогда
		Если КартинкаСтрока = "" Тогда
			ТекущийОбъект.Картинка = "";
		Иначе 
			//Каталог = Константы.КаталогХраненияКартинокМПВВ.Получить();
			...
			ПолноеИмя = Каталог + ИмяФайла; 
			КартинкаКартинка.Записать(ПолноеИмя);
			ТекущийОбъект.Картинка = ПолноеИмя;
		КонецЕсли;
		ЗаписатьКартинку = Ложь;
	КонецЕсли;
...
Процедура ВыбратьКартинку(Команда)
	...
	ДиалогФайла.Фильтр = "(*.jpg;*.jpeg)|*.jpg;*.jpeg";	
	...
	//СтруктураВозврат = ОткрытьФормуМодально("Документ.РассылкаТелеграм.Форма.ФормаВыбораФайлаКартинки");
	Если БылВыборФайла Тогда
		КартинкаКартинка = Новый Картинка(ИмяФайла);
	КонецЕсли;
    УстановитьКартинку();
	ЗаписатьКартинку = Истина;
	
КонецПроцедуры

&НаСервере
Функция УстановитьКартинку()	
	
	КартинкаСтрока = ПоместитьВоВременноеХранилище(КартинкаКартинка);
