...
Функция Печать(ПараметрКоманды)
	...
	Запрос.Текст = 
		...
		|	И ЗаявкаНаРоуминг.Ссылка = &Ссылка";
	...
	Если Результат.Пустой() Тогда
		Сообщить("Печать форм возможна только после проведения!");		
	Конецесли;	

	ВыборкаДетальныеЗаписи = Результат.Выбрать();
    Мас=Новый Массив;
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Мас.Добавить(Документы.ЗаявкаНаРоуминг.ПечатьЗаявкаНаРоуминг(ВыборкаДетальныеЗаписи.Ссылка, Документы.ЗаявкаНаРоуминг.ПолучитьМакет("МакетЗаявки")));
	КонецЦикла;
