...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Выручка_ПространствоВкуса = ПланыСчетов.Финансовый.ВыручкаПрочая;
	//---АК
	...
	Если ЗначениеЗаполнено(ТорговаяТочка) и ТорговаяТочка.ТипРозничнойТочки = Перечисления.ТипыРозничныхТочек.ПространствоВкуса Тогда
		Проводка.СчетКт										= Выручка_ПространствоВкуса;
	Иначе	
		Проводка.СчетКт										= ПланыСчетов.Финансовый.ВыручкаТорговыхТочек;
	КонецЕсли;
	//---АК
	...
	Если СчитыватьТаблицуТоваровСЧековДляПроведения
		...
		КонецЕсли;	
		//+++АК sils 16.12.2018 ИП-00020532
		Если ДополнительныеСвойства.Свойство("ТаблицаТоваровКПроведению") и
			не ДополнительныеСвойства.Свойство("ТаблицаУслугКПроведению") Тогда
			ТабТовары = ДополнительныеСвойства.ТаблицаТоваровКПроведению;
			...
			Если ТабУслуги.Количество() > 0 Тогда
				ДополнительныеСвойства.Вставить("ТаблицаУслугКПроведению", ТабУслуги);
				ДополнительныеСвойства.Вставить("ТаблицаТоваровКПроведению", ТабТовары);
			КонецЕсли;
		КонецЕсли;	
		//---АК
	...
	//+++АК sils 16.12.2018 ИП-00020532
	ТаблицаУслугКПроведению = Новый ТаблицаЗначений();
	ТаблицаУслугКПроведению.Колонки.Добавить("Номенклатура");
	ТаблицаУслугКПроведению.Колонки.Добавить("ПродажиККМ");
	
	Если ЗначениеЗаполнено(ДатаВключенияДинамическогоСчитывания)
		И ДатаВключенияДинамическогоСчитывания <= Дата Тогда
		Если ДополнительныеСвойства.Свойство("ТаблицаУслугКПроведению") Тогда
			Для Каждого СтрокаТовар Из ДополнительныеСвойства.ТаблицаУслугКПроведению Цикл
				СтрокаДоб		 				= ТаблицаУслугКПроведению.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаДоб, СтрокаТовар);
			КонецЦикла;	
		КонецЕсли;
	КонецЕсли;
	...
	//+++АК sils 16.12.2018 ИП-00020532
	ПроведениеУслугПоРегиструТоваров(ТаблицаУслугКПроведению);
	//---АК
...
//+++АК sils 16.12.2018 ИП-00020532
Процедура ПроведениеУслугПоРегиструТоваров(ТабУслуг)
	Движения.ДвиженияТоваровПоЛистамУчета.Записывать = Истина;
	Движения.ДвиженияТоваровПоЛистамУчета.Очистить();
	...
	ЗапросКешТовары.УстановитьПараметр("Товары", ТабУслуг.ВыгрузитьКолонку("Номенклатура"));
	ТабКеш = ЗапросКешТовары.Выполнить().Выгрузить();
	
	Если ЗначениеЗаполнено(СкладТТ) Тогда
		Для Каждого СтрокаТовар Из ТабУслуг Цикл
			СтрокаКеш = ТабКеш.Найти(СтрокаТовар.Номенклатура, "Ссылка");
			Если СтрокаТовар.ПродажиККМ <> 0 Тогда
				Движение = Движения.ДвиженияТоваровПоЛистамУчета.Добавить();
				Движение.Период = Дата;
				Движение.СтруктурнаяЕдиница = ТорговаяТочка;
				Движение.ВидДвиженияТовара = Перечисления.ВидДвиженияТовараПоЛистуУчета.Продажа;
				Движение.Номенклатура = СтрокаТовар.Номенклатура;
				Движение.Количество = СтрокаТовар.ПродажиККМ * -1;
			КонецЕсли;
		КонецЦикла;	
	КонецЕсли;
КонецПроцедуры	
