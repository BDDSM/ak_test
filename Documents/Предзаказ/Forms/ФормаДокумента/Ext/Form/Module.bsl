...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если НЕ Отказ И ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение И КонтролироватьНулевыеЦены Тогда
		...
		Если ТоварыБезЦен.НетРозничнойЦены <> Неопределено ИЛИ ТоварыБезЦен.НетЗакупочнойЦены <> Неопределено Тогда
			...
			//+++АК MOSD 2018.12.14 ИП-00017119.02 добавил в текст вопроса "бренд-менеджера"
			ПоказатьВопрос(Оп, "ВНИМАНИЕ! Документ не проведен! Для указанных в документе товаров отсутствуют розничные/закупочные цены." + Символы.ПС  + Символы.ПС + "Уведомить технолога и бренд-менеджера об отсутствии цен?", РежимДиалогаВопрос.ДаНет, 0, КодВозвратаДиалога.Нет, "Вопрос");
			//---АК MOSD 
			Отказ = Истина;	
		КонецЕсли;
	КонецЕсли;
...
//+++АК MOSD 2018.12.14 ИП-00017119.02 
Процедура ОтправитьПисьмоОбОтсутствииЦен(ТоварыБезЦен, ПолучательEmail)
	...
	НовоеПисьмо.Кому 				= ПолучательEmail;
	НовоеПисьмо.КомуПредставление 	= ПолучательEmail;
	Получатель = НовоеПисьмо.КомуТЧ.Добавить();
	Получатель.АдресЭлектроннойПочты 	= ПолучательEmail;
	Получатель.Представление 			= ПолучательEmail;
...
//+++АК MOSD 2018.12.14 ИП-00017119.02 
Функция ПолучитьАдресЭлПочтыБрендМенеджераТехнолога(Технолог)
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
		|	РолиПользователейСоставРоли.Ссылка КАК РольТехнолога
		|ПОМЕСТИТЬ втРолиТехнолога
		|ИЗ
		...
		|			И (РолиПользователейСоставРоли.Сотрудник = &Технолог)
		|			И (РолиПользователейТипыРолей.ТипРоли = ЗНАЧЕНИЕ(ПланВидовХарактеристик.ТипыРолейПользователя.ТехнологПоКачеству))
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
		|	СоответствиеОбъектРольСрезПоследних.РольПользователя КАК РольТовароведа
		|ПОМЕСТИТЬ втРолиТовароведа
		|ИЗ
		...
		|						вт.РольТехнолога
		|					ИЗ
		|						втРолиТехнолога КАК вт)) КАК СоответствиеОбъектРольСрезПоследних
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	РольТовароведа
		|;
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
		|	РолиПользователейСоставРоли.Сотрудник КАК Товаровед
		|ПОМЕСТИТЬ втТовароведы
		|ИЗ
		|	втРолиТовароведа КАК втРолиТовароведа
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.РолиПользователей.СоставРоли КАК РолиПользователейСоставРоли
		|		ПО втРолиТовароведа.РольТовароведа = РолиПользователейСоставРоли.Ссылка
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ИЗ
		|	втТовароведы КАК втТовароведы
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.КонтактнаяИнформация КАК КонтактнаяИнформация
		|		ПО втТовароведы.Товаровед = КонтактнаяИнформация.Объект
	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда
		Рез = ВыборкаДетальныеЗаписи.Представление
	КонецЕсли;
...
//		ОтправитьПисьмоОбОтсутствииЦен(ТоварыБезЦен, Выборка.Email);
	КонецЕсли;
	
	//отправка бренд-менеджеру
	АдресЭлПочтыБрендМенеждера = ПолучитьАдресЭлПочтыБрендМенеджераТехнолога(Технолог);
	Если ЗначениеЗаполнено(АдресЭлПочтыБрендМенеждера) Тогда
		ОтправитьПисьмоОбОтсутствииЦен(ТоварыБезЦен, АдресЭлПочтыБрендМенеждера);
	КонецЕсли;
	//---АК MOSD 	
КонецПроцедуры
