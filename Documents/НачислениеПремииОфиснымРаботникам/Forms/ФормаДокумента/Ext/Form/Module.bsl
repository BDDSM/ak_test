...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОперацияАпдекс = APDEX_ОценкаПроизводительностиКлиентСервер.ПолучитьОперацию("Открытие документа Начисление премии офисным работникам");
	APDEX_ОценкаПроизводительностиКлиентСервер.НачатьЗамерВремени(ОперацияАпдекс);
	//---АК
	
	//+++АК ILIK 2018.07.06 ИП-00019079
	//Элементы.Утверждено.Доступность = ЭтоРуководительПодразделения();
	Элементы.Утверждено.Доступность = ЭтоРуководительПодразделения() И Не УправлениеДопПравамиПользователей.ПолучитьПравоПользователяУпр(ПланыВидовХарактеристик.ПраваПользователей.РазрешитьРедактированиеПремийТолькоПоСебе, Ложь);
	//---АК ILIK
	
	//+++АК sole 2018.09.06 ИП-00019467.01
	Элементы.Состав.ТолькоПросмотр = Объект.Утверждено;
	//---АК sole 2018.09.06 ИП-00019467.01
	...
	//+++АК kats 2018.03.15 17322
	Элементы.АК_ВыгруженоВЗУП.Доступность = РольДоступна("ВыплатаЗарплаты");
	//---АК kats 2018.03.15 17322
	
	//+++АК sole 2018.08.13 ИП-00019467
	ЗаполнитьУтверждающего();
	//---АК sole 2018.08.13 ИП-00019467
...
//+++АК sole 2018.08.13 ИП-00019467
&НаСервере
Процедура ЗаполнитьУтверждающего()
	
	Перем Запрос;
	
	Если Не ЭтаФорма.Объект.Утверждающий.Пустая() Тогда
		Возврат;
	КонецЕсли;
	...
	Запрос.Текст =
"ВЫБРАТЬ ПЕРВЫЕ 1
|	СписокУтверждающихНачислениеПремии.Утверждающий
|
|ИЗ РегистрСведений.СписокУтверждающихНачислениеПремии КАК СписокУтверждающихНачислениеПремии
|ГДЕ 
|		СписокУтверждающихНачислениеПремии.АвторДокумента = &ТекущийПользователь
|";
	...
	Пока Выборка.Следующий() Цикл
		ЭтаФорма.Объект.Утверждающий = Выборка.Утверждающий;
	КонецЦикла;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	//ОтправитьПисьмоПроКривенкоСервер();
...
Процедура ОтправитьПисьмоСервер()
	...
	//НастройкиСКД.ПараметрыДанных.УстановитьЗначениеПараметра("Ссылка", ЭтотОбъект.ЗаявкаНаРасходованиеСредств);
...
Процедура АкцептоватьЗаявкуОтправкаПочты(Адрес) Экспорт
	...
	СсылкаАкцептант								= Объект.Утверждающий; //ПараметрыСеанса.ТекущийПользователь; //+++АК LAGP 2018.10.12 ИП-00020142 По логике здесь должен быть акцептант (утверждающий)
	СписокКому 									= Новый СписокЗначений;
	Если Адрес<>"" Тогда
		СпАдресов								= Новый СписокЗначений;
	...
	|ИЗ
	|	Документ.НачислениеПремииОфиснымРаботникам КАК ДокументНаАкцепт
	|ГДЕ
	...
	|ИЗ
	|	Документ.НачислениеПремииОфиснымРаботникам.Состав КАК ДокументТЧДляПисьма
	|ГДЕ
	...
	Тема										= "Начисление премии офисным работникам №" + СокрЛП(РезЗапросДоки_Номер) + " от " + Формат(РезЗапросДоки_Дата,"ДФ=dd.MM.yyyy");
...
&НаСервере
Процедура ОтправитьПисьмоПроКривенкоСервер()
		//+++АК ZHAS 16-09-17 ИП-00015200.02.000.00001
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
...
;
	...
	Кривенко = Справочники.Пользователи.НайтиПоНаименованию("Кривенко Андрей");
	СтрокиКривенко =  Объект.Состав.НайтиСтроки(Новый Структура("ФизЛицо",Кривенко.ФизЛицо));
	ЭтоКривенко = Объект.Утверждающий = Кривенко И СтрокиКривенко.Количество()>0;
	
	Если Не ЭтоКривенко Тогда
		Возврат;
	КонецЕсли;	
	//Отправка письма
	...
	//СписокКому.Добавить(ВыборкаДетальныеЗаписи.Представление);
	СписокКому.Добавить("buhzp2@gmail.com");
	//СписокКому.Добавить("pozm@automacon.ru");
	...
	Почта.Подключиться(Профиль);
	Письмо.Тема = ""+Объект.Ссылка+" - документ по Кривенко и про Кривенко записан! Ахтунг! Готовность номер 1!";
	Письмо.ИмяОтправителя 	= "" + УчетнаяЗапись + "";
	Письмо.ИмяОтправителя  	= "" + СокрЛП(УчетнаяЗапись) + "";
	Письмо.Отправитель     	= "" + СокрЛП(УчетнаяЗапись) + "";
	Для Каждого ПолучательЭлемент Из СписокКому Цикл
	...
	ТекстСообщения = Письмо.Тексты.Добавить();
	ТекстСообщения.Текст     = ""+Объект.Ссылка+" - ждёт реакции";
	ТекстСообщения.ТипТекста = ТипТекстаПочтовогоСообщения.ПростойТекст;
...
Процедура ПериодПриИзменении()
	...
	Если Не ПроверкиРоли("ВыплатаЗарплаты") Тогда
		ТекДата = ТекущаяДата();
		...
		//	Объект.Дата = НачалоМесяца(ДобавитьМесяц(Объект.ПериодНачисления, -1));
		//Иначе
