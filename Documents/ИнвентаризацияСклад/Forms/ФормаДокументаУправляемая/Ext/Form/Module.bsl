...
Процедура ПерезаполнитьОстаткиПоТоварам(ЗаполнятьКоличествоУчетным = Ложь)
	...
	ДополнитьОстаткамиСоСкладовРезерваИМ();
	//---
КонецПроцедуры

&НаСервере
//+++AK GREK 10.12.2018 ИП-00020678
Процедура ДополнитьОстаткамиСоСкладовРезерваИМ()
	Если Объект.ПоНесколькимСкладам ИЛИ Объект.Склад.Код <> "000000316" Тогда
		Возврат;
	КонецЕсли;		   
	...
	Для Каждого СтрТовары Из Объект.Товары Цикл
		СтрокиОстаток = ТЗ_ОстаткиНаСкладахРезерваДляИМ.НайтиСтроки(Новый Структура("Номенклатура, Характеристика, ДатаПроизводства, ЕдиницаИзмерения, Склад", СтрТовары.Номенклатура, СтрТовары.Характеристика, СтрТовары.ДатаПроизводства, СтрТовары.ЕдиницаИзмерения, Объект.Склад));
		Если СтрокиОстаток.Количество() > 0 Тогда
			СтрТовары.УчетноеКоличество = СтрТовары.УчетноеКоличество + СтрокиОстаток[0].КоличествоОстаток ;
		КонецЕсли;	
	КонецЦикла;
