...
Процедура ОбработкаКоманды(ПараметрКоманды, ПараметрыВыполненияКоманды)
	...
	//+++АК ILIK 2018.07.03 ИП-00018770
	Если ПараметрыВыполненияКоманды.Источник.ИмяФормы = "Документ.ЗаявкаНаУслугиМатериалы.Форма.ФормаДокумента" Тогда
		ОткрытьФормуМодально("Документ.ЗаявкаНаУслугиМатериалы.Форма.ФормаПолучателиРассылки", Новый Структура("МассивДокументов", ПараметрКоманды), ПараметрыВыполненияКоманды.Источник);
		Возврат;
	КонецЕсли;
	//---АК ILIK
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Выборка.ВариантРассылкиЗаявкиНаУслугиИМатериалы = Перечисления.ВариантыРассылокЗаявкиНаУслугиИМатериалы.ОтдельнымиПисьмами Тогда
			...
			Пока РезЗапросДоки.Следующий() Цикл
				...
				TYPE_MESSAGE                                = 1;
				GUID_Заявки									= РезЗапросДоки.Ссылка.УникальныйИдентификатор();
