	...
	Для Каждого СтрокаТЧ Из ТаблицаСРаспределениями Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТЧ.СуммаРаспределено) Тогда	
			...
			ИначеЕсли ТранспортныеУслуги Тогда
				...
				//+++ AK suvv 2018.12.13 ИП-00020688
				СтрокаТЧСуммаСписания = ?(ВариантРасчетаНДС = Перечисления.ВариантыРасчетаНДС.НДСвТомЧисле, СтрокаТЧ.СуммаРаспределено - СтрокаТЧ.СуммаНДСРаспределено, СтрокаТЧ.СуммаРаспределено);
				//--- AK suvv
