...
Процедура СписокПриАктивизацииСтроки(Элемент)
	ПодключитьОбработчикОжидания("УстановитьОтборПоЗаявке",1/10,Истина);
КонецПроцедуры
...
Процедура ОтменитьЗаявкуСервер()
	Для Каждого Документ Из Элементы.Список.ВыделенныеСтроки Цикл
		ДокументОбкт = Документ.ПолучитьОбъект();
		ДокументОбкт.СтатусЗаявки = Перечисления.АК_СтатусыЗаявокНаПечатьЭтикетки.Отменена;
		ДокументОбкт.Записать();
	КонецЦикла;
