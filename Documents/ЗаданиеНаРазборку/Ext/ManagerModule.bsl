...
Функция ДокументыБезУпорядочиванияТоваров(МассивСсылок)
	...
	Возврат Результат; //LATV Попросили отключить, не успели подготовится к этим изменениям
	...
	Запрос = Новый Запрос(
		...
		|	И ЗаданиеНаРазборкуТовары.Ссылка.Склад.Владелец = &Кавказский
	...
	Запрос.УстановитьПараметр("Кавказский",		Справочники.СтруктурныеЕдиницы.НайтиПоКоду("P00000231", Истина));
...
Процедура УпорядочитьСтрокиТоваров(ЗаданиеНаРазборкуОбъект) Экспорт
	...
	//LATV Попросили отключить, не успели подготовится к этим изменениям
	//Кавказский = Справочники.СтруктурныеЕдиницы.НайтиПоКоду("P00000231", Истина);
	//Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗаданиеНаРазборкуОбъект.Склад, "Владелец") <> Кавказский Тогда
		// По-старому
...
Процедура ЗаполнитьПечатнуюФорму_ЗаданиеНаРазборку(ТабДок, МассивСсылок)
	...
	Запрос.Текст = 
		...
		//|	ЗаданиеНаРазборкуТовары.НомерСтроки //LATV Попросили отключить, не успели подготовится к этим изменениям
		|	ЗаданиеНаРазборкуТовары.СтруктурнаяЕдиница.НомерТочки
		|ИТОГИ ПО
		|	Ссылка";
