...
Перем Поз, СтрокаТЗн, ТекТипЗадания;
	...
	Если ЭтоНеПервоеЧтение = Ложь Тогда
		...
		Если НЕ ЭтоНовыйОбъект Тогда
			...
			УстановитьПривилегированныйРежим(Истина);
			ТекПользователь = ПараметрыСеанса.ТекущийПользователь.ФизЛицо;
			...
			Если ЗначениеЗаполнено(ТекПользователь) Тогда
				...
				МассивНайденныхСтрок = ОтветственныеЛица.НайтиСтроки(Новый Структура("Сотрудник", ТекПользователь));
