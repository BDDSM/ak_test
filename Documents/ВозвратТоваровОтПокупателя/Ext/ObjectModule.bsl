...
Процедура ПроведениеДокументаПоРБФинансовый(Отказ)
	...
	СтруктураНастроек = ОбщегоНазначенияСервер.ПолучитьНастройкиОтраженияОперацийВУчете(Перечисления.ВидыОперацийВУчете.ВозвратТоваровОтПокупателя, ЭтотОбъект.Дата);
	СтруктураНастроекВн = ОбщегоНазначенияСервер.ПолучитьНастройкиОтраженияОперацийВУчете(Перечисления.ВидыОперацийВУчете.ВозвратТоваровОтПокупателя_Вн, ЭтотОбъект.Дата, Организация);
	СторонняяРозница = ОбщегоНазначенияКлиентСервер.ЭтоСторонняяРозница(СтруктурнаяЕдиница);
	Счет904 = ПланыСчетов.Финансовый.ВыручкаТорговыхТочек;
	СтатьяВыручка = Справочники.СтатьиДоходовРасходов.ПолучитьСсылку(Новый УникальныйИдентификатор("50e3f40d-f8d8-11e6-a30f-005056a00106"));
	...
	Если СуммаДокумента <> 0 Тогда  	
		...
		Если СторонняяРозница Тогда
			Движение.СчетДт = ?(ЗначениеЗаполнено(СтруктураНастроекВн.Счет), СтруктураНастроекВн.Счет, Счет904);
			Движение.СубконтоДт.СтатьиДоходовРасходов = ?(ЗначениеЗаполнено(СтруктураНастроекВн.СтатьяДоходовРасходов),СтруктураНастроекВн.СтатьяДоходовРасходов,СтатьяВыручка);			
		Иначе
			Движение.СчетДт = ?(ЗначениеЗаполнено(СтруктураНастроекВн.Счет), СтруктураНастроекВн.Счет, Счет904);
			Движение.СубконтоДт.СтатьиДоходовРасходов = ?(ЗначениеЗаполнено(СтруктураНастроекВн.СтатьяДоходовРасходов),СтруктураНастроекВн.СтатьяДоходовРасходов,СтатьяВыручка);
		КонецЕсли;
	...
	Для Каждого Стр из ТЗТоварыПоСтавкам Цикл
		Если Стр.СуммаНДС <> 0 Тогда
			...
			Если СторонняяРозница Тогда
				Движение.СчетКт = ?(ЗначениеЗаполнено(СтруктураНастроекВн.Счет), СтруктураНастроекВн.Счет, Счет904);
				Движение.СубконтоКт.СтатьиДоходовРасходов = ?(ЗначениеЗаполнено(СтруктураНастроекВн.СтатьяДоходовРасходов),СтруктураНастроекВн.СтатьяДоходовРасходов,СтатьяВыручка);
			Иначе
				Движение.СчетКт = ?(ЗначениеЗаполнено(СтруктураНастроекВн.Счет), СтруктураНастроекВн.Счет, Счет904);
				Движение.СубконтоКт.СтатьиДоходовРасходов = ?(ЗначениеЗаполнено(СтруктураНастроекВн.СтатьяДоходовРасходов),СтруктураНастроекВн.СтатьяДоходовРасходов,СтатьяВыручка);
			КонецЕсли;
...
Функция ПолучитьНастройкуОтраженияВУчете_НеПриниматьКВычетуНДС()
	...
	Возврат ОбщегоНазначенияСервер.ПолучитьНастройкиОтраженияОперацийВУчете(Перечисления.ВидыОперацийВУчете.ВозвратОтПокупателя_НеПриниматьКВычетуНДС, Дата, Организация);	
...
Процедура СоздатьДвиженияФинансовый_НеПриниматьКВычетуНДС(НастройкаОтраженияВУчете, ТаблицаПоСтавкамНДС, Отказ)
	...
	Для Каждого Стр Из ТаблицаПоСтавкамНДС Цикл
		...
		Движение.СчетДт = НастройкаОтраженияВУчете.Счет;
		Движение.СубконтоДт.ТорговыеТочки = НастройкаОтраженияВУчете.СтруктурнаяЕдиница;
		Движение.СубконтоДт.СтатьиДоходовРасходов = НастройкаОтраженияВУчете.СтатьяДоходовРасходов;
		Движение.СубконтоДт.ЦФО = НастройкаОтраженияВУчете.ЦФО;
		Движение.СчетКт = ПланыСчетов.Финансовый.НалогиУН;
		Движение.СубконтоКт.Организации = НастройкаОтраженияВУчете.ДопРеквизит;	
		Движение.СубконтоКт.ВидыНалогов = НастройкаОтраженияВУчете.ДопРеквизит1;
