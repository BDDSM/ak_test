...
Функция ПрочитатьPDFДепозит(ПутьДоФайла) Экспорт
	...
	Если Найти(ВесьТекст, "Банке ВТБ (ПАО)") > 0 Тогда
		...
		СтавкаДоговора = Найти(ВесьТекст, "6.Процентная ставка по депозиту:");
		Если СтавкаДоговора > 0 Тогда
			СтрокаСоСтавкойДоговора = Прав(ВесьТекст, СтрДлина(ВесьТекст) - СтавкаДоговора - 32);	
		...
		СтавкаДоговора = "" + СтрокаСоСтавкой + "%";
