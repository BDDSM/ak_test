...
// {{ АК Молчанов С.А. 08.12.18 ИП-00020041.02
Процедура ОпределитьБлижайшееМетро() Экспорт
	КоличествоСтанций = СтанцииМетро.Количество();
	Пока КоличествоСтанций > 0 Цикл
		КоличествоСтанций = КоличествоСтанций - 1;
		Если Не ЗначениеЗаполнено(СтанцииМетро[КоличествоСтанций].Метро) Тогда
			СтанцииМетро.Удалить(СтанцииМетро[КоличествоСтанций]);
		КонецЕсли;
	КонецЦикла;	
	
	Если СтанцииМетро.Количество() = 0 Или Не ЗначениеЗаполнено(КоординатыДолгота) Или Не ЗначениеЗаполнено(КоординатыШирота) Тогда
		Возврат;
	ИначеЕсли СтанцииМетро.Количество() = 1 Тогда
		СтанцииМетро[0].Ближайшая = Истина;
	Иначе
		ВыбранныеСтанции = СтанцииМетро.НайтиСтроки(Новый Структура("Ближайшая",Истина));
		Если ВыбранныеСтанции.Количество() Тогда
			Возврат;
		КонецЕсли;		
		ОпределитьБлижайшееМетроПоКоординатам();		
	КонецЕсли;	
КонецПроцедуры

Процедура ОпределитьБлижайшееМетроПоКоординатам()
	
	Обработка = Обработки.АК_ОтображениеНаКарте.Создать();
	БлижайшаяСтанция = Неопределено;
	СтрокаКоординат = "";
	МинимальноеРастояние = 0;
	
	Для Каждого СтрокаСтанции Из СтанцииМетро Цикл
		Если НЕ ЗначениеЗаполнено(СтрокаСтанции.Метро) Тогда
			Продолжить;
		КонецЕсли;
		
		лАдрес = "Метро " + Строка(СтрокаСтанции.Метро);
		
		КоординатыОпределены = Обработка.ПолучитьСтрокуКоординатПоАдресу(лАдрес,СтрокаКоординат);
		Если НЕ КоординатыОпределены Тогда
			Продолжить;
		КонецЕсли;
		
		МассивСтрок = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СтрокаКоординат,",");
		Если МассивСтрок.Количество() <> 2 Тогда
			Продолжить;
		КонецЕсли;
		
		РасстояниеДоМетро = ПросчитатьРастояниеОтДо(МассивСтрок[0],МассивСтрок[1]); //широта,долгота
		
		Если МинимальноеРастояние = 0 Или МинимальноеРастояние > РасстояниеДоМетро Тогда
			БлижайшаяСтанция = СтрокаСтанции.Метро;
			МинимальноеРастояние = РасстояниеДоМетро;
		КонецЕсли;		
	КонецЦикла;
	
	Если БлижайшаяСтанция <> Неопределено Тогда
		СтрокаСтанции = СтанцииМетро.Найти(БлижайшаяСтанция,"Метро");
		Если СтрокаСтанции <> Неопределено Тогда
			СтрокаСтанции.Ближайшая = Истина;
		КонецЕсли;		
	КонецЕсли;
	
КонецПроцедуры

Функция ПросчитатьРастояниеОтДо(ШиротаМетро,ДолготаМетро)
    
    Возврат ASin(sin(ШиротаМетро)*sin(КоординатыШирота) + cos(ШиротаМетро)*cos(КоординатыШирота)*cos(ДолготаМетро - КоординатыДолгота));
