...
Процедура ОбработатьПодтверждениеЗакрытияИзПисьма(СЕСсылка, СтруктураПараметров) Экспорт	
	Если ТипЗнч(СЕСсылка) = Тип("СправочникСсылка.СтруктурныеЕдиницы") И СтруктураПараметров.Согласовано И НЕ СтруктураПараметров.Отклонено Тогда
		МЗ = РегистрыСведений.АК_МагазиныКЗакрытию.СоздатьМенеджерЗаписи();
		МЗ.Магазин = СЕСсылка;
		МЗ.Прочитать();
		Если МЗ.Выбран() Тогда
			Если МЗ.ДатаПланируемогоЗакрытия = СЕСсылка.ДатаПланируемогоЗакрытия Тогда
				Возврат;
			КонецЕсли;
		КонецЕсли;
		МЗ.Магазин = СЕСсылка;
		МЗ.ДатаПланируемогоЗакрытия = СЕСсылка.ДатаПланируемогоЗакрытия;
		МЗ.Записать();
		...
		Письмо.Тема = "Уведомление о закрытии торговой точки" + СЕСсылка.Наименование + " начиная с " + Формат(СЕСсылка.ДатаПланируемогоЗакрытия, "ДФ=dd.MM.yyyy");
		Письмо.ИмяОтправителя  	= "" + СокрЛП(УчетнаяЗапись) + "";
		Письмо.Отправитель     	= "" + СокрЛП(УчетнаяЗапись) + "";
		ТекстУведомления = "";
		ТекстУведомления = ТекстУведомления + "Добрый день!" + Символы.ПС + Символы.ПС;
		ТекстУведомления = ТекстУведомления + "Магазин " + СЕСсылка.Наименование + " закрывается начиная с " + Формат(СЕСсылка.ДатаПланируемогоЗакрытия, "ДФ=dd.MM.yyyy") + 
							" Причина закрытия: " + СЕСсылка.ПричинаЗакрытия + ". Просьба переводить покупателей в магазин " + СЕСсылка.КудаПеренаправляемПокупателейПослеЗакрытия + Символы.ПС;
		ТекстУведомления = ТекстУведомления + "Дата последней инкассации " + Формат(СЕСсылка.ДатаПоследнейИнкасации, "ДФ=dd.MM.yyyy") + Символы.ПС;
		ТекстУведомления = ТекстУведомления + "Закрывает " + СЕСсылка.КтоЗакрывает + Символы.ПС;
		...
		Запрос.УстановитьПараметр("Группа",ПредопределенноеЗначение("Справочник.АК_ГруппыРассылки.ИнформированиеОЗакрытииМагазина"));
		Рез = Запрос.Выполнить();
		Если НЕ Рез.Пустой() Тогда
		...
		ТекстСообщения = Письмо.Тексты.Добавить(ТекстУведомления);
		Попытка
...
Функция ПолучитьОграниченияВыручкиПоМагазинам(Магазин = Неопределено) Экспорт
	
	ПараметрКонецПериода  = КонецДня (ТекущаяДата() - 86400);
	ПараметрНачалоПериода = НачалоДня(ПараметрКонецПериода - 31 * 86400);
	НижняяГраницаСтатистики = НачалоДня(ПараметрКонецПериода - 14 * 86400);
	
	ТекстЗапроса = 
	"SELECT id_tt,date_tt, SUM(summa) AS Eearnings
	|FROM Reports.dbo.DTT 
	|WHERE (date_tt BETWEEN " +ВнешниеДанные.ФорматПоля(ПараметрНачалоПериода)+ "
	|                       AND " +ВнешниеДанные.ФорматПоля(ПараметрКонецПериода) + ")
	|		УсловиеПоТТ
	|GROUP BY id_tt,date_tt
	|ORDER BY id_tt, date_tt DESC";
	
	ТекстЗапроса = СтрЗаменить(
		ТекстЗапроса, 
	    "УсловиеПоТТ", 
	    ?(НЕ ЗначениеЗаполнено(Магазин)," ","AND id_tt = " + ВнешниеДанные.ФорматПоля(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Магазин,"id_TT"))));
	
	ADOСоединение = ВнешниеДанные.ПолучитьADOСоединение();
	rs = ADOСоединение.Execute(ТекстЗапроса);	
	тзВыручка = ВнешниеДанные.ПреобразоватьРезультатВТаблицуЗначений(rs);
	ADOСоединение.Close();
	
	Результат = Новый Соответствие;
	
	//считаем максимальную выручку
	Для Каждого ТекСтр Из тзВыручка Цикл
		
		Если Результат.Получить(ТекСтр.id_tt) = Неопределено Тогда
			Результат.Вставить(ТекСтр.id_tt, Новый Структура("ВерхнийПредел,НижнийПредел,Статистика",0,0,Новый Массив));
		КонецЕсли;
		
		ДанныеПоМагазину = Результат[ТекСтр.id_tt];
		ДанныеПоМагазину.Вставить("ВерхнийПредел",Макс(ДанныеПоМагазину.ВерхнийПредел, ТекСтр.Eearnings));
		ДанныеПоМагазину.Вставить("НижнийПредел", ДанныеПоМагазину.ВерхнийПредел);
		
	КонецЦикла;
	
	//считаем минимальную выручку
	Для Каждого ТекСтр Из тзВыручка Цикл
		
		ДанныеПоМагазину = Результат[ТекСтр.id_tt];
		
		Если ТекСтр.Eearnings < ДанныеПоМагазину.ВерхнийПредел * 0.2 Тогда
			Продолжить;
		КонецЕсли;
		
		ДанныеПоМагазину.Вставить("НижнийПредел", Мин(ДанныеПоМагазину.НижнийПредел, ТекСтр.Eearnings));
		
		ТекСтрДата = Дата(СтрЗаменить(ТекСтр.date_tt,"-","")+"000000");
		Если  ТекСтрДата >= НижняяГраницаСтатистики Тогда
			ДанныеПоМагазину.Статистика.Добавить(Новый Структура("Дата,Выручка",ТекСтрДата,Окр(ТекСтр.Eearnings/1000)));
		КонецЕсли;
		
	КонецЦикла;
	
	//считаем ограничения
	Для Каждого ДанныеПоМагазину Из Результат Цикл
		ДанныеПоМагазину.Значение.Вставить("ВерхнийПредел", Окр(ДанныеПоМагазину.Значение.ВерхнийПредел * 2 / 1000));
		ДанныеПоМагазину.Значение.Вставить("НижнийПредел", Окр(ДанныеПоМагазину.Значение.НижнийПредел * 0.6 / 1000));
	КонецЦикла;
