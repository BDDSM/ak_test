...
Процедура ЗакрытьФормуСВозвратомКодировки()
	...
	СтруктураВозврата = Новый Структура("Значение, Представление", 
		Элементы.СписокКодировок.ТекущиеДанные.Значение, Представление);
...
Процедура ЗаполнитьСписокКодировок(ПолныйСписок)
	...
	Если Не ПолныйСписок Тогда
		СписокКодировокЛокальный = РаботаСФайламиКлиентСервер.ПолучитьСписокКодировок();
	Иначе
