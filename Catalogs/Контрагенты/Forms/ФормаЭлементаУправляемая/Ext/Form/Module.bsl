...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	//+++ АК luza ИП-00019743
	Если ТекГрузоотправитель <> Грузоотправитель Тогда
		ЗаписатьГрузоотправителяСервер();
	КонецЕсли;	
	//-- АК luza ИП-00019743
...
//+++АК luza ИП-00019743
&НаСервере
Процедура ЗаполнитьГрузоотправителяСервер()
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	...
	Если ТЗ.Количество() > 0 Тогда
		Грузоотправитель = ТЗ[0].Значение;
		ТекГрузоотправитель = Грузоотправитель; 
	КонецЕсли;	
...
//+++АК luza ИП-00019743
&НаСервере
Процедура ЗаписатьГрузоотправителяСервер()
	Запись = РегистрыСведений.ЗначенияСвойствОбъектов.СоздатьМенеджерЗаписи();
	Запись.Объект = Объект.Ссылка;
	Запись.Свойство = ПланыВидовХарактеристик.СвойстваОбъектов.Грузоотправитель_По_Умолчанию;	
	Запись.Значение = Грузоотправитель;
	Запись.Записать();	
КонецПроцедуры
