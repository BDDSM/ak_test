...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ШиринаЗаголовка = 1.3 * Макс(Параметры.Длина, СтрДлина(Заголовок));
	...
	ТекстСтроки = Параметры.ТекстСтроки;
	
	// Размещение текста.
	Элементы.ТекстСтроки.МногострочныйРежим = Параметры.Многострочность;
	
	// Размещение кнопок.
	ДобавитьКомандыИКнопкиНаФорму("РежимДиалогаВопрос.ОКОтмена");
...
Процедура Подключаемый_ОбработчикКоманды(Команда)
	...
	РезультатВыбора.Вставить("Значение", ?(ВыбранноеЗначение = "КодВозвратаДиалога.ОК", ТекстСтроки, Неопределено));
	Если НЕ ПустаяСтрока(Параметры.ОписаниеОповещения) Тогда
		РезультатВыбора.Вставить("ОписаниеОповещения", Параметры.ОписаниеОповещения);
	КонецЕсли;
...
Процедура СброситьРазмерыИПоложениеОкна()
	...
	Если ПравоДоступа("СохранениеДанныхПользователя", Метаданные) Тогда
		ХранилищеСистемныхНастроек.Удалить("ОбщаяФорма.ВводСтроки", "", ИмяПользователя);
	КонецЕсли;
