...
Функция ПолучитьПояснения(ТолькоПоля, НаименованиеПояснения = "")
	...
	СтруктураПояснения = Новый Структура("Изготовитель,ЮридическийАдрес,ФактическийАдрес,Производство,Импортёр,Местонахождение,
		...
		|МассаВес,ЦветТон,МассоваяДоляФторида,
		|ОбогащениеПродуктаМинераламиИВитаминами,СодержаниеПродуктаДополнительно,ДополнительныеУсловияХранения,
		|СрокГодности,УсловияХранения,Влажность,
		|ЗнакиСоответствия,ЗначенияПетлиМёбиуса,ЗнакПредназначенияДляПарфюмерноКосметическойПродукции,СпециальныеЗнаки,
		|СпособПриготовленияПрименения,УсловияХраненияПослеВскрытияУпаковки,РекомендацииПоПотреблению,
		|СведенияОПопаданииПродукцииВГлаза,ФразаДляБезопасностиУпотребления,
		|ОбразецУпаковкиДляВкусВилл,
		|ОбразецУпаковкиЭтикетки,РабочиеФайлыУпаковкиЭтикетки,ЧертёжДляЭтикетокФлоупак,
		...
		|ОсобенностиПродукта,ОсобенностиПроизводстваПродукта,ОсобенностиПродуктаПрочие,ОбластьПримененияПродукта");
	Если ТолькоПоля Тогда Возврат СтруктураПояснения; КонецЕсли;
	
	Если НЕ ПустаяСтрока(НаименованиеПояснения) Тогда Возврат РегистрыСведений.ПоясненияКартДизайнПроектов.ПолучитьПояснение(НаименованиеПояснения, 2); КонецЕсли;
	
	СтруктураПояснения.Вставить("Изготовитель", "Полное наименование, полный Юр адрес с индексом. Для импортных товаров обязательно указывать наименование и место нахождения импортера.
		|Наименование и адрес изготовителя писать на языке страны или буквами латинского языка, но страну обязательно прописывать на русском языке. Обязательно указывать сторону, принимающую претензии от потребителя с указанием наименования организации, адреса почтового и электронной почты.
		|Если претензии принимает сам изготовитель, то указывать фразу “Претензии от потребителей принимает изготовитель”. Претензии принимает Вкусвилл.");
	...
	СтруктураПояснения.Вставить("НаименованиеПродуктаПоТехническойДокументации", "Указываемое наименование должно соответствовать наименованию в нормативно-технической документации. (Если продукция предназначена для детей, то указывается возраст ребенка и информируется о том, что подходит для детей)");
	СтруктураПояснения.Вставить("СоставПродуктаДляЭтикетки", "Ингредиенты указывают в порядке уменьшения их массовой доли в рецептуре, при этом парфюмерную (ароматическую) композицию указывают как единый ингредиент без раскрытия её состава.
	...
	СтруктураПояснения.Вставить("МассаВес", "Для мыла твёрдого туалетного - номинальная масса куска, на момент упаковки, за исключением парфюмерно-косметической продукции номинальной массой менее 5 г, или номинальным объёмом менее 5 мл, или пробника парфюмерно-косметической продукции");
	СтруктураПояснения.Вставить("ЦветТон", "Для декоративной косметики или окрашивающих средств");
	СтруктураПояснения.Вставить("МассоваяДоляФторида", "Указывается для средств для полости рта, содержащих соединения фтора");
	...
	СтруктураПояснения.Вставить("СодержаниеПродуктаДополнительно", "Специфическая информация, которую также нужно добавить (например, содержание ароматических масел в продукте)");
	СтруктураПояснения.Вставить("СрокГодности", "Указывается в месяцах или годах / рекомендуем указывать");
	...
	СтруктураПояснения.Вставить("ЗначенияПетлиМёбиуса", "Обязательный знак согласно ТРТС 005/2011. В треугольнике может указываться цифра –код материала, из которого изготавливается упаковка. Если в упаковке использованы 2 и более материала нужно указать значения для каждого из них в поле через запятую");
	СтруктураПояснения.Вставить("ЗнакПредназначенияДляПарфюмерноКосметическойПродукции", "Если этого знака нет на таре, нужно поставить в пустое поле слово “Да”, чтобы мы указали его на этикетке");
	СтруктураПояснения.Вставить("СпециальныеЗнаки", "Указать при необходимости наличие специальных знаков, например: “температурные режимы”; “защита от прямых солнечных лучей”; “не тестировано на животных”");
	...
	СтруктураПояснения.Вставить("СпособПриготовленияПрименения", "Если отсутствие сведений может привести к неправильному использованию потребителем парфюмерно-косметической продукции");
	...
	СтруктураПояснения.Вставить("СведенияОПопаданииПродукцииВГлаза", "Инструкция по мерам предосторожности");
	СтруктураПояснения.Вставить("ФразаДляБезопасностиУпотребления", "Напишите фразу для безопасности использования");
	СтруктураПояснения.Вставить("ОбразецУпаковкиДляВкусВилл", "При небольших размерах потребительской тары, на которой невозможно изложить необходимый текст информации полностью,
		|а также на сувенирных и подарочных наборах, по усмотрению изготовителя допускается часть информации располагать на потребительской таре, а часть - на ярлыке, открытке, листе-вкладыше,
		|прилагаемом к каждой единице ПКП. В последнем случае, а также если к ПКП есть сопроводительная информация на продукцию, наносится графический знак в виде кисти руки на открытой книге,
		|указывающий на то, что часть информации для потребителя находится в прилагаемом ярлыке, открытке, листе-вкладыше.");
	СтруктураПояснения.Вставить("ОсобенностиПродукта", "Самая привлекательная для покупателя информация, которая отличает данный продукт от аналогов. Например, с гиалуроновой кислотой; не содержит спирт и т.п.");
	СтруктураПояснения.Вставить("ОбластьПримененияПродукта", "Описание для чего использовать данный продукт.
		|
		|Например, для жирной/сухой/нормальной кожи (волос и т.п.) - для косметики;
		|
		|для стирки тёмных/светлых/детских вещей - для средств для стирки;
		|
		|для мытья полов/посуды и т.д.");
	СтруктураПояснения.Вставить("ОсобенностиПроизводстваПродукта", "Например, информация о производстве продукта, о удобном применении/использовании и т.д.	
		|
		|Описание отдельных компонентов, их полезные свойства.	
		|
		|Информация об отсутствии вредных компонентов, которые обычно используют в аналогах.	
		|
		|Другая полезная, интересная информация для покупателя о продукте.");
	СтруктураПояснения.Вставить("ОсобенностиПродуктаПрочие", "Что делает данный продукт.
		|
		|Например, улучшает цвет лица, борется с морщинами и т.д. (для косметики);
		|
		|эффективно очищает от жира и т.д. (для бытовой химии);
		|
		|и т.д.");
...
Функция ЗаполнитьЛистыФайловДляЗагрузки(ГруппыСФайламиСтрокой = "", Знач НачНомерФайла = 0)
	...
	Если ПустаяСтрока(ГруппыСФайламиСтрокой) Тогда
		ГруппыСФайламиСтрокой = "ДекларацияСоответствия,ТитульныйЛистНаПродукцию,ОбластьПрименения,АссортиментныйПеречень,ТребованияКСырью,ПодтверждениеМассы,
			|НазначениеПарфюмерноКосметическойПродукцииФайл,ПодтверждениеМолярнойМассы,
			|УсловияТранспортировкиИХраненияПродукции,
			...
			|ПротоколИспытанийНаКонкретныйПродукт,Сертификаты,МикробиологическиеИХимическиеПоказатели,ИнструкцияПоПрименению,РецептураПродукции";
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.ТипПродукции = 0 Тогда Объект.ТипПродукции = 2; КонецЕсли;
	...
	Если ЭтоПоставщик Тогда
		...
		Если ТекСтатус = ПредопределенноеЗначение("Перечисление.СтатусыКарточекТовараПоставщиков.НаИсправлениеОтСОУ") И МассивСтрокДляОтбора.Количество() > 0 Тогда
	...
	ИначеЕсли ЭтоПоставщик И ТекСтатус = ПредопределенноеЗначение("Перечисление.СтатусыКарточекТовараПоставщиков.НаИсправлениеОтСОУ") Тогда
		...
		СтруктураПроверяемыеРеквизиты = Новый Структура("Страница1,Страница2,Страница3,Страница4,Страница5,Страница7,Страница9,Страница10");
		Для Каждого КлючИЗначение Из СтруктураПроверяемыеРеквизиты Цикл
	...
	ИначеЕсли ЭтоАудитор И ТекСтатус = ПредопределенноеЗначение("Перечисление.СтатусыКарточекТовараПоставщиков.ВРаботеУАудитора") Тогда
		...
		СтруктураНевидимости = Новый Структура("Статус");
...
Перем ВидСтандартаКачества;
	...
	// Если в графе проставляется ГОСТ то поля: 2-10, 17 неактивны (было 18, но 15-го поля нет)
	...
	МассивИндексовДляНевидимостиДляГОСТ.Добавить(17);
...
Перем ОписаниеОповещения;
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") И ИсточникВыбора.ИмяФормы = "ОбщаяФорма.Вопрос" Тогда
		...
		ИначеЕсли Найти(ОписаниеОповещения, "ПоказатьПояснениеЗавершение") = 1 И ВыбранноеЗначениеРезультат = "Редактировать" Тогда
			...
			СтруктураКлючаПояснения = Новый Структура("ИмяПоля,ТипПродукции", ИмяПоляПояснения, 2);
...
Функция ПолучитьПроверяемыеРеквизиты(Объект, ПолучатьСинонимыРеквизитов = Ложь, ЭтаФорма = Неопределено)
	...
	СтруктураПроверяемыеРеквизиты = Новый Структура("Наименование,Владелец,Поставщик,Технолог,
		...
		|НазначениеПарфюмерноКосметическойПродукции,
		|ОбогащениеПродуктаМинераламиИВитаминами,СрокГодности,ТипСрокаГодности,Влажность,УсловияХраненияПослеВскрытияУпаковки,СпособПриготовленияПрименения");
	...
	Если Объект.ВидСтандартаКачества <> ПредопределенноеЗначение("Перечисление.ВидыСтандартовКачества.ГОСТ") Тогда
		...
		СтруктураФайлы = Новый Структура(
			"ТитульныйЛистНаПродукцию,ОбластьПрименения,АссортиментныйПеречень,ТребованияКСырью,ПодтверждениеМассы,НазначениеПарфюмерноКосметическойПродукцииФайл,
			|ПодтверждениеМолярнойМассы,УсловияТранспортировкиИХраненияПродукции,МикробиологическиеИХимическиеПоказатели");
	КонецЕсли;
...
Функция ПолучитьИспользуемыеРеквизиты(Объект, ПолучатьСинонимыРеквизитов = Ложь, ЭтаФорма = Неопределено)
	...
	СтруктураИспользуемыеРеквизиты = Новый Структура("Наименование,Владелец,НаименованиеДляЦенника,Поставщик,Технолог,ТипПозиционирования,ШтрихКод,
		...
		|ВидСтандартаКачества,НомерНормативногоДокумента,НаименованиеПродуктаПоТехническойДокументации,СоставПродуктаДляЭтикетки,МассаНетто,
		|НазначениеПарфюмерноКосметическойПродукции,ПродукцияПредназначенаДляДетей,
		|ЦветТон,МассоваяДоляФторида,ОбогащениеПродуктаМинераламиИВитаминами,СодержаниеПродуктаДополнительно,НомерПроизводственнойПартии,
		|СрокГодности,ТипСрокаГодности,ТемператураХраненияОт,ТемператураХраненияДо,Влажность,ДополнительныеУсловияХранения,УсловияХраненияПослеВскрытияУпаковки,СпособПриготовленияПрименения,
		|РСТ,ЕАС,ЗначенияПетлиМёбиуса,ЗнакПредназначенияДляПарфюмерноКосметическойПродукции,СпециальныеЗнаки,
		|РекомендацииПоПотреблению,СведенияОПопаданииПродукцииВГлаза,ФразаДляБезопасностиУпотребления,
		|КтоОсуществляетПечать,ПараметрыМестаДатыИзготовления,МатериалПечати,
		|ОбластьПримененияПродукта,ОсобенностиПродуктаПрочие,ОсобенностиПродукта,ОсобенностиПроизводстваПродукта,
...
Процедура ЗаполнитьКарточкуТекстНаСервере(Объект, ТабДок)
	...
	// добавляем синонимы
	СтруктураИспользуемыеРеквизиты.Вставить("РекомендацииПоПотреблению", "Рекомендации по применению аллергической продукции (особые меры предостороженности)");
	СтруктураИспользуемыеРеквизиты.Вставить("ОсобенностиПродуктаПрочие", "Основное действие продукта");
	СтруктураИспользуемыеРеквизиты.Вставить("ОсобенностиПродукта", "Главная фишка");
	СтруктураИспользуемыеРеквизиты.Вставить("ОсобенностиПроизводстваПродукта", "Общее описание продукта");
	
	Справочники.КартыДизайнПроектов.ЗаполнитьТекстовоеНаполнениеКарточки(Объект.Ссылка, СтруктураИспользуемыеРеквизиты, ТабДок);
