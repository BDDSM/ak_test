...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоПоставщик Тогда
		СтруктураНевидимости = Новый Структура("Статус,ГруппаПродакт,РольТехнолога");
		...
		Если ТекСтатус = ПредопределенноеЗначение("Перечисление.СтатусыКарточекТовараПоставщиков.НаИсправлениеОтТехнолога") И МассивСтрокДляОтбора.Количество() > 0 Тогда
	...
	Если НЕ ЭтоПоставщик И ТекСтатус = ПредопределенноеЗначение("Перечисление.СтатусыКарточекТовараПоставщиков.НаПроверке") Тогда
		...
		ОтметитьПоляСНеисправленнымиЗамечаниями(Объект.Замечания, Элементы);
		
	ИначеЕсли ЭтоПоставщик И ТекСтатус = ПредопределенноеЗначение("Перечисление.СтатусыКарточекТовараПоставщиков.НаИсправлениеОтТехнолога") Тогда
		
		ОтметитьПоляСНеисправленнымиЗамечаниями(Объект.Замечания, Элементы, СтруктураРеквизитыДляЗамечаний);
	...
	ИначеЕсли ЭтоАналитик И ТекСтатус = ПредопределенноеЗначение("Перечисление.СтатусыКарточекТовараПоставщиков.ВРаботеУМастераДанных") Тогда
...
Процедура ОтправитьНаПроверку(Команда)
	...
	Если Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыКарточекТовараПоставщиков.НаИсправлениеОтТехнолога")
	...
	ИначеЕсли Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыКарточекТовараПоставщиков.ВРаботеУМастераДанных")
	  И Объект.Замечания.НайтиСтроки(Новый Структура("Исправлено", Ложь)).Количество() = 0 Тогда
...
Процедура ПроверкаПройдена(Команда)
	...
	СменитьСтатус("Внимание! После прохождения проверки редактирование будет запрещено!
		|Продолжить?", "ВРаботеУМастераДанных");
...
Процедура ОтправитьНаИсправление(Команда)
	...
	СменитьСтатус("Отправить на исправление?", "НаИсправлениеОтТехнолога");
...
Перем ТекСтатус;
	...
	ТолькоПросмотр = ТолькоПросмотр ИЛИ (ЭтоПоставщик И ТекСтатус = ПредопределенноеЗначение("Перечисление.СтатусыКарточекТовараПоставщиков.НаПроверке")
		ИЛИ НЕ ЭтоАналитик И ТекСтатус = ПредопределенноеЗначение("Перечисление.СтатусыКарточекТовараПоставщиков.ВРаботеУМастераДанных")
	...
	Элементы.ФормаОтправитьНаПроверку.Видимость = 
		...
		ИЛИ ТекСтатус = ПредопределенноеЗначение("Перечисление.СтатусыКарточекТовараПоставщиков.НаИсправлениеОтТехнолога")
		ИЛИ ЭтоАналитик И ТекСтатус = ПредопределенноеЗначение("Перечисление.СтатусыКарточекТовараПоставщиков.ВРаботеУМастераДанных"));
	
	ТолькоПросмотр = ТолькоПросмотр ИЛИ
		(НЕ ЭтоПоставщик И Элементы.ФормаОтправитьНаПроверку.Видимость И НЕ (ЭтоАналитик И ТекСтатус = ПредопределенноеЗначение("Перечисление.СтатусыКарточекТовараПоставщиков.ВРаботеУМастераДанных")));
...
Перем СтрокаОшибки, СтруктураОшибки;
	...
	Если СтруктураОшибки.Количество() = 0 Тогда
		...
		Если НовыйСтатус = "ВРаботеУМастераДанных" Тогда
			...
			//СтруктураПроверяемыеРеквизиты.Вставить("РольТехнолога", "Роль технолога");
		КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Процедура ОтметитьПоляСНеисправленнымиЗамечаниями(Замечания, Элементы, СтруктураРеквизитыДляЗамечаний = Неопределено)
	
	// выбираем "активные" замечания
	МассивСтрокДляОтбора = Замечания.НайтиСтроки(Новый Структура("Исправлено", Ложь));
	Если МассивСтрокДляОтбора.Количество() > 0 Тогда
		
		ЦветФонаОсобыйТекст = Новый Цвет(255, 0, 0);
		Если СтруктураРеквизитыДляЗамечаний = Неопределено Тогда СтруктураРеквизитыДляЗамечаний = Новый Структура; КонецЕсли;
		
		Для Каждого СтрокаТЧ Из МассивСтрокДляОтбора Цикл
			СтруктураРеквизитыДляЗамечаний.Вставить(СтрокаТЧ.ИмяПоля);
			Элементы[СтрокаТЧ.ИмяПоля].ЦветФона = ЦветФонаОсобыйТекст;
		КонецЦикла;
...
Процедура ОбновитьЗначенияПолей(Команда = Неопределено)
	...
	Для Каждого СтрокаТЧ Из Объект.Замечания Цикл
		//+++АК SHEP 2018.10.25 ИП-00018753.05: обход ошибок ЛК поставщика
		//СтрокаТЧ.ЗначениеПоля = Объект[СтрокаТЧ.ИмяПоля];
		Объект.Свойство(СтрокаТЧ.ИмяПоля, СтрокаТЧ.ЗначениеПоля);
		//---АК SHEP 2018.10.25
	КонецЦикла;
