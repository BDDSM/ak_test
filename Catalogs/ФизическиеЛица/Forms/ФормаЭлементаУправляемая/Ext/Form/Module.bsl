...
//+++ЗНК Zionec 2018.12.07 ИП-00020645
&НаСервере
...
Процедура ПечатьНаправлений(Команда)
	...
	СписокНаправлений.Добавить("ИзменитьДатыОкончания", "Изменить даты окончания");
	СписокНаправлений.Добавить("НаправлениеПрофмедлаб", "Направление Профмедлаб");
	СписокНаправлений.Добавить("НаправлениеНаПсихОсвидетельствование", "Направление на псих. освидетельствование");
	
	ВыбраннаяПФ = ВыбратьИзСписка(СписокНаправлений, Элементы.ПечатьНаправлений);
	Если ВыбраннаяПФ = Неопределено Тогда 
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Или ЭтаФорма.Модифицированность Тогда
		ПоказатьПредупреждение(, "Перед печатью необходимо записать элемент!");
		Возврат;
	КонецЕсли;
	
	Если ВыбраннаяПФ.Значение = "НаправлениеПрофмедлаб" Тогда
		ТабДокумент = ПечатьНаправлениеПрофмедлаб();
		ТабДокумент.Показать();	
	ИначеЕсли ВыбраннаяПФ.Значение = "НаправлениеНаПсихОсвидетельствование" Тогда 
		ТабДокумент = ПечатьНаправлениеНаПсихОсвидетельствование();
		ТабДокумент.Показать();
	ИначеЕсли ВыбраннаяПФ.Значение = "ИзменитьДатыОкончания" Тогда
		ПараметрыОткрытия = Новый Структура();
		ПараметрыОткрытия.Вставить("Ссылка", Объект.Ссылка);
		ПараметрыОткрытия.Вставить("ДатаОкончанияДействияАнализовПоМедКнижке", Объект.ДатаОкончанияДействияАнализовПоМедКнижке);
		ПараметрыОткрытия.Вставить("ДатаОкончанияДействияПсихологОсв", Объект.ДатаОкончанияДействияПсихологОсв);
		ОписаниеОповещенияФормы = Новый ОписаниеОповещения("ЗаполнениеДатОкончанияЗавершение", ЭтаФорма);
		ОткрытьФорму("Справочник.ФизическиеЛица.Форма.ЗНК_ЗаполнениеДатОкончания", ПараметрыОткрытия, ЭтаФорма,,,,ОписаниеОповещенияФормы);
	КонецЕсли;	
...
//+++ЗНК Zionec 2018.12.07 ИП-00020645
&НаКлиенте
Процедура ЗаполнениеДатОкончанияЗавершение(Результат, ДопПараметры) Экспорт  
...
//+++ЗНК Zionec 2018.12.07 ИП-00020645
&НаСервере
Функция ПечатьНаправлениеПрофмедлаб(Профосмотр = "") 
	...
	|	ПОДСТРОКА(КонтактнаяИнформация.Представление, 1, 500) КАК Адрес
	|ИЗ
	...
	Иначе
		...
		Если Выборка.Количество() = 1 Тогда 
			Выборка.Следующий();
			ТекущаяДолжность = Выборка.Должность;
		Иначе
			Пока Выборка.Следующий()Цикл
				ТекущаяДолжность = ТекущаяДолжность + ВРег(Лев(СокрЛП(Выборка.Должность),1)) + Сред(СокрЛП(Выборка.Должность),2) + ", ";
			КонецЦикла;
			ТекущаяДолжность = Сред(ТекущаяДолжность,1, СтрДлина(ТекущаяДолжность)-2);
		КонецЕсли;
	КонецЕсли;
	
	Макет = РеквизитФормыВЗначение("Объект").ПолучитьМакет("ЗНК_НаправлениеПрофмедлаб");
	
	ОбластьМакета = Макет.ПолучитьОбласть("Шапка");
    ТабДокумент.Вывести(ОбластьМакета);
	
	ОбластьМакета = Макет.ПолучитьОбласть("Профосмотр" + Профосмотр);
    ТабДокумент.Вывести(ОбластьМакета);
	
	ОбластьМакета = Макет.ПолучитьОбласть("Данные");
	ЗаполнитьЗначенияСвойств(ОбластьМакета.Параметры, Выборка);
	ОбластьМакета.Параметры.ДатаРождения = Формат(Выборка.ДатаРождения,"ДФ=dd.MM.yyyy");
	ОбластьМакета.Параметры.Должность = ТекущаяДолжность;	
	ТабДокумент.Вывести(ОбластьМакета);
...
//+++ЗНК Zionec 2018.12.07 ИП-00020645
&НаСервере
Функция ПечатьНаправлениеНаПсихОсвидетельствование() 

	ТабДок = Новый ТабличныйДокумент;
	Макет = РеквизитФормыВЗначение("Объект").ПолучитьМакет("ЗНК_Направление");
	
	Данные = Новый Структура("Объект, Тип, Вид", Объект.Ссылка, Перечисления.ТипыКонтактнойИнформации.Адрес, Справочники.ВидыКонтактнойИнформации.ЮрАдресФизЛица);
	Результат = РегистрыСведений.КонтактнаяИнформация.Получить(Данные);	
	Адрес = Результат.Представление;
	
	
	ОбластьМакета = Макет.ПолучитьОбласть("Приложение2");
	ОбластьМакета.Параметры.Дата = ТекущаяДата();
	ОбластьМакета.Параметры.ДатаРождения = Формат(Объект.ДатаРождения, "ДФ='дд ММММ гггг'");
	ОбластьМакета.Параметры.Адрес = Адрес;
	ОбластьМакета.Параметры.Должность = Объект.Должность;
	ОбластьМакета.Параметры.ФИО = СокрЛП(Объект.Наименование);
	ОбластьМакета.Параметры.ФИнициалы = Фамилия + " " + Лев(Имя, 1) + "." + Лев(Отчество, 1) + ".";
	ТабДок.Вывести(ОбластьМакета);
	
	ТабДок.ВывестиГоризонтальныйРазделительСтраниц();
	
	ОбластьМакета = Макет.ПолучитьОбласть("Приложение3");
	ОбластьМакета.Параметры.Дата = ТекущаяДата();
	ОбластьМакета.Параметры.ДатаРождения = Формат(Объект.ДатаРождения, "ДФ='дд ММММ гггг'");
	ОбластьМакета.Параметры.Адрес = Адрес;
	ОбластьМакета.Параметры.ФИО = СокрЛП(Объект.Наименование);
	...
	ОбластьМакета = Макет.ПолучитьОбласть("СхемаПроезда");
	ТабДок.Вывести(ОбластьМакета);
