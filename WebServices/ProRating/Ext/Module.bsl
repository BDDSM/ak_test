...
Функция SaveSupplierReview(ФильтрСтрокой, Период, Оценка, Комментарий, РекомендуемДрузьям) Экспорт
	
	ОбработкаОбъект = ПолучитьИПодключитьВнешнююОбработку();
	Если ОбработкаОбъект <> Неопределено Тогда
		Возврат ОбработкаОбъект.SaveSupplierReview(ФильтрСтрокой, Период, Оценка, Комментарий, РекомендуемДрузьям);
	КонецЕсли;
