...
Функция CreateNomenclatureCharacteristics(КарточкаТовараПоставщика, ТаблицаСкладов) Экспорт
	
	ОбработкаОбъект = ПолучитьИПодключитьВнешнююОбработку();
	Если ОбработкаОбъект <> Неопределено Тогда
		Возврат ОбработкаОбъект.CreateNomenclatureCharacteristics(КарточкаТовараПоставщика, ТаблицаСкладов);
	КонецЕсли;
