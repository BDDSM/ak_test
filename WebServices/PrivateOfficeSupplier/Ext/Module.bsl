...
//+++АК SHEP 2018.12.11 ИП-00018753.05
//Создать характеристики номенклатуры
Функция CreateNomenclatureCharacteristics(КарточкаТовараПоставщика, ТаблицаСкладов, МастерДанных) Экспорт
	
	ОбработкаОбъект = ПолучитьИПодключитьВнешнююОбработку();
	Если ОбработкаОбъект <> Неопределено Тогда
		Возврат ОбработкаОбъект.CreateNomenclatureCharacteristics(КарточкаТовараПоставщика, ТаблицаСкладов, МастерДанных);
	КонецЕсли;
