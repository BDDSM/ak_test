...
Функция ПолучитьДанныеШтрихкодаОптимиз(ШтрихКод)
	...
	Возврат ОбменМобильноеПриложениеПереопределяемый.ПолучитьДанныеШтрихкодаОптимиз(ШтрихКод);
	//---АК BELN 
...
Функция ПаллетаНаДебаркадере(ДанныеМобильногоПриложения)
	...
	Исключение
		...
		Возврат Новый ХранилищеЗначения(СтруктураВозврат);
	КонецПопытки;
	Если СтруктураПараметры.Свойство("ДатаНачала") Тогда
		КодОрг="";
		Если СтруктураПараметры.Свойство("Организация") Тогда
			КодОрг=СтруктураПараметры.Организация;
		КонецЕсли;
		//++ХЖК 15.08.2018 ИП-00019563
		СписокВорот = "";
		Если СтруктураПараметры.Свойство("СписокВорот") Тогда 
			СписокВорот = СтруктураПараметры.СписокВорот;
		КонецЕсли;
		//++ХЖК 15.08.2018 ИП-00019563
		ТД=Отчеты.ОтчетКладовщикаНаДебаркадере.СформироватьОтчет(
		СтруктураПараметры.ДатаНачала,СтруктураПараметры.Датаокончания,Справочники.СтруктурныеЕдиницы.НайтиПоКоду(СтруктураПараметры.КодСклада),
		СтруктураПараметры.ОтборСтатус,КодОрг, СписокВорот,РасходныйОрдер);
...
Функция ЗаписатьРасходник(ДанныеМобильногоПриложения)
	...
	//АК БЕЛН 05.10.2017 ИП-00016830-
	Попытка
		Если ТЗТовары.Итог("Количество")=0 ИЛИ ТЗТовары.Итог("Количество")=Неопределено Тогда
			СтруктураВозврат.ОписаниеОшибок = СтруктураВозврат.ОписаниеОшибок + ?(ЗначениеЗаполнено(СтруктураВозврат.ОписаниеОшибок), Символы.ПС, "")
			+ "Пустая таблица товаров";
			СтруктураВозврат.Результат = Ложь;
			Возврат Новый ХранилищеЗначения(СтруктураВозврат);
