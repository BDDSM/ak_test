...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЕстьМетаданныеТипЕдиницы = МодульОбъекта().ЕстьМетаданныеТипЕдиницы;
	...
	ИмяСправочникаКонтрагенты 	= МодульОбъекта().ПолучитьТипЗначенияОбъекта("ЮрФизЛицоСтороннее",,Истина);
	ИмяСправочникаДоговоры 		= МодульОбъекта().ПолучитьТипЗначенияОбъекта("Договор",,Истина);
...
Процедура ЗаполнитьСпискиВыбораКолонок()
	...
	СписокЕдиницEDI 		= МодульОбъекта().ПолучитьСписокЕдиницEDI();
    ЗаполнитьСписокВыбораЭлемента(Элементы.СписокНоменклатурыЕдиницаEDI, СписокЕдиницEDI);
	
	СписокДействийПересчета = МодульОбъекта().ПолучитьСписокДействийПересчета();
    ЗаполнитьСписокВыбораЭлемента(Элементы.СписокНоменклатурыДействиеПересчета, СписокДействийПересчета);
...
Процедура ОформитьПодсказкиЭлементовУправления()
	...
	Элементы.ПризнакНоменклатуры.СписокВыбора.Добавить("Товар");
	Элементы.ПризнакНоменклатуры.СписокВыбора.Добавить("Услуга");
	Элементы.ПризнакНоменклатуры.СписокВыбора.Добавить("Возвратная тара");
	
	Если НЕ ЕстьМетаданныеТипЕдиницы
		И НЕ МодульОбъекта().ВнешнееХранилище Тогда
		Элементы.НадписьПризнакНоменклатурыНастроить.Видимость = Истина;
	КонецЕсли;
	
	Если НЕ ЕстьМетаданныеТипЕдиницы Тогда
		ПризнакНоменклатуры = "Товар";
		Элементы.ПризнакНоменклатуры.Доступность = Ложь;
	Иначе
		Если ТипЕдиницы = "SER" Тогда
			ПризнакНоменклатуры = "Услуга";
		ИначеЕсли ТипЕдиницы = "RC" Тогда
			ПризнакНоменклатуры = "Возвратная тара";
		Иначе
			ПризнакНоменклатуры = "Товар";
		КонецЕсли;	
	КонецЕсли;
...
Процедура ЗаполнитьРеквизитыПересчетаКорректно(СтрокаНаФорме, СтрокаСоответствия)
	...
	Если НЕ ЗначениеЗаполнено(ТипЕдиницы) И ЗначениеЗаполнено(СтрокаСоответствия.ТипЕдиницы) Тогда
		ТипЕдиницы = СтрокаСоответствия.ТипЕдиницы;
	КонецЕсли;
...
Функция СохранитьСоответствиеСервер()
	РезультатСохранения = МодульОбъекта().СоответствиеТоваров_Сохранить(СписокНоменклатуры,GTIN,КодТовараПартнера,Наименование,Параметры.Партнер,ТекущееСоответствие,ТипЕдиницы);//+
	КоличествоОшибок=1;
...
&НаКлиенте
Процедура НадписьПризнакНастроитьНажатие(Элемент)
	
	ТекстВопроса = "Для работы с дополнительными признаками номенклатуры (услуга, возвратная тара) в модуле Контур.EDI требуется обновить конфигурацию:
					| + реквизит ""ТипЕдиницы"" в рег. сведений ""КонтурEDI_СоответствияТоваров"".
					|
					|Сохранить файл обновления на диск?";
	
	ПредложитьОбновитьОбъектыМетаданныхКонтурEDI(, ТекстВопроса);
...
&НаКлиенте
Процедура ПризнакНоменклатурыПриИзменении(Элемент)
	
	Если ПризнакНоменклатуры = "Услуга" Тогда
		ТипЕдиницы = "SER";
	ИначеЕсли ПризнакНоменклатуры = "Возвратная тара" Тогда
		ТипЕдиницы = "RC";
	Иначе
		ТипЕдиницы = "";
	КонецЕсли;	
