...
Процедура ИнициализироватьСтруктуруВХСервер()
	...
	АдресаВХ.Вставить("КлючАвторизации",				ПоместитьВоВременноеХранилище("1C-edi-60703a2a-f03d-47b4-b806-de22a02d8da7",УникальныйИдентификатор));	//Строка
...
Процедура ИнтерфейсныеПравкиДляПлатформы83()
	...
	Если СейчасВыбранТакси Тогда
		...
		Если Найти(Строка(Метаданные.РежимСовместимости), "Версия8_2_") > 0
			...
			И Число(Прав(Строка(Метаданные.РежимСовместимости), 1)) < 7)
			...
			Элементы.ГруппаТПЛевая.Ширина = 20;
			Элементы.ГруппаТППравая.Ширина = 18;
			Элементы.ГруппаНастройки.Ширина = 25;
		...
		ИначеЕсли ((ВерсияПлатформы.Младшая = 3	И ВерсияПлатформы.Релиз >= 7)
			...
			Элементы.ГруппаТП.РастягиватьПоГоризонтали = Истина;
...
Функция ПолучитьДвоичныеДанныеФайлаИнструкцииВызовСервера()
	...
	Попытка
		Соединение = МодульОбъекта().ПолучитьСоединениеHTTP("edi.kontur.ru");
...
Процедура ПриОткрытии(Отказ)
	...
	Если ПериодВСекундах > 2 * 24 * 3600
		...
		НастройкаПериода.ДатаНачала 	= НачалоДня(ТекущаяДата() - 2 * 24 * 3600);
	...
	//+++АК SaMi 2018.04.13 ИП-00018178 
	ЗапуститьАвтообмен();
	//---АК SaMi  2018.04.13 
...
Функция ПолучитьСоединениеHTTPКлиент(Адрес) Экспорт
	...
	Если ВерсияПрограмногоПродуктаРавнаИлиВышеСервер(ВерсияПлатформы, "8.2.19.130") Тогда 
		Выполнить("СоединениеHTTP = Новый HTTPСоединение(Адрес, , , , Прокси, , Новый ЗащищенноеСоединениеOpenSSL)");
	Иначе
		Выполнить("СоединениеHTTP = Новый HTTPСоединение(Адрес, , , , Прокси, Истина)");
	КонецЕсли;
...
&НаСервере
Функция ВерсияПрограмногоПродуктаРавнаИлиВышеСервер(ВерсияПродукта, ЭталоннаяВерсия)
	
	Возврат МодульОбъекта().ВерсияПрограмногоПродуктаРавнаИлиВыше(ВерсияПродукта, ЭталоннаяВерсия);
...
Процедура Удалить_ИнициализироватьРаботуСДиадоком()
	...
	//	ДиадокАПИ.ApiClientId = "1C-edi-60703a2a-f03d-47b4-b806-de22a02d8da7";
...
Процедура ПредоставитьУдаленныйДоступКонтурVNC(Команда)
	...
	СоединениеHTTP_VNC = ПолучитьСоединениеHTTPКлиент("help.kontur.ru"); 
...
Функция ПолучитьВерсиюБСПСервер()
	//+++АК SaMi 2018.04.12 ИП-00018178 
	Возврат "";
	//---АК SaMi  2018.04.12 
...
Процедура ОткрытьФормуУдаленияСообщений(Команда)
	...
	Если Параметры.МодальностьЗапрещена Тогда 
		Выполнить("ОткрытьФормуОбъектаМодально(ПутьКФормам + ""Сервис_УдалениеСообщенийУправляемая"", ПараметрыФормы, ""ОбработчикПослеУдаленияСообщений"")");
	Иначе
		ПолучитьФормуОбработки("Сервис_УдалениеСообщенийУправляемая", ПараметрыФормы).ОткрытьМодально();
		ОбработчикПослеУдаленияСообщений();
	КонецЕсли;
...
Процедура ОПрограмме(Команда)
	...
	Если Параметры.МодальностьЗапрещена Тогда 
		Выполнить("ОткрытьФормуОбъектаМодально(ПутьКФормам + ""Сервиc_ОПрограммеУправляемая"", ПараметрыФормы)");
	Иначе
		ПолучитьФормуОбработки("Сервиc_ОПрограммеУправляемая", ПараметрыФормы).ОткрытьМодально();
	КонецЕсли;
...
Процедура ОткрытьНастройкиМеркурий(Команда)
	...
	МодульОбъектаОбновитьПриНеобходимости();//обновим обработку во временном хранилище, если устарела и получим новый Параметры.АдресХранилища
...
&НаКлиенте
Функция МеркурийПолучитьКлиентскийМодульEDI()
	...
	МеркурийКлиентскийМодульEDI = ПолучитьФормуОбработки("КлиентскийМодульEDI_Управляемая",ПараметрыФормы,,,,ПутьКФормамМеркурий);
...
&НаКлиенте
Функция ОткрытьКарточкуМеркурий(ВыбраннаяСтрока, ПереотправкаСообщения = Ложь, Сообщение=Неопределено )
...
//+++АК SaMi 2018.04.13 ИП-00018178 
&НаКлиенте
Процедура АК_ПериодАвтообменаПриИзменении(Элемент)
...
//+++АК SaMi 2018.04.13 ИП-00018178 
&НаКлиенте
Процедура ЗапуститьАвтообмен()
	
	ОтключитьОбработчикОжидания("ЗапуститьОбмен");
	Если АК_ПериодАвтообмена > 0 Тогда
		ПодключитьОбработчикОжидания("ЗапуститьОбмен", АК_ПериодАвтообмена);
	КонецЕсли; 
