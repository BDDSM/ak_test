...
Процедура УстановитьВидимость()
	...
	Если СообщениеМожетСодержатьДанныеОбУпаковках Тогда
		...
		Если ИспользоватьДопПараметрыУпаковки Тогда
			Элементы.ТранспортныеУпаковкиВесНетто.Видимость 	= Истина;		
			Элементы.ТранспортныеУпаковкиВесБрутто.Видимость 	= Истина;		
			Элементы.ТранспортныеУпаковкиДлина.Видимость 		= Истина;		
			Элементы.ТранспортныеУпаковкиШирина.Видимость 		= Истина;		
			Элементы.ТранспортныеУпаковкиГруппаТемпературныйРежим.Видимость = Истина;		
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьПоляНаФорме()
	...
	Если НЕ Сообщение = Неопределено Тогда
		...
		Если ОтправлятьДанныеОбУпаковках И СообщениеСодержитДанныеОбУпаковках Тогда
			МодульОбъекта().ДополнитьДеревоТранспортныхУпаковокНедостающимиКолонками(Сообщение.ТранспортныеУпаковки);
...
КонецПроцедуры // ПолучитьДанныеДляОтклоненияЗаказаПолностью()
...
Процедура ТоварыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)//!!
	...
	Если НЕ ТолькоПросмотрСообщения
		И ИмяКолонки = "ТоварыРешениеПоВозврату" Тогда
...
Процедура ОбработчикВыбораРешенияПоВозврату(ВыбранноеЗначение, Параметры = Неопределено) Экспорт
	...
	Если НЕ ВыбранноеЗначение = Неопределено Тогда
		...
		//ПроверитьЗаполнениеПолей();
		//НайденныеСтроки = ТаблицаОшибок.НайтиСтроки(Новый Структура("ИмяПоля,НомерСтроки", "РешениеПоВозврату", ТекДанные.НомерСтроки));
...
Функция СоставитьСписокВыбораПоСтрокеТоваров(GTIN,КодТовараПокупателя,Отправитель1С)
	...
	Для Каждого Стр Из СоответствиеТоваров Цикл
		...
		СтруктураТовара.Вставить("ТипЕдиницы",					Стр.ТипЕдиницы);
...
Процедура ТранспортныеУпаковкиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ТранспортныеУпаковкиСоставУпаковки" И Поле.Доступность Тогда
		...
		ОбновитьДанныеРазмещения();
	ИначеЕсли ЗначениеЗаполнено(Элемент.ТекущиеДанные.ТипУпаковки) 
		И Поле.Имя = "ТранспортныеУпаковкиКоличество" Тогда
		СтандартнаяОбработка = Ложь;
	ИначеЕсли НЕ ЗначениеЗаполнено(Элемент.ТекущиеДанные.ТипУпаковки) 
...
Процедура ДобавитьОсобенностьШапки(Особенность) //Особенность - строка таблицы Особенностей
	...
	ПутьВСтруктуре 	= ?(ЗначениеЗаполнено(Особенность.ИмяСтруктуры), 
						Особенность.ИмяСтруктуры + "." + Особенность.Идентификатор, 
						Особенность.Идентификатор);
						
	Если ЗначениеЗаполнено(Особенность.ИмяСтруктуры) Тогда
		Если МодульОбъекта().ПроверитьСвойствоПоляВСтруктуре(Сообщение, ПутьВСтруктуре, ЗначениеСвойстваСообщения) Тогда 
			НоваяОсобенность.Значение 	= ЗначениеСвойстваСообщения;
		КонецЕсли;	
	ИначеЕсли Сообщение.Свойство(Особенность.Идентификатор,ЗначениеСвойстваСообщения) Тогда
		НоваяОсобенность.Значение 	= ЗначениеСвойстваСообщения;
	КонецЕсли;
...
Процедура ДобавитьОсобенностьТоваров(Особенность)
	...
	Для каждого РеквизитТоваров Из ЭтаФорма.ПолучитьРеквизиты("Товары") Цикл
		Если РеквизитТоваров.Имя = Особенность.Идентификатор Тогда
	...
	Если НЕ РеквизитДобавлен Тогда	
		...
		НовыеРеквизиты.Добавить(Новый РеквизитФормы(Особенность.Идентификатор, ТипЗначенияКолонкиОсобенность, "Товары", Особенность.Идентификатор));
		ИзменитьРеквизиты(НовыеРеквизиты);
	КонецЕсли;
	
	КолонкаНаФорме = Элементы.Найти("Товары" + Особенность.Идентификатор);
...
Процедура ПеренестиОсобенностиВСообщение()
	...
	Для каждого ОсобенностьСообщения Из ОсобенностиСообщений Цикл
		...
		ЭтоРеквизитТоваров = (НЕ Найти(ОсобенностьСообщения.Путьxml,"lineItem.") = 0);
		
		Если ЭтоРеквизитТоваров Тогда
		...
		Иначе
			...
			МассивОсобенностей 	= 	ВремОсобенности.НайтиСтроки(Новый Структура("Идентификатор,ИмяСтруктуры", 
																ОсобенностьСообщения.Идентификатор, 
																ОсобенностьСообщения.ИмяСтруктуры));
			Если МассивОсобенностей.Количество() = 1 Тогда
				НайденнаяОсобенность = МассивОсобенностей[0];
			Иначе
				НайденнаяОсобенность = Неопределено;
			КонецЕсли;
			
			Если НЕ НайденнаяОсобенность = Неопределено Тогда
				
				ПутьВСтруктуре 	= ?(ЗначениеЗаполнено(ОсобенностьСообщения.ИмяСтруктуры), 
									ОсобенностьСообщения.ИмяСтруктуры + "." + ОсобенностьСообщения.Идентификатор, 
									ОсобенностьСообщения.Идентификатор);
				
				Если ЗначениеЗаполнено(ОсобенностьСообщения.ИмяСтруктуры) Тогда 
					Если МодульОбъекта().ПроверитьСвойствоПоляВСтруктуре(Сообщение, ПутьВСтруктуре) Тогда
						Сообщение[ОсобенностьСообщения.ИмяСтруктуры].Вставить(ОсобенностьСообщения.Идентификатор, НайденнаяОсобенность.Значение);
					КонецЕсли;	
				Иначе
...
Процедура ОсобенностиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ОсобенностиЗначение"
		И ЗначениеЗаполнено(Элемент.ТекущиеДанные.СписокВыбора) Тогда
		
		Если Параметры.МодальностьЗапрещена Тогда 
			Выполнить("ПоказатьВыборИзСписка(Новый ОписаниеОповещения(""СписокЗначенийОсобенностиВыборЗавершение"", ЭтаФорма), Элемент.ТекущиеДанные.СписокВыбора, Элемент)");
		Иначе
			ВыбранноеЗначение = ВыбратьИзСписка(Элемент.ТекущиеДанные.СписокВыбора, Элемент);
