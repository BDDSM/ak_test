...
Функция ПеренестиВЗаказСервер()
	...
	Возврат МодульОбъекта().ОбработкаСобытияПодключаемогоМодуля("ЗагрузитьРасхождения", , Новый Структура("ДокументСсылка,ТаблицаСравнения,ТипСообщения, ВызовИзФормы", Документ, ТабСоответствий,"ORDERS", Истина));
	//---АК SaMi  2018.04.12 
...
Функция ПеренестиВДокументРеализацияСервер()
	...
	//+++АК SaMi 2018.04.12 ИП-00018178 
	Возврат МодульОбъекта().ОбработкаСобытияПодключаемогоМодуля("ЗагрузитьРасхождения", , Новый Структура("ДокументСсылка,ТаблицаСравнения,ТипСообщения, ВызовИзФормы", Документ, ТабСоответствий,"RECADV", Истина));
	//---АК SaMi  2018.04.12 
...
Процедура ЗаполнитьТоварыДокумента_Возврат(Док,ТабСоответствий,ВариантЗаполнения)
	...
	Для Каждого СтрокаСоответствий Из ТабСоответствий Цикл
		...
		ИмяТабЧасти = ?(МодульОбъекта().ИмяКонфигурации1С = "УФ_УНФ" , "Запасы" , "Товары"); 
		//поищем строку с таким товаром в документе
		НайденнаяСтрока = МодульОбъекта().НайтиСоответствиеСтрокиВДокументе(СтрокаСоответствий, Док, ИмяТабЧасти);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		СверятьВRECADVЦеныИСтавки 		= (МодульОбъекта().ПолучитьЗначениеСвойстваОбъектаEDI(Партнер, "СверятьВRECADVЦеныИСтавки") = Истина);
		ОтправлятьВозвратнуюТаруВDESADV	= (МодульОбъекта().ПолучитьЗначениеСвойстваОбъектаEDI(Партнер, "ОтправлятьВозвратнуюТаруВDESADV") = Истина);
		РаботаСУслугами 				= (МодульОбъекта().ПолучитьЗначениеСвойстваОбъектаEDI(Партнер, "РаботаСУслугами") = Истина);
...
Процедура ЗаполнитьТоварыДокумента_Реализация_Заказ(Док,ТабСоответствий,ВариантЗаполнения)
	...
	Для Каждого СтрокаСоответствий Из ТабСоответствий Цикл
		...
		ИначеЕсли НЕ СтрокаСоответствий.КоличествоПринято = 0 Тогда
			...
			Если НЕ МодульОбъекта().Имяконфигурации1С = "УФ_БП" Тогда
