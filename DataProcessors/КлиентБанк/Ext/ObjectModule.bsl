...
Функция ПривестиЗначение(Значение)
	...
	Если ТипЗнч(Значение) = Тип("Строка") Тогда
		...
		//+++АК LAGP 2018.12.13 ИП-00020638 Замена кавычек «» на человеческие "
		ЗначениеОбработанное = Значение;
		ЗначениеОбработанное = СтрЗаменить(ЗначениеОбработанное, "«", """");
		ЗначениеОбработанное = СтрЗаменить(ЗначениеОбработанное, "»", """");
		//---АК LAGP
		
		Возврат СокрЛП(ЗначениеОбработанное);
...
//
Функция НайтиДоговорКонтрагента(ВладелецДоговора, ОрганизацияДоговора, СписокВидовДоговора = Неопределено, СтруктураПараметров = Неопределено)
	...
	//+++АК LAGP 2018.12.10 ИП-00020300 Депозиты ТИЛСИ /Возврат /перечисление %
	Если ЗначениеЗаполнено(СтруктураПараметров) Тогда
		Для каждого ЭлементСтруктурыПараметров Из СтруктураПараметров Цикл
			...
			|	И ДоговорыКонтрагентов." + ЭлементСтруктурыПараметров.Ключ + " = &" + ЭлементСтруктурыПараметров.Ключ;
	...
	Если ТипДокумента = "РасходИзБанка" Тогда
		...
		ИначеЕсли ОбъектДляЗаписи.СтатьяДвиженияДенежныхСредств.Код = "139391" Тогда //Услуги банка прочие постоянные
			...
			Если Найти(НП,"комиссиявнутри")>0 ИЛИ Найти(НП,"комиссиязаперечислениесредств")>0 ИЛИ Найти(НП,"комиссиявдруг")>0
			 //+++АК ILIK 2018.11.16 ИП-00020354, 2018.12.06 ИП-00020354.01
			 ...
			 Или Найти(НП, "платазавыдачуналичныхкчеку") > 0 Или Найти(НП, "комиссиязаорганизациюобслуживаниясиспользованиемсистемы""сбербанкбизнесонл@йн""") > 0
			 Или Найти(НП, "комиссиязаведениесчета") > 0 Или Найти(НП, "комиссияинобанкасогласнодокументу") > 0 Тогда
			 //---АК ILIK
...
//СуммаДокумента
	...
	Если ОбъектДокумента.ВидОперации = Перечисления.ВидыОперацийППИсходящее.РазмещениеДепозита Тогда
		...
		ОбъектДокумента.СтатьяДвиженияДенежныхСредствБУ = ОбщегоНазначенияПовтИсп.ВернутьСтатьюДДСПоУсловию("000000011", Истина);		
		//---АК LAGP
...
//НазначениеПлатежа
	...
	Если ОбъектДокумента.ВидОперации = Перечисления.ВидыОперацийПоступлениеБезналичныхДенежныхСредств.ВозвратДепозита Тогда
		...
		ОбъектДокумента.СчетУчетаРасчетовСКонтрагентомБУ = ПланыСчетов.Хозрасчетный.НайтиПоКоду("55.03");
		Если НЕ ЗначениеЗаполнено(ОбъектДокумента.СтатьяДвиженияДенежныхСредствБУ) Тогда  //+++АК LAGP 2018.12.10 ИП-00020300 Депозиты ТИЛСИ /Возврат /перечисление %. Статья определяется регистром Соответствия статей. Оставил код, но добавлено условие заполненности.
...
//				Если Не ОбщегоНазначения.ЕстьНеЦифры(СтрокаДокумента.Номер) Тогда
	...
	Если НЕ ЗначениеЗаполнено(СтрокаДокумента.ВидОперации) Тогда
		...
		//+++АК LAGP 2018.12.10 ИП-00020300 Депозиты ТИЛСИ /Возврат /перечисление %
		ЭтоРазмещениеДепозита 	= (ВидДокумента = "РасходИзБанка"
									И Найти(НРег(СтрокаДокумента.НазначениеПлатежа), "размещение") > 0
									И Найти(НРег(СтрокаДокумента.НазначениеПлатежа), "депозит") > 0);
		ЭтоВозвратДепозита 		= (ВидДокумента = "ПоступлениеВБанк"
									И Найти(НРег(СтрокаДокумента.НазначениеПлатежа), "возврат") > 0
									И Найти(НРег(СтрокаДокумента.НазначениеПлатежа), "депозит") > 0);
		ЭтоПроцентПоДепозиту 	= (ВидДокумента = "ПоступлениеВБанк"
									И Найти(НРег(СтрокаДокумента.НазначениеПлатежа), "процент") > 0
									И Найти(НРег(СтрокаДокумента.НазначениеПлатежа), "депозит") > 0);							
		//---АК LAGP
		...
		Если Исходяший Тогда
			...
			//+++АК LAGP 2018.12.10 ИП-00020300 Депозиты ТИЛСИ /Возврат /перечисление %
			ИначеЕсли ЭтоРазмещениеДепозита Тогда
				ВидОперацииДокумента = ПредопределенноеЗначение("Перечисление.ВидыОперацийППИсходящее.РазмещениеДепозита");
				СтрокаДокумента.СтатьяДДС 			= ОбщегоНазначенияПовтИсп.ВернутьСтатьюДДСПоУсловию("310310"); 				//Выдача займов
				СтрокаДокумента.СтатьяДДС_Бух 		= ОбщегоНазначенияПовтИсп.ВернутьСтатьюДДСПоУсловию("000000011", Истина); 	//Депозиты
		...
		Иначе
			...
			//+++АК LAGP 2018.12.10 ИП-00020300 Депозиты ТИЛСИ /Возврат /перечисление %
			ИначеЕсли ЭтоВозвратДепозита Тогда
				ВидОперацииДокумента = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеБезналичныхДенежныхСредств.ВозвратДепозита");
				СтрокаДокумента.СтатьяДДС 			= ОбщегоНазначенияПовтИсп.ВернутьСтатьюДДСПоУсловию("310410"); 				//Возврат займов
				СтрокаДокумента.СтатьяДДС_Бух 		= ОбщегоНазначенияПовтИсп.ВернутьСтатьюДДСПоУсловию("000000011", Истина); 	//Депозиты
				СтрокаДокумента.Контрагент 			= Справочники.Контрагенты.НайтиПоКоду("Т0002799"); //ПАО ВТБ
			ИначеЕсли ЭтоПроцентПоДепозиту Тогда
				ВидОперацииДокумента = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеБезналичныхДенежныхСредств.ПоступлениеПроцентовПоДепозиту");
				СтрокаДокумента.СтатьяДДС 			= ОбщегоНазначенияПовтИсп.ВернутьСтатьюДДСПоУсловию("310410"); 				//Возврат займов
				СтрокаДокумента.СтатьяДДС_Бух 		= ОбщегоНазначенияПовтИсп.ВернутьСтатьюДДСПоУсловию("000000017", Истина); 	//Проценты по займам/депозитам полученные
				СтрокаДокумента.Контрагент 			= Справочники.Контрагенты.НайтиПоКоду("Т0002799"); //ПАО ВТБ
			//---АК LAGP	
...
//			СтрокаДокумента.ВидОперации = Результат.ВидОперации;
		...
		Если ВсеРеквизитыПоискаЕсть Тогда
			...
			Пока Результат.Следующий() Цикл
				...
				Если ((ВыборкаНомер = НомерДляПоискаДок)
					...
					НазначениеСовпадает = Ложь;
					СтрокаНазначенияВБазеСЦифрами = ОбщегоНазначенияПовтИсп.ВернутьСтрокуПоУсловию(Результат.Ссылка.НазначениеПлатежа, ЛОЖЬ, ИСТИНА);
					СтрокаНазначенияВВыпискеСЦифрами = ОбщегоНазначенияПовтИсп.ВернутьСтрокуПоУсловию(СтрокаДокумента.НазначениеПлатежа, ЛОЖЬ, ИСТИНА);
					Если (СтрокаНазначенияВБазеСЦифрами = СтрокаНазначенияВВыпискеСЦифрами) ИЛИ НЕ ЗначениеЗаполнено(Результат.Ссылка.НазначениеПлатежа) ИЛИ НЕ ЗначениеЗаполнено(СтрокаДокумента.НазначениеПлатежа) Тогда
						НазначениеСовпадает = Истина;
					КонецЕсли;
					
					Если НЕ НазначениеСовпадает Тогда
						СтрокаНазначенияВБазеСБуквами = ОбщегоНазначенияПовтИсп.ВернутьСтрокуПоУсловию(Результат.Ссылка.НазначениеПлатежа, ИСТИНА, ИСТИНА);
						СтрокаНазначенияВВыпискеСБуквами = ОбщегоНазначенияПовтИсп.ВернутьСтрокуПоУсловию(СтрокаДокумента.НазначениеПлатежа, ИСТИНА, ИСТИНА);
						Если НЕ СтрДлина(СтрокаНазначенияВБазеСБуквами) = СтрДлина(СтрокаНазначенияВВыпискеСБуквами) Тогда
							Если СтрДлина(СтрокаНазначенияВБазеСБуквами) > СтрДлина(СтрокаНазначенияВВыпискеСБуквами) Тогда
								СтрокаНазначенияВБазеСБуквами = Лев(СтрокаНазначенияВБазеСБуквами, СтрДлина(СтрокаНазначенияВВыпискеСБуквами));		
							Иначе
								СтрокаНазначенияВВыпискеСБуквами = Лев(СтрокаНазначенияВВыпискеСБуквами, СтрДлина(СтрокаНазначенияВБазеСБуквами));
							КонецЕсли;
						КонецЕсли;	
						
						Если СтрокаНазначенияВБазеСБуквами = СтрокаНазначенияВВыпискеСБуквами Тогда
							НазначениеСовпадает = Истина;	
						КонецЕсли;
					КонецЕсли;	
					//---АК LAGP
					
					Если КоличествоДок = 0 И НазначениеСовпадает Тогда //+++АК LAGP 2018.11.26 Оптимизация. От МКБ часто приходят совпадающие входящие номера, проверка назначения добавлена
						СтрокаДокумента.Документ = Результат.Ссылка;
						СтрокаДокумента.Проведен = Результат.Проведен;
						СтрокаДокумента.НомерДок = Результат.Номер;
						СтрокаДокумента.ДатаДок = Результат.Дата;
	...
	//17)Дополнительная обработка
	
	//+++АК LAGP 2018.12.10 ИП-00020300 Депозиты ТИЛСИ /Возврат /перечисление %     ЭтоПроцентПоДепозиту  ЭтоВозвратДепозита
	Если ЭтоРазмещениеДепозита ИЛИ ЭтоВозвратДепозита ИЛИ ЭтоПроцентПоДепозиту Тогда
		СтруктураДополнительныхПараметров = Новый Структура;
		ДатаДоговораНайдена = Истина;
		Если ЭтоРазмещениеДепозита Тогда
			СтруктураДополнительныхПараметров.Вставить("Дата", СтрокаДокумента.ДатаДок);
		Иначе
			ДатаИзНазначения = Неопределено;
			
			СтрокаЦифрИТочекИзНазначения = ОбщегоНазначенияПовтИсп.ВернутьСтрокуПоУсловию(СтрокаДокумента.НазначениеПлатежа, Ложь, Истина, ".");
			
			Пока Найти(СтрокаЦифрИТочекИзНазначения, ".") > 2 И ДатаИзНазначения = Неопределено Цикл //получает последнюю дату в строке формата дд.ММ.гггг
				Если Сред(СтрокаЦифрИТочекИзНазначения, Найти(СтрокаЦифрИТочекИзНазначения, ".") + 3, 1) = "." Тогда
					СтрокаСДатой = Сред(СтрокаЦифрИТочекИзНазначения, Найти(СтрокаЦифрИТочекИзНазначения, ".") - 2, 10);
					Если СтрЧислоВхождений(СтрокаСДатой, ".") = 2 Тогда
						СтрокаГод 	= Прав(СтрокаСДатой, 4);
						СтрокаМесяц = Сред(СтрокаСДатой, 4, 2);
						СтрокаДень 	= Лев(СтрокаСДатой, 2);
						СтрокаЦифрИТочекИзНазначения = Прав(СтрокаЦифрИТочекИзНазначения, СтрДлина(СтрокаЦифрИТочекИзНазначения) - Найти(СтрокаЦифрИТочекИзНазначения, ".") - 5);
						Попытка
							ДатаИзНазначения = Дата(СтрокаГод, СтрокаМесяц, СтрокаДень);	
						Исключение
						КонецПопытки;
					Иначе
						СтрокаЦифрИТочекИзНазначения = Прав(СтрокаЦифрИТочекИзНазначения, СтрДлина(СтрокаЦифрИТочекИзНазначения) - Найти(СтрокаЦифрИТочекИзНазначения, "."));	
					КонецЕсли;	
				Иначе
					СтрокаЦифрИТочекИзНазначения = Прав(СтрокаЦифрИТочекИзНазначения, СтрДлина(СтрокаЦифрИТочекИзНазначения) - Найти(СтрокаЦифрИТочекИзНазначения, "."));	
				КонецЕсли;	
			...
			Если ДатаИзНазначения = Неопределено Тогда
				Сообщить("Договор депозита не найден, т.к. не определена дата из назначения: " + СтрокаДокумента.НазначениеПлатежа);
				ДатаДоговораНайдена = Ложь;
			КонецЕсли;	
			
			СтруктураДополнительныхПараметров.Вставить("Дата", ДатаИзНазначения);
		КонецЕсли;	
			
		Если НЕ ЭтоПроцентПоДепозиту Тогда
			СтруктураДополнительныхПараметров.Вставить("СуммаДоговора", СтрокаДокумента.СуммаДокумента);
		КонецЕсли;	
		СписокВидовДоговоров = Новый Массив;
		СписокВидовДоговоров.Добавить(ПредопределенноеЗначение("Перечисление.ВидыДоговоровКонтрагентов.Финансовый"));	
		                             
		Если ЗначениеЗаполнено(СтрокаДокумента.СчетОрганизации) И ДатаДоговораНайдена Тогда
			СтрокаДокумента.ДоговорКонтрагента  = НайтиДоговорКонтрагента(СтрокаДокумента.Контрагент, ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаДокумента.СчетОрганизации, "Владелец"), СписокВидовДоговоров, СтруктураДополнительныхПараметров);
		КонецЕсли;	
	КонецЕсли;	
	//---АК LAGP
