		...
		Если ВсеРеквизитыПоискаЕсть Тогда
			...
			Пока Результат.Следующий() Цикл
				...
				Если ((ВыборкаНомер = НомерДляПоискаДок)
					...
					СтрокаНазначенияВБазе = ОбщегоНазначенияПовтИсп.ВернутьСтрокуПоУсловию(Результат.Ссылка.НазначениеПлатежа, ЛОЖЬ, ИСТИНА);
					СтрокаНазначенияВВыписке = ОбщегоНазначенияПовтИсп.ВернутьСтрокуПоУсловию(СтрокаДокумента.НазначениеПлатежа, ЛОЖЬ, ИСТИНА);
