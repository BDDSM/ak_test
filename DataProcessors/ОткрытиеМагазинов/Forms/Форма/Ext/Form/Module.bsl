...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьЗакрытыеТорговыеТочки();				
	//---AK
...
Процедура НастроитьОповещение(Команда)
	...
	//+++AK GREK 12.12.2018 ИП-00020570
	СписокГрупп.Добавить(ПредопределенноеЗначение("Справочник.АК_ГруппыРассылки.ИнформированиеОЗакрытииМагазина"));				
	//---AK
...
//+++AK GREK 12.12.2018 ИП-00020570
&НаКлиенте
Процедура КомандаОбновитьТаблицуЗакрытыеТорговыеТочки(Команда)
	Объект.ЗакрытыеТорговыеТочки.Очистить();
	ЗаполнитьЗакрытыеТорговыеТочки();
КонецПроцедуры
...
//+++AK GREK 12.12.2018 ИП-00020570
&НаСервере
Процедура ЗаполнитьЗакрытыеТорговыеТочки()
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	СтруктурныеЕдиницы.Ссылка КАК Магазин
	               |ИЗ
	               |	Справочник.СтруктурныеЕдиницы КАК СтруктурныеЕдиницы
	               |ГДЕ
	               |	СтруктурныеЕдиницы.СтатусТорговойТочки = &СтатусЗакрыт";
	Запрос.УстановитьПараметр("СтатусЗакрыт", ПредопределенноеЗначение("Перечисление.СтатусыТорговыхТочек.Закрыт"));
	Рез = Запрос.Выполнить();
	Если НЕ Рез.Пустой() Тогда
		Объект.ЗакрытыеТорговыеТочки.Загрузить(Рез.Выгрузить());
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ЗакрытьМагазин(Команда)
	СтруктураПараметров = Новый Структура("УказыватьДопИнформациюПриЗакрытии", Истина);
	ОткрытьФорму("Обработка.ОповещенияОЗакрытииМагазинов.Форма.Форма", СтруктураПараметров, ЭтаФорма);
КонецПроцедуры
