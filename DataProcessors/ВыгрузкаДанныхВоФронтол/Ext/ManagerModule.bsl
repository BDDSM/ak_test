...
Процедура ВыгрузитьДанныеВоФронтол(ТабКВыгрузке, СтруктурнаяЕдиница = Неопределено) Экспорт
	...
	ЗапросАдреса = Новый Запрос(
		...
		|	И СтруктурныеЕдиницы.ТипРозничнойТочки В (ЗНАЧЕНИЕ(Перечисление.ТипыРозничныхТочек.Магазин), ЗНАЧЕНИЕ(Перечисление.ТипыРозничныхТочек.Микромаркет))
	...
	Пока Выборка.Следующий() Цикл
		...
		//---АК mika
			...
			ФайлСПользователями.УстановитьТекст(ФайлВыгрузкиБезПользователей.ПолучитьТекст());
