...
Процедура ВыгрузитьДанныеВоФронтол_(ТабКВыгрузке) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		//---АК mika
			Попытка
				//+++АК BELN 2018.12.06 ИП-00020532.01 
				Если Выборка.ТипРозничнойТочки = Перечисления.ТипыРозничныхТочек.ПространствоВкусвилл Тогда
...
Процедура ВыгрузитьДанныеВоФронтол(ТабКВыгрузке, СтруктурнаяЕдиница = Неопределено) Экспорт
	...
	ЗапросАдреса = Новый Запрос(
		...
		|	И СтруктурныеЕдиницы.ТипРозничнойТочки В (ЗНАЧЕНИЕ(Перечисление.ТипыРозничныхТочек.Магазин), ЗНАЧЕНИЕ(Перечисление.ТипыРозничныхТочек.Микромаркет), ЗНАЧЕНИЕ(Перечисление.ТипыРозничныхТочек.ПространствоВкусвилл))
