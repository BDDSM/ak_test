...
Процедура ОбновитьГрафикНаСервере()
	...
	|ИЗ
	|	РегистрСведений.СпецификаФормированияТабеляПекарей КАК СпецификаФормированияТабеля
	|ГДЕ
	...
	Если ВариантПостроения = 1 Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "РегистрСведений.СпецификаФормированияТабеляПекарей", "РегистрСведений.СпецификаФормированияТабеляПекарейПомощники");
	КонецЕсли;
	//---АК mika
	...
	|ИЗ
	|	РегистрСведений.ТабельРаботыПекарей КАК ТабельРаботы
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СпецификаФормированияТабеляПекарей КАК СпецификаФормированияТабеля
...
Процедура ОбновитьГрафикНаСервереСКД() //+++АК mika 2018.08.20 ИП-00019541
	...
	//Если ВариантПостроения = 1 Тогда  //+++АК mika 2018.08.22 ИП-00019541 Формирования порядка всегда в разрезе групп помощников
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "РегистрСведений.СпецификаФормированияТабеляПекарей", "РегистрСведений.СпецификаФормированияТабеляПекарейПомощники");
	//КонецЕсли;
...
Процедура ДобавитьПродавца(Команда)
	...
	Результат = ОткрытьФормуМодально("Обработка.ГрафикРаботыПекарей.Форма.ФормаВыбораФизическогоЛица", ПараметрыОтбора);
...
Процедура ДобавитьПродавцаСлева(Команда)
	...
	Если ТипЗнч(Расшифровка) <> Тип("ФиксированнаяСтруктура") тогда
		Предупреждение("Встаньте в колонку с пекарем относительно которой необходимо выполнить действие. Операция не выполнена.");
		Возврат;
	КонецЕсли;
...
Процедура ОткрытьОтчетПоОтклонениюГрафиковРаботыОтРеальныхВыходов(Команда)
	...
	ОткрытьФорму("Отчет.ОтклонениеГрафиковРаботыПекарейОтРеальныхВыходов.Форма.ФормаОтчета",, ЭтаФорма);	
...
Процедура ОткрытьФормуСпискаГрафика(Команда)
	...
	Форма = ПолучитьФорму("РегистрСведений.ТабельРаботыПекарей.Форма.ФормаСписка");
...
Процедура ПечатьБейджев(Команда)
	...
	Результат = ОткрытьФормуМодально("Обработка.ГрафикРаботыПекарей.Форма.ФормаПодбораПродавцов", ПараметрыОткрытия);
...
Функция ПолучитьТабличныйДокумент(МассивФизическихЛиц)
	...
	Возврат Обработки.ГрафикРаботыПекарей.СформироватьТабличныйДокумент(МассивФизическихЛиц);
...
Процедура ПоставитьСотрудникаПеред(Команда)
	...
	Если ТипЗнч(Расшифровка) <> Тип("ФиксированнаяСтруктура") тогда
		Предупреждение("Встаньте в колонку с пекарем относительно котрой необходимо выполнить действие. Операция не выполнена.");
		Возврат;
	КонецЕсли;
...
Процедура ГруппаТорговыхТочекНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Результат = ОткрытьФормуМодально("Обработка.ГрафикРаботыПекарей.Форма.ФормаВыбораСтруктурнойЕдиницы", ПараметрыОтбора);
...
Процедура ПроверитьРаботуСотрудниковВДругихГруппах(ТЗ, ГруппаТорговыхТочек, Отказ)
	...
	|ИЗ
	|	ВТ КАК ВТ
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ТабельРаботыПекарей КАК ТабельРаботы
...
Процедура ЗаписатьДанныеОПорядкеВывода()
	...
	Пока НачальнаяКолонка <= КонечнаяКолонка Цикл
		...
		//+++АК mika 2018.07.17 ИП-00019209
		//Запись = РегистрыСведений.СпецификаФормированияТабеляПекарей.СоздатьМенеджерЗаписи();
		Если ВариантПостроения = 1 Тогда
			Запись = РегистрыСведений.СпецификаФормированияТабеляПекарейПомощники.СоздатьМенеджерЗаписи();
		Иначе
			Запись = РегистрыСведений.СпецификаФормированияТабеляПекарей.СоздатьМенеджерЗаписи();
		КонецЕсли;
		//---АК mika 
...
Функция ЗаписатьИзмененияВРегистр(ПараметрыСотрудника)
	...
	Если ЗначениеЗаполнено(ПараметрыСотрудника.Период) И ЗначениеЗаполнено(ГруппаТорговыхТочек) И ЗначениеЗаполнено(ПараметрыСотрудника.Сотрудник)Тогда
		...
		НЗ = РегистрыСведений.ТабельРаботыПекарей.СоздатьНаборЗаписей();
	...
	Если ЗначениеЗаполнено(ПараметрыСотрудника.ТорговаяТочка) ИЛИ ЗначениеЗаполнено(ПараметрыСотрудника.Отсутствие)   ИЛИ ЗначениеЗаполнено(ПараметрыСотрудника.Тренинг) Тогда	
		...
		Запись = РегистрыСведений.ТабельРаботыПекарей.СоздатьМенеджерЗаписи();
		Запись.Группа 	 = ГруппаТорговыхТочек;
	...
	//ЗаписьЖурнала = РегистрыСведений.ЖурналИзмененийТабеляРаботыПекарей.СоздатьМенеджерЗаписи();
...
Процедура ИзменитьПорядокПродавцовВТабеле_Отключена(НовоеФизЛицо, ТекущееФизЛицо) //+++АК mika 2018.08.22 ИП-00019541 процедура отключена
	...
	//+++АК mika 2018.07.17 ИП-00019209
	//НаборЗаписей = РегистрыСведений.СпецификаФормированияТабеляПекарей.СоздатьНаборЗаписей();
	Если ВариантПостроения = 1 Тогда
		НаборЗаписей = РегистрыСведений.СпецификаФормированияТабеляПекарейПомощники.СоздатьНаборЗаписей();
	Иначе
		НаборЗаписей = РегистрыСведений.СпецификаФормированияТабеляПекарей.СоздатьНаборЗаписей();
	КонецЕсли;
	//---АК mika 
...
Процедура СкопироватьГрафикСДругогоПродавца(Команда)
	...
	Если ЗначениеЗаполнено(Результат)Тогда
		...
		ДанныеКопирования = ОткрытьФормуМодально("Обработка.ГрафикРаботыПекарей.Форма.ФормаКопированияГрафика", ПараметрыОткрытия, ЭтаФорма);
...
Процедура СкопироватьГрафикСДругогоПродавцаНаСервере(ДанныеКопирования)
	...
	Иначе
		Сообщить("Пекарь с которого производится копирование не найден в графике");
		Возврат;
	КонецЕсли;
...
Процедура ОчиститьЗаписиПоСтаройГруппеТорговыхТочек(ПараметрыСотрудника) //+++АК mika 2017.02.09 ИП-00016863.02
	...
	Если ЗначениеЗаполнено(ПараметрыСотрудника.Период) И ЗначениеЗаполнено(ПараметрыСотрудника.ГруппаСтарая) И ЗначениеЗаполнено(ПараметрыСотрудника.Сотрудник)Тогда
		...
		НаборЗаписей = РегистрыСведений.ТабельРаботыПекарей.СоздатьНаборЗаписей();
...
Процедура ОбновитьТаблицуСпецификиФормирования() //+++АК mika 2018.08.22 ИП-00019541
	...
	|	СпецификаФормированияТабеля.Порядок КАК Порядок
	|ИЗ
	|	РегистрСведений.СпецификаФормированияТабеляПекарейПомощники КАК СпецификаФормированияТабеля
	|ГДЕ
	|	СпецификаФормированияТабеля.Группа = &Группа
	|
	|УПОРЯДОЧИТЬ ПО
