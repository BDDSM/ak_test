...
Процедура ЗаполнитьСписокНовинокНаСервере()
	...
	МассивНовинок = Обработки.НародныйГурман.ПолучитьМассивИдТоваров(Истина);
...
Процедура ЗаполнитьСписокНеНовинокНаСервере()
	...
	МассивНеНовинок = Обработки.НародныйГурман.ПолучитьМассивИдТоваров(Ложь);
	...
	|	И НЕ (Номенклатура.Наименование ПОДОБНО ""Зам. %"" ИЛИ Номенклатура.Наименование ПОДОБНО ""Эт %"")
	|
	|СГРУППИРОВАТЬ ПО
	|	Номенклатура.Ссылка,
	|	ЕСТЬNULL(НГОтзывыСобраны.ОтзывыСобраны, ЛОЖЬ)
