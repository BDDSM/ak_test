...
Процедура СформироватьЭтикеткиНаСервере()
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ТЗ", ТаблицаТоваров.Выгрузить());
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ТЗ.Номенклатура,
	|	ТЗ.Характеристика,
	...
	Для Каждого Выб Из ТаблицаТоваров Цикл
		...
		ТекстНадШтрихКодом = Строка(СтрокиКеш[0].Значение)
				+ Символы.ПС + ?(ЗначениеЗаполнено(выб.ДатаПроизводства), " Дата произв. " + Формат(выб.ДатаПроизводства, "ДФ=dd.MM.yy"), "00.00.00")
				+ Символы.ПС +?(ЗначениеЗаполнено(выб.КоличествоКоробок), " Количество коробок " + Формат(выб.КоличествоКоробок, "ДФ=dd.MM.yy"), "0");
		Область = Макет.ПолучитьОбласть("Этикетка");
		Область.Параметры.Товар = Выб.Номенклатура.НаименованиеПолное;
		Область.Параметры.Заголовок = ТекстНадШтрихКодом;
		
		
		ШтрихКод = "88" + Прав(СтрокиКеш[0].Штрихкод,СтрДлина(СтрокиКеш[0].Штрихкод)-2)+Строка(Формат(Выб.КоличествоКоробок, "ЧЦ=5; ЧВН=; ЧГ="));
	...
	Этикетки.КлючПараметровПечати = "ПараметрыПечати_ЭтикеткиБольшихКоробок";
