...
Функция ЭтоПомощникУправляющего()
	...
	Возврат (НЕ Результат.Пустой())
				ИЛИ ПараметрыСеанса.ТекущийПользователь.ПросмотрВсехМагазиновВНовойВкусвилл;
...
Функция ЭкстренныйВызовДоступен()
	...
	Возврат ЭтоУправляющий()
				ИЛИ ЭтоПомощникУправляющего()
				ИЛИ РольДоступна("ПолныеПрава")
				ИЛИ РольДоступна("АК_РаботаСГрафикамиПродавцов");
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("СвойствоСотрудника")Тогда
		...
		ТекущееСвойство    = СвойствоСотрудника; //+++АК mika 2018.11.20 ИП-00020498
	КонецЕсли;
...
Процедура ТорговаяТочкаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Результат)Тогда
		...
		ОбновитьГруппуТорговойТочки();//+++АК mika 2018.02.10 ИП-00016863.02
	КонецЕсли;
...
Процедура СвойствоСотрудникаПриИзменении(Элемент)
	...
	ПроверитьКорректностьЗаполненияКоличестваЧасов(); //+++АК mika 2018.12.05 ИП-00020643
	
	ТекущееСвойство = СвойствоСотрудника;
...
Процедура РаботаИлиОтсутствиеПриИзменении(Элемент)
	...
	Иначе
		ТорговаяТочка 		= ПредопределенноеЗначение("Справочник.СтруктурныеЕдиницы.ПустаяСсылка");
		Если ЗначениеЗаполнено(Элементы.СотрудникТипВахтыВГрафике.ТекстРедактирования) Тогда //+++АК mika 2018.12.05 ИП-00020643
...
&НаКлиенте
Процедура ОбновитьГруппуТорговойТочки()//+++АК mika 2018.02.10 ИП-00016863.02
...
Функция ПолучитьЦФОМагазинаСервер(СтруктурнаяЕдиница) //+++АК mika 2018.02.10 ИП-00016863.02
...
&НаКлиенте
Процедура ПроверитьКорректностьЗаполненияКоличестваЧасов() //+++АК mika 2018.12.05 ИП-00020643

	Если СвойствоСотрудника > 2 И НЕ ЗначениеЗаполнено(Элементы.СотрудникТипВахтыВГрафике.ТекстРедактирования) Тогда
		СвойствоСотрудника = ТекущееСвойство;
		ПоказатьПредупреждение(,"Для планирования ""вахтовых"" выходов заполните ""Тип вахты в графике""(карточка физ. лица)!");
		Возврат;
	КонецЕсли;                              
	
	Если СвойствоСотрудника = 5 Тогда
		Если КоличествоЧасовПочасовика > 8 Тогда
			КоличествоЧасовПочасовика = 8;
			ПоказатьПредупреждение(, "Сотрудники со свойством Вахтовый (Стажер) могут планироваться максимум на 8 ч.!");
			Возврат;
		КонецЕсли;
	КонецЕсли;

КонецПроцедуры //ПроверитьКорректностьЗаполненияКоличестваЧасов()
