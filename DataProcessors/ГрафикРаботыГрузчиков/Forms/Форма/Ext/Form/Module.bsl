...
Процедура ОбновитьГрафикНаСервере()
	...
	|ИЗ
	|	РегистрСведений.СпецификаФормированияТабеляГрузчиков КАК СпецификаФормированияТабеля
	|ГДЕ
	...
	Если ВариантПостроения = 1 Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "СпецификаФормированияТабеляГрузчиков", "СпецификаФормированияТабеляГрузчиковПомощники");
	КонецЕсли;
	//---АК mika 
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СпецификаФормированияТабеляГрузчиков КАК СпецификаФормированияТабеля
...
Процедура ОбновитьГрафикНаСервереСКД() //+++АК mika 2018.08.20 ИП-00019541
	...
	|	ТабельРаботы.Тренинг,
	|	ТабельРаботы.ЭкстренныйВызов,
	|	ISNULL(ТипыВахтГрафикиРабот.Порядок, -1) КАК ТипВахты
	|ИЗ
	|	РегистрСведений.ТабельРаботыГрузчиков КАК ТабельРаботы
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СпецификаФормированияТабеляГрузчиков КАК СпецификаФормированияТабеля
	|		ПО ТабельРаботы.Группа = СпецификаФормированияТабеля.Группа
	...
	|		ПО ТабельРаботы.Сотрудник = КонтактнаяИнформация.Объект
	|			И (КонтактнаяИнформация.Вид = ЗНАЧЕНИЕ(Справочник.ВидыКонтактнойИнформации.ТелефонСлужебный))
	|		ЛЕВОЕ СОЕДИНЕНИЕ Перечисление.ТипыВахтГрафикиРабот КАК ТипыВахтГрафикиРабот
	|		ПО ТабельРаботы.Сотрудник.ТипВахтыВГрафике = ТипыВахтГрафикиРабот.Ссылка
	|ГДЕ
	|	ТабельРаботы.Группа = &Группа
	|	И ТабельРаботы.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|
	...
	//Если ВариантПостроения = 1 Тогда  //+++АК mika 2018.08.22 ИП-00019541 Формирования порядка всегда в разрезе групп помощников
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "РегистрСведений.СпецификаФормированияТабеляГрузчиков", "РегистрСведений.СпецификаФормированияТабеляГрузчиковПомощники");
	//КонецЕсли;
...
Процедура ДобавитьПродавца(Команда)
	...
	Результат = ОткрытьФормуМодально("Обработка.ГрафикРаботыГрузчиков.Форма.ФормаВыбораФизическогоЛица", ПараметрыОтбора);
...
Процедура ДобавитьПродавцаСлева(Команда)
	...
	Если ТипЗнч(Расшифровка) <> Тип("ФиксированнаяСтруктура") тогда
		Предупреждение("Встаньте на колонку с сотрудником относительно котрой необходимо выполнить действие. Операция не выполнена.");
		Возврат;
	КонецЕсли;
...
Процедура ОткрытьОтчетПоОтклонениюГрафиковРаботыОтРеальныхВыходов(Команда)
	...
	ОткрытьФорму("Отчет.ОтклонениеГрафиковРаботыГрузчиковОтРеальныхВыходов.Форма.ФормаОтчета",, ЭтаФорма);	
...
Процедура ОткрытьФормуСпискаГрафика(Команда)
	...
	Форма = ПолучитьФорму("РегистрСведений.ТабельРаботыГрузчиков.Форма.ФормаСписка");
...
Процедура ПечатьБейджев(Команда)
	...
	Результат = ОткрытьФормуМодально("Обработка.ГрафикРаботыГрузчиков.Форма.ФормаПодбораПродавцов", ПараметрыОткрытия);
...
Функция ПолучитьТабличныйДокумент(МассивФизическихЛиц)
	...
	Возврат Обработки.ГрафикРаботыГрузчиков.СформироватьТабличныйДокумент(МассивФизическихЛиц);
...
Процедура ЗаписатьТелефонВКИНаСервере()
	...
	Если ЗначениеЗаполнено(ТелефонТекущегоПродавца)
		...
		Если ТипЗнч(Расшифровка) = Тип("ФиксированнаяСтруктура") Тогда
			ТабличныйДокумент.Область(3, НомерКолонки, 3, НомерКолонки).Расшифровка =
				Новый ФиксированнаяСтруктура("Сотрудник, КодСотрудника, Телефон", Расшифровка.Сотрудник, Расшифровка.КодСотрудника, ТелефонТекущегоПродавца);
		КонецЕсли;
...
Функция ЗаписатьИзмененияВРегистр(ПараметрыСотрудника)
	...
	Если ЗначениеЗаполнено(ПараметрыСотрудника.ТорговаяТочка) Тогда
		...
		//+++АК mika 2018.02.10 ИП-00016863.02
		Если ВариантПостроения = 1 Тогда
	...
	Если ЗначениеЗаполнено(ПараметрыСотрудника.Период)
			И ЗначениеЗаполнено(ГруппаТорговыхТочек)
			И ЗначениеЗаполнено(ПараметрыСотрудника.Сотрудник)Тогда
		
		НЗ = РегистрыСведений.ТабельРаботыГрузчиков.СоздатьНаборЗаписей();
	...
	//
	Если ЗначениеЗаполнено(ПараметрыСотрудника.ТорговаяТочка)
			ИЛИ ЗначениеЗаполнено(ПараметрыСотрудника.Отсутствие)
			ИЛИ ЗначениеЗаполнено(ПараметрыСотрудника.Тренинг) Тогда	
		
		Запись = РегистрыСведений.ТабельРаботыГрузчиков.СоздатьМенеджерЗаписи();
		...
		//+++АК mika 2018.06.13 Без задачи. Письмо: Установить группу при записи «Отсутствий» (перед записью в модуле объекта контроль)
	...
	//ЗаписьЖурнала = РегистрыСведений.ЖурналИзмененийТабеляРаботыГрузчиков.СоздатьМенеджерЗаписи();
	...
	//ЗаписьЖурнала.СвойствоСотрудника= ПараметрыСотрудника.СвойствоСотрудника;
...
Процедура УдалитьЗаписиНаСервере();
	...
	Для Каждого СтрокаМассива Из МассивВыделенныхСтрок Цикл
		...
		// Очистим запись по полям Период, Группа, Сотрудник
		Если ЗначениеЗаполнено(СтруктураДанных.Период)
				И ЗначениеЗаполнено(ГруппаТорговыхТочек)
				И ЗначениеЗаполнено(СтруктураДанных.Сотрудник) И ВариантПостроения = 0 Тогда //+++АК mika 2018.02.10 ИП-00016863.02 Добавлено "И ВариантПостроения = 0", данная очистка актуальна только для "стандартного алгоритма".
...
Процедура ИзменитьПорядокПродавцовВТабеле_Отключена(НовоеФизЛицо, ТекущееФизЛицо) //+++АК mika 2018.08.22 ИП-00019541 процедура отключена
	...
	//+++АК mika 2018.07.17 ИП-00019209
	//НаборЗаписей = РегистрыСведений.СпецификаФормированияТабеляГрузчиков.СоздатьНаборЗаписей();
	Если ВариантПостроения = 1 Тогда
		НаборЗаписей = РегистрыСведений.СпецификаФормированияТабеляГрузчиковПомощники.СоздатьНаборЗаписей();
	Иначе
		НаборЗаписей = РегистрыСведений.СпецификаФормированияТабеляГрузчиков.СоздатьНаборЗаписей();
	КонецЕсли;
	//---АК mika 
...
Процедура СкопироватьГрафикСДругогоПродавца(Команда)
	...
	Если ТипЗнч(Расшифровка) <> Тип("ФиксированнаяСтруктура") тогда
		Предупреждение("Встаньте на колонку с физ. лицом для которго необходимо скопировать график. Операция не выполнена.");
		Возврат;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Результат)Тогда
		...
		ДанныеКопирования = ОткрытьФормуМодально("Обработка.ГрафикРаботыГрузчиков.Форма.ФормаКопированияГрафика", ПараметрыОткрытия, ЭтаФорма);
...
Процедура СкопироватьГрафикСДругогоПродавцаНаСервере(ДанныеКопирования)
	...
	Иначе
		Сообщить("Физ. лицо с которого производится копирование не найдено в графике");
		Возврат;
	КонецЕсли;
...
Процедура ОсновнаяТаблицаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если НЕ ПроверитьВозможностьПланированияВахтовыйМетод() Тогда
		Возврат;
	КонецЕсли;
	...
	Если ТипЗнч(Результат) = Тип("Структура")Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(ТорговаяТочка) Тогда
			...
			ИначеЕсли СвойствоСотрудника = 3 Тогда //+++АК mika 2018.09.19 ИП-00019805 
				Представление = Представление + "вх, ";
			ИначеЕсли СвойствоСотрудника = 4 Тогда //+++АК mika 2018.11.20 ИП-00020498 
				Представление = Представление + "нвх, ";
			ИначеЕсли СвойствоСотрудника = 5 Тогда //+++АК mika 2018.12.05 ИП-00020643 
				Представление = Представление + "свх, ";
			КонецЕсли;
...
&НаКлиенте
Процедура ВариантПостроенияПриИзменении(Элемент) //+++АК mika 2018.02.10 ИП-00016863.02 
...
&НаКлиенте
Процедура ВопросСменитьВариантПостроленияЗавершение(Результат, ДопПараметры) Экспорт //+++АК mika 2018.02.10 ИП-00016863.02
...
//
Процедура ОбновитьВозможностьИзмененияВариантаПостроения(ОбновитьГруппу) //+++АК mika 2017.02.10 ИП-00016863.02
...
//
Функция ПолучитьГруппуТорговыхТочекСерверБезКонтекста(ТекущийПомощник) //+++АК mika 2017.02.10 ИП-00016863.02
...
//
Процедура ОбновитьВозможностьИзмененияГруппыТорговыйТочек() //+++АК mika 2017.02.10 ИП-00016863.02
...
//
Процедура ОчиститьЗаписиПоСтаройГруппеТорговыхТочек(ПараметрыСотрудника) //+++АК mika 2017.02.10 ИП-00016863.02
	...
	Если ЗначениеЗаполнено(ПараметрыСотрудника.Период) И ЗначениеЗаполнено(ПараметрыСотрудника.ГруппаСтарая) И ЗначениеЗаполнено(ПараметрыСотрудника.Сотрудник)Тогда
		...
		НаборЗаписей = РегистрыСведений.ТабельРаботыГрузчиков.СоздатьНаборЗаписей();
...
&НаКлиенте
Процедура ТекущийПомощникНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка) //+++АК mika 2018.02.10 ИП-00016863.02
...
&НаКлиенте
Процедура ОбработатьВыборПомощника(Результат, ДополнительныеПараметры) Экспорт //+++АК mika 2018.02.10 ИП-00016863.02
...
//+++АК mika 2018.07.17 ИП-00019209
&НаКлиенте
Процедура ПоставитьСотрудникаПередКлиент(СписокСотрудников, НомерВыделеннойКолонки)
	
     ПоказатьВыборИзСписка(Новый ОписаниеОповещения("ОбработатьВыборСотрудникаПеремещение",ЭтаФорма, Новый Структура("НомерВыделеннойКолонки",НомерВыделеннойКолонки)), СписокСотрудников);	
	
КонецПроцедуры

//+++АК mika 2018.07.17 ИП-00019209
&НаКлиенте
Процедура ОбработатьВыборСотрудникаПеремещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено
		...
		ПеренестиПродавцаНаСервере(ДополнительныеПараметры.НомерВыделеннойКолонки, Результат);
...
Процедура ОбновитьТаблицуСпецификиФормирования() //+++АК mika 2018.08.22 ИП-00019541
	...
	|	СпецификаФормированияТабеля.Порядок КАК Порядок
	|ИЗ
	|	РегистрСведений.СпецификаФормированияТабеляГрузчиковПомощники КАК СпецификаФормированияТабеля
	|ГДЕ
	|	СпецификаФормированияТабеля.Группа = &Группа
	|
	|УПОРЯДОЧИТЬ ПО
...
&НаКлиенте
Функция ПроверитьВозможностьПланированияВахтовыйМетод() //+++АК mika 2018.10.03 ИП-00019755
	
	МассивПериодов = Новый Массив();
	Для Каждого СтрокаМассива Из Элементы.ОсновнаяТаблица.ВыделенныеСтроки Цикл
		ТекущаяСтрока = ОсновнаяТаблица.НайтиПоИдентификатору(СтрокаМассива);
		МассивПериодов.Добавить(ТекущаяСтрока.Период);
	КонецЦикла;
	
	ТекстУведомления = "";
	ЗапретПланирования = ПроверитьВозможностьПланированияВахтовыйМетодСервер(МассивПериодов, ТекущийПродавец, ТекстУведомления);
	
	Если ЗначениеЗаполнено(ТекстУведомления) Тогда
		ПоказатьПредупреждение(,ТекстУведомления);
	КонецЕсли;
	
	Возврат ЗапретПланирования

КонецФункции

&НаСервереБезКонтекста
Функция ПроверитьВозможностьПланированияВахтовыйМетодСервер(МассивПериодов, ТекущийПродавец, ТекстУведомления) //+++АК mika 2018.10.03 ИП-00019755
	
	ТипВахтыВГрафике = ОбщегоНазначения.ПолучитьЗначениеРеквизита(ТекущийПродавец, "ТипВахтыВГрафике");
	
	Если НЕ ЗначениеЗаполнено(ТипВахтыВГрафике) Тогда
		Возврат Истина;
	КонецЕсли;
	
	ТаблицаПериодов = Новый ТаблицаЗначений();
	ТаблицаПериодов.Колонки.Добавить("Дата", Новый ОписаниеТипов("Дата"));
	ТаблицаПериодов.Колонки.Добавить("Сотрудник", Новый ОписаниеТипов("СправочникСсылка.ФизическиеЛица"));
	ТаблицаПериодов.Колонки.Добавить("ТипВахтыВГрафике"    , Новый ОписаниеТипов("ПеречислениеСсылка.ТипыВахтГрафикиРабот"));
	
	Для каждого ЭлементМассива Из МассивПериодов Цикл
		НоваяСтрока = ТаблицаПериодов.Добавить();
		НоваяСтрока.Дата = ЭлементМассива;
		НоваяСтрока.Сотрудник = ТекущийПродавец;
		НоваяСтрока.ТипВахтыВГрафике = ТипВахтыВГрафике;
	КонецЦикла;
	
	Запрос = Новый Запрос("Выбрать * ПОМЕСТИТЬ ВТ Из &ВременнаяТаблица Как ВременнаяТаблица");
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("ВременнаяТаблица", ТаблицаПериодов);
	Запрос.Выполнить();
	
	Если ТипВахтыВГрафике = Перечисления.ТипыВахтГрафикиРабот.Помесячно Тогда
		
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	Таб_ПрошлыйМесяц.ТекущийМесяц,
		|	Таб_ПрошлыйМесяц.ПрошлыйМесяц,
		|	Таб_ПрошлыйМесяц.Сотрудник,
		|	НАЧАЛОПЕРИОДА(ТабельРаботыГрузчиков.Период, МЕСЯЦ) КАК Период
		|ИЗ
		|	(ВЫБРАТЬ
		|		НАЧАЛОПЕРИОДА(ВТ.Дата, МЕСЯЦ) КАК ТекущийМесяц,
		|		НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(ВТ.Дата, МЕСЯЦ, -1), МЕСЯЦ) КАК ПрошлыйМесяц,
		|		ВТ.Сотрудник КАК Сотрудник
		|	ИЗ
		|		ВТ КАК ВТ) КАК Таб_ПрошлыйМесяц
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ТабельРаботыГрузчиков КАК ТабельРаботыГрузчиков
		|		ПО Таб_ПрошлыйМесяц.Сотрудник = ТабельРаботыГрузчиков.Сотрудник
		|			И (Таб_ПрошлыйМесяц.ПрошлыйМесяц = НАЧАЛОПЕРИОДА(ТабельРаботыГрузчиков.Период, МЕСЯЦ))
		|ГДЕ
		|	НЕ ТабельРаботыГрузчиков.Период ЕСТЬ NULL
		|
		|УПОРЯДОЧИТЬ ПО
		|	Период";
		...
		Если НЕ Результат.Пустой() Тогда
			...
			ТекстУведомления = СтрЗаменить(СтрЗаменить(СтрЗаменить(СтрЗаменить("Запрещено планирование сотрудника ФизЛицо на ТекущийМесяц, поскольку в графике есть записи за ПрошлыйМесяц (установлен тип вахты: ТипВахты)!",
				"ФизЛицо",ТекущийПродавец), "ТекущийМесяц", Формат(Выборка.ТекущийМесяц, "ДФ='ММММ гггг'")), "ТипВахты", ТипВахтыВГрафике), "ПрошлыйМесяц", Формат(Выборка.ПрошлыйМесяц, "ДФ='ММММ гггг'")) ; 
	...
	Иначе
		...
		|	Таб_Итоговая.ТипВахтыВГрафике,
		|	Таб_Итоговая.ЗапретПланирования
		|ИЗ
		|	(ВЫБРАТЬ
		|		ВТ.Дата КАК Дата,
		|		ВТ.Сотрудник КАК Сотрудник,
		|		ВТ.ТипВахтыВГрафике КАК ТипВахтыВГрафике,
		|		ВЫБОР
		|			КОГДА ВТ.ТипВахтыВГрафике = ЗНАЧЕНИЕ(Перечисление.ТипыВахтГрафикиРабот.ПерваяПоловинаМесяца)
		|					И ДЕНЬ(ВТ.Дата) > 15
		|				ТОГДА ИСТИНА
		|			КОГДА ВТ.ТипВахтыВГрафике = ЗНАЧЕНИЕ(Перечисление.ТипыВахтГрафикиРабот.ВтораяПоловинаМесяца)
		|					И ДЕНЬ(ВТ.Дата) < 16
		|				ТОГДА ИСТИНА
		|			ИНАЧЕ ЛОЖЬ
		|		КОНЕЦ КАК ЗапретПланирования
		|	ИЗ
		|		ВТ КАК ВТ) КАК Таб_Итоговая
		|ГДЕ
		|	Таб_Итоговая.ЗапретПланирования";
		...
		Если НЕ Результат.Пустой() Тогда
			...
			ДатыПланирования = "";
			Пока Выборка.Следующий() Цикл
				ДатыПланирования = ДатыПланирования + Формат(Выборка.Дата, "ДФ=dd.MM.yyyy") + ", ";
			КонецЦикла;
			
			ТекстУведомления = СтрЗаменить(СтрЗаменить(СтрЗаменить("Запрещено планирование сотрудника ФизЛицо на ДатыПланирования (установлен тип вахты: ТипВахты)!",
						"ФизЛицо",ТекущийПродавец), "ДатыПланирования", Лев(СокрЛП(ДатыПланирования),СтрДлина(ДатыПланирования)-2)), "ТипВахты", ТипВахтыВГрафике); 
