...
Процедура СформироватьЭтикеткиНаСервере()
	...
	Для Каждого Выб Из ТаблицаТоваров Цикл
		Если Не ЗначениеЗаполнено(Выб.ТТ) или Не ЗначениеЗаполнено(Выб.Номенклатура) Тогда
			Продолжить;
		КонецЕсли; 
		...
		УмножатьНаТысячу =  Ложь;
		
		ТекстНадШтрихКодом =  ?(ЗначениеЗаполнено(Объект.Дата), " Дата " + Формат(Объект.Дата, "ДФ=dd.MM.yy"), "00.00.00")
				+ Символы.ПС +" Магазин " + выб.ТТ;
		Область = Макет.ПолучитьОбласть("Этикетка");
		Область.Параметры.Товар = Выб.Номенклатура.НаименованиеПолное;
		Область.Параметры.Заголовок = ТекстНадШтрихКодом;
		
		                                                                                             
		ШтрихКод = "86" +Строка(Формат(Выб.Номенклатура.id_tov, "ЧЦ=6; ЧВН=; ЧГ="))+Строка(Формат(Выб.ТТ.id_TT, "ЧЦ=5; ЧВН=; ЧГ="))
		+Строка(Формат(День(Объект.Дата), "ЧЦ=2; ЧВН=; ЧГ="))+Строка(Формат(Месяц(Объект.Дата), "ЧЦ=2; ЧВН=; ЧГ="))+Строка(Формат(Год(Объект.Дата), "ЧЦ=4; ЧВН=; ЧГ="));
	...
	Этикетки.КлючПараметровПечати = "ПараметрыПечати_ПечатьШККонвертовРекламныхМатериалов";
