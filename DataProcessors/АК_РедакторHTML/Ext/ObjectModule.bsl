...
Функция ПолучитьТекст(Параметр = 0) Экспорт
...
Процедура УстановитьИПроверитьТекст() Экспорт
	...
	//JScript = "tinyMCE.execCommand('mceInsertContent',false,'"+ИзТекстаВХТМЛ(ТекстHTML)+"');";
	//Эксплорер.Документ.parentWindow.ExecScript(JScript,"JavaScript");
	//ПроверитьТекст(ЭтотОбъект.ТекстHTML);
...
Функция ИзТекстаВХТМЛ(Текст)
    Возврат "<p>"+СтрЗаменить(ТекстHTML, Символы.ПС, "<br />")+"</p>";
КонецФункции	
