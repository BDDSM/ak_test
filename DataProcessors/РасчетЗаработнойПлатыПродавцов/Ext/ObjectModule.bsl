...
Процедура РасчитатьЗпНоваяСхемаСхема(ДатаНач, ДатаКон)
	...
	//+++АК mika 2018.05.19 ИП-00018349
	//Опримизация (обнаружена неоптимальность запроса в рамках ИП-00018349 (Бонус промоутера))
	
#Область НеактуальныйКод
	
	////+++АК MIND 2017.11.08 доработал расчет в рамках появления в расчетах уборщиц
	//
	////+++АК gusd (ИП-00018118) 2018.03.14 добавлен расчет с учетом "заморозки з/п"
	//
	//Запрос = Новый Запрос;
	//Запрос.УстановитьПараметр("НачалоПериода", ДатаНач);
	//Запрос.УстановитьПараметр("КонецПериода" , КонецДня(ДатаКон));
	...
	//|	Продавцы.ЭкстренныйВызов,
	//|	ЕСТЬNULL(ВЗ_ВРазвитии.Период, ДАТАВРЕМЯ(3000, 1, 1)) КАК ВРазвитииСДаты,
	//|	ЕСТЬNULL(ЗамороженнаяЗарплатаПродавцов.Сумма, 0) КАК ФиксированнаяСтавкаЗП,
	...
	//|		СГРУППИРОВАТЬ ПО
	//|			ЦФОСтруктурныхЕдиниц.ТорговаяТочка) КАК ВЗ_ВРазвитии
	//|		ПО Продавцы.Ссылка.ТорговаяТочка = ВЗ_ВРазвитии.СтруктурнаяЕдиница
	//|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЗамороженнаяЗарплатаПродавцов КАК ЗамороженнаяЗарплатаПродавцов
	...
	//|	ЕСТЬNULL(ВЗ_ВРазвитии.Период, ДАТАВРЕМЯ(3000, 1, 1)),
	//|	0,
	//|	ЛОЖЬ
	//|ИЗ
	//|	Документ.ЛистУчета.Кассиры КАК Кассиры
	...
	//|		СГРУППИРОВАТЬ ПО
	//|			ЦФОСтруктурныхЕдиниц.ТорговаяТочка) КАК ВЗ_ВРазвитии
	//|		ПО Кассиры.Ссылка.ТорговаяТочка = ВЗ_ВРазвитии.СтруктурнаяЕдиница
	//|ГДЕ
	//|	Кассиры.Ссылка.Проведен
	//|	И Кассиры.Ссылка.Дата МЕЖДУ &НачалоПериода И &КонецПериода
	//|
	...
	//|		СГРУППИРОВАТЬ ПО
	//|			ЦФОСтруктурныхЕдиниц.ТорговаяТочка) КАК ВЗ_ВРазвитии
	//|		ПО Грузчики.Ссылка.ТорговаяТочка = ВЗ_ВРазвитии.СтруктурнаяЕдиница
	//|ГДЕ
	//|	Грузчики.Ссылка.Проведен
	//|	И Грузчики.Ссылка.Дата МЕЖДУ &НачалоПериода И &КонецПериода
	//|
	...
	//|		СГРУППИРОВАТЬ ПО
	//|			ЦФОСтруктурныхЕдиниц.ТорговаяТочка) КАК ВЗ_ВРазвитии
	//|		ПО Уборщицы.Ссылка.ТорговаяТочка = ВЗ_ВРазвитии.СтруктурнаяЕдиница
	//|ГДЕ
	...
	//|		СГРУППИРОВАТЬ ПО
	//|			ЦФОСтруктурныхЕдиниц.ТорговаяТочка) КАК ВЗ_ВРазвитии
	//|		ПО Пекари.Ссылка.ТорговаяТочка = ВЗ_ВРазвитии.СтруктурнаяЕдиница
	//|ГДЕ
	//|	Пекари.Ссылка.Проведен
	//|	И Пекари.Ссылка.Дата МЕЖДУ &НачалоПериода И &КонецПериода";
	////---АК mika 2018.04.09 ИП-00018000
	...
	Пока Выборка.Следующий()Цикл
		...
		Для каждого СтрокаТаблицы Из Таблица Цикл
			...
			Иначе 
				...
				Если СтрокаТаблицы.Праздник ИЛИ ЭкстренныйВызов Тогда
		...
		СуммаВыручка = 0;
		Если Не Выборка.Практикант И Не Выборка.Стажер Тогда
...
Функция ПолучитьТекстЗапросаРассчетаЗпПродавцам()
	...
	//+++АК mika 2018.09.19 ИП-00019805 
	|	ВЫБОР
	|		КОГДА Грузчики.СвойствоСотрудника = 3 ИЛИ Грузчики.СвойствоСотрудника = 4
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ,
	//---АК mika
...
Процедура РассчитатьДоплаты_ВахтовыйМетодГрузчики(ДатаНач, ДатаКон) //+++АК mika 2018.10.09 ИП-00019755 
	...
	|	И (Таб_Грузчики.СвойствоСотрудника = 3
	|			ИЛИ Таб_Грузчики.СвойствоСотрудника = 4)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	И ТабельРаботыГрузчиков.Сотрудник.ТипВахтыВГрафике <> ЗНАЧЕНИЕ(Перечисление.ТипыВахтГрафикиРабот.ПустаяСсылка)
