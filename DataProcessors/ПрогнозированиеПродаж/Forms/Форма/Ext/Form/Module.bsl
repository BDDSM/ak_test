		...
		Пока ВыборкаПоТТ.Следующий() Цикл
			...
			Для Каждого СтрокаМеньше Из ТаблицаТТМеньше Цикл
				...
				Иначе
					...
					//+++АК mika 2018.12.14 ИП-00020392.02
					Если РегистрыСведений.ТоварныйАссортиментТочек.ПроверкаПрограмногоВыводаПозицииИзАссортиментаАвто(ТекТорговаяТочка, ТекНоменклатура, Ложь, "БезУведомлений") Тогда
						МенеджерЗаписи.Комментарий = РегистрыСведений.ТоварныйАссортиментТочек.ПолучитьКомментарийОбходаКонтроляАвтовывода(МенеджерЗаписи.Комментарий, "РаспределитьХарактеристикиМеждуПоставщиками"); 
					КонецЕсли;
					//---АК mika
