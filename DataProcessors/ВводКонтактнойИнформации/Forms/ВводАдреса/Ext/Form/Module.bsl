...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЦветФонаУправляющегоПоля = Новый Цвет(255, 232, 179); //ЦветаСтиля.ФонУправляющегоПоля;
	ЦветФонаФормы            = WebЦвета.Кремовый; //ЦветаСтиля.ЦветФонаФормы;
	...
	Если Параметры.Свойство("СкрытьКомментарий") Тогда
		Элементы.АдресСтраницаКомментарий.Видимость = Ложь;
	КонецЕсли;
	//---АК SHEP 2018.09.25
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	Оповещение = Новый ОписаниеОповещения("ПодтвердитьИЗакрыть", ЭтаФорма);
...
Процедура УлицаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если КодКлассификатораНаселенногоПункта > 0 Тогда
		ПараметрыФормы = Новый Структура("СкрыватьНеактуальныеАдреса, КодКлассификатораНаселенногоПункта, Улица, ДополнительнаяТерритория", 
			СкрыватьНеактуальныеАдреса, КодКлассификатораНаселенногоПункта, Улица, ДополнительнаяТерритория);
	Иначе
		// Родитель улицы неопределен, открываем форму в которой ничего нет
		ПараметрыФормы = Новый Структура("СкрыватьНеактуальныеАдреса, КодКлассификатораНаселенногоПункта, Улица, Заголовок, ДополнительнаяТерритория", 
			СкрыватьНеактуальныеАдреса, -1, Улица, НаселенныйПункт + " " + НСтр("ru = '(не найден)'"), ДополнительнаяТерритория);
	КонецЕсли;
...
Процедура ДобавитьОбъект(Команда)
	...
	Если КоличествоВариантов>0 Тогда
		...
		Оповещение = Новый ОписаниеОповещения("ДобавитьОбъектЗавершение", ЭтаФорма, ДополнительныеПараметры);
...
Процедура ВвестиАдресВСвободнойФорме(Команда)
	...
	Оповещение = Новый ОписаниеОповещения("ВвестиАдресВСвободнойФормеЗавершение", ЭтаФорма);
...
Процедура УстановитьПиктограммуКомментария()
	...
	Иначе
		Элементы.АдресСтраницаКомментарий.Картинка = БиблиотекаКартинок.ПрочаяКонтактнаяИнформация;
	КонецЕсли;
...
Процедура ПодтвердитьИЗакрыть(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Модифицированность Тогда
		...
		Если (Не РазрешитьВводАдресаВСвободнойФорме) И Результат.ОшибкиЗаполнения.Количество()>0 Тогда
...
Процедура ЗаполнитьПредставлениеАдреса(Контекст, XDTOКонтактная=Неопределено)
	...
	Если ТипЗнч(Контекст.Страна)=Тип("СправочникСсылка.КлассификаторСтранМира") Тогда
...
Процедура ЗначениеРеквизитовПоКонтактнойИнформации(Контекст, РедактируемаяИнформация)
	...
	Если ПустаяСтрока(НаименованиеСтраны) Тогда
		Контекст.Страна = Справочники.КлассификаторСтранМира.ПустаяСсылка();
	Иначе
		СсылкаНаРоссию = Справочники.КлассификаторСтранМира.Россия;
		...
		Иначе
			ДанныеСтраны = Справочники.КлассификаторСтранМира.ДанныеСтраныМира(, НаименованиеСтраны);
	...
	НаселенныйПунктПолноеПредставление = КонтактнаяИнформацияСлужебный.ПредставлениеНаселенногоПункта(ДанныеАдреса, Ложь);
	Контекст.НаселенныйПункт = НаселенныйПунктПолноеПредставление;
	Контекст.Улица = КонтактнаяИнформацияСлужебный.ПредставлениеУлицы(ДанныеАдреса);
	
	НаселенныйПунктВФорматеКЛАДР = КонтактнаяИнформацияСлужебный.ПредставлениеНаселенногоПункта(ДанныеАдреса);
	АнализКлассификатора = КонтактнаяИнформацияСлужебный.НаселенныеПунктыПоПредставлению(НаселенныйПунктВФорматеКЛАДР, , , Контекст.Улица);
	...
	УлицаОпределена = Ложь;
	ДополнительнаяТерритория = "";
	Если ДанныеАдреса.Состав <> Неопределено И ТипЗнч(ДанныеАдреса.Состав) <> Тип("Строка") Тогда
		Свойство = КонтактнаяИнформацияСлужебный.ЗначениеСвойстваПоXPath(ДанныеАдреса.Состав, "ДопАдрЭл[ТипАдрЭл='10200000']");
		Если Свойство <> Неопределено Тогда
			ДополнительнаяТерритория = Свойство.Значение;
			АнализКлассификатора = КонтактнаяИнформацияСлужебный.УлицыПоПредставлению(Контекст.КодКлассификатораНаселенногоПункта, ДополнительнаяТерритория);
			...
			Если НайденоУлиц = 1 Тогда
				...
				Контекст.УлицаНайденаТочно      = Истина;
				Контекст.Улица                  = ВыбранноеЗначение.Представление;
				УлицаОпределена = Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	
	Если УлицаОпределена= Ложь Тогда
		...
		Если НайденоУлиц = 1 Тогда
			...
			Контекст.УлицаНайденаТочно      = Истина;
			Контекст.Улица                  = ВыбранноеЗначение.Представление;
			Контекст.ДополнительнаяТерритория = ДополнительнаяТерритория;
		КонецЕсли;
	КонецЕсли;
...
Функция ПутьКДаннымФормыПоПутиXPath(ПутьXPath) 
	...
	Если Строки.Количество() > 0 Тогда
		...
		Для Каждого СтрокаЗдания Из Строки Цикл
			...
			Если ЗначениеЗаполнено(ЭтаФорма[ИмяРеквизита]) Тогда
...
Функция КонтекстФормыКлиент(НовыеДанные=Неопределено)
	СписокРеквизитов = "
		...
		|Модифицированность, ДополнительнаяТерритория
		|";
