...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли ТипКонтактнойИнформации = Перечисления.ТипыКонтактнойИнформации.Телефон Тогда
		...
		//+CRM
		Если ПустаяСтрока(XDTOКонтактная.Представление) И НЕ (Параметры.Представление = Неопределено) Тогда
			XDTOКонтактная.Представление = Параметры.Представление;
		КонецЕсли;	
		//-CRM
	Иначе
	...
	//+CRM
	//+СофтФон
	Элементы.НомерТелефона.АктивизироватьПоУмолчанию = Истина;	
	Если Параметры.Свойство("БылиВнесеныИзменения") Тогда
		ЭтаФорма.Модифицированность = Параметры.БылиВнесеныИзменения;
	КонецЕсли;
	//-СофтФон
	//-CRM
