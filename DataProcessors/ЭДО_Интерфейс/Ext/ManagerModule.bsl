...
Функция СоздатьПТУ(СтруктураДокумента, ТЗТовары, ТЗУслги, ТЗОборудование)
	...
	//+++ AK suvv 2018.12.10 ИП-00020446
	СуммаНДСВПакетеЭДО = СтруктураДокумента.ЭДО_ВходящийДокумент.СуммаНДС;
	СуммаНДСНДок       = НДок.Товары.Итог("СуммаНДС") + НДок.Услуги.Итог("СуммаНДС") + НДок.Оборудование.Итог("СуммаНДС");
	РазницаСуммНДС     = СуммаНДСВПакетеЭДО - СуммаНДСНДок;
	Если РазницаСуммНДС <= 1 и РазницаСуммНДС >= -1 Тогда
		СтрТЧДляИзменения = неопределено;
		Если НДок.Оборудование.Количество() > 0 Тогда
			СтрТЧДляИзменения = НДок.Оборудование[0];
		ИначеЕсли НДок.Услуги.Количество() > 0 Тогда
			СтрТЧДляИзменения = НДок.Услуги[0];
		ИначеЕсли НДок.Товары.Количество() > 0 Тогда
			СтрТЧДляИзменения = НДок.Товары[0];
		КонецЕсли;
		Если СтрТЧДляИзменения <> неопределено Тогда 
			СтрТЧДляИзменения.СуммаНДС = СтрТЧДляИзменения.СуммаНДС + РазницаСуммНДС;
		КонецЕсли;
	КонецЕсли;
	//--- AK suvv 
...
Функция СоздатьПДР(СтруктураДокумента, ТЗДопРасход, Партия)
	...
	//+++ AK suvv 2018.12.24 ИП-00020593.01
	НДок.Товары.Очистить();
	//--- AK suvv
...
Функция СоздатьВозвратТоваровОтПокупателя(СтруктураДокумента, ТЗТовары)
	...
	Попытка		
		...
		//+++ AK suvv 2018.12.17 ИП-00019917.10
		ЗаписьСО = РегистрыСведений.ЗначенияСвойствОбъектов.СоздатьМенеджерЗаписи();
		ЗаписьСО.Объект = НДок.Ссылка;
		ЗаписьСО.Свойство = ПредопределенноеЗначение("ПланВидовХарактеристик.СвойстваОбъектов.ОплатаВозврата");
		ЗаписьСО.Значение = Истина; 
		ЗаписьСО.Записать();
		Попытка
			НДок.Записать(РежимЗаписиДокумента.Проведение);
			Сообщить(?(СтруктураДокумента.СоздаватьНовый, "Создан и проведен документ ", "Перезаполнен и перепроведен документ ") + строка(НДок));
		Исключение
			Сообщить(?(СтруктураДокумента.СоздаватьНовый, "Создан документ ", "Перезаполнен документ ") + строка(НДок));
		КонецПопытки;
		//--- AK suvv 
