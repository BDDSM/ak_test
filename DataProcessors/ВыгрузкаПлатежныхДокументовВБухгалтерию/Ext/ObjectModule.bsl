...
//+++АК Susk (Суслин К.В.) 2018.02.12 б/н оптимизация
Перем ОбъектПодключения Экспорт;
//ИП-00020101
Перем ПодключениеУстановлено Экспорт;
//---АК Susk (Суслин К.В.) 
...
Функция ВыполнитьВыгрузкуИзмененныхДанныхДляУзлаОбмена(УзелОбмена, МассивПравилКонвертации, СтруктураДляУдаленияРегистрацииИзменений)
	...
	Попытка
		...
		//+++АК Susk (Суслин К.В.) 2017.12.13 ИП-00016868
		МассивСтрокПравилДляУдаления = Новый Массив;
		//---АК Susk (Суслин К.В.) 		 
		...
		Пока ВыборкаИзменений.Следующий() Цикл
			...
			Если ТекущаяСтрокаПравилаВыгрузки <> Неопределено Тогда
				...
				Иначе	
					...
					//+++АК Susk (Суслин К.В.) 2017.12.11 ИП-00016868
					//для того, чтобы в случае ошибки записи не рубился обмен целиком ставлю попытку. 
					//обработка снятия с регистрации прописана в самих правилах обмена, т.е. если будет ошибка, то объект просто пропустится и регистрация не снимется.
					// при возникновении сбоя загрузки объекта в базу БУ - вылетит исключение, которое я тут обрабатываю.
					
					//ВыгрузкаОбъектаВыборки(ОбъектДляПравилВыгрузки, ТекущаяСтрокаПравилаВыгрузки, , ИсходящиеДанные);
					
					тмпФлагРежимОтладки = мОбработкаДляЗагрузкиДанных.ФлагРежимОтладки;									
					...
					Исключение
						...
						Попытка
							Если ЗП.Свойство("ОписаниеОшибки") Тогда
								//ОписаниеОшибки = СокрЛ(СтрЗаменить(СокрЛП(СтрЗаменить(СтрПолучитьСтроку(ЗП.ОписаниеОшибки, 6), "ОписаниеОшибки", "")), "=", ""));
								Стр = ЗП.ОписаниеОшибки;
								ОписаниеОшибки = СокрЛ(СтрЗаменить(СокрЛП(СтрЗаменить(Сред(Стр, Найти(Стр, "ОписаниеОшибки"), 200), "ОписаниеОшибки", "")), "=", ""));
							Иначе
								ОписаниеОшибки = "НЕИЗВЕСТНАЯ ОШИБКА!";
							КонецЕсли;
							
							ЗаписатьВРегистрОшибкуВыгрузкиОбъекта(ОписаниеОшибки, ОбъектДляПравилВыгрузки);
						Исключение
						КонецПопытки;
							
						СтрокаСообщенияОбОшибке = ЗаписатьВПротоколВыполнения(72, ЗП, Истина);
						МассивСтрокПравилДляУдаления.Добавить(ТекущаяСтрокаПравилаВыгрузки);
						
						мОбработкаДляЗагрузкиДанных.ФлагРежимОтладки = тмпФлагРежимОтладки;						
					...
					мОбработкаДляЗагрузкиДанных.ФлагРежимОтладки = тмпФлагРежимОтладки;
					ЗаписатьВРегистрОшибкуВыгрузкиОбъекта("", ОбъектДляПравилВыгрузки, Истина);	
					//---АК Susk (Суслин К.В.)
		...
		//+++АК Susk (Суслин К.В.) 2017.12.13 ИП-00016868
		ОбщегоНазначения.УдалитьПовторяющиесяЭлементыМассива(МассивСтрокПравилДляУдаления);
		
		Для Каждого ТекущаяСтрокаПравилаВыгрузки Из МассивСтрокПравилДляУдаления Цикл
			УдалитьСтрокуДереваПравилВыгрузкиПоТипуВыгрузкиИзМассива(ВременныйМассивПравилКонвертации, ТекущаяСтрокаПравилаВыгрузки);
		КонецЦикла;
		//---АК Susk (Суслин К.В.) 
...
Процедура ЗагрузитьПравилаОбмена(Источник = "", ТипИсточника = "XMLФайл") Экспорт
	...
	Если НЕ ((ПравилаОбмена.ЛокальноеИмя = "ПравилаОбмена")
...
Процедура ВыполнитьВыгрузку() Экспорт
	...
	Иначе
		...
		//+++АК Susk (Суслин К.В.) 2017.12.12 ИП-00016868
...
Функция ВыполнитьПодключениеКИБПриемнику() Экспорт
	...
	//в рамках этой задачи мне требуется по кому сперва кое-чего проверить, потом прогрузить РТУ недостающие этой обработкой и потом еще по кому
	//делать действия. Чтобы не подключаться 20 раз, я подключаюсь по кому в самом начале и сюда передаю подключение. Для этого делаю переменную "Подключение установлено"
	//и если я уже установил подключение, то сюда просто присваиваю его извне. Иначе в других случаях подключаюсь к базе уже отсюда.
...
//+++АК Susk (Суслин К.В.) 2017.12.12 ИП-00016868
Процедура ЗаписатьВРегистрОшибкуВыгрузкиОбъекта(СтрокаОшибки, Документ, ФлагОчистки = Ложь)
	
	Набор = РегистрыСведений.ЛогНеудачногоОбмена.СоздатьНаборЗаписей();
	Набор.Отбор.Документ.Установить(Документ);
	Набор.Прочитать();
	
	Если ФлагОчистки Тогда
		Набор.Очистить();
		Набор.Записать();
		Возврат;
	КонецЕсли;
	...
	Запись.СчетчикНеудачнойВыгрузки = Запись.СчетчикНеудачнойВыгрузки + 1;
	Запись.ОписаниеОшибки = СтрокаОшибки;
	Запись.ПериодПоследнейВыгрузки = ТекущаяДата();
...
Функция ПолучитьТекстВременныхПравилОбмена(Разрез = "") Экспорт
	
	ТекстПравил = "";
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               |	ВременныеПравилаОбмена.ДействуютДо,
	               |	ВременныеПравилаОбмена.Правила
	               |ИЗ
	               |	РегистрСведений.ВременныеПравилаОбмена КАК ВременныеПравилаОбмена
	               |ГДЕ
	               |	ВременныеПравилаОбмена.ДействуютДо >= &ДействуютДо
	               |	И ВременныеПравилаОбмена.Разрез = &Разрез";
	
	Запрос.УстановитьПараметр("ДействуютДо", ТекущаяДата());
	Запрос.УстановитьПараметр("Разрез", Разрез);
	                                                       
	Выборка = Запрос.Выполнить().Выбрать();
	
	Если Выборка.Следующий() Тогда
		ТекстПравил = Выборка.Правила.Получить();
	КонецЕсли;
...
ИмяМакета = "ПравилаОбменаДанными";
