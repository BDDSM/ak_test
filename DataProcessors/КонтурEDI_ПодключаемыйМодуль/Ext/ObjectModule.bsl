...
Функция ОбработатьСобытиеКонтурEDI(ИмяСобытия, Параметры, СтандартнаяОбработкаEDI = Истина) Экспорт
	...
	//----------------------------------------------------------------------------------------------------------------
	//+++AK GREK 11.11.2018 ИП-00019473
	Если ИмяСобытия = "ПолучитьТоварыДокумента" И ТипЗнч(Параметры.Документ) = Тип("ДокументСсылка.КонтурEDI_ТестовыйЗаказВX5") Тогда
		СтандартнаяОбработкаEDI = Ложь;
		Запрос = Новый Запрос;
		Запрос.Текст =  "ВЫБРАТЬ
		                |	МИНИМУМ(ТестовыйЗаказВX5Товары.НомерСтроки) КАК НомерСтроки,
		                |	ТестовыйЗаказВX5Товары.Номенклатура,
		                |	ТестовыйЗаказВX5Товары.Характеристика КАК ХарактеристикаНоменклатуры,
		                |	ТестовыйЗаказВX5Товары.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
		                |	ТестовыйЗаказВX5Товары.Номенклатура.Наименование КАК Наименование,
		                |	ВЫРАЗИТЬ(ТестовыйЗаказВX5Товары.Номенклатура.НаименованиеПолное КАК СТРОКА(200)) КАК НаименованиеПолное,
		                |	ТестовыйЗаказВX5Товары.Цена,
		                |	ТестовыйЗаказВX5Товары.СтавкаНДС,
		                |	НЕОПРЕДЕЛЕНО КАК СерияНоменклатуры,
		                |	ЗНАЧЕНИЕ(Справочник.КлассификаторСтранМира.РОССИЯ) КАК Страна,
		                |	"""" КАК НомерГТД,
		                |	СУММА(ТестовыйЗаказВX5Товары.Количество) КАК Количество,
		                |	СУММА(ТестовыйЗаказВX5Товары.Сумма) КАК Сумма,
		                |	СУММА(ТестовыйЗаказВX5Товары.СуммаНДС) КАК СуммаНДС,
		                |	NULL КАК ТипЕдиницы
		                |ИЗ
		                |	Документ.КонтурEDI_ТестовыйЗаказВX5.Товары КАК ТестовыйЗаказВX5Товары
		                |ГДЕ
		                |	ТестовыйЗаказВX5Товары.Ссылка В(&Ссылка)
		                |
		                |СГРУППИРОВАТЬ ПО
		                |	ТестовыйЗаказВX5Товары.Номенклатура,
		                |	ТестовыйЗаказВX5Товары.ЕдиницаИзмерения,
		                |	ТестовыйЗаказВX5Товары.Цена,
		                |	ТестовыйЗаказВX5Товары.СтавкаНДС,
		                |	ТестовыйЗаказВX5Товары.Номенклатура.Наименование,
		                |	ТестовыйЗаказВX5Товары.Характеристика,
		                |	ВЫРАЗИТЬ(ТестовыйЗаказВX5Товары.Номенклатура.НаименованиеПолное КАК СТРОКА(200))";
...
Функция Плагин_НестандартнаяКонфигурация(ИмяСобытия, Параметры, СтандартнаяОбработкаEDI)
	...
	Если ИмяСобытия = "ПолучитьРеквизитДокументаДляСообщения" Тогда
		...
		Если ТипЗнч(Документ) = Тип("ДокументСсылка.РеализацияТоваровУслуг") ИЛИ ТипЗнч(Документ) = Тип("ДокументОбъект.РеализацияТоваровУслуг") ИЛИ 
				 ТипЗнч(Документ) = Тип("ДокументСсылка.ВозвратТоваровОтПокупателя") ИЛИ ТипЗнч(Документ) = Тип("ДокументОбъект.ВозвратТоваровОтПокупателя") Тогда
		...
		//+++AK GREK 11.11.2018 ИП-00019473                    
		Если ТипЗнч(Документ) = Тип("ДокументСсылка.КонтурEDI_ТестовыйЗаказВX5") Тогда
			Если ИмяРеквизита = "Грузополучатель" Тогда
				Результат = Документ.ТочкаДоставки;
			КонецЕсли;	
		КонецЕсли;
		//---AK
	...
	Если ИмяСобытия = "ФинальныйЗапросПостроенияСписка" Тогда
		...
		| И (Документ.EDI_ГотовКОбмену = Истина ИЛИ ТипЗначения(Документ) = Тип(Документ.ВозвратТоваровОтПокупателя) ИЛИ Документ.Ссылка Есть NULL)";
		
		Возврат Неопределено;
	КонецЕсли; 	
	
	Если ИмяСобытия = "ЗагрузитьРасхождения" Тогда
		//+++AK GREK 02.11.2018 ИП-00019569              
		//Иногда пресылают повторные корректировки по 1-2 позициям, нужно не заменять предыдущую корректировку на новую, а дополнять ее.
		ЭтоДополняющаяКорректировка = ЭтоНеПерваяКорректировкаРеализации(Параметры.ДокументСсылка);
		//---AK
		...
		Для Каждого Строка Из ТаблицаСравнения Цикл
			Если Строка[ИмяПоляЦенаБезНДСПринято] = Строка[ИмяПоляЦенаБезНДСВНакладной] ИЛИ Строка[ИмяПоляЦенаБезНДСПринято] = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			...
			//+++AK GREK 06.09.2018 ИП-00018902.01^01 Цены обновлять не нужно, оставляем свои
			Если НЕ Параметры.ТипСообщения = "RECADV" Тогда
		...
		Для Каждого Строка Из ТаблицаСравнения Цикл
			...
			//+++AK GREK 02.11.2018 ИП-00019569              
			Если ЭтоДополняющаяКорректировка Тогда
				Если Строка[ИмяПоляКоличествоПринято] = Неопределено ИЛИ Строка[ИмяПоляКоличествоПринято] = 0 Тогда
					ТекстОшибки = Параметры.ТипСообщения + ". " + "Номенклатура: <" + Строка.Номенклатура + ">. Характеристика: <" + Строка.ХарактеристикаНоменклатуры + ">" + Символы.ПС +
										"Повторная корректировка, измеять строку не требуется.";
					ЗаписьЖурналаРегистрации("EDI. Расхождения при загрузке", УровеньЖурналаРегистрации.Предупреждение, , Параметры.ДокументСсылка, ТекстОшибки);
					Продолжить; //Не нужно обрабатывать отсутствующие строки
				КонецЕсли;
			КонецЕсли;
			//---AK
			
			Если Строка[ИмяПоляКоличествоПринято] = Неопределено Тогда
				Строка[ИмяПоляКоличествоПринято] = 0;
			КонецЕсли; 
			...
			Если НайденныеСтроки.Количество() = 0 Тогда
				Если Строка[ИмяПоляКоличествоПринято] = 0 Тогда
					Продолжить;
				Иначе
					ЕстьОшибки = Истина;
					Прервать;
		...
		Для Каждого Строка Из ТаблицаСравнения  Цикл
			...
			СтрокиEDI = ДокументОбъект.EDI_Товары.НайтиСтроки(Новый Структура("Номенклатура", Строка.Номенклатура));
			
			МассивКоличество = ДокументОбъект.EDI_Товары.Выгрузить(СтрокиEDI).ВыгрузитьКолонку("Количество");
			МассивСумма = ОбщегоНазначения.РаспределитьПропорционально(СтрокаТовара.Сумма, МассивКоличество);
			МассивСуммаНДС = ОбщегоНазначения.РаспределитьПропорционально(СтрокаТовара.СуммаНДС, МассивКоличество);
			
			Для Сч = 0 По СтрокиEDI.ВГраница() Цикл
				Если МассивСумма <> Неопределено Тогда
					СтрокиEDI[Сч].Сумма = МассивСумма[Сч];
				КонецЕсли; 
				Если МассивСуммаНДС <> Неопределено Тогда
					СтрокиEDI[Сч].СуммаНДС = МассивСуммаНДС[Сч];
				КонецЕсли; 
			КонецЦикла;
		КонецЦикла;  
		...
		Если НЕ ЕстьОшибки Тогда
			Попытка 
				ДокументОбъект.ДополнительныеСвойства.Вставить("НеПроверятьБлокировкуОбменомПоEDI", Истина);
		...
		Если ЕстьОшибки Тогда
			ТекстОшибки = Параметры.ТипСообщения + ". " + "Не удалось перенести корректировки в документ отгрузки."; 
			ЗаписьЖурналаРегистрации("EDI. Ошибка при загрузке", УровеньЖурналаРегистрации.Предупреждение, , Параметры.ДокументСсылка, ТекстОшибки);				
	...
	Если ИмяСобытия = "ИзменитьМетаданныеСообщения" Тогда
		...
		Если ТипСообщения = "DESADV" И Направление = "Исходящее" Тогда
			...
			ОсновнойМодуль.ДобавитьОписаниеМетаданныхСообщения(ТаблицаМетаданных, "СерийныйНомер",					"Товары",		"=",				,	"Строка178"	,	,	);
			ОсновнойМодуль.ДобавитьОписаниеМетаданныхСообщения(ТаблицаМетаданных, "СрокГодности",					"Товары",		"=",				,	"Дата"		,	,	);
			ОсновнойМодуль.ДобавитьОписаниеМетаданныхСообщения(ТаблицаМетаданных, "ДатаПроизводства",					"Товары",		"=",				,	"Дата"		,	,	);
		КонецЕсли;		
	КонецЕсли;		
		...
		Если Сообщение.ТипСообщения = "DESADV" Тогда
			...
			Если НЕ Сообщение.Товары.Колонки.Найти("СрокГодности") = Неопределено Тогда
				Если ЗначениеЗаполнено(Параметры.СтрокаТоваров.СрокГодности) Тогда
					...
					ЗначениеXDTO = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип("http://www.w3.org/2001/XMLSchema","string"),ОсновнойМодуль.Дата1С_в_ДатаXML(Параметры.СтрокаТоваров.ДатаПроизводства));
	...
	Если ИмяСобытия = "ПередВыполнениемОбмена" Тогда
		//+++AK GREK 06.11.2018 ЗА-00018906  
		Запрос = Новый Запрос("ВЫБРАТЬ
		                      |	ВозвратТоваровОтПокупателя.Ссылка КАК Объект,
		                      |	ВозвратТоваровОтПокупателя.Представление
		                      |ИЗ
		                      |	Документ.ВозвратТоваровОтПокупателя КАК ВозвратТоваровОтПокупателя
		                      |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КонтурEDI_Сообщения КАК КонтурEDI_Сообщения
		                      |		ПО (КонтурEDI_Сообщения.ТипСообщения = ""RETREC"")
		                      |			И ВозвратТоваровОтПокупателя.Ссылка = КонтурEDI_Сообщения.Документ
		                      |ГДЕ
		                      |	КонтурEDI_Сообщения.Документ ЕСТЬ NULL
		                      ...
		                      |	И ВозвратТоваровОтПокупателя.ПредъявленСчетФактура = ИСТИНА
		                      |	И ВозвратТоваровОтПокупателя.НомерВходящегоСчетФактуры <> """"
		                      |	И ВозвратТоваровОтПокупателя.ДатаВходящегоСчетаФактуры <> """"
		                      |	И ВозвратТоваровОтПокупателя.Дата > &ДатаНачалаОтправки");
		
		Запрос.УстановитьПараметр("ДатаНачалаОтправки",'20181101');
		
		ТЗ = Запрос.Выполнить().Выгрузить();
		
		Для каждого Стр из ТЗ Цикл
			Сообщение = ОсновнойМодуль.ПодготовитьИсходящееСообщение("RETREC", Стр.Объект);  
			...
			Если НЕ (Сообщение.СодержитОшибки = Истина) Тогда
				...
				ОсновнойМодуль.ОтправитьСообщение("RETREC",Стр.Объект,Параметры);
				
				ОсновнойМодуль.Сообщить_КонтурEDI("Создано сообщение: RETREC по документу: " + Стр.Объект);
			КонецЕсли;
		КонецЦикла;
		//---AK
	КонецЕсли;
...
//+++AK GREK 02.11.2018 ИП-00019569
Функция ЭтоНеПерваяКорректировкаРеализации(ДокументСсылка)
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	КонтурEDI_Сообщения.Ссылка
	               |ИЗ
