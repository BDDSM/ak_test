...
//+++АК LATV 2018.07.02 ИП-00019101
&НаКлиенте
Функция ТекущиеТорговыеТочки(ИспользоватьВсеЕслиНеВыбраны = Ложь, ВыбраныВсе = Ложь);
	...
	Если ТекВыделенныеСтроки.Количество() > 0 Тогда
		...
		Для Каждого СтрокаТаблицы Из ТекВыделенныеСтроки Цикл
			...
			Если ТекДанные <> Неопределено
			   И Не ТекДанные.НеУчаствуетВРаспределении
		...
		//Если ПроверитьТТ Тогда
		//	МассивТТ = ТорговыеТочкиУчаствующиеВРаспределении(МассивТТ);
		//КонецЕсли;
...
//+++АК KIRN 2018.09.07 ИП-00019773
	ТекстЗапроса = "select raspr_disable, CAST(date_add as datetime) date_add, d.date1 					
					...
					|	  where b.rn=1";
	//ТекстЗапроса = "select raspr_disable, CAST(date_add as datetime) date_add					
	//				|from (SELECT 
	//				|	  [raspr_disable], date_add, 
	//				|	  row_number() over (order by [date_add] desc) rn
	//				|	  FROM [M2].[dbo].[raspr_disable] as a with(nolock)) b
	...
	Попытка
		...
		Если НЕ rs.EOF() Тогда
			Если Rs.Fields("raspr_disable").Value = 1 Тогда
				...
				//Сообщить("Все действия по распределения пока запрещены. В SQL идет пересчет данных. Примерное время окончания работ " + Формат(Rs.Fields("date_add").Value, "ДФ='dd.MM.yyyy HH:mm'"));
				//---АК KIRN 
				Возврат Ложь;
			КонецЕсли;	
			rs.MoveNext();
...
//+++АК LATV 2018.07.05 ИП-00019101
&НаКлиенте
Процедура ОтправитьДанныеДляРаспределенияПоТТ(ПризнакТТ, мСписокТорговыхТочек, мСписокНоменклатуры, ПроверятьБлокировку = Истина, ВыбраныВсе=Истина)

	Если ПризнакТТ = "2016" Тогда
		ТекстНомер_Р = ДопМодульСервер.ОтправитьДанныеДляРаспределенияПоТТ(ЭтаФорма.ДатаЗаказов
			, мСписокТорговыхТочек, мСписокНоменклатуры, Объект.Склад,, ЭтаФорма.ОстаткиСледДнем, ПроверятьБлокировку, Объект.СкладХранения, ВыбраныВсе);
	//+++АК KIRN 2018.08.02 ИП-00019359.000.00000002
	ИначеЕсли ПризнакТТ = "2016_" Тогда //предварительное
		ТекстНомер_Р = ДопМодульСервер.ОтправитьДанныеДляРаспределенияПоТТ(ЭтаФорма.ДатаЗаказов
			, мСписокТорговыхТочек, мСписокНоменклатуры, Объект.Склад,, ЭтаФорма.ОстаткиСледДнем, ПроверятьБлокировку, Объект.СкладХранения, ВыбраныВсе, 1);
	//---АК KIRN 
	ИначеЕсли ПризнакТТ = "18421" Тогда
		ТекстНомер_Р = ДопМодульСервер.ОтправитьДанныеДляРаспределенияПоТТ_18421(ЭтаФорма.ДатаЗаказов
...
Процедура СделатьРаспределениеПоТТ(ПризнакТТ)
	...
	//
	ВыбраныВсе = Ложь;
	мСписокТорговыхТочек = ТекущиеТорговыеТочки(Истина, ВыбраныВсе);
	
	//
	мСписокНоменклатуры = ТекущиеТовары(Истина);
	ВыбранныеТовары = ОбщегоНазначенияКлиентСервер_СкопироватьСписокЗначений(мСписокНоменклатуры);
	...
	ОтправитьДанныеДляРаспределенияПоТТ(ПризнакТТ, мСписокТорговыхТочек, мСписокНоменклатуры, ВыбраныВсе);
...
//+++АК LATV 2018.07.05 ИП-00019101
&НаКлиенте
Функция ОбщегоНазначенияКлиентСервер_СкопироватьСписокЗначений(СписокИсточник)
...
//+++АК KIRN 2018.08.02 ИП-00019359 
&НаКлиенте
Процедура СделатьПредварительноеРаспределениеПоТТ_(Команда)
	СделатьРаспределениеПоТТ("2016_");
КонецПроцедуры
