...
&НаКлиенте
Процедура ДобавитьУборщицу(Команда)
	
	ПараметрыОтбора = Новый Структура("ЦФО, ТекущийПомощник", ГруппаТорговыхТочек, ТекущийПомощник);
	Результат = ОткрытьФормуМодально("Обработка.ГрафикРаботыУборщиц.Форма.ФормаВыбораФизическогоЛица", ПараметрыОтбора);
	
	Если ЗначениеЗаполнено(Результат)Тогда
		
		ДобавитьПервоеФизЛицоВГрафик(Результат);
		ИзменитьПорядокУборщицВТабеле(Результат, ТекущийПродавец);
...
&НаКлиенте
Процедура ДобавитьУборщицуСлева(Команда)
	...
	Если ТипЗнч(Расшифровка) <> Тип("ФиксированнаяСтруктура") тогда
		Предупреждение("Встаньте в колонку с уборщицей, относительно которой необходимо выполнить действие. Операция не выполнена.");
		Возврат;
	КонецЕсли;
...
Процедура ГруппаТорговыхТочекНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Результат = ОткрытьФормуМодально("Обработка.ГрафикРаботыУборщиц.Форма.ФормаВыбораСтруктурнойЕдиницы", ПараметрыОтбора);
...
&НаКлиенте
Процедура ПереместитьТекущуюУборщицу(Команда)
	...
	Результат = СписокСотрудников.ВыбратьЭлемент("Выбор уборщицы");
	Если Результат <> Неопределено Тогда
		
		
		//+++АК mika 2018.07.25 ИП-00019350 Некорректный порядок
		//ПеренестиУборщицуВТабеле(НомерКолонкиРасшифровки, Результат.Значение);
		Попытка
			ПеренестиУборщицуВТабеле(НомерКолонкиРасшифровки, Результат.Значение);
		Исключение
		...
		ИзменитьПорядокУборщицВТабеле(ТекущийПродавец, Результат.Значение);
		ОбновитьГрафикНаСервере();
		УстановитьВидимость();
...
Процедура СкопироватьГрафик(Команда)
	...
	Если ТипЗнч(Расшифровка) <> Тип("ФиксированнаяСтруктура") тогда
		Предупреждение("Встаньте в колонку с уборщицей, для которой необходимо скопировать график. Операция не выполнена.");
		Возврат;
	КонецЕсли;
	
	СписокСотрудников = ПолучитьСписокСотрудников();
	Результат = СписокСотрудников.ВыбратьЭлемент("Выбор уборщицы, с которой копируется график");
	Если Результат <> Неопределено Тогда
...
Процедура ОткрытьФормуСпискаГрафика(Команда)
	...
	Форма = ПолучитьФорму("РегистрСведений.ТабельРаботыУборщиц.Форма.ФормаСписка");
...
Процедура ПечатьБейджев(Команда)
	...
	Результат = ОткрытьФормуМодально("Обработка.ГрафикРаботыУборщиц.Форма.ФормаПодбораПродавцов", ПараметрыФормы);
...
Процедура ОткрытьОтчетОтклонениеГрафиковРаботыОтРеальныхВыходов(Команда)
	ОткрытьФорму("Отчет.ОтклонениеГрафиковРаботыУборщицОтРеальныхВыходов.Форма.ФормаОтчета",, ЭтаФорма);	
КонецПроцедуры
...
Процедура ЗаполнитьТаблицуСотрудникПериодНаСервере()
	...
	|	И ТабельРаботыУборщиц.Группа = &Группа
	|ИТОГИ
	...
	Если ВариантПостроения = 1 Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ТабельРаботыУборщиц.Группа", "ТабельРаботыУборщиц.ГруппаСотрудника");
		//Запрос.Текст = СтрЗаменить(Запрос.Текст, "ТабельРаботыУборщиц.Период,","ТабельРаботыУборщиц.Период, ТабельРаботы.Группа,");
		...
		Если ЗначениеЗаполнено(ТекущийУправляющий) Тогда
			...
			Если ЗначениеЗаполнено(ТекущийПомощник) Тогда
				Запрос.Текст = СтрЗаменить(Запрос.Текст, "И ТабельРаботыУборщиц.Период МЕЖДУ &НачалоПериода И &КонецПериода","И ТабельРаботыУборщиц.Период МЕЖДУ &НачалоПериода И &КонецПериода И ТабельРаботыУборщиц.Сотрудник.ПомощникТУ = &ТекущийПомощник");
	...
	Если ВариантПостроения = 1 Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "РегистрСведений.СпецификаФормированияТабеляУборщиц", "РегистрСведений.СпецификаФормированияТабеляУборщицПомощники");
	КонецЕсли;
	//---АК mika
...
Процедура ТаблицаСотрудникПериодИзменитьДанные()
	...
	Если МассивСтрок.Количество() = 1 Тогда
		...
		Результат 		  = ОткрытьФормуМодально("Обработка.ГрафикРаботыУборщиц.Форма.ФормаРедактированияПериода", ПараметрыОткрытия, ЭтаФорма);	
		
	Иначе
		
		// Без привязки к периоду
		ПараметрыОткрытия = Новый Структура("Сотрудник, ТаблицаДанных", ТекущийПродавец, ТЗ);
		Результат 		  = ОткрытьФормуМодально("Обработка.ГрафикРаботыУборщиц.Форма.ФормаРедактированияПериодаМассивСтрок", ПараметрыОткрытия, ЭтаФорма);
...
Процедура УстановитьВидимость(Открытие = Ложь)
	...
	Элементы.ДобавитьУборщицу.Видимость 			= НЕ ЕстьДанные
														И НЕ Открытие;
	Элементы.ДобавитьУборщицуСлева.Видимость 		= ЕстьДанные;
	Элементы.ПереместитьТекущуюУборщицу.Видимость= ЕстьДанные;
...
&НаСервере
Процедура ИзменитьПорядокУборщицВТабеле(НовоеФизЛицо, ТекущееФизЛицо)
	...
	//+++АК mika 2018.07.17 ИП-00019209
	//НаборЗаписей = РегистрыСведений.СпецификаФормированияТабеляУборщиц.СоздатьНаборЗаписей();
	Если ВариантПостроения = 1 Тогда
		НаборЗаписей = РегистрыСведений.СпецификаФормированияТабеляУборщицПомощники.СоздатьНаборЗаписей();
	Иначе
		НаборЗаписей = РегистрыСведений.СпецификаФормированияТабеляУборщиц.СоздатьНаборЗаписей();
	КонецЕсли;
	//---АК mika 
...
&НаСервере
Процедура ПеренестиУборщицуВТабеле(ТекущаяКолонка, ФизЛицо)
...
Процедура СкопироватьГрафикНаСервере(ДанныеКопирования)
	...
	Иначе
		Сообщить("Уборщица, с которой производится копирование не найдена в графике");
		Возврат;
	КонецЕсли;
...
Процедура ОчиститьЗаписиВРегистреЗапрос(Данные) //+++АК mika 2018.09.13 ИП-00019819
	...
	|ИЗ
	|	РегистрСведений.ТабельРаботыУборщиц КАК ТабельРаботы
	|ГДЕ
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			НаборЗаписей = РегистрыСведений.ТабельРаботыУборщиц.СоздатьНаборЗаписей();
