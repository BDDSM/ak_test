...
Процедура УбратьИзАссортиментаВсехМагазиновНаСервере(НоменклатураСплитТестированияСсылка)
	...
	Запрос = Новый Запрос(
		...
		|	ТоварныйАссортиментТочекСрезПоследних.ТорговаяТочка
		|ИЗ
		|	ТоварныйАссортиментТочекСрезПоследних КАК ТоварныйАссортиментТочекСрезПоследних
		...
		|		ПО ТоварныйАссортиментТочекСрезПоследних.ТорговаяТочка.ТипРозничнойТочки = СоответствиеТиповРозничныхТочекИТорговыхМарок.ТипРозничнойТочки
		|			И ТоварныйАссортиментТочекСрезПоследних.Номенклатура.ТорговаяМарка = СоответствиеТиповРозничныхТочекИТорговыхМарок.ТорговаяМарка
		|ГДЕ
		|	ЕСТЬNULL(СоответствиеТиповРозничныхТочекИТорговыхМарок.ТорговаяМарка, &ПустаяТМ) = ТоварныйАссортиментТочекСрезПоследних.Номенклатура.ТорговаяМарка");
	Запрос.УстановитьПараметр("ТекДата", КонецДня(ТекущаяДата()));
	Запрос.УстановитьПараметр("ПустаяТМ", ПредопределенноеЗначение("Справочник.ТорговыеМарки.ПустаяСсылка"));
	...
	НаборЗаписейТАТ = РегистрыСведений.ТоварныйАссортиментТочек.СоздатьНаборЗаписей();
	НаборЗаписейТАТ.ОбменДанными.Загрузка = Истина;
	МенеджерЗаписиТАТ = РегистрыСведений.ТоварныйАссортиментТочек.СоздатьМенеджерЗаписи();
	
	НаборЗаписейАСТ = РегистрыСведений.АссортиментСплитТестирования.СоздатьНаборЗаписей();
	НаборЗаписейАСТ.ОбменДанными.Загрузка = Истина;
	...
	Пока ВыборкаЗапроса.Следующий() Цикл
		...
		Если МенеджерЗаписиАСТ.Выбран() Тогда
			Если МенеджерЗаписиАСТ.Добавлен = -ДобавленаТТ Тогда
				МенеджерЗаписиАСТ.Удалить();
			КонецЕсли;
		КонецЕсли;
		
		Если МенеджерЗаписиТАТ.Выбран() Тогда 
			Если МенеджерЗаписиТАТ.Выведена = Ложь Тогда
				МенеджерЗаписиТАТ.Удалить();
			Иначе
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		
		ЗаписьТАТ = НаборЗаписейТАТ.Добавить();
		ЗаполнитьЗначенияСвойств(ЗаписьТАТ, СтруктураОтбор);
		ЗаписьТАТ.Выведена = Истина;
		ЗаписьТАТ.Комментарий = "Сплит-тестирование товаров: выводим из ассортимента";
		
		ЗаписьАСТ = НаборЗаписейАСТ.Добавить();
		ЗаполнитьЗначенияСвойств(ЗаписьАСТ, СтруктураОтбор);
		ЗаписьАСТ.Добавлен = ДобавленаТТ;
		
	КонецЦикла;
	
	Если НаборЗаписейТАТ.Количество() > 0 Тогда
		НаборЗаписейТАТ.Записать(Ложь);
	КонецЕсли;
	
	Если НаборЗаписейАСТ.Количество() > 0 Тогда
		НаборЗаписейАСТ.Записать(Ложь);
	КонецЕсли;
