...
Процедура ОбновитьСписокСОстатками()
	...
	Запрос.Текст = "ВЫБРАТЬ
				   ...
				   //ITseboev + 12.12.2018 ИП-00020699 Исключаем незаполненный вид рекламного материала
				   |		И НЕ ЗаявкиНаРекламныеМатериалыОстатки.Номенклатура.ПометкаУдаления
				   |		И ЗаявкиНаРекламныеМатериалыОстатки.Номенклатура.ВидРекламногоМатериала <> ЗНАЧЕНИЕ(Перечисление.ПодразделРекламныхМатериалов.ПустаяСсылка)
				   //ITseboev - 12.12.2018 ИП-00020699
	               |	
	               |	ОБЪЕДИНИТЬ ВСЕ
	               |	
	               |	ВЫБРАТЬ
				   ...
				   |		И НЕ ЗаявкаНаРекламныеМатериалыТовары.Номенклатура.ПометкаУдаления				   
				   //---АК mika ИП-00018800
				   //ITseboev + 12.12.2018 ИП-00020699 Исключаем незаполненный вид рекламного материала  
				   |		И ЗаявкаНаРекламныеМатериалыТовары.Номенклатура.ВидРекламногоМатериала <> ЗНАЧЕНИЕ(Перечисление.ПодразделРекламныхМатериалов.ПустаяСсылка)) КАК ВЗ_Запрос
				   //ITseboev - 12.12.2018 ИП-00020699
				   |СГРУППИРОВАТЬ ПО
				   |	ВЗ_Запрос.Номенклатура
				   |;
				   ...
				   //ITseboev + 12.12.2018 ИП-00020699 Исключаем незаполненный вид рекламного материала
				   |				И Номенклатура.ВидРекламногоМатериала <> ЗНАЧЕНИЕ(Перечисление.ПодразделРекламныхМатериалов.ПустаяСсылка)) КАК ТоварыНаСкладахОстатки
				   //ITseboev - 12.12.2018 ИП-00020699
	               |ГДЕ
				   ...
				   |ГДЕ
	               |	НЕ НоменклатураСпр.Ссылка.ПометкаУдаления
				   |	И НоменклатураСпр.ВидРекламногоМатериала <> ЗНАЧЕНИЕ(Перечисление.ПодразделРекламныхМатериалов.ПустаяСсылка)
	               |
	               |УПОРЯДОЧИТЬ ПО
				   //ITseboev + 12.12.2018 ИП-00020699 Сортировка по виду рекламного материала
				   |	Вид,
				   //ITseboev - 12.12.2018 ИП-00020699
...
Процедура СоздатьЗаявку(Команда)
	...
	ФормаДок = ОткрытьФорму("Документ.ЗаявкаНаРекламныеМатериалы.ФормаОбъекта");	
	СтруктураЗаявки = ПараметрыЗаявки(); 
	Если СтруктураЗаявки.ТекМагазин <> НЕОПРЕДЕЛЕНО Тогда 
		ФормаДок.Объект.ТорговаяТочка = СтруктураЗаявки.ТекМагазин;
	КонецЕсли;
	ФормаДок.Объект.ТипЗаявки = СтруктураЗаявки.ТипЗаявки;	
	Если СтруктураЗаявки.РольПродавец = Истина Тогда 
		ФормаДок.Элементы.ТорговаяТочка.Доступность = Ложь;
		ФормаДок.Элементы.ТипЗаявки.Доступность = Ложь;
		ФормаДок.Элементы.ЗаявкаНаНовыйМагазин.Доступность = Ложь;
	КонецЕсли;
...
Процедура СоздатьЗаявкуПоОтмеченным(Команда)
	...
	Если МассивТовары.Количество() > 0 Тогда	
		ФормаДок = ОткрытьФорму("Документ.ЗаявкаНаРекламныеМатериалы.ФормаОбъекта", Новый Структура("МассивТоварыПриСоздании", МассивТовары));	
		СтруктураЗаявки = ПараметрыЗаявки(); 
		Если СтруктураЗаявки.ТекМагазин <> НЕОПРЕДЕЛЕНО Тогда 
			ФормаДок.Объект.ТорговаяТочка = СтруктураЗаявки.ТекМагазин;
		КонецЕсли;
		...
		КонецЕсли;
	Иначе	
		Сообщить("Не отмечено не одной позиции из списка!");	
	КонецЕсли;
	//ITseboev - 12.12.2018 ИП-00020699
...
//ITseboev + 12.12.2018 ИП-00020699
&НаСервере
Функция ПараметрыЗаявки()
	
	СтруктураПараметров = Новый Структура();
	СтруктураПараметров.Вставить("ТипЗаявки", Перечисления.ТипЗаявкиНаРекламныеМатериалы.ЗаявкаМагазина); 
	СтруктураПараметров.Вставить("ТекМагазин", ПараметрыСеанса.ТорговаяТочкаПоАйпи);
	СтруктураПараметров.Вставить("РольПродавец", РольДоступна("Продавец"));	
