...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Если Параметры.Свойство("СтруктураТаблиц") = Истина Тогда
		Объект.Магазины.Очистить();
		
		Для каждого Стр Из Параметры.СтруктураТаблиц.ТЗМагазинов Цикл
			Нстр = Объект.Магазины.Добавить();
			Нстр.Выбран = Истина;
			Нстр.Магазин = Стр.Магазин;
			Нстр.Тип = Стр.Тип;			
			Нстр.Статус = Стр.Статус;			
			Нстр.Формат = Стр.Формат;			
		КонецЦикла;
		
		Объект.Товары.Очистить();
		Для каждого Стр Из Параметры.СтруктураТаблиц.ТЗНоменклатуры Цикл
...
Процедура ОтчетПоАссортименту(Команда)
	ПараметрыФормы = Новый Структура("Ключ", ГУИД);
	ОткрытьФорму("Отчет.ОтчетПоАссортиментуТочекЗЛ.Форма.ФормаОтчета",ПараметрыФормы ,ЭтаФорма, ГУИД);
КонецПроцедуры
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
		...
		Для каждого Стр Из ВыбранноеЗначение.ТЗМагазинов Цикл
			Нстр = Объект.Магазины.Добавить();
			Нстр.Выбран = Истина;
			Нстр.Магазин = Стр.Магазин;
			Нстр.Тип = Стр.Тип;			
		...
		Объект.Товары.Очистить();
		Для каждого Стр Из ВыбранноеЗначение.ТЗНоменклатуры Цикл
			Нстр = Объект.Товары.Добавить();
			Нстр.Номенклатура = Стр.Номенклатура;
			Нстр.Характеристика = Стр.Характеристика;			
		КонецЦикла;  	
