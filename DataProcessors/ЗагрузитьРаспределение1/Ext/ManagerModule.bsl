...
Функция СодатьДокументыПоРаспределениюНаСервере_РО(СтруктураНастроек)
	...
	ТабСформированныеМаршЛисты = СоответствиеДляСформированныхМаршрутов(Результаты[4]); // Получить из результата запроса соответствие маршрутных листов
...
Функция СодатьДокументыПоРаспределениюНаСервереПоЗонам_РО(СтруктураНастроек)
	...
	ТабОстаткиНоменклатуры = Результаты[4].Выгрузить();
	ТабОстаткиНоменклатуры.Индексы.Добавить("Склад, Номенклатура, Характеристика");
	
	ТабСформированныеМаршЛисты = Результаты[5].Выгрузить();
