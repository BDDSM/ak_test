...
Функция СодатьДокументыПоРаспределениюНаСервере_РО(СтруктураНастроек, СписокОшибок)
	...
	Пока Выборка.СледующийПоЗначениюПоля("Маршрут") Цикл
		...
		Если ЗначениеЗаполнено(Выборка.Маршрут) Тогда
			// Маршрутные листы создаются вне зависимости от того, есть маршрут или нет. Так было, надо или нет не понятно. В ЗаполнитьРасходникиПоЗаданиямНаРазборку не создаются
			ЕстьМаршрут = Истина;
		КонецЕсли;
					...
					Если Не ЕстьМаршрут Тогда
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Есть товары, без маршрута. ТорговаяТочка = %1; Номенклатура = %2; Характеристика = %3'")
								, Выборка.ТорговаяТочка, Выборка.Номенклатура, Выборка.Характеристика);
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
						
						СкладыСервер.ЗаписатьЛогЗагрузкиДокументовПоРаспределению(Склад, РасхОрдерОбъект.Ссылка, 0, ТекстСообщения);
					КонецЕсли;
...
Функция ПолучитьРейсПоМаршруту(Маршрут, ДатаРаспределения, Организация, Автомобиль, тзТарифы)
	...
	Иначе
		...
		Если Не РезультатДанныеМаршрута.Пустой() Тогда
