...
Процедура СвойствоСотрудникаПриИзменении(Элемент)
	...
	ПроверитьКорректностьЗаполненияКоличестваЧасов(); //+++АК mika 2018.12.07 ИП-00020658
...
&НаКлиенте
Процедура ПроверитьКорректностьЗаполненияКоличестваЧасов() //+++АК mika 2018.12.07 ИП-00020658

	Если СвойствоСотрудника = 2 Тогда
		Если КоличествоЧасовПочасовика > 5 Тогда //+++АК mika 2018.12.13 ИП-00020658^01
			КоличествоЧасовПочасовика = 5;
			ПоказатьПредупреждение(, "Сотрудники со свойством Почасовик могут планироваться максимум на 5 ч.!");
			Возврат;
		КонецЕсли;
	КонецЕсли;
