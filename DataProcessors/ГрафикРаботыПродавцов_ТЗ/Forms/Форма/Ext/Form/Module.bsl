...
&НаКлиенте
Процедура ПриОткрытии(Отказ) //+++АК mika 2017.12.27 ИП-00016863.01
...
Процедура ОбновитьГрафикНаСервере()
	...
	|ИЗ
	|	РегистрСведений.СпецификаФормированияТабеля КАК СпецификаФормированияТабеля
	|ГДЕ
	...
	Если ВариантПостроения = 1 Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "РегистрСведений.СпецификаФормированияТабеля", "РегистрСведений.СпецификаФормированияТабеляПомощники");
	КонецЕсли;
	//---АК mika
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СпецификаФормированияТабеля КАК СпецификаФормированияТабеля
	|		ПО ТабельРаботыПродавцов.Группа = СпецификаФормированияТабеля.Группа
	|			И ТабельРаботыПродавцов.Сотрудник = СпецификаФормированияТабеля.ФизическоеЛицо
	...
	|ГДЕ
	|	ТабельРаботыПродавцов.Группа = &Группа
	...
	//Возможность "Помощнику" формировать график по "своим" продавцам по подчиненным сотрудникам,
	//независимо от группы на которою было распределение 
	Если ВариантПостроения = 1 Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ТабельРаботыПродавцов.Группа", "ТабельРаботыПродавцов.ГруппаСотрудника");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ТабельРаботыПродавцов.Период,","ТабельРаботыПродавцов.Период, ТабельРаботыПродавцов.Группа,");
		...
		Если ЗначениеЗаполнено(ТекущийУправляющий) Тогда
			...
			Если ЗначениеЗаполнено(ТекущийПомощник) Тогда
				Запрос.Текст = СтрЗаменить(Запрос.Текст, "И ТабельРаботыПродавцов.Период МЕЖДУ &НачалоПериода И &КонецПериода","И ТабельРаботыПродавцов.Период МЕЖДУ &НачалоПериода И &КонецПериода И ТабельРаботыПродавцов.Сотрудник.ПомощникТУ = &ТекущийПомощник");
...
Процедура ОбновитьГрафикНаСервереСКД() //+++АК mika 2018.08.20 ИП-00019541
	...
	|	ТабельРаботыПродавцов.Период,
	|	ТабельРаботыПродавцов.Сотрудник,
	|	ТабельРаботыПродавцов.Сотрудник.ПомощникТУ КАК Помощник,
	|	ТабельРаботыПродавцов.ТорговаяТочка,
	|	ТабельРаботыПродавцов.Отсутствие,
	|	ТабельРаботыПродавцов.СвойствоПродавца,
	|	ТабельРаботыПродавцов.КоличествоЧасов,
	...
	Если ВариантПостроения = 1  Тогда
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ТабельРаботыПродавцов.Группа", "ТабельРаботыПродавцов.ГруппаСотрудника");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ТабельРаботыПродавцов.Период,","ТабельРаботыПродавцов.Период, ТабельРаботыПродавцов.Группа,");
		...
		Если НЕ ИспользоватьОтборы Тогда
			...
			Если ЗначениеЗаполнено(ТекущийУправляющий) Тогда
				//Для управляющего использовать отбор по помощнику если он установлен, иначе отображать данные по группе сотрудника "в целом".
				Если ЗначениеЗаполнено(ТекущийПомощник) Тогда
					ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "И ТабельРаботыПродавцов.Период МЕЖДУ &НачалоПериода И &КонецПериода","И ТабельРаботыПродавцов.Период МЕЖДУ &НачалоПериода И &КонецПериода И ТабельРаботыПродавцов.Сотрудник.ПомощникТУ = &ТекущийПомощник");
				КонецЕсли;
			Иначе
				ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "И ТабельРаботыПродавцов.Период МЕЖДУ &НачалоПериода И &КонецПериода","И ТабельРаботыПродавцов.Период МЕЖДУ &НачалоПериода И &КонецПериода И ТабельРаботыПродавцов.Сотрудник.ПомощникТУ = &ТекущийПомощник");
			КонецЕсли;
	...
	//Если ВариантПостроения = 1 Тогда  //+++АК mika 2018.08.22 ИП-00019541 Формирования порядка всегда в разрезе групп помощников
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "РегистрСведений.СпецификаФормированияТабеля", "РегистрСведений.СпецификаФормированияТабеляПомощники");
	//КонецЕсли;
	...
	Для Каждого Строка Из ТаблицаЗначений Цикл
		...
		СтруктураДанных = Новый Структура;
		СтруктураДанных.Вставить("СвойствоПродавца"					, Строка.СвойствоПродавца);
		...
		СтруктураДанных.Вставить("ТорговаяТочкаТипРозничнойТочки"	, Строка.ТорговаяТочкаТипРозничнойТочки);
	...
	ТЗСотрудники = ТЗОсновная.Скопировать();
	ТЗСотрудники.Свернуть("Помощник, Сотрудник, Порядок, КодСотрудника, Телефон, Статус, БлижайшийМагазин, ПЧ");
...
Процедура ОтктытьРегистрСведений_ТабельРаботыПродавцов(Команда)
	...
	Форма = ПолучитьФорму("РегистрСведений.ТабельРаботыПродавцов.Форма.ФормаСписка");
...
&НаКлиенте
Процедура ВариантПостроенияПриИзменении(Элемент) //+++АК mika 2017.12.27 ИП-00016863.01
...
&НаКлиенте
Процедура ВопросСменитьВариантПостроленияЗавершение(Результат, ДопПараметры) Экспорт //+++АК mika 2017.12.27 ИП-00016863.01
...
Процедура ОсновнаяТаблицаПередНачаломИзменения(Элемент, Отказ)
	...
	//										ТекущийПродавец, ТТ, Отсутствие, СвойствоПродавца, КоличествоЧасов,Элементы.ОсновнаяТаблица.ТекущиеДанные.Период,ТекДанные.Тренинг, ТекДанные.ЭкстренныйВызов);
	//	
	...
	//		КонецЕсли;
	//		
	//		Если ЭтоМагазин(ТорговаяТочка) Тогда
...
Процедура ОбработатьВыделенныеСтрокиНаСервере(СтруктураДанных)
	...
	Для Каждого СтрокаМассива Из МассивСтрок Цикл
		...
		//+++АК mika 2017.12.28 ИП-00016863.01
		Если ВариантПостроения = 1 Тогда
			...
			ТекСтр.Группа = СтруктураДанных.ГруппаНовая;
...
Функция ЗаписатьИзмененияВРегистр(ПараметрыСотрудника)
	...
	Если ЗначениеЗаполнено(ПараметрыСотрудника.Период) И ЗначениеЗаполнено(ГруппаТорговыхТочек) И ЗначениеЗаполнено(ПараметрыСотрудника.Сотрудник) И ВариантПостроения = 0 Тогда //+++АК mika 2017.12.28 ИП-00016863.01 Добавлено "И ВариантПостроения = 0", данная очистка актуальна только для "стандартного алгоритма".
		
		НЗ = РегистрыСведений.ТабельРаботыПродавцов.СоздатьНаборЗаписей();
...
Процедура УдалитьЗаписиНаСервере();
	...
	Для Каждого СтрокаМассива Из МассивВыделенныхСтрок Цикл
		...
		// Очистим запись по полям Период, Группа, Сотрудник
		Если ЗначениеЗаполнено(СтруктураДанных.Период) И ЗначениеЗаполнено(ГруппаТорговыхТочек) И ЗначениеЗаполнено(СтруктураДанных.Сотрудник) И ВариантПостроения = 0 Тогда //+++АК mika 2017.12.28 ИП-00016863.01 Добавлено "И ВариантПостроения = 0", данная очистка актуальна только для "стандартного алгоритма".
...
&НаСервере
Процедура ИзменитьПорядокПродавцовВТабеле_Отключен(НовоеФизЛицо, ТекущееФизЛицо) //+++АК mika 2018.08.22 ИП-00019541 процедура отключена
	...
	//+++АК mika 2018.07.17 ИП-00019209
	//НаборЗаписей = РегистрыСведений.СпецификаФормированияТабеля.СоздатьНаборЗаписей();;
	Если ВариантПостроения = 1 Тогда
		НаборЗаписей = РегистрыСведений.СпецификаФормированияТабеляПомощники.СоздатьНаборЗаписей();
	Иначе
		НаборЗаписей = РегистрыСведений.СпецификаФормированияТабеля.СоздатьНаборЗаписей();
	КонецЕсли;
	//---АК mika 
...
Процедура СкопироватьГрафикСДругогоПродавцаНаСервере(ДанныеКопирования)
	...
	ТаблицаДанных = ТЗ.СкопироватьКолонки("Период, ТорговаяТочка, Отсутствие, СвойствоПродавца, Представление,Тренинг");
	...
	Иначе
		Сообщить("Продавец с которого производится копирование не найден в графике");
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ОсновнаяТаблицаПредставлениеИзменениеТекстаРедактирования(Элемент, Текст, СтандартнаяОбработка)
...
Процедура ОсновнаяТаблицаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ОсновнаяТаблицаПредставление" Тогда
		СтандартнаяОбработка = Ложь;
		...
		Если ЗначениеЗаполнено(ТекущийПродавец)Тогда
			...
			Если ВариантПостроения = 1 Тогда
				...
				ПараметрыОткрытия = Новый Структура("Сотрудник, ТорговаяТочка, Отсутствие, СвойствоПродавца, КоличествоЧасов,ТекДата,Тренинг, ЭкстренныйВызов, Группа, ГруппаПомощника, ВариантПостроения",
						ТекущийПродавец, ТТ, Отсутствие, СвойствоПродавца, КоличествоЧасов,Элементы.ОсновнаяТаблица.ТекущиеДанные.Период,ТекДанные.Тренинг, ТекДанные.ЭкстренныйВызов, ТекДанные.Группа, ГруппаТорговыхТочек, 1);
...
//
Процедура ОбновитьВозможностьИзмененияВариантаПостроения(ОбновитьГруппу) //+++АК mika 2017.12.27 ИП-00016863.01 
...
//
Функция ПолучитьГруппуТорговыхТочекСерверБезКонтекста(ТекущийПомощник) //+++АК mika 2017.12.27 ИП-00016863.01
...
//
Процедура ОбновитьВозможностьИзмененияГруппыТорговыйТочек() //+++АК mika 2017.12.27 ИП-00016863.01
...
//
Процедура ОчиститьЗаписиПоСтаройГруппеТорговыхТочек(ПараметрыСотрудника) //+++АК mika 2017.12.27 ИП-00016863.01
	...
	Если ЗначениеЗаполнено(ПараметрыСотрудника.Период) И ЗначениеЗаполнено(ПараметрыСотрудника.ГруппаСтарая) И ЗначениеЗаполнено(ПараметрыСотрудника.Сотрудник)Тогда
		...
		НаборЗаписей = РегистрыСведений.ТабельРаботыПродавцов.СоздатьНаборЗаписей();
...
&НаКлиенте
Процедура ТекущийПомощникНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка) //+++АК mika 2018.01.09 ИП-00016863.01
...
&НаКлиенте
Процедура ОбработатьВыборПомощника(Результат, ДополнительныеПараметры) Экспорт //+++АК mika 2018.01.09 ИП-00016863.01
...
Процедура ОбновитьТаблицуСпецификиФормирования() //+++АК mika 2018.08.22 ИП-00019541
	...
	|	СпецификаФормированияТабеля.Порядок КАК Порядок
	|ИЗ
	|	РегистрСведений.СпецификаФормированияТабеляПомощники КАК СпецификаФормированияТабеля
	|ГДЕ
	|	СпецификаФормированияТабеля.Группа = &Группа
	|
	|УПОРЯДОЧИТЬ ПО
