...
Процедура РассчитатьНаборДанных(ВнешнийНаборДанных, НастройкиВывода) //+++АК mika 2017.12.26 ИП-00017572 Процедура модифицирована (Актуальная)
	
	Сообщить(СтрЗаменить(НСтр("ru = 'Начало сопоставления: Период';"), "Период", ТекущаяДата()));
	...
	ВнешнийНаборДанных.Колонки.Добавить("ЗадолженностьФин",Новый ОписаниеТипов("Число",Новый КвалификаторыЧисла(15,2)));
	ВнешнийНаборДанных.Колонки.Добавить("ЗадолженностьБух",Новый ОписаниеТипов("Число",Новый КвалификаторыЧисла(15,2)));
	...
	СчетаФинансы = Новый Массив();
	СчетаФинансы.Добавить(ПланыСчетов.Финансовый.НайтиПоКоду("60.1"));
	СчетаФинансы.Добавить(ПланыСчетов.Финансовый.НайтиПоКоду("60.2"));
	СчетаФинансы.Добавить(ПланыСчетов.Финансовый.НайтиПоКоду("62.2"));
	//+++АК sils 28.01.2018 ИП-00017572.01
	НайдКод = ПланыСчетов.Финансовый.НайтиПоКоду("60.22");
	Если НайдКод <> ПланыСчетов.Финансовый.ПустаяСсылка() Тогда
		СчетаФинансы.Добавить(НайдКод);
	КонецЕсли;
	//---АК

	МассивКонтрагентов = ВернутьМассивКонтрагентов(СчетаФинансы, Период); 	
	...
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Остатки(&Период, Счет В ИЕРАРХИИ(&Счета), ,(Субконто1 В ИЕРАРХИИ (&ПапкаАренда) И Субконто1 В ИЕРАРХИИ (&Контрагенты)) И Организация В (&Организации)) КАК ХозрасчетныйОстатки";
	
	Запрос.УстановитьПараметр("Период", СоединениеСБазой.NewObject("Граница", КонецДня(Период), СоединениеСБазой.ВидГраницы.Включая));
	
	Счета = СоединениеСБазой.NewObject("Массив");
	
	Счета.Добавить(СоединениеСБазой.ПланыСчетов.Хозрасчетный.НайтиПоКоду("60.01"));
	Счета.Добавить(СоединениеСБазой.ПланыСчетов.Хозрасчетный.НайтиПоКоду("60.02"));
	
	Счета.Добавить(СоединениеСБазой.ПланыСчетов.Хозрасчетный.НайтиПоКоду("60.31"));
	Счета.Добавить(СоединениеСБазой.ПланыСчетов.Хозрасчетный.НайтиПоКоду("60.32"));
	
	Счета.Добавить(СоединениеСБазой.ПланыСчетов.Хозрасчетный.НайтиПоКоду("62.01"));
	Счета.Добавить(СоединениеСБазой.ПланыСчетов.Хозрасчетный.НайтиПоКоду("62.02"));
	Счета.Добавить(СоединениеСБазой.ПланыСчетов.Хозрасчетный.НайтиПоКоду("62.31"));
	
	Счета.Добавить(СоединениеСБазой.ПланыСчетов.Хозрасчетный.НайтиПоКоду("76.05"));
	Счета.Добавить(СоединениеСБазой.ПланыСчетов.Хозрасчетный.НайтиПоКоду("76.35"));
	...
	МассивОрганизацийБУ = ТаблицаОрганизацийБУ.ВыгрузитьКолонку("ОрганизацияБУ");
	Для каждого Организация из МассивОрганизацийБУ Цикл
		Организации.Добавить(Организация);
	КонецЦикла;
	Запрос.УстановитьПараметр("Организации",Организации);

	Если ПолноеСравнение Тогда	
		
		Запрос.УстановитьПараметр("Контрагенты", Запрос.Parameters.ПапкаАренда);

	Иначе
		
		Контрагенты = СоединениеСБазой.NewObject("Массив");
		МассивКонтрагентовБУ = ТаблицаКонтрагентовБУ.ВыгрузитьКолонку("КонтрагентБУ");              
		Для каждого Контрагент из МассивКонтрагентовБУ Цикл
	...
	ФинБухРезультат.Колонки.Добавить("Остаток",Новый ОписаниеТипов("Число"));
	...
	Для каждого СтрокаРез из ТЗРезультатВБух Цикл
		...
		Если ПолноеСравнение И СтрокаСопоставленияКон = Неопределено Тогда
			
			ГУИД = СоединениеСБазой.String(СтрокаРез.Контрагент.УникальныйИдентификатор());
			
			УникальныйИдентификатор = Новый УникальныйИдентификатор(ГУИД);
			Контрагент = Справочники.Контрагенты.ПолучитьСсылку(УникальныйИдентификатор);

			Если Лев(СокрЛП(Контрагент),1) = "<" Тогда
				
				КонтрагентТекущаяБаза = ПолучитьКонтрагентаПоИННТекущаяБаза(СтрокаРез.Контрагент);
				
				Если КонтрагентТекущаяБаза = Неопределено Тогда
					
					Сообщить("Контрагент бух. базы не найден обратным поиском! Контрагент: " + СтрокаРез.Контрагент.Наименование + " сумма:" + СтрокаРез.Остаток, СтатусСообщения.ОченьВажное);
					
				Иначе
					
					Контрагент = КонтрагентТекущаяБаза;
					
					НоваяСтрока = ФинБухРезультат.Добавить();
					НоваяСтрока.Контрагент = Контрагент;
					НоваяСтрока.Организация = СтрокаСопоставленияОрг.Организация;	
					НоваяСтрока.Остаток = СтрокаРез.Остаток;
					
					Продолжить;
					
				КонецЕсли;
			Иначе
				//Сообщить("Контрагент бух. базы обнаружен обратным поиском! Контрагент: " + СтрокаРез.Контрагент.Наименование + " сумма:" + СтрокаРез.Остаток, СтатусСообщения.Информация);
			КонецЕсли;
		...
		Если СтрокаСопоставленияКон = Неопределено или СтрокаСопоставленияОрг = Неопределено Тогда
			Сообщить("Не найдено соответствие контрагенту бух базы! Контрагент: " + СтрокаРез.Контрагент.Наименование+ " Организация: " + СтрокаРез.Организация.Наименование + " сумма:" + СтрокаРез.Остаток);
			ИтоговаяСумма = ИтоговаяСумма + СтрокаРез.Остаток;
			Продолжить;
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ИтоговаяСумма)Тогда
		Сообщить(СтрЗаменить(НСтр("ru = 'Итог отклонения: ИтоговаяСумма';"), "ИтоговаяСумма", Окр(ИтоговаяСумма, 2)));
		Сообщить(НСтр("ru = 'ПРИМЕЧАНИЕ: Для полного поиска контрагентов необходимо установить флажки ""Все группы"" и ""Полное сопоставление""';") + Символы.ПС);
	КонецЕсли;
	...
	|	ЕСТЬNULL(ФинансовыйОстатки.СуммаОстаток,0) КАК ЗадолженностьФин,
	|	ЕСТЬNULL(ДанныеБУ.Остаток,0) КАК ЗадолженностьБух
	|ИЗ
	|	РегистрБухгалтерии.Финансовый.Остатки(
	|			&Период,
	|			Счет В ИЕРАРХИИ (&Счета),
	|			,
	|			(Субконто2 В (&Контрагенты) ИЛИ ИСТИНА)    
	|				И Организация В (&Организации)) КАК ФинансовыйОстатки
	...
	Запрос.УстановитьПараметр("Контрагенты", ФинБухРезультат.ВыгрузитьКолонку("Контрагент"));;
	...
	Запрос.УстановитьПараметр("Счета",  СчетаФинансы);
	...
	Если не Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			ЗаполнитьЗначенияСвойств(ВнешнийНаборДанных.Добавить(), Выборка);
		КонецЦикла;
		
	КонецЕсли;	
	
	Сообщить(СтрЗаменить(НСтр("ru = 'Конец сопоставления: Период';"), "Период", Строка(ТекущаяДата()) + Символы.ПС));

КонецПроцедуры

Процедура РассчитатьНаборДанных_(ВнешнийНаборДанных,НастройкиВывода) //+++АК mika 2017.12.26 ИП-00017572 Оригинальная процедура (не используется)
	...
	//+++АК mika 2017.12.26 ИП-00017572
	СчетаФинансы = Новый Массив();
	СчетаФинансы.Добавить(ПланыСчетов.Финансовый.НайтиПоКоду("60.1"));
	СчетаФинансы.Добавить(ПланыСчетов.Финансовый.НайтиПоКоду("60.2"));
	СчетаФинансы.Добавить(ПланыСчетов.Финансовый.НайтиПоКоду("62.2"));
	...
	МассивКонтрагентов = ВернутьМассивКонтрагентов(СчетаФинансы, Период); //+++АК mika 2017.12.26 ИП-00017572 Передается параметр	
	МассивОрганизаций = ВернутьМассивОрганизаций();	
	#Если ТолстыйКлиентОбычноеПриложение Тогда
	Состояние("Сопоставление контрагентов и организаций между базами");
	#КонецЕсли 
	...
	Запрос.Текст = 
					...
					|	ХозрасчетныйОстатки.СуммаОстаток КАК Остаток					
					|ИЗ
					|	РегистрБухгалтерии.Хозрасчетный.Остатки(&Период, Счет В ИЕРАРХИИ(&Счета), ,Субконто1 В ИЕРАРХИИ (&ПапкаАренда) И Организация В (&Организации)) КАК ХозрасчетныйОстатки";
					
	//+++АК mika Дата: 2017.08.01
	...
	//+++АК mika ИП-00016272 Дата: 2017.07.25
	...
	//Комментарий: Некорректное ТЗ, счет отсутствует в бух. базе
	//Счета.Добавить(СоединениеСБазой.ПланыСчетов.Хозрасчетный.НайтиПоКоду("62.2"));
	
	//Комментарий: Дополнение к ТЗ от 03.08.2017, добавить два счета: 60.31, 60.32 
	Счета.Добавить(СоединениеСБазой.ПланыСчетов.Хозрасчетный.НайтиПоКоду("60.31"));
	Счета.Добавить(СоединениеСБазой.ПланыСчетов.Хозрасчетный.НайтиПоКоду("60.32"));
	
	//Комментарий: Дополнение к ТЗ от 03.08.2017, добавить два счета: 62.01, 62.02 
	Счета.Добавить(СоединениеСБазой.ПланыСчетов.Хозрасчетный.НайтиПоКоду("62.01"));
	Счета.Добавить(СоединениеСБазой.ПланыСчетов.Хозрасчетный.НайтиПоКоду("62.02"));
	...
	Для каждого СтрокаРез из ТЗРезультатВБух Цикл
		...
		Если СтрокаСопоставленияКон = Неопределено Тогда
	...
	//+++АК mika Дата: 2017.08.01 ИП-00016272
	//Запрос.УстановитьПараметр("Период", Период);
	Запрос.УстановитьПараметр("Период", Новый Граница(КонецДня(Период), ВидГраницы.Включая));
	//---АК
	...
	////Счета.Добавить(ПланыСчетов.Финансовый.НайтиПоКоду("60.4"));
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		ЗаполнитьЗначенияСвойств(ВнешнийНаборДанных.Добавить(),ВыборкаДетальныеЗаписи);
	КонецЦикла;
...
Функция ВернутьТаблицуОрганизацийБУ(МассивОрганизаций)
	...
	Для каждого Организация из МассивОрганизаций Цикл       
		...
		Если ОрганизацияБУ = Неопределено Тогда
			Сообщить("Не удоалось найти в бухгалтерской базе Организацию "+Организация);
			Продолжить;		
		КонецЕсли;
...
Функция ВернутьМассивКонтрагентов(СчетаФинансы, Период) //+++АК mika 2017.12.26 ИП-00017572 В функцию добавлены параметры "СчетаФинансы, Период"
	...
	КомпоновщикНастроекКонтрагентов.ЗагрузитьНастройки(КомпоновщикНастроек.ПолучитьНастройки());
	...
	//mika Дата: 2017.09.07 
	//С переходом на 8.3, Параметр перестал переносится в макет компоновки из компоновщика настроек
	...
	СхемаЗапросКонтрагентов.Параметры.ГруппаАренда.Значение = Справочники.Контрагенты.НайтиПоНаименованию("Аренда", Истина); //+++АК mika 2017.12.26 ИП-00017572 отключать отбор по группе "Аренда" в фин. базе нельзя, слишком большая выборка.
	СхемаЗапросКонтрагентов.Параметры.ВсеГруппы.Значение = ВсеГруппы; //+++АК mika 2017.12.26 ИП-00017572 При снятом флаге будут анализирвоаться все контрагенты в базе Финансы (с последующим отбором по счетам)
	СхемаЗапросКонтрагентов.Параметры.ДоступныеКонтрагенты.Значение  = ПолучитьДоступныеКонтрагенты(СчетаФинансы, Период); //+++АК mika 2017.12.26 ИП-00017572 Нельзя выборку добавлять в запрос
	//mika
...
Функция ВернутьМассивОрганизаций()
	...
	КомпоновщикНастроекОрганизаций.ЗагрузитьНастройки(КомпоновщикНастроек.ПолучитьНастройки());
...
Процедура РасчитатьНачальныйОстаток(ДатаАктуальности,ДатаРасчета,Организация,РасчетныйСчет,ВнешнийНаборДанных,НачальныйОстаток)
	
	НоваяСтрокаКалендаря = ВнешнийНаборДанных.Добавить();
	НоваяСтрокаКалендаря.Период = ДатаРасчета;
	НоваяСтрокаКалендаря.Организация = Организация;
	НоваяСтрокаКалендаря.РасчетныйСчет = РасчетныйСчет;
	
	
	
	Если ДатаАктуальности < ДатаРасчета Тогда
		
		СтрокиРасчитанногоРанееОстатка = ВнешнийНаборДанных.НайтиСтроки(Новый Структура("Организация,РасчетныйСчет",Организация,РасчетныйСчет));
		Если СтрокиРасчитанногоРанееОстатка.Количество() Тогда
			//возьмем последнее значение
			НоваяСтрокаКалендаря.СуммаНачальныйОстаток = СтрокиРасчитанногоРанееОстатка[СтрокиРасчитанногоРанееОстатка.ВГраница()].СуммаКонечныйОстаток;			
		Иначе
			НоваяСтрокаКалендаря.СуммаНачальныйОстаток = 0;
		КонецЕсли;
		
	КонецЕсли;
	
	Если ДатаАктуальности >= ДатаРасчета или НоваяСтрокаКалендаря.СуммаНачальныйОстаток = 0 Тогда
		//рассчитаем по базе
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	СУММА(ЕСТЬNULL(ФинансовыйОстатки.СуммаОстаток, 0)) КАК СуммаОстаток
		|ИЗ
		|	РегистрБухгалтерии.Финансовый.Остатки(
		|			&ДатаОстатка,
		|			Счет = &Счет,
		...
		Запрос.УстановитьПараметр("ДатаОстатка", ДатаРасчета); //КонецДня(ДатаАктуальности)
		Запрос.УстановитьПараметр("Субконто1", Организация);
		Запрос.УстановитьПараметр("Субконто2", РасчетныйСчет);
		Запрос.УстановитьПараметр("Счет", ПланыСчетов.Финансовый.РасчетныйСчет);
		...
		Если ВыборкаДетальныеЗаписи.Следующий() Тогда
			НоваяСтрокаКалендаря.СуммаНачальныйОстаток = ?(ВыборкаДетальныеЗаписи.СуммаОстаток = NULL,0,ВыборкаДетальныеЗаписи.СуммаОстаток);			
		Иначе			
			НоваяСтрокаКалендаря.СуммаНачальныйОстаток = 0;			
		КонецЕсли;		
	КонецЕсли;
	...
	НачальныйОстаток = НоваяСтрокаКалендаря.СуммаНачальныйОстаток;
	
КонецПроцедуры	

Процедура РасчитатьПриходИнкассаций(ДатаАктуальности,ДатаРасчета,ДатаПрошлогоРабДня, Организация,РасчетныйСчет,ВнешнийНаборДанных,Приход)
	...
	Запрос.Текст = 
		...
		|	ФинансовыйОбороты.Субконто2 КАК Источник,
		|	ФинансовыйОбороты.СуммаОборотДт КАК ЗапланированноИнкассаций
		|ИЗ
		|	РегистрБухгалтерии.Финансовый.Обороты(НАЧАЛОПЕРИОДА(&ДатаНачала, ДЕНЬ), КОНЕЦПЕРИОДА(&ДатаОкончания, ДЕНЬ), , Счет В (ЗНАЧЕНИЕ(ПланСчетов.Финансовый.ДенежныеСредстваВПутиИнкассация), ЗНАЧЕНИЕ(ПланСчетов.Финансовый.ДенежныеСредстваВПутиЭквайринг)), , Субконто1 = &Субконто1, , ) КАК ФинансовыйОбороты";

	Запрос.УстановитьПараметр("ДатаНачала", ДатаПрошлогоРабДня);
	Запрос.УстановитьПараметр("ДатаОкончания", ДатаРасчета-1);
	Запрос.УстановитьПараметр("Субконто1", Организация);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		НоваяСтрокаКалендаря.Организация = Организация;
		НоваяСтрокаКалендаря.РасчетныйСчет = РасчетныйСчет;
		НоваяСтрокаКалендаря.Контрагент = ВыборкаДетальныеЗаписи.Источник;
		НоваяСтрокаКалендаря.Документ = ВыборкаДетальныеЗаписи.Счет.Наименование;           		
		НоваяСтрокаКалендаря.СтатьяДДС = ВыборкаДетальныеЗаписи.Счет;//ПланыСчетов.Финансовый.ДенежныеСредстваВПутиИнкассация;
		НоваяСтрокаКалендаря.СуммаПриход =  ВыборкаДетальныеЗаписи.ЗапланированноИнкассаций;		
		Приход = Приход + ВыборкаДетальныеЗаписи.ЗапланированноИнкассаций;
...
Процедура РасчитатьПриходФакт(ДатаАктуальности,ДатаРасчета,Организация,РасчетныйСчет,ВнешнийНаборДанных,ПриходФакт)
	...
	//Если ДатаАктуальности >= ДатаРасчета Тогда
		...
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ЕСТЬNULL(ФинансовыйОбороты.СуммаОборотДт, 0) КАК СуммаОборотДт,
		|	ФинансовыйОбороты.Регистратор,
		|	ФинансовыйОбороты.Субконто3 КАК СтатьяДДС,
	...
	//	
	//	Запрос.Текст = "ВЫБРАТЬ
	//	|	ПоступлениеВБанк.СуммаДокумента КАК СуммаОборотДт,
	//	|	ПоступлениеВБанк.Ссылка КАК Регистратор,
	//	|	ПоступлениеВБанк.СтатьяДвиженияДенежныхСредств КАК СтатьяДДС,
	//	|	ПоступлениеВБанк.Контрагент КАК Контрагент
	//	|ИЗ
	//	|	Документ.ПоступлениеВБанк КАК ПоступлениеВБанк
	//	|ГДЕ
	//	|	ПоступлениеВБанк.Проведен
	//	|	И ПоступлениеВБанк.Дата МЕЖДУ &НачалоДня И &ОкончаниеДня
	//	|	И ПоступлениеВБанк.Организация = &Субконто1
	//	|	И ПоступлениеВБанк.СчетОрганизации = &Субконто2";	
	//КонецЕсли;
			   
	Запрос.УстановитьПараметр("НачалоДня", НачалоДня(ДатаРасчета));
	Запрос.УстановитьПараметр("ОкончаниеДня", КонецДня(ДатаРасчета));
	Запрос.УстановитьПараметр("Субконто1", Организация);
	Запрос.УстановитьПараметр("Субконто2", РасчетныйСчет);
	Запрос.УстановитьПараметр("Счет", ПланыСчетов.Финансовый.РасчетныйСчет);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		НоваяСтрокаКалендаря.Организация = Организация;
		НоваяСтрокаКалендаря.РасчетныйСчет = РасчетныйСчет;
		НоваяСтрокаКалендаря.СтатьяДДС = ВыборкаДетальныеЗаписи.СтатьяДДС;
		НоваяСтрокаКалендаря.Контрагент = ВыборкаДетальныеЗаписи.Контрагент;
		НоваяСтрокаКалендаря.Документ = ВыборкаДетальныеЗаписи.Регистратор;
		НоваяСтрокаКалендаря.СуммаПриход =  ВыборкаДетальныеЗаписи.СуммаОборотДт;
		
		ПриходФакт = ПриходФакт + ВыборкаДетальныеЗаписи.СуммаОборотДт;
	КонецЦикла;
		
КонецПроцедуры	

Процедура РасчитатьРасходПланПлатежейПоОрганизации(ДатаАктуальности,ДатаРасчета,Организация,ВнешнийНаборДанных,РасходПлан, Договоры, Долги)	
	
	
	//ИмяКолонки = "Дата_" + Формат(ДатаРасчета, "ДФ=yyyyMMdd");
	//ИмяКолонкиДок = "Документ_" + Формат(ДатаРасчета, "ДФ=yyyyMMdd");
	//Если Договоры.Колонки.Найти(ИмяКолонки) = Неопределено Тогда
	//	возврат;
	//КонецЕсли;
	
	ОсновнойБанковскийСчет = Организация.ОсновнойБанковскийСчет;
	СтрокиПоОрганизации =  Договоры.Скопировать(Договоры.НайтиСтроки(Новый Структура("Организация,ДатаОплаты",Организация,ДатаРасчета)));
	СтатьяДДС = Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоКоду("131010");
	Для каждого Строка из СтрокиПоОрганизации Цикл
			Если НЕ ЗначениеЗаполнено(Строка.Сумма) Тогда
				продолжить;
			КонецЕсли;	
			
			мКурсВалюты = ?(ЗначениеЗаполнено(Строка.Валюта),ПроцедурыНачисления.ПолучитьКурсВалютыПоДоговору(ДатаРасчета, Строка).Курс,1);
			
			Сумма = Строка.Сумма*?(НЕ ЗначениеЗаполнено(мКурсВалюты), 1, мКурсВалюты);
            
			Если Сумма<>0 Тогда
				НоваяСтрокаКалендаря = ВнешнийНаборДанных.Добавить();
				НоваяСтрокаКалендаря.Период = ДатаРасчета;
				НоваяСтрокаКалендаря.Организация = Организация;
				НоваяСтрокаКалендаря.РасчетныйСчет = ОсновнойБанковскийСчет;		
				НоваяСтрокаКалендаря.СтатьяДДС = СтатьяДДС;
				НоваяСтрокаКалендаря.СуммаПланРасход = Сумма;			
				НоваяСтрокаКалендаря.Контрагент = Строка.Контрагент;
				НоваяСтрокаКалендаря.Документ = Строка.ПлатежныйДокумент;//ДоговорКонтрагента;//;
				РасходПлан = РасходПлан + Сумма;
			КонецЕсли;
	...
	СтрокиДолги = Долги.НайтиСтроки(Новый Структура("Организация",Организация));
	Для каждого СтрокаДолга из СтрокиДолги Цикл
	...
	НоваяСтрокаКалендаря.РасчетныйСчет = ОсновнойБанковскийСчет;
	НоваяСтрокаКалендаря.СтатьяДДС =  СтатьяДДС;
	НоваяСтрокаКалендаря.Контрагент = СтрокаДолга.Контрагент;
///	НоваяСтрокаКалендаря.Документ = СтрокаДолга.Документ;
	НоваяСтрокаКалендаря.СуммаПланРасход = СтрокаДолга.СуммаКОплате;
	РасходПлан = РасходПлан + СтрокаДолга.СуммаКОплате;	
...
Процедура РасчитатьРасходПлан(ДатаАктуальности,ДатаРасчета,Организация,РасчетныйСчет,ВнешнийНаборДанных,РасходПлан)
	...
	Запрос.Текст = 
		...
		|	ЗаявкаНаРасходованиеСредств.БанковскийСчетКасса КАК РасчетныйСчет,
		|	ЗаявкаНаРасходованиеСредств.Организация,
		|	ЗаявкаНаРасходованиеСредств.СуммаДокумента КАК СуммаПланРасход
		|ИЗ
		|	Документ.ЗаявкаНаРасходованиеСредств КАК ЗаявкаНаРасходованиеСредств
		|ГДЕ
		|	ЗаявкаНаРасходованиеСредств.ФормаОплаты = ЗНАЧЕНИЕ(Перечисление.ФормыОплаты.Безналичные)
		|	И ЗаявкаНаРасходованиеСредств.ДатаРасхода = &ДатаРасхода
		|	И ЗаявкаНаРасходованиеСредств.БанковскийСчетКасса = &БанковскийСчетКасса
		|	И ЗаявкаНаРасходованиеСредств.Организация = &Организация
		|	И ЗаявкаНаРасходованиеСредств.Проведен";

		
	Запрос.УстановитьПараметр("ДатаРасхода",ДатаРасчета);
	Запрос.УстановитьПараметр("БанковскийСчетКасса",РасчетныйСчет);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		НоваяСтрокаКалендаря = ВнешнийНаборДанных.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрокаКалендаря,ВыборкаДетальныеЗаписи);
		НоваяСтрокаКалендаря.Период = ДатаРасчета;
		//НоваяСтрокаКалендаря = ВнешнийНаборДанных.Добавить();
		//НоваяСтрокаКалендаря.Период = ДатаРасчета;
		//НоваяСтрокаКалендаря.Организация = Организация;
...
Процедура РасчитатьРасходФакт(ДатаАктуальности,ДатаРасчета,Организация,РасчетныйСчет,ВнешнийНаборДанных,РасходФакт)
	...
	Если ДатаАктуальности >= ДатаРасчета Тогда	
		...
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ЕСТЬNULL(ФинансовыйОбороты.СуммаОборотКт, 0) КАК СуммаОборотКт,
		|	ФинансовыйОбороты.Регистратор,
		|	ФинансовыйОбороты.Субконто3 КАК СтатьяДДС
		|ИЗ
		|	РегистрБухгалтерии.Финансовый.Обороты(
	...
	Иначе
		...
		Запрос.Текст = "ВЫБРАТЬ
		               |	РасходИзБанка.СуммаДокумента КАК СуммаОборотКт,
		               |	РасходИзБанка.Ссылка КАК Регистратор,
		               ...
		               |	И РасходИзБанка.Дата МЕЖДУ &НачалоДня И &ОкончаниеДня
		               |	И РасходИзБанка.Организация = &Субконто1
		               |	И РасходИзБанка.СчетОрганизации = &Субконто2";	
	КонецЕсли;
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		НоваяСтрокаКалендаря.Организация = Организация;
		НоваяСтрокаКалендаря.РасчетныйСчет = РасчетныйСчет;
		НоваяСтрокаКалендаря.Контрагент = ВыборкаДетальныеЗаписи.Регистратор.Контрагент;
		НоваяСтрокаКалендаря.Документ = ВыборкаДетальныеЗаписи.Регистратор;
		НоваяСтрокаКалендаря.СтатьяДДС = ВыборкаДетальныеЗаписи.СтатьяДДС;
		НоваяСтрокаКалендаря.СуммаРасход =  ВыборкаДетальныеЗаписи.СуммаОборотКт;
		
		РасходФакт = РасходФакт + ВыборкаДетальныеЗаписи.СуммаОборотКт;
	КонецЦикла;

	
КонецПроцедуры	

Процедура РасчитатьКонечныйОстаток(ДатаАктуальности,ДатаРасчета,Организация,РасчетныйСчет,ВнешнийНаборДанных,НачальныйОстаток,Приход,РасходПлан,РасходФакт)
	
	НоваяСтрокаКалендаря = ВнешнийНаборДанных.Добавить();
	НоваяСтрокаКалендаря.Период = ДатаРасчета;
	НоваяСтрокаКалендаря.Организация = Организация;
	НоваяСтрокаКалендаря.РасчетныйСчет = РасчетныйСчет;
	НоваяСтрокаКалендаря.СуммаКонечныйОстаток = НачальныйОстаток +  Приход - РасходФакт;
...
// Формирует и возвращает контрагентов, по которым были обороты за три последних года
//
// Параметры:
//  СчетаФинансы  - <Тип.ПланСчетов.Финансовый> - 60.1, 60.2, 62.2
//  Период  - <Тип.Дата> - Дата формирования отчета
//
// Возвращаемое значение:
//   <Тип.Массив>   -  Массив контрагентов
//
Функция ПолучитьДоступныеКонтрагенты(СчетаФинансы, Период) //+++АК mika 2017.12.28 ИП-00017572
	...
	|ИЗ
	|	РегистрБухгалтерии.Финансовый.Обороты(&НачалоПериода {(&НачалоПериода)}, &КонецПериода {(&КонецПериода)}, , Счет В ИЕРАРХИИ (&Счета) {(Счет В ИЕРАРХИИ (&Счета)) КАК Поле2}, , , , ) КАК ФинансовыйОбороты";
	
	Запрос.УстановитьПараметр("КонецПериода", Новый Граница(КонецДня(Период), ВидГраницы.Включая));
	Запрос.УстановитьПараметр("НачалоПериода", ДобавитьМесяц(НачалоДня(Период), - 120));
	Запрос.УстановитьПараметр("Счета", СчетаФинансы);
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Возврат ТаблицаКонтрагентов.ВыгрузитьКолонку("Контрагент");
		
	КонецЕсли;
	
	Возврат Неопределено;
	
КонецФункции // ПолучитьДоступныеКонтрагенты()

// Осуществляет Обратный поиск контрагента по ИНН, если обратный поиск по GUID не дал результатов
//
// Параметры:
//  КонтрагентУдаленнойБазыGUID  - <Тип.COMobject> - COMobject контрагент удаленной базы
//
// Возвращаемое значение:
//   <Тип.СправочникСсылка.Контрагенты>  - Ссылка на найденный контрагент, или неопределено 
//
Функция ПолучитьКонтрагентаПоИННТекущаяБаза(КонтрагентУдаленнойБазыCOM) //+++АК mika 2017.12.28 ИП-00017572
	...
	Запрос.УстановитьПараметр("ИНН",КонтрагентУдаленнойБазыCOM.ИНН);
	Запрос.УстановитьПараметр("КПП",КонтрагентУдаленнойБазыCOM.КПП);
	Запрос.УстановитьПараметр("Наименование",КонтрагентУдаленнойБазыCOM.Наименование);
...
КонецФункции // ПолучитьКонтрагентаПоИННТекущаяБаза()
