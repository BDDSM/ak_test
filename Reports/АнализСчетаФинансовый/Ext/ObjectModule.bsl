...
Процедура СформироватьОтчет(ДокументРезультат,  ПоказыватьЗаголовок = Истина, ВысотаЗаголовка = 0) Экспорт
	...
	Если ЕстьИтог Тогда
		Если Счет.Вид = ВидСчета.Активный Тогда 
			...
			СуммаКонДт = ОбщийИтог.СуммаНачДт - ОбщийИтог.СуммаНачКт + ОбщийИтог.СуммаОборотДт - ОбщийИтог.СуммаОборотКт;
			СуммаКонКт = 0;
		ИначеЕсли Счет.Вид = ВидСчета.Пассивный Тогда
			СуммаКонДт = 0;
			СуммаКонКт = ОбщийИтог.СуммаНачКт - ОбщийИтог.СуммаНачДт + ОбщийИтог.СуммаОборотКт - ОбщийИтог.СуммаОборотДт;
		Иначе
...
Процедура НачалоВыбораЗначенияСубконто(Элемент, СтандартнаяОбработка, ТипЗначенияПоля=Неопределено) Экспорт
	...
	Для Инд=0 По ПостроительОтчета.Отбор.Количество()-1 Цикл
		...
		ИначеЕсли СтрокаОтбора.ТипЗначения = ПланыВидовХарактеристик.ВидыСубконто.Склады.ТипЗначения Тогда
		...
		//ИначеЕсли СтрокаОтбора.ТипЗначения = ПланыВидовХарактеристик.ВидыСубконто.Договоры.ТипЗначения Тогда
