...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка) Экспорт
	...
	//КонецЦикла;
	ТабДанные.Свернуть("Дата, ТорговаяТочка, Расходник,Склад,ПриемкаВМагазинеБезПересчета,ПриемкаТовараБезПересчета", "НехваткаКоробок");
	...
	|		ПО втТабДанные.ТорговаяТочка = ЗначенияСвойствСтруктурныхЕдиниц.СтруктурнаяЕдиница
	...
	|		ПО втТабДанные.ТорговаяТочка = СтруктурныеЕдиницы.Ссылка
	|ГДЕ
	|	СтруктурныеЕдиницы.ТипРозничнойТочки = &ТипРозничнойТочки";
	Запрос.УстановитьПараметр("ТипРозничнойТочки",Перечисления.ТипыРозничныхТочек.Вкусомат);
	//---АК BELN 2018.11.26 
	Запрос.УстановитьПараметр("ТабДанные",ТабДанные);
	МассивТТ = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("ТорговаяТочка");
	Отбор = Новый Структура;
	Для Каждого ТТ Из МассивТТ Цикл
    	Отбор.Вставить("ТорговаяТочка", ТТ);
    	Строки = ТабДанные.НайтиСтроки(Отбор);
		Для Каждого Стр Из Строки Цикл
			Если Стр.НехваткаКоробок <> 0 Тогда
        		Стр.НехваткаКоробок = 0;
			КонецЕсли;
		КонецЦикла;
		Отбор.Очистить();
    КонецЦикла;	
	...
	|	Группы.НомерСтр КАК НомерСтроки,
	...
	|	КОЛИЧЕСТВО(втТабДанные.НехваткаКоробок) КАК НехваткаКоробок
	|ПОМЕСТИТЬ вт
	|ИЗ
	...
	|	втГруппы.Представление,
	|	вт.НехваткаКоробок,
	|	втГруппы.НомерСтроки Как НомСтр
	|ИЗ
	|	втГруппы КАК втГруппы
	|		ЛЕВОЕ СОЕДИНЕНИЕ вт КАК вт
	|		ПО втГруппы.Представление = вт.Представление
	|
	|УПОРЯДОЧИТЬ ПО
	|	втГруппы.НомерСтроки";
...
Функция ПолучитьДанныеДляОтчета(ДатаОтчета, НоменклатураОтбор = Неопределено, СкладОтбор = Неопределено, СворачиватьДанные = Ложь,ТипТТ,ПодразделениеОтбор,ОбычныеПоставки = Ложь) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	//НоменклатураОтбор=Справочники.Номенклатура.НайтиПоКоду("000622087");
	Запрос = Новый Запрос();
	Запрос.УстановитьПараметр("ДатаОтчета", ДатаОтчета);
	//+++АК KIRN 2018.10.05 
	Запрос.УстановитьПараметр("ОбычныеПоставки", ОбычныеПоставки);
	//---АК KIRN 
	...
	МасСкладИскл=Новый Массив;
	МасСкладИскл.Добавить(Справочники.Склады.НайтиПоКоду("100004507"));
	
	
	Запрос.УстановитьПараметр("МасСкладИскл", МасСкладИскл);  
...
//|	МАКСИМУМ(ВЫБОР
//|			КОГДА НЕ МагазиныКПересчетуНаДату.Магазин есть Null И МагазиныКПересчетуНаДатуИскл.Магазин есть Null  Тогда 2
//|			КОГДА РасходныйОрдерСкладТовары.Ссылка.ПриемкаВМагазинеБезПересчета < 0
//|				ТОГДА -РасходныйОрдерСкладТовары.Ссылка.ПриемкаВМагазинеБезПересчета
//|			ИНАЧЕ РасходныйОрдерСкладТовары.Ссылка.ПриемкаВМагазинеБезПересчета
...
//|	МАКСИМУМ(ВЫБОР
//|			КОГДА НЕ МагазиныКПересчетуНаДату.Магазин есть Null И МагазиныКПересчетуНаДатуИскл.Магазин есть Null 
//|			И ВЫРАЗИТЬ(РасходныйОрдерСкладТовары.Ссылка.Получатель КАК Справочник.СтруктурныеЕдиницы).ПриемкаТовараБезПересчета=ИСТИНА Тогда Истина
//|					когда выбор КОГДА РасходныйОрдерСкладТовары.Ссылка.ПриемкаВМагазинеБезПересчета < 0
//|						ТОГДА -РасходныйОрдерСкладТовары.Ссылка.ПриемкаВМагазинеБезПересчета
//|					ИНАЧЕ РасходныйОрдерСкладТовары.Ссылка.ПриемкаВМагазинеБезПересчета
//|				КОНЕЦ В (1, 2)
//|				ТОГДА ИСТИНА
//|			ИНАЧЕ ЛОЖЬ  КОНЕЦ) КАК ПриемкаТовараБезПересчета 
...
//|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МагазиныКПересчетуНаДату КАК МагазиныКПересчетуНаДату
//|		ПО РасходныйОрдерСкладТовары.Ссылка.Получатель = МагазиныКПересчетуНаДату.Магазин
//|			И РасходныйОрдерСкладТовары.Ссылка.ДатаРаспределения = МагазиныКПересчетуНаДату.Дата
//|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МагазиныКПересчетуНаДатуИскл КАК МагазиныКПересчетуНаДатуИскл
//|		ПО РасходныйОрдерСкладТовары.Ссылка.Получатель = МагазиныКПересчетуНаДатуИскл.Магазин
//|			И РасходныйОрдерСкладТовары.Ссылка.ДатаРаспределения = МагазиныКПересчетуНаДатуИскл.Дата
//|ГДЕ
...
//|	И НЕ РасходныйОрдерСкладТовары.Номенклатура В (&НоменклИскл)
//|
//|СГРУППИРОВАТЬ ПО
...
//|	ВЫРАЗИТЬ(РасходныйОрдерСкладТовары.Ссылка.Получатель КАК Справочник.СтруктурныеЕдиницы).ТипРозничнойТочки"
	...
	//|	 Максимум(Выбор когда РасходныйОрдерСкладТовары.Ссылка.ПриемкаВМагазинеБезПересчета<0 тогда -РасходныйОрдерСкладТовары.Ссылка.ПриемкаВМагазинеБезПересчета 
	//| иначе РасходныйОрдерСкладТовары.Ссылка.ПриемкаВМагазинеБезПересчета конец) как ПриемкаВМагазинеБезПересчета,
	//|	Максимум(Выбор когда Выбор когда РасходныйОрдерСкладТовары.Ссылка.ПриемкаВМагазинеБезПересчета<0 тогда -РасходныйОрдерСкладТовары.Ссылка.ПриемкаВМагазинеБезПересчета 
	//|	иначе РасходныйОрдерСкладТовары.Ссылка.ПриемкаВМагазинеБезПересчета конец  в (1,2) Тогда Истина Иначе Ложь Конец)  как ПриемкаТовараБезПересчета
|	МАКСИМУМ(ВЫБОР
|			КОГДА НЕ МагазиныКПересчетуНаДату.Магазин есть Null  Тогда 2
|			КОГДА РасходныйОрдерСкладТовары.Ссылка.ПриемкаВМагазинеБезПересчета < 0
|				ТОГДА -РасходныйОрдерСкладТовары.Ссылка.ПриемкаВМагазинеБезПересчета
|			ИНАЧЕ РасходныйОрдерСкладТовары.Ссылка.ПриемкаВМагазинеБезПересчета
...
|	МАКСИМУМ(ВЫБОР
|			КОГДА НЕ МагазиныКПересчетуНаДату.Магазин есть Null  
|			И ВЫРАЗИТЬ(РасходныйОрдерСкладТовары.Ссылка.Получатель КАК Справочник.СтруктурныеЕдиницы).ПриемкаТовараБезПересчета=ИСТИНА Тогда Истина
|					когда выбор КОГДА РасходныйОрдерСкладТовары.Ссылка.ПриемкаВМагазинеБезПересчета < 0
|						ТОГДА -РасходныйОрдерСкладТовары.Ссылка.ПриемкаВМагазинеБезПересчета
|					ИНАЧЕ РасходныйОрдерСкладТовары.Ссылка.ПриемкаВМагазинеБезПересчета
...
//|			И РасходныйОрдерСкладТовары.Ссылка.ДатаРаспределения = МагазиныКПересчетуНаДатуИскл.Дата
	...
	|	И РасходныйОрдерСкладТовары.Ссылка.Проведен = ИСТИНА
	//+++АК KIRN 2018.10.05  ИП-00019999
	|	И Выбор когда &ОбычныеПоставки = Истина Тогда РасходныйОрдерСкладТовары.Ссылка.ПриемкаВМагазинеБезПересчета = 0 Иначе Истина Конец
	//---АК KIRN 
	|	И РасходныйОрдерСкладТовары.Ссылка.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыРасходныхОрдеров.ВСборке)
	|	И РасходныйОрдерСкладТовары.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходСкладскойУчет.ОтгрузкаВТорговуюТочку)
	|	И (РасходныйОрдерСкладТовары.Номенклатура = &Номенкл
	...
	//|			ИЛИ &Склад = НЕОПРЕДЕЛЕНО)
	|	И (РасходныйОрдерСкладТовары.Ссылка.Склад = &Склад
	|			ИЛИ &Склад = НЕОПРЕДЕЛЕНО) и Не РасходныйОрдерСкладТовары.Номенклатура в (&НоменклИскл) И Не РасходныйОрдерСкладТовары.Ссылка.Склад в (&МасСкладИскл)
	|
	|СГРУППИРОВАТЬ ПО
	|	Выразить(РасходныйОрдерСкладТовары.Ссылка.Получатель как Справочник.СтруктурныеЕдиницы),
	|	Выразить(РасходныйОрдерСкладТовары.Ссылка.Получатель как Справочник.СтруктурныеЕдиницы).ТипРозничнойТочки
	...
	|	РасходныйОрдерСкладТовары.Ссылка.Склад.Владелец КАК Склад,  Максимум(вт.ПриемкаВМагазинеБезПересчета) как ПриемкаВМагазинеБезПересчета,
	|	Максимум(вт.ПриемкаТовараБезПересчета)  как ПриемкаТовараБезПересчета,
	|	РасходныйОрдерСкладТовары.Характеристика
	|ИЗ
	|	Документ.РасходныйОрдерСклад.Товары КАК РасходныйОрдерСкладТовары
	|внутреннее соединение вт по вт.ТорговаяТочка=РасходныйОрдерСкладТовары.Ссылка.Получатель
	|ГДЕ
	|	РасходныйОрдерСкладТовары.Ссылка.ДатаРаспределения = &ДатаОтчета                                          //5
	|	И РасходныйОрдерСкладТовары.Ссылка.Проведен = ИСТИНА
	//+++АК KIRN 2018.10.05  ИП-00019999
	...
	|			ИЛИ &СкладВладелец = НЕОПРЕДЕЛЕНО) И Номенклатура.Ссылка в(&МасНом)
	|;";
	...
	|			КонецПериода(&ДатаОтчетаДо,день), Характеристика в(&МасХар)
	...
	|			ИЛИ &Номенкл = НЕОПРЕДЕЛЕНО) И ХарактеристикиНоменклатуры.Ссылка в(&МасХар)
	|;";
	...
	|			ИЛИ &Номенкл = НЕОПРЕДЕЛЕНО) И КоличествоВКоробкеПоДням.Характеристика в(&МасХар)";
	...
	ТЗНомБезРасп=Новый ТаблицаЗначений;
	ТЗНомБезРасп.Колонки.Добавить("ТорговаяТочка");
	ТЗНомБезРасп.Колонки.Добавить("Номенклатура");
	ТабОтгруженоГрупп=ТабОтгружено.Скопировать();
	ТабОтгруженоГрупп.Свернуть("Расходник,Номенклатура,ТорговаяТочка","КоличествоУРЗ,Количество");
	Для каждого СтрОтгр Из ТабОтгруженоГрупп Цикл
		Если СтрОтгр.КоличествоУРЗ=0 И НЕ СтрОтгр.Количество=0 Тогда
			НовСтрОтгр=ТЗНомБезРасп.Добавить();
			ЗаполнитьЗначенияСвойств(НовСтрОтгр,СтрОтгр);
		КонецЕсли; 
	КонецЦикла; 
	...
	Попытка
		...
		Пока НЕ rs.EOF() Цикл
			...
			ПолучилаТТШтук = Rs.Fields("post").Value;
			КолПост=СтрокиДанных.Количество();
			СчПост=1;
			Для каждого СтрокаДоб Из СтрокиДанных Цикл
				МасСтрНомБезРасп=ТЗНомБезРасп.НайтиСтроки(Новый Структура("ТорговаяТочка, Номенклатура", ТТ, Товар));
				КолвоПоИтерации = ?(СчПост=КолПост И  МасСтрНомБезРасп.Количество()=0,ПолучилаТТШтук,Мин(СтрокаДоб.РаспределеноШтук, ПолучилаТТШтук));
				...
				СтрокаДоб.ПолучилаТТШтук = КолвоПоИтерации;
				...
				ПолучилаТТШтук = ПолучилаТТШтук - КолвоПоИтерации;
				СчПост=СчПост+1;
			КонецЦикла; 
