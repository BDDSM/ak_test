...
Процедура ОбработкаИзмененияСоставаСубконто(ПолнаяОбработка = Истина) Экспорт
	...
	Если ПолнаяОбработка Тогда
		...
		|ИЗ
		|	ПланСчетов.Финансовый.ВидыСубконто КАК ВС1
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПланСчетов.Финансовый.ВидыСубконто КАК ВС2
		|		ПО ВС1.Ссылка = ВС2.Ссылка
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПланСчетов.Финансовый.ВидыСубконто КАК ВС3
		...
		Иначе
			//ДанныеОтчета.ПоказателиОтчета.Упр.Использование = Ложь;
			//ДанныеОтчета.ПоказателиОтчета.Упр.Значение      = Ложь;
...
Процедура ДоработатьКомпоновщикПередВыводом(ВнешниеНаборыДанных) Экспорт
	...
	//АК_СтандартныеОтчеты.ДобавитьОтборДляПоказателяКонтроль(ЭтотОбъект);
...
Функция ПолучитьТекстЗаголовка(ОрганизацияВНачале = Истина) Экспорт 
	...
	ЗаголовокОтчета = "Анализ субконто " + ТекстСубконто + АК_СтандартныеОтчеты.ПолучитьПредставлениеПериода(ЭтотОбъект);
