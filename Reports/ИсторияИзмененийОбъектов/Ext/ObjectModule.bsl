	...
	Для Каждого ЭлементМодРеквизит Из ТаблицаИзмененийРеквизитов Цикл
		...
		Если ЭлементМодРеквизит._Модификация = Истина Тогда
			...
			Если ОписаниеРеквизита = Неопределено Тогда
				Если ВыводимоеНаименование = "Дата" Тогда
					ОписаниеРеквизита = Новый Структура("Тип", Новый ОписаниеТипов("Дата"));
				ИначеЕсли ВыводимоеНаименование = "Код" Или ВыводимоеНаименование = "Номер" Тогда
			...
			Пока ИндексПоВерсиямРеквизитов >= 1 Цикл
				...
				Если ТипЗнч(СтруктураХарактеристикаИзменения) = Тип("Строка") Тогда
					...
					Если НЕ ОписаниеРеквизита = Неопределено Тогда
						ОписаниеТипаРеквизита = ОписаниеРеквизита.Тип;
						// пытаемся найти прямое соответствие
						ПредставлениеЗначенияРеквизита = СформироватьПредставлениеПоОписаниюТипа(ОписаниеТипаРеквизита, ЗначениеРеквизита);
					КонецЕсли;
				ИначеЕсли СтруктураХарактеристикаИзменения <> Неопределено Тогда
					
					Если НЕ СтруктураХарактеристикаИзменения.Значение = Неопределено Тогда
...
Функция ПолучитьОбъектИзXML(СтрокаXML)
	...
	Если ЧтениеXML.Прочитать() Тогда
		Если ВозможностьЧтенияXML(ЧтениеXML) Тогда
			//изменения Кудяков Г.А.   28 декабря 2009 г.
			Попытка
				Объект = ПрочитатьXML(ЧтениеXML);
			Исключение
				Сообщить("Изменена конфигурация объекта, производится востановление версии", СтатусСообщения.Информация);
				//в этом случае изменен порядок или реквизиты объекта
				Структура	= РазборОбъектаXML(СтрокаXML, СсылкаНаОбъект);
				Объект		= СсылкаНаОбъект.ПолучитьОбъект();
				Для Каждого ТЧ ИЗ Объект.Метаданные().ТабличныеЧасти Цикл
		        	Объект[ТЧ.Имя].Очистить();
					Если Не Объект.Метаданные().ТабличныеЧасти[ТЧ.Имя]=Неопределено Тогда
						Объект[ТЧ.Имя].Загрузить(Структура.ТабличныеЧасти[ТЧ.Имя]);
					КонецЕсли;	
				КонецЦикла;
				ЗаполнитьЗначенияСвойств(Объект,Структура);
			КонецПопытки;
			//окнчание изменений Кудяков Г.А.   28 декабря 2009 г.	
			ЧтениеXML.Закрыть();
			Возврат Объект;
		Иначе
			ЧтениеXML.Закрыть();
...
Функция ВосстановитьВерсиюОбъекта(Объект,ТекущиеДанные) Экспорт
	Если ТекущиеДанные = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	Если ЗначениеЗаполнено(ТекущиеДанные.НомерВерсии) Тогда
		ИдентификаторВерсии=ТекущиеДанные.НомерВерсии;
		Объект=ПолучитьОбъектИзXML(ПолучитьСтрокуXMLПоНомеруВерсии(ИдентификаторВерсии));
		Возврат Истина;
	Иначе
...
//          ИмяРеквизитаN -> ЗначениеN
//
Функция РазборОбъектаXML(СтрокаXML, Ссылка)
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		Если ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			...
			ИначеЕсли УровеньЧтения = 2 Тогда // указатель на втором уровне - это реквизит или имя табличной части
				...
				Если ТабличныеЧастиМТД.Найти(ИмяРеквизита) <> Неопределено Тогда
					...
					// создаем новую таблицу значений в таблице соответствий
					Если ТабличныеЧасти[ИмяТабличнойЧасти] = Неопределено Тогда
						ТабличныеЧасти.Вставить(ИмяТабличнойЧасти, Ссылка[ИмяТабличнойЧасти].Выгрузить().СкопироватьКолонки());
					КонецЕсли;
				КонецЕсли;
				НовоеЗР = ЗначенияРеквизитов.Добавить();
				НовоеЗР.НаименованиеРеквизита = ИмяРеквизита;
		...
		ИначеЕсли ЧтениеXML.ТипУзла = ТипУзлаXML.Текст Тогда
			Если (УровеньЧтения = 2) Тогда // значение реквизита
				...
				//изменения Кудяков Г.А.   28 декабря 2009 г.
				Если НовоеЗР.НаименованиеРеквизита="Ref" Тогда
					НовоеЗР.ТипРеквизита=СтрЗаменить(ИмяОбъекта,"Object","Ref");
				КонецЕсли;
				НовоеЗР.ЗначениеРеквизита=ПривесстиЗначениеКТипу(НовоеЗР,Ссылка);
				//изменения Кудяков Г.А.   28 декабря 2009 г.
			...
			ИначеЕсли (УровеньЧтения = 4) Тогда // значение реквизита
				...
				ИсходныеДанные.Вставить("НаименованиеРеквизита",ИмяПоляТЧ);
				ИсходныеДанные.Вставить("ТипРеквизита",ТипЗначения);
				ИсходныеДанные.Вставить("ЗначениеРеквизита",ЧтениеXML.Значение);
				ПоследняяСтрока[ИмяПоляТЧ] = ПривесстиЗначениеКТипу(ИсходныеДанные,ТабличныеЧасти[ИмяТабличнойЧасти]);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция вПреобразоватьИдентификатор(Строка,Прямое=Ложь) 
	Если Прямое Тогда
...
//Приводим значение к типу, на основе объекта и данных записанных в XML файле
//Праметры:
//	ИсходныеДанные	-<Структура>. Структура исходных данных (НаименованиеРеквизита,ТипРеквизита,ЗначениеРеквизита)
//  Ссылка			-<ЛюбаяСсылка>. Ссылка на исходный объект
//
Функция ПривесстиЗначениеКТипу(ИсходныеДанные,Ссылка)
	Если ЗначениеЗаполнено(ИсходныеДанные.ТипРеквизита) Тогда 
		ОписаниеТипа=Новый ОписаниеТипов(ИсходныеДанные.ТипРеквизита);
	Иначе
		МассивТипа=Новый Массив;
		//тип берем из Oбъекта
		Попытка
			Если ТипЗнч(Ссылка)=Тип("ТаблицаЗначений") Тогда
				ТипЗначенияВОбъекте	= Ссылка.Колонки[ИсходныеДанные.НаименованиеРеквизита].ТипЗначения;
			Иначе	
				Значение			= Ссылка[ИсходныеДанные.НаименованиеРеквизита];
				ТипЗначенияВОбъекте	= ТипЗнч(Значение);
			КонецЕсли;	
		Исключение
			Сообщить("Реквизит """+ИсходныеДанные.НаименованиеРеквизита+""" у объекта не существует.",СтатусСообщения.Внимание);
			Возврат ИсходныеДанные.ЗначениеРеквизита;
		КонецПопытки;
		Если Не ТипЗнч(ТипЗначенияВОбъекте)=Тип("ОписаниеТипов") Тогда
			МассивТипа.Добавить(ТипЗначенияВОбъекте);
			ИсходныеДанные.ТипРеквизита=СтрЗаменить(ТипЗначенияВОбъекте," ссылка: ","Ссылка.");
			ОписаниеТипа=Новый ОписаниеТипов(МассивТипа);
		Иначе
			ИсходныеДанные.ТипРеквизита=СтрЗаменить(ТипЗначенияВОбъекте," ссылка: ","Ссылка.");
			ОписаниеТипа=ТипЗначенияВОбъекте;
		КонецЕсли;	
	КонецЕсли;
	
	//приводим значение к ТИПУ
	Если  Найти(ИсходныеДанные.ТипРеквизита,"Перечисление")=1 Тогда
		//у перечисления нет ссылки поэтому его по имени
		ПустоеЗначениеТипа						= ОписаниеТипа.ПривестиЗначение(Неопределено);
		ИсходныеДанные.ЗначениеРеквизита		= Перечисления[ПустоеЗначениеТипа.Метаданные().Имя][ИсходныеДанные.ЗначениеРеквизита]	
	ИначеЕсли	Найти(ИсходныеДанные.ТипРеквизита,".")>0 Тогда
		//это ссылочный тип
		ПустоеЗначениеТипа						= ОписаниеТипа.ПривестиЗначение(Неопределено);
		ИсходныеДанные.ЗначениеРеквизита		= ЗначениеИзСтрокиВнутр(СтрЗаменить(ЗначениеВстрокуВнутр(ПустоеЗначениеТипа),":00000000000000000000000000000000",":"+вПреобразоватьИдентификатор(ИсходныеДанные.ЗначениеРеквизита)));
	Иначе
		//простой тип
		Если ОписаниеТипа.СодержитТип(Тип("Дата")) Тогда
			ИсходныеДанные.ЗначениеРеквизита	= ОписаниеТипа.ПривестиЗначение(СтрЗаменитьИзСписка(ИсходныеДанные.ЗначениеРеквизита));
		Иначе
			ИсходныеДанные.ЗначениеРеквизита	= ОписаниеТипа.ПривестиЗначение(ИсходныеДанные.ЗначениеРеквизита);
		КонецЕсли;	
	КонецЕсли;
	Возврат ИсходныеДанные.ЗначениеРеквизита;
КонецФункции
...
Функция СтрЗаменитьИзСписка(Знач ИсходнояСтрока,СоотвесвтиеЗамены=Неопределено)
	Если СоотвесвтиеЗамены=Неопределено Тогда
		СоотвесвтиеЗамены=Новый Соответствие;
		СоотвесвтиеЗамены.Вставить("T","");
		СоотвесвтиеЗамены.Вставить("-","");
		СоотвесвтиеЗамены.Вставить(":","");
	КонецЕсли;	
	Для Каждого Элемент ИЗ СоотвесвтиеЗамены Цикл
		ИсходнояСтрока=СтрЗаменить(ИсходнояСтрока,Элемент.Ключ,Элемент.Значение);			
	КонецЦикла;
	Возврат ИсходнояСтрока;
КонецФункции	
