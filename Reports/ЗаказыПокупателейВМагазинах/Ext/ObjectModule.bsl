...
Функция ТаблицаДанных(ДатаНачала, ДатаОкончания, Магазин, ТолькоБулево = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(Магазин) Тогда
		...
		УсловиеОтбораSQL2 = "DTT.id_tt = " + ВнешниеДанные.ФорматПоля(МагазинДанные.id_TT) + Символы.ПС;
	КонецЕсли;
	
	УсловиеОтбораПоПериодуSQL = "";
	Если ДатаНачала <> Дата(1,1,1) И ДатаОкончания <> Дата(1,1,1) Тогда
		УсловиеОтбораПоПериодуSQL = УсловиеОтбораПоПериодуSQL + "AND Date_proizv BETWEEN " + ВнешниеДанные.ФорматПоля(ДатаНачала, Истина) + " AND " + ВнешниеДанные.ФорматПоля(ДатаОкончания, Истина);
	ИначеЕсли ДатаНачала <> Дата(1,1,1) Тогда
		УсловиеОтбораПоПериодуSQL = УсловиеОтбораПоПериодуSQL + "AND Date_proizv >= " + ВнешниеДанные.ФорматПоля(ДатаНачала, Истина);
	ИначеЕсли ДатаОкончания <> Дата(1,1,1) Тогда
		УсловиеОтбораПоПериодуSQL = УсловиеОтбораПоПериодуSQL + "AND Date_proizv <= " + ВнешниеДанные.ФорматПоля(ДатаОкончания, Истина);
	КонецЕсли;
	
	УсловиеОтбораПоПериодуSQL2 = СтрЗаменить(СтрЗаменить(УсловиеОтбораПоПериодуSQL, "Date_proizv", "date_tt"), "'", "''");
	...
	// Пришло: берём из "Получено ТТ" расходника (из SQL)
	//ТекстЗапросаSQL =
	//	"SELECT
	...
	//	|	" + УсловиеОтбораSQL2 + "
	//	|	" + СтрЗаменить(УсловиеОтбораПоПериодуSQL, "Date_proizv", "rz.Date_r") + "
	//	|";
	ТекстЗапросаSQL =
		"SET NOCOUNT ON
		|
		|create table #ls
		|	(date_tt date, id_tt int, id_tov int, post numeric(15, 3))
		|
		|Insert into #ls
		|exec ('
		|SELECT
		...
		|
		|SELECT
		|	CONVERT(datetime, ar.date_tt) AS ДатаПоступления,
		|	tt.N AS ShopNo,
		|	ar.id_tov,
		|	ar.post AS Пришло
		|FROM #ls ar (nolock) 
	...
	Для Каждого СтрокаПришло Из ТаблицаДанныхПришло Цикл
		...
		СтруктураОтбор = Новый Структура("ДатаПоступления,ShopNo,id_tov");
		//---АК SHEP 2018.12.20
