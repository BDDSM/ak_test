...
Процедура СформироватьОтчетВТаблицу()
	...
	МакетКомпоновки = КомпоновщикМакета.Выполнить(Отчеты.ОтклонениеГрафиковРаботыПромоутеровОтРеальныхВыходов.ПолучитьМакет("ОсновнаяСхемаКомпоновкиДанных")
...
Процедура ЗаписатьДанныеВЛистыСервер()
	...
	Пока Выборка.СледующийПоЗначениюПоля("ЛистУчета") Цикл
		...
		ТабПродавцы = ДокОбъект.Промоутеры.Выгрузить();
		Пока Выборка.Следующий() Цикл
			СтрокаТаб = ТабПродавцы.Найти(Выборка.Сотрудник, "Промоутер");
			...
			Иначе	
				Если СтрокаТаб = Неопределено Тогда
					...
					СтрокаТаб.Промоутер = Выборка.Сотрудник;
			...
			КонецЕсли;	
		КонецЦикла;
		ДокОбъект.Промоутеры.Загрузить(ТабПродавцы);
