...
Функция СформироватьОтчет(Результат = Неопределено, ДанныеРасшифровки = Неопределено, ВыводВФормуОтчета = Истина) Экспорт
	...
	//+++АК BELN 2018.02.19 Не работает отбор по счету пришлось обходить
	Сп=Новый СписокЗначений;
	Для каждого Эл Из ЭтотОбъект.КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
		Если Эл.Использование И Строка(Эл.ЛевоеЗначение)="Счет" И НЕ Эл.Представление="Исключить счета (60.1, 60.2)" 
			И (Эл.ВидСравнения=ВидСравненияКомпоновкиДанных.Равно ИЛИ Эл.ВидСравнения=ВидСравненияКомпоновкиДанных.ВСписке
			ИЛИ Эл.ВидСравнения=ВидСравненияКомпоновкиДанных.ВИерархии ИЛИ Эл.ВидСравнения=ВидСравненияКомпоновкиДанных.ВСпискеПоИерархии) Тогда
			Если ТипЗнч(Эл.ПравоеЗначение)=Тип("СписокЗначений") Тогда
				Сп=Эл.ПравоеЗначение;
			Иначе
				Сп.Добавить(Эл.ПравоеЗначение);	
			КонецЕсли; 
		КонецЕсли; 
	КонецЦикла; 
	Если Сп.Количество()=0 Тогда
		Сп.Добавить(ПланыСчетов.Финансовый.ПустаяСсылка());
	КонецЕсли; 
	Для каждого Эл Из ЭтотОбъект.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы Цикл
		Если Строка(Эл.Параметр)="СчетОтбора" Тогда
			Эл.Значение=Сп;
...
//Раков П.С. ++
Процедура ОбработкаКомментариев(ДокументКомментария, ДатаОтчета, МенеджерКонтрагента, ПДЗВСтрокеРасшифровки) Экспорт

	Регистратор = ПолучитьРегистраторАктуальногоКомментария(ДокументКомментария, ДатаОтчета);
	//+++АК sils 19.07.2017 ИП-00015657.07
	//Если НЕ МенеджерКонтрагента = ПараметрыСеанса.ТекущийПользователь и Регистратор = Неопределено тогда
	//	Предупреждение("Вносить и редактировать комментарии могут только ответственные менеджеры контрагентов!");
	//	Возврат;
	//КонецЕсли;	
	//Если НЕ Регистратор = Неопределено тогда 
	//	Форма = Регистратор.ПолучитьФорму();
	//	Форма.Открыть();
	//	форма.ЭлементыФормы.Пользователь.ТолькоПросмотр = Истина;
	//	форма.ЭлементыФормы.Дата.ТолькоПросмотр = Истина;
	//	форма.ЭлементыФормы.Сумма.ТолькоПросмотр = Истина;
	//	форма.ЭлементыФормы.ДействияФормы.Доступность = Ложь;
	//	форма.ЭлементыФормы.ДокументЗадолженности.ТолькоПросмотр = Истина;
	//	Если НЕ МенеджерКонтрагента = ПараметрыСеанса.ТекущийПользователь Тогда 
	//		форма.ТолькоПросмотр = Истина;
	//	КонецЕсли;	
	//ИначеЕсли МенеджерКонтрагента = ПараметрыСеанса.ТекущийПользователь тогда
	//+++АК sils 19.07.2017 ИП-00015657.07
	Если НЕ Регистратор = Неопределено тогда 
		Форма = Документы.АК_КомментарииКДебЗадолженности.ПолучитьФормуСписка();
		Форма.Отбор.ДокументЗадолженности.ВидСравнения = ВидСравнения.Равно;
		Форма.Отбор.ДокументЗадолженности.Использование = Истина;
		Форма.Отбор.ДокументЗадолженности.Значение = Регистратор.ДокументЗадолженности;
		Форма.ПараметрТекущаяСтрока = Регистратор.Ссылка;
		форма.ЭлементыФормы.ДокументСписок.НастройкаОтбора.ДокументЗадолженности.Доступность = Ложь;
		Форма.Открыть();
	Иначе
	//---АК
		Форма = ОткрытьФорму("Документ.АК_КомментарииКДебЗадолженности.Форма.ФормаДокумента");
		форма.ЭлементыФормы.ДокументЗадолженности.Значение = ДокументКомментария;
		форма.ЭлементыФормы.Пользователь.Значение = ПараметрыСеанса.ТекущийПользователь; 
		форма.ЭлементыФормы.Сумма.Значение = ПДЗВСтрокеРасшифровки;
		форма.ЭлементыФормы.Пользователь.ТолькоПросмотр = Истина;
		форма.ЭлементыФормы.Дата.ТолькоПросмотр = Истина;
		форма.ЭлементыФормы.Сумма.ТолькоПросмотр = Истина;
		форма.ЭлементыФормы.ДействияФормы.Доступность = Ложь;
...
Процедура ОбработкаКомментариевБухгалтером(ДокументКомментария, ДатаОтчета, БухгалтерКонтрагента) Экспорт
	...
	Если НЕ БухгалтерКонтрагента = ПараметрыСеанса.ТекущийПользователь  тогда
		Предупреждение("Обрабатывать комментарии менеджеров могут только ответственные бухгалтера контрагентов.");
		Возврат;
	КонецЕсли;	
	Если НЕ Регистратор.ВидКомментария = Справочники.АК_ВидыКомментариев.Сверка тогда
		Предупреждение("Выберите комментарий с видом ""Сверка""");
		Возврат;
	КонецЕсли;	
	Если НЕ Регистратор = Неопределено тогда 
		НЗ = РегистрыСведений.АК_КомментарииКДебЗадолженности.СоздатьНаборЗаписей();
		НЗ.Отбор.Регистратор.Установить(Регистратор);
		НЗ.Прочитать();
		Если НЗ.Количество()>0 тогда 
			Кнопки = Новый СписокЗначений;
			Кнопки.Добавить(КодВозвратаДиалога.Да, "Подтвердить");
			Кнопки.Добавить(КодВозвратаДиалога.Нет, "Отклонить");
			Кнопки.Добавить(КодВозвратаДиалога.Отмена, "Отмена");
			Ответ = Вопрос("Выберите действие с комментарием.", Кнопки);
			Если Ответ = КодВозвратаДиалога.Да тогда
				НЗ[0].ПодтвержденБухгалтером = Перечисления.АК_ВидыПодтверждения.Подтвержден;
				НЗ.Записать();
	Предупреждение("Статус комментария изменен на ""подтвержден""." + Символы.ПС +"Для отображения нового комментария в отчете необходимо переформировать отчет.");
			ИначеЕсли Ответ = КодВозвратаДиалога.Нет тогда
				НЗ[0].ПодтвержденБухгалтером = Перечисления.АК_ВидыПодтверждения.Отклонен;
				НЗ.Записать();
	Предупреждение("Статус комментария изменен на ""Отклонен""." + Символы.ПС +"Для отображения нового комментария в отчете необходимо переформировать отчет.");
			КонецЕсли;	
		КонецЕсли;	
	Иначе
		Возврат;
...
Процедура ОбработкаКомментариевЮристом(ДокументКомментария, Регистратор, ДатаОтчета) Экспорт
	Если Регистратор = Неопределено тогда
		Возврат;
	КонецЕсли;	
	
	ТекПользователь = ПараметрыСеанса.ТекущийПользователь;
	Если не РольДоступна(Метаданные.Роли.АК_ПодтверждениеОтклонениеСудебнойЗадолженности) Тогда
		Предупреждение("Обрабатывать комментарии могут только пользователи с ролью ""Подтверждение/отклонение судебной задолженности"".");
		Возврат;
	КонецЕсли;	
...
//+++АК sils 21.07.2017 ИП-00015657.07
//Функция ПолучитьРегистраторАктуальногоКомментария(ДокументКомментария, ДатаОтчета)
//+++АК sils 21.07.2017 ИП-00015657.07
Функция ПолучитьРегистраторАктуальногоКомментария(ДокументКомментария, ДатаОтчета) Экспорт
//---АК

	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	АК_КомментарииКДебЗадолженностиСрезПоследних.Регистратор
		|ИЗ
		|	РегистрСведений.АК_КомментарииКДебЗадолженности.СрезПоследних(, ДокументЗадолженности = &Документ) КАК АК_КомментарииКДебЗадолженностиСрезПоследних
		|
		|УПОРЯДОЧИТЬ ПО
		|	АК_КомментарииКДебЗадолженностиСрезПоследних.Период УБЫВ";

	Запрос.УстановитьПараметр("Документ", ДокументКомментария);
...
КонецФункции // ПолучитьДатуАктуальногоКомментария()
