...
Процедура СформироватьОтчетВТаблицу()
	...
	МакетКомпоновки = КомпоновщикМакета.Выполнить(Отчеты.ОтклонениеГрафиковРаботыКассировОтРеальныхВыходов.ПолучитьМакет("ОсновнаяСхемаКомпоновкиДанных")
...
Процедура ЗаписатьДанныеВЛистыСервер()
	...
	Пока Выборка.СледующийПоЗначениюПоля("ЛистУчета") Цикл
		...
		ТабКассиры = ДокОбъект.Кассиры.Выгрузить();
		
		//
		Пока Выборка.Следующий() Цикл
			
			//
			СтрокаТаб = ТабКассиры.Найти(Выборка.Сотрудник, "Кассир");
			Если СтрокаТаб = Неопределено И ЗначениеЗаполнено(Выборка.Свойство_Гр) Тогда
				СтрокаТаб = ТабКассиры.Добавить();
				СтрокаТаб.Кассир = Выборка.Сотрудник;
			...
			Если СтрокаТаб <> Неопределено Тогда
				...
				Если Выборка.Свойство_Гр = "Кассир" Тогда
				...
				ИначеЕсли Выборка.Свойство_Гр = "Стажер-кассир" Тогда
					...
					КонецЕсли;
					СтрокаТаб.КоличествоЧасов    = Выборка.КоличествоЧасов_Гр; 
				ИначеЕсли Выборка.Свойство_Гр = "Почасовик" Тогда //+++АК mika 2018.12.09 ИП-00020658
		...
		//
		ДокОбъект.Кассиры.Загрузить(ТабКассиры);
