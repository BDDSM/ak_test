//+++АК KIRN 2018.07.17 ИП-00019036 
Функция ПолучитьДанныеДляФормирования(мДатаРаспределения, мРасчетчик, мСклад, ДнейПродажВГруппировке, ГлубинаАнализаПродаж,
										ВыводитьТТМиниТТПусто = Ложь, ВыводитьТолькоВВ = Ложь, ВыводитьДополнительныеСтолбцы = Ложь, ВсеРаспределения = Ложь, ИспользоватьНормативныйКвантДляРасчетаПлановогоОстатка = Истина,
										ВыводитьТолькоПерекрестки = Ложь, ВыводитьТолькоПятерочки = Ложь) Экспорт
	...
	КонецЕсли;
	//+++AK GREK 19.11.2018 ЗА-00018923
	Если ВыводитьТолькоПерекрестки Тогда
		ТекстОтбораТТ = ТекстОтбораТТ + "
	|		And Spr_StrEd._Fld2375RRef = 0x9B0B23FEC8C16D0B42EF28BF4B072914";
	ИначеЕсли ВыводитьТолькоПятерочки Тогда
		ТекстОтбораТТ = ТекстОтбораТТ + "
	|		And Spr_StrEd._Fld2375RRef = 0xBF719BBC13518AEB48B29D32DF5A4884";
	КонецЕсли;
	//---AK
	...
	ТаблицаДанные.Колонки.Добавить("ПрогнозныйОстаток"		, мТипЧисло);
	ТаблицаДанные.Колонки.Добавить("ФактическийОстаток0"	, мТипЧисло); //+++АК SHEP 2018.10.24 ИП-00020237
	...
	//+++АК KIRN 2018.02.26 ИП-00017989
	ТаблицаДанные.Колонки.Добавить("СрокГодности"		, мТипЧисло);
	//---АК KIRN 
	...
	Если ВыводитьДополнительныеСтолбцы Тогда
	...
	//+++АК BELN 2018.10.17 ИП-00020134 
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	|ГДЕ
	|	(СтруктурныеЕдиницы.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Розница) ИЛИ СтруктурныеЕдиницы.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.СторонняяРозница))
	...
	|	И СтруктурныеЕдиницы.СтатусТорговойТочки <> ЗНАЧЕНИЕ(Перечисление.СтатусыТорговыхТочек.Приостановлен)
	...
	|ГДЕ
	|	ЕСТЬNULL(Номенклатура.id_tov, 0) <> 0
	|	И Номенклатура.ЭтоГруппа = ЛОЖЬ
	|	И Номенклатура.ПометкаУдаления = ЛОЖЬ
	|	И Номенклатура.ВидНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ВидыНоменклатуры.Товар)
	|	И (Номенклатура.ТипТовара = ЗНАЧЕНИЕ(Перечисление.ТипыТоваров.Продукция)
	|			ИЛИ Номенклатура.ТипТовара = ЗНАЧЕНИЕ(Перечисление.ТипыТоваров.ПокупнойТовар))";
	...
	//+++АК SHEP 2018.10.24 ИП-00020237: в запросе добавил заполнение Fact_Ostatok0
	ТекстЗапроса =
	...
	//+++АК BELN 2018.10.15 ИП-00020134      
	|, @dayr integer, @monthr integer
	//---АК BELN 2018.10.15 
	|
	...
	//+++АК BELN 2018.10.15  ИП-00020134  
	|Set @dayr	= "+Строка(День(мДатаРаспределения))+"
	|Set @monthr		= "+Строка(Месяц(мДатаРаспределения))+"
	//---АК BELN 2018.10.15 
	|
	...
	|create table #ostatki_tt_tov_currdate(id_tov int, id_tt int, kol real, q_fo real)
	|declare @tbl varchar(36)
	|     ,@SQL varchar(max)
	|SET @tbl = REPLACE(NEWID(),'-','_')
	|exec M2..[ostatki_tt_tov_currdate] '" + ТекстИДТоваров + "', '" + ТекстИДТТ + "', 0, @tbl
	|SET @SQL = '
	|select id_tov, id_tt, kol, q_fo
	|from ##'+@tbl+'
	|where kol != 0
	|drop table ##'+@tbl
	|insert into #ostatki_tt_tov_currdate (id_tov, id_tt, kol, q_fo)
	|EXEC (@SQL)
	...
	|Where
	//+++АК KIRN 2018.09.16 
	|	Poryadok_Obespecheniya._Fld2728RRef = @Binary_Sklad	
	...
	|	 and Predzakazy._Fld3057RRef = @Binary_Sklad
	//---АК KIRN 
	...
	|		and Spr_Sklady._OwnerIDRRef = @Binary_Sklad
	//---АК KIRN 
	...
	|	SUM(Progn_Ostatok) as Progn_Ostatok,
	|	SUM(Fact_Ostatok0) as Fact_Ostatok0,
	...
	|	SUM(Fact_Ostatok0) +	SUM(Raspredeleno) - SUM(Plan_Prodaj) as Plan_Konec_Dnya
	|Into #VT_Osnovnaya
	|From (
	|	Select
	...
	|		PodZapros_.Progn_Ostatok,
	|		PodZapros_.Fact_Ostatok0,
	...
	|		Union All
	|		
	//+++АК SHEP 2018.10.24 ИП-00020237: вернул обратно (разкомментировал), только вместо Fact_Ostatok заполняем Fact_Ostatok0
	////+++АК BELN 2018.10.15 ИП-00020134      
	|		Select
	...
	//|			Day(Reg_Fact_Ost._Fld3195) = @Dayr and month(Reg_Fact_Ost._Fld3195) = @monthr
	////---АК BELN 2018.10.15 
	|
	|		Union All
	//---АК SHEP 2018.10.24
	...
	|			TovSpr._idrref,
	|			0,
	|			ostatki_tt_tov_currdate.q_fo,
	|			ostatki_tt_tov_currdate.kol,
	|			0,
	|			0
	|	From #ostatki_tt_tov_currdate ostatki_tt_tov_currdate
	|inner JOIN IzbenkaFin.dbo._Reference29 TovSpr (nolock) ON ostatki_tt_tov_currdate.id_tov = TovSpr._Fld760
	|inner JOIN IzbenkaFin.dbo._Reference42 ttSpr (nolock) ON ostatki_tt_tov_currdate.id_tt = ttSpr._Fld758
	|		Union All
	//+++АК KIRN 2018.07.17 ИП-00019036 
	|"+?(НЕ ВсеРаспределения,"
	|		
	...
	|		Where
	|			Reg_Ost_Raspr._Fld4021 = @DateRaspr","
	|		
	|		select 
	|		sel1.TT,
	|		sel1.Tov,
	...
	|		(
	|		select 
	|			doc._Fld3090_RRRef as TT,
	|			doc_tab._Fld3102RRef as Tov, 
	|			--doc_tab._Fld3103RRef as Ch,
	|			--doc_tab._Fld5085 as Number_r,
	|			SUM(doc_tab._Fld3106) as Rasp
	|		from
	|		IzbenkaFin.dbo._Document3034_VT3100 doc_tab (nolock)
	|		inner join IzbenkaFin.dbo._Document3034 doc (nolock) on doc._IDRRef = doc_tab._Document3034_IDRRef 
	|		where doc._Date_Time = @DateRaspr 
	|		group by 
	|			doc._Fld3090_RRRef,
	|		doc_tab._Fld3102RRef 
	|			--doc_tab._Fld3103RRef,
	|			--doc_tab._Fld5085
	|		)sel1")+"
	//|		Select
	//|			Reg_Ost_Raspr._Fld4023RRef,
	...
	//+++АК KIRN 2018.02.26 ИП-00017989 
	|	Spr_Har._Fld6855 as Expiration, -- срок годности после изготовления
	//---АК KIRN 
	|	CASE
	...
	| On VT_Daty_Postavki.Tovar = VT_Fact_Prodaj.id_tov
	...
	|	VT_Osnovnaya.Progn_Ostatok as Progn_Ostatok,
	|	VT_Osnovnaya.Fact_Ostatok0 as Fact_Ostatok0,
	...
	//+++АК KIRN 2018.02.26  ИП-00017989
	|	ISNULL(VTKolVUpakovke.Expiration, 0) as Expiration,
	//---АК KIRN 
	...
	|		ELSE CAST(ISNULL(VT_Fact_Prodaj_Mejdu.kol, 0) as Numeric(15,0))
	|	END as Fact_Razm", ",
	|	0 as Kol_Fact_Prodaj,
	|	0 as Ost_Sklad,
	|	0 as Plan_Ost_Do_Razm,
	|	0 as KolTTWith05,
	|	0 as Plan_Ost_Razm,
	|	0 as Fact_Razm") + "
	|-------------------------------------------------------------------------------------
	...
	//+++АК BELN 2018.11.01 ИП-00019917.05
	МасНом=Новый Массив;
	//---АК BELN 2018.11.01 
	//
		...
		Если Выборка.Fields.Count > 0 Тогда
			Пока НЕ Выборка.EOF Цикл
				...
				Если НЕ ЗначениеЗаполнено(НоваяСтрока.Поставщик) Тогда
					МасНом.Добавить(НоваяСтрока.Номенклатура);
				КонецЕсли; 
				...
				НоваяСтрока.ПрогнозныйОстаток		= Выборка.Fields("Progn_Ostatok").Value;
				НоваяСтрока.ФактическийОстаток0		= Выборка.Fields("Fact_Ostatok0").Value; //+++АК SHEP 2018.10.24 ИП-00020237
				...
				//+++АК KIRN 2018.02.26 ИП-00017989
				НоваяСтрока.СрокГодности	 		= Выборка.Fields("Expiration").Value;
				//---АК KIRN 
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	РегистрСведений.ТоварныйАссортиментТочек.СрезПоследних(&дата, Номенклатура В (&МасНом) И Характеристика<>Значение(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)) КАК ТоварныйАссортиментТочекСрезПоследних
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныПоставщиков.СрезПоследних(&дата,Номенклатура В (&МасНом) ) КАК ЦеныПоставщиковСрезПоследних
		|		ПО ТоварныйАссортиментТочекСрезПоследних.Номенклатура = ЦеныПоставщиковСрезПоследних.Номенклатура
		|			И ТоварныйАссортиментТочекСрезПоследних.Характеристика = ЦеныПоставщиковСрезПоследних.Характеристика
		|			";
	
	Запрос.УстановитьПараметр("дата", мДатаРаспределения);
	Запрос.УстановитьПараметр("МасНом", МасНом);
	РезультатЗапроса = Запрос.Выполнить();
	
	ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();

	Для каждого НоваяСтрока Из ТаблицаДанные Цикл
		Если НЕ ЗначениеЗаполнено(НоваяСтрока.Поставщик) Тогда
			Если ВыборкаДетальныеЗаписи.НайтиСледующий(Новый Структура("Номенклатура,ТорговаяТочка",НоваяСтрока.Номенклатура,НоваяСтрока.ТорговаяТочка)) Тогда
				НоваяСтрока.Поставщик 	=  ВыборкаДетальныеЗаписи.Поставщик;
			КонецЕсли; 
