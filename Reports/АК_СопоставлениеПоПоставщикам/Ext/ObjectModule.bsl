...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Для каждого Параметр из КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы Цикл
		...
		ИначеЕсли СокрЛП(Параметр.Параметр) = "Организация" Тогда
			...
			Запрос.УстановитьПараметр("Организация", Параметр.Значение);
	...
	ЗапросДанныеФинансов.Текст = "ВЫБРАТЬ
	                             ...
	                             |ИЗ
	                             |	РегистрБухгалтерии.Финансовый.ОстаткиИОбороты(&НачалоПериода, &КонецПериода, , , Счет В ИЕРАРХИИ (&СписокСчетов), , ) КАК ФинансовыйОстатки
	                             |
	                             |ОБЪЕДИНИТЬ ВСЕ
	                             |
	                             |ВЫБРАТЬ
	                             ...
	                             |ИЗ
	                             |	РегистрБухгалтерии.Финансовый.Обороты(&НачалоПериода, &КонецПериода, Регистратор, Счет В ИЕРАРХИИ (&СписокСчетов), , , , ) КАК ФинансовыйОбороты
	                             |
	                             |ОБЪЕДИНИТЬ ВСЕ
	                             |
	                             |ВЫБРАТЬ
	...
	Для Каждого Стр Из ТаблицаФинансыВрем Цикл
		...
		НовСтр.КонтрагентНаименованиеПолное = СокрЛП(Стр.Контрагент.НаименованиеПолное);
		НовСтр.КонтрагентИНН = Стр.Контрагент.Инн;
		НовСтр.ГУИД = Строка(Стр.Контрагент.УникальныйИдентификатор());
	КонецЦикла;	
	...
	ТекстЗапроса="ВЫБРАТЬ
			...
			|ПОМЕСТИТЬ СводнаяТаблица
			|ИЗ
			|	РегистрБухгалтерии.Хозрасчетный.ОстаткиИОбороты(&НачалоПериода, &КонецПериода, , , Счет В ИЕРАРХИИ (&СписокСчетов), , ) КАК ФинансовыйОстатки
			...
			|	ФинансовыйОбороты.Организация
			|ИЗ
			|	РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоПериода, &КонецПериода, Регистратор, Счет В ИЕРАРХИИ (&СписокСчетов), , , , ) КАК ФинансовыйОбороты
