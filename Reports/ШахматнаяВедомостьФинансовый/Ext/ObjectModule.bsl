...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок = Истина, ВысотаЗаголовка = 0) Экспорт
	...
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ФинансовыйОборотыДтКт.СчетДт КАК СчетДт,
	|	ФинансовыйОборотыДтКт.СчетКт КАК СчетКт,
	|	ФинансовыйОборотыДтКт.СчетДт.Представление КАК СчетДтПредставление,
	|	ФинансовыйОборотыДтКт.СчетКт.Представление КАК СчетКтПредставление,
	|	ФинансовыйОборотыДтКт.СчетДт.Валютный КАК СчетДтВалютный,
	|	ФинансовыйОборотыДтКт.СчетКт.Валютный КАК СчетКтВалютный,
	|	ФинансовыйОборотыДтКт.СуммаОборот КАК СуммаОборот,
	|	ФинансовыйОборотыДтКт.ВалютнаяСуммаОборотДт КАК ВалютнаяСуммаОборотДт,
	|	ФинансовыйОборотыДтКт.ВалютнаяСуммаОборотКт КАК ВалютнаяСуммаОборотКт
	|ИЗ
	|	РегистрБухгалтерии.Финансовый.ОборотыДтКт(&ДатаНач, &ДатаКон, , " 
	+ ТекстОтборСчетДт + ", , " 
	+ ТекстОтборСчетКт + ", , "
	+ СтрокаОграниченийПоРеквизитам +"
	|) КАК ФинансовыйОборотыДтКт
	|
	...
	Если Не ПоСубсчетам Тогда
		...
		|
		|ГДЕ
		|	Финансовый.Родитель = &ПустойРодитель");
		ЗапросСчетаВерх.УстановитьПараметр("ПустойРодитель", ПланыСчетов.Финансовый.ПустаяСсылка());
...
Процедура ОбработкаРасшифровкиСтандартногоОтчета(Расшифровка) Экспорт
	...
	Отчет = Отчеты.ОтчетПоПроводкамФинансовый.Создать();
