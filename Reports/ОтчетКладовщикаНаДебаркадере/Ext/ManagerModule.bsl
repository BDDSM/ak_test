...
Функция СформироватьОтчет(ДатаНачала,ДатаОкончания,СкладВладелец,ОтборСтатусПаллетта="",КодОрганизации="", СписокВорот="",Расходник=Неопределено) Экспорт
	...
	Запрос.Текст = 
		...
		|ПОМЕСТИТЬ втЗабытые
		|ИЗ
		|	РегистрСведений.СканированныеПаллеты.СрезПоследних(&Дата2, Ордер=&Расходник ИЛИ &Расходник=Неопределено) КАК СканированныеПаллетыСрезПоследних
		|ГДЕ
		...
		|ГДЕ  (МаршрутныйЛистРасходныеОрдера.Документ=&Расходник ИЛИ &Расходник=Неопределено) И
		...
		|ГДЕ
		|	СоставПаллеты.ПометкаУдаления = ЛОЖЬ И (СоставПаллеты.РасходныйОрдер=&Расходник ИЛИ &Расходник=Неопределено)
		|
		|СГРУППИРОВАТЬ ПО
		|	СоставПаллеты.РасходныйОрдер
		|;
		...
		|		ИЗ
		|			РегистрСведений.СканированныеПаллеты КАК СканированныеПаллетыСрезПоследних
		|		ГДЕ  (СканированныеПаллетыСрезПоследних.Ордер=&Расходник ИЛИ &Расходник=Неопределено) И
