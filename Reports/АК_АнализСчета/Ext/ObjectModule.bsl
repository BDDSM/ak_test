...
Процедура ПередВыводомОтчета(МакетКомпоновки) Экспорт
	...
	ЗавершающаяЧастьЗапроса = ") КАК ФинансовыйОбороты";//") КАК ХозрасчетныйОбороты";
		...
		Если СтрокаТаблицы.Использование И ЗначениеЗаполнено(СтрокаТаблицы.Счет) Тогда
			...
			Для Каждого ВидКорСубконто Из СтрокаТаблицы.ПоСубконто Цикл
				Если ВидКорСубконто.Пометка Тогда
					//+++АК sils 21.08.2018 ИП-00019605
					//Если ТипЗнч(ВидКорСубконто.Значение) = Тип("ПланВидовХарактеристикСсылка.ВидыСубконтоХозрасчетные") Тогда
					Если ТипЗнч(ВидКорСубконто.Значение) = Тип("ПланВидовХарактеристикСсылка.ВидыСубконто") Тогда
					//---АК
						...
						ТекстЗапросаЗамены  = СтрЗаменить(ТекстЗапросаЗамены, "КорАналитика" + КоличествоАналитики, "ФинансовыйОбороты.КорСубконто" + инд);
						//---АК
						КоличествоАналитики = КоличествоАналитики + 1;
					КонецЕсли;
				КонецЕсли;
			...
			Для сч = 1 По 3 Цикл
				...
				СтрокаДляПоиска = "ФинансовыйОбороты.КорСубконто" + сч + " КАК КорСубконто" + сч + ",";
				//---АК
				НачалоСтроки = Найти(ПромежуточныйТекстЗапроса,  СтрокаДляПоиска);
				Если НачалоСтроки > 0 Тогда
					СтрокаДляЗамены = СтрокаДляЗамены + "@";
	...
	Для инд = 1 По 3 Цикл
		...
		ПостояннаяЧастьЗапроса = СтрЗаменить(ПостояннаяЧастьЗапроса, "ФинансовыйОбороты.КорСубконто" + инд + " КАК", "NULL КАК");
		//---АК
	КонецЦикла;
	...
	МакетГруппировкиСчетЗаголовок = АК_СтандартныеОтчеты.ПолучитьМакетГруппировкиПоПолюГруппировки(МакетКомпоновки, "Счет", , "Заголовок");
	МакетГруппировкиСчетПодвал    = АК_СтандартныеОтчеты.ПолучитьМакетГруппировкиПоПолюГруппировки(МакетКомпоновки, "Счет", , "Подвал");
	
	МакетГруппировкиСубконтоЗаголовок = АК_СтандартныеОтчеты.ПолучитьМакетГруппировкиПоПолюГруппировки(МакетКомпоновки, "Субконто", , "Заголовок");
	МакетГруппировкиСубконтоПодвал    = АК_СтандартныеОтчеты.ПолучитьМакетГруппировкиПоПолюГруппировки(МакетКомпоновки, "Субконто", , "Подвал");
	
	МакетГруппировкиПодразделениеЗаголовок = АК_СтандартныеОтчеты.ПолучитьМакетГруппировкиПоПолюГруппировки(МакетКомпоновки, "Подразделение", , "Заголовок");
	МакетГруппировкиПодразделениеПодвал    = АК_СтандартныеОтчеты.ПолучитьМакетГруппировкиПоПолюГруппировки(МакетКомпоновки, "Подразделение", , "Подвал");

	МакетГруппировкиОрганизацияЗаголовок = АК_СтандартныеОтчеты.ПолучитьМакетГруппировкиПоПолюГруппировки(МакетКомпоновки, "Организация", , "Заголовок");
	МакетГруппировкиОрганизацияПодвал    = АК_СтандартныеОтчеты.ПолучитьМакетГруппировкиПоПолюГруппировки(МакетКомпоновки, "Организация", , "Подвал");

	МакетГруппировкиВалютаЗаголовок = АК_СтандартныеОтчеты.ПолучитьМакетГруппировкиПоПолюГруппировки(МакетКомпоновки, "Валюта", , "Заголовок");
	МакетГруппировкиВалютаПодвал    = АК_СтандартныеОтчеты.ПолучитьМакетГруппировкиПоПолюГруппировки(МакетКомпоновки, "Валюта", , "Подвал");

	МакетГруппировкиПериодЗаголовок = АК_СтандартныеОтчеты.ПолучитьМакетГруппировкиПоПолюГруппировки(МакетКомпоновки, "Период", , "Заголовок");
	МакетГруппировкиПериодПодвал    = АК_СтандартныеОтчеты.ПолучитьМакетГруппировкиПоПолюГруппировки(МакетКомпоновки, "Период", , "Подвал");

	
	МакетГруппировкиКорЗаголовок = АК_СтандартныеОтчеты.ПолучитьМакетГруппировкиПоПолюГруппировки(МакетКомпоновки, "Кор", , "Заголовок");
...
Процедура ДоработатьКомпоновщикПередВыводом(ВнешниеНаборыДанных) Экспорт
	...
	ТиповыеОтчеты.УстановитьПараметр(КомпоновщикНастроек, "НачалоПериода", НачалоДня(НачалоПериода), ЗначениеЗаполнено(НачалоПериода));
	ТиповыеОтчеты.УстановитьПараметр(КомпоновщикНастроек, "КонецПериода", КонецДня(КонецПериода), ЗначениеЗаполнено(КонецПериода));
	...
	//+++ AK suvv 18.07.2018 ИП-00019308
	//неверно добавляется отбор, отчет по проводкам формируется неправильно
	//Если ПоСубсчетам Тогда 
	//	ПолеГруппировки.Поле           = Новый ПолеКомпоновкиДанных("Счет");
	//Иначе
	//	ПолеГруппировки.Поле           = Новый ПолеКомпоновкиДанных("СчетОтбор");
	//КонецЕсли;
	ПолеГруппировки.Поле           = Новый ПолеКомпоновкиДанных("Счет");
	//--- AK suvv
	...
	Если Не ПоСубсчетамКорСчетов Тогда
		...
		СписокСчетовПоСубсчетам = АК_СтандартныеОтчеты.ПолучитьСписокСчетовПоСубсчетам(ДанныеОтчета.ГруппировкаКор);
	...
	СхемаКомпоновкиДанных.НаборыДанных.ОсновнойНаборДанных.Запрос = Схема.НаборыДанных.ОсновнойНаборДанных.Запрос;
	СхемаКомпоновкиДанных.НаборыДанных.Обороты.Запрос             = Схема.НаборыДанных.Обороты.Запрос;
	
	УстановитьУсловиеВЗапросеНаПоказатели(ПоказателиОтчета, СхемаКомпоновкиДанных);
	...
	//
	//Поле = УсловноеОформление.Поля.Элементы.Добавить();
	//Поле.Поле = Новый ПолеКомпоновкиДанных("СальдоНаКонецПериода.КоррКонечныйОстатокДт");
	//Поле = УсловноеОформление.Поля.Элементы.Добавить();
	//Поле.Поле = Новый ПолеКомпоновкиДанных("СальдоНаКонецПериода.МСФОКонечныйОстатокДт");
...
Функция ПолучитьТекстЗаголовка(ОрганизацияВНачале = Истина) Экспорт 
	...
	ЗаголовокОтчета = "Анализ счета " + Счет + АК_СтандартныеОтчеты.ПолучитьПредставлениеПериода(ЭтотОбъект);
...
Процедура УстановитьУсловиеВЗапросеНаПоказатели(ПоказателиОтчета, Схема)
	...
	Для Каждого Показатель Из Показатели Цикл
		...
		Если ПоказателиОтчета[Показатель].Значение Тогда
			Если Показатель = "Упр" Тогда
				Поля.Добавить("Сумма");
			ИначеЕсли Показатель = "Корр" ИЛИ Показатель = "МСФО" Тогда
...
Функция ПолучитьНаборПоказателей()
	...
	НаборПоказателей.Добавить("Корр");
	НаборПоказателей.Добавить("МСФО");
