...
Функция ПолучитьТаблицуРасхожденийСчета44_3(ДатаНачала, ДатаОкончания, ЕстьОтборТТ, МассивТТ) Экспорт
	...
	ТаблицаРезультат.Колонки.Добавить("Терминал", Новый ОписаниеТипов("СправочникСсылка.Терминалы"));
	ТаблицаРезультат.Колонки.Добавить("Отклонение", Новый ОписаниеТипов("Число"));
	ТаблицаРезультат.Колонки.Добавить("Расчетчик", Новый ОписаниеТипов("СправочникСсылка.Пользователи"));
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |			ЛистУчета.Расчетчик КАК Расчетчик,
	               |			ЛистУчета.Комментарий КАК Комментарий
	               |		ИЗ
	               |			Документ.ЛистУчета КАК ЛистУчета
	               |		ГДЕ
	               |			ЛистУчета.Дата МЕЖДУ &ДатаНачала И &ДатаКонца
	               |			И ЛистУчета.Проведен = ИСТИНА) КАК ВЗ_Листы
	               |		ПО ФинансовыйОбороты.Субконто1 = ВЗ_Листы.ТорговаяТочка
	               |			И (НАЧАЛОПЕРИОДА(ФинансовыйОбороты.Период, ДЕНЬ) = НАЧАЛОПЕРИОДА(ВЗ_Листы.Дата, ДЕНЬ))
	               |ГДЕ
