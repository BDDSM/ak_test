...
Функция Расшифровать_ПоНоменклатуре(Расшифровка)
	...
	ПараметрВладелец= Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы.Найти(Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("Владелец")).ИдентификаторПользовательскойНастройки);
	...
	Хран=ПолучитьИзВременногоХранилища(Отчет.ПутьКТаблице);
	...
	//+++АК BELN 2018.04.03 
	Запрос = Новый Запрос;
	Запрос.Текст = ПолучитьЗапросПоРО("РасходныйОрдерСкладТовары.Номенклатура = &Номенклатура ");
	...
	Запрос.УстановитьПараметр("МасРО", МасРО);
	...
	Запрос = Новый Запрос;
	Запрос.Текст = ПолучитьЗапросПоРОИПоступ("&Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
...
&НаСервере
Функция ПолучитьЗапросПоРО(ТекстОтбора1,ТекстОтбора2="Истина")
	Текст=		"ВЫБРАТЬ
		...
		|ГДЕ РасходныйОрдерСкладТовары.Ссылка в (&МасРО) И &Отбор2
		...
		//|			КОНЕЦ, ДЕНЬ) МЕЖДУ &НачалоПериода И &КонецПериода И РасходныйОрдерСкладТовары.Склад.Владелец = &Склад
		...
		|	Номенклатура,
		|	Документ";
	Текст=СтрЗаменить(Текст,"&Отбор1",ТекстОтбора1);	
	Текст=СтрЗаменить(Текст,"&Отбор2",ТекстОтбора2);	
...
&НаСервере
Функция ПолучитьЗапросПоРОИПоступ(ТекстОтбора1,ТекстОтбора2,ТекстОтбора3="Истина",ТекстОтбора4="Истина")
	Текст=		"ВЫБРАТЬ
	               |	ТаблицаПоПоступлению.Номенклатура,
	               |	ТаблицаПоПоступлению.ТТ,
	               |	ТаблицаПоПоступлению.Поступило,
	               |	ТаблицаПоПоступлению.Бой,
	               |	ТаблицаПоПоступлению.День,ТаблицаПоПоступлению.Расходник,ТаблицаПоПоступлению.ЗаданиеНаРазборку
	               ...
	               |			День,
	               |			,
	               |			&Отбор1) КАК ТоварыНаСкладахОстаткиИОбороты
	               |ГДЕ
				   ...
				   |	РасходныйОрдерСкладТовары.Дата,
				   |	РасходныйОрдерСкладТовары.ДатаРаспределения,
				   |	Значение(Перечисление.ТипыСтруктурныхЕдиниц.Склад) КАК ТипСтруктурнойЕдиницы,
				   |	РасходныйОрдерСкладТовары.ПриемкаВМагазинеБезПересчета,
				   ...
				   |	Документ.РасходныйОрдерСклад КАК РасходныйОрдерСкладТовары
				   |ГДЕ
				   |	РасходныйОрдерСкладТовары.Ссылка в (&МасРО) И &Отбор3
	               |;
	               ...
	               |			И РасходныйОрдерСкладТовары.СкладВладелец = ЗначенияСвойствОбъектов.СтруктурнаяЕдиница
	               |			И (ЗначенияСвойствОбъектов.День = НАЧАЛОПЕРИОДА(РасходныйОрдерСкладТовары.Дата, ДЕНЬ))
				   ...
				   //|	И РасходныйОрдерСкладТовары.Ссылка.ВидОперации = &ВидОперации  И РасходныйОрдерСкладТовары.Ссылка.Склад.Владелец = &Склад
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	РасходныйОрдерСкладТовары.Номенклатура,
	               |	РасходныйОрдерСкладТовары.Ссылка ;
	               ...
	               |	РасходныйОрдерСкладТовары.Отклонение,
					...
					|	КОНЕЦ КАК Комплектовщик,
				   
	               |	РасходныйОрдерСкладТовары.Склад как Склад,
	               |	РасходныйОрдерСкладТовары.Получатель КАК ТТ,
	               |	РасходныйОрдерСкладТовары.Ссылка КАК Документ,
	               |	НАЧАЛОПЕРИОДА(РасходныйОрдерСкладТовары.Дата, ДЕНЬ) КАК Дата,
	               |	ВЫБОР
	               ...
	               |	МЕСЯЦ(РасходныйОрдерСкладТовары.Дата) КАК Месяц,
	               |	ГОД(РасходныйОрдерСкладТовары.Дата) КАК Год,
	               |	НЕДЕЛЯ(РасходныйОрдерСкладТовары.Дата) КАК Неделя,
	               ...
	               |	ВЫБОР Когда РасходныйОрдерСкладТовары.ВыезднаяТорговля Тогда 0
	               |		КОГДА (ВЫРАЗИТЬ((РасходныйОрдерСкладТовары.КоличествоУРЗ - ЕСТЬNULL(ТаблицаДанныхПоПоступлению.Поступило, 0)) / ТабТовары.КоличествоВУпаковке КАК ЧИСЛО(15, 0))) > 0
	               |			ТОГДА ВЫРАЗИТЬ((РасходныйОрдерСкладТовары.КоличествоУРЗ - ЕСТЬNULL(ТаблицаДанныхПоПоступлению.Поступило, 0)) / ТабТовары.КоличествоВУпаковке КАК ЧИСЛО(15, 0))
	               |		ИНАЧЕ 0
	               ...
	               |			И РасходныйОрдерСкладТовары.Получатель = ТаблицаДанныхПоПоступлению.ТТ
	               |			И (НАЧАЛОПЕРИОДА(ВЫБОР
	               |					КОГДА РасходныйОрдерСкладТовары.ДатаРаспределения <> ДАТАВРЕМЯ(1, 1, 1)
	               |						ТОГДА РасходныйОрдерСкладТовары.ДатаРаспределения
	               |					ИНАЧЕ РасходныйОрдерСкладТовары.Дата
	               |				КОНЕЦ, ДЕНЬ) = ТаблицаДанныхПоПоступлению.День) И РасходныйОрдерСкладТовары.Ссылка=ТаблицаДанныхПоПоступлению.Расходник И РасходныйОрдерСкладТовары.ЗаданиеНаРазборку=ТаблицаДанныхПоПоступлению.ЗаданиеНаРазборку
	               ...
	               |			И РасходныйОрдерСкладТовары.Склад = Остатки.Склад
	               |			И (НАЧАЛОПЕРИОДА(ВЫБОР
	               |					КОГДА РасходныйОрдерСкладТовары.ДатаРаспределения <> ДАТАВРЕМЯ(1, 1, 1)
	               |						ТОГДА РасходныйОрдерСкладТовары.ДатаРаспределения
	               |					ИНАЧЕ РасходныйОрдерСкладТовары.Дата
	               ...
	               |		ПО РасходныйОрдерСкладТовары.Получатель = ПоздноПришло.ТорговаяТочка
	               |			И (НАЧАЛОПЕРИОДА(ВЫБОР
	               |					КОГДА РасходныйОрдерСкладТовары.ДатаРаспределения <> ДАТАВРЕМЯ(1, 1, 1)
	               |						ТОГДА РасходныйОрдерСкладТовары.ДатаРаспределения
	               |					ИНАЧЕ РасходныйОрдерСкладТовары.Дата
	...
	Текст=СтрЗаменить(Текст,"&Отбор1",ТекстОтбора1);	
	Текст=СтрЗаменить(Текст,"&Отбор2",ТекстОтбора2);	
	Текст=СтрЗаменить(Текст,"&Отбор3",ТекстОтбора3);
	Текст=СтрЗаменить(Текст,"&Отбор4",ТекстОтбора4);
...
Функция Расшифровать_ПоСкладу(Расшифровка)
	...
	//+++АК BELN 2018.04.03 
	Запрос = Новый Запрос;
	Запрос.Текст = ПолучитьЗапросПоРО("Истина","РасходныйОрдерСкладТовары.Склад = &Склад");
	...
	Запрос = Новый Запрос;
	Запрос.Текст = ПолучитьЗапросПоРОИПоступ("Склад = &Склад",
	"Истина","РасходныйОрдерСкладТовары.Склад = &Склад"
...
Функция Расшифровать_ПоКомплектовщикуИНоменклатуреИСкладу(Расшифровка)
	СтруктураВозврата = Новый Структура("Сборщик, Склад, Номенклатура");
	...
	//	НоменклатураСсылка = Справочники.Номенклатура.ПустаяСсылка();
	//КонецПопытки;
	КомплектовщикСсылка = СтруктураВозврата.Сборщик;
	СкладСсылка = СтруктураВозврата.Склад;
	НоменклатураСсылка = СтруктураВозврата.Номенклатура;
	Если НЕ ЗначениеЗаполнено(НоменклатураСсылка) Тогда
		НоменклатураСсылка = Справочники.Номенклатура.ПустаяСсылка();
	КонецЕсли; 
	
	Хран=ПолучитьИзВременногоХранилища(Отчет.ПутьКТаблице);
	Если Хран=Неопределено ИЛИ СтруктураВозврата.Сборщик=Неопределено ИЛИ СтруктураВозврата.Склад=Неопределено Тогда
	...
	//+++АК BELN 2018.04.03 
	Запрос = Новый Запрос;
	Запрос.Текст = ПолучитьЗапросПоРО("(&Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка) или РасходныйОрдерСкладТовары.Номенклатура = &Номенклатура) ","РасходныйОрдерСкладТовары.Склад = &Склад");
	...
	Запрос.Текст = ПолучитьЗапросПоРОИПоступ("(&Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	               ...
	               |				И Склад = &Склад",
	"(&Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	               |			ИЛИ РасходныйОрдерСкладТовары.Номенклатура = &Номенклатура)","РасходныйОрдерСкладТовары.Склад = &Склад"
	,
	"	ВЫБОР
					|		КОГДА РасходныйОрдерСкладТовары.ЗаданиеНаРазборку = ЗНАЧЕНИЕ(Документ.ЗаданиеНаРазборку.ПустаяСсылка)
					|			ТОГДА РасходныйОрдерСкладТовары.Сборщик
