...
Процедура СформироватьОтчетВТаблицу()
	...
	МакетКомпоновки = КомпоновщикМакета.Выполнить(Отчеты.ОтклонениеГрафиковРаботыУборщицОтРеальныхВыходов.ПолучитьМакет("ОсновнаяСхемаКомпоновкиДанных")
...
Процедура ЗаписатьДанныеВЛистыСервер()
	...
	Пока Выборка.СледующийПоЗначениюПоля("ЛистУчета") Цикл
		...
		ТабУборщицы = ДокОбъект.Уборщицы.Выгрузить();
		Пока Выборка.Следующий() Цикл
			СтрокаТаб = ТабУборщицы.Найти(Выборка.Сотрудник, "ФизЛицо");
			Если НЕ ЗначениеЗаполнено(Выборка.ТТ_Гр) Тогда
				Если СтрокаТаб <> Неопределено Тогда
					ТабУборщицы.Удалить(СтрокаТаб);
				КонецЕсли;	
			Иначе	
				Если СтрокаТаб = Неопределено Тогда
					СтрокаТаб = ТабУборщицы.Добавить();
			...
			КонецЕсли;	
		КонецЦикла;
		ДокОбъект.Уборщицы.Загрузить(ТабУборщицы);
