...
Процедура ВыводПодписейОтчета(ОтчетОбъект, Результат)
	...
	СтандартныеОтчеты.ВыводПодписейОтчета(ОтчетОбъект, Результат, ДополнительныеПараметры);
...
Процедура УстановитьУсловиеВЗапросеНаПоказатели(ПоказателиОтчета, Схема)
	...
	ТекстДляЗаменыУсловиеОтборКонтрольОстатки = СтандартныеОтчеты.ПолучитьТекстПоМаркерам(Схема.НаборыДанных.ОсновнойНаборДанных.Запрос, "// Начало ОтборКонтроль", "// Конец ОтборКонтроль");
	ТекстДляЗаменыУсловиеОтборКонтрольОбороты = СтандартныеОтчеты.ПолучитьТекстПоМаркерам(Схема.НаборыДанных.Обороты.Запрос, "// Начало ОтборКонтроль", "// Конец ОтборКонтроль");
