...
Процедура ПередВыводомОтчета(МакетКомпоновки) Экспорт
	...
	КоличествоПоказателей = 0 + ПоказателиОтчета.Упр.Значение + ПоказателиОтчета.Корр.Значение + ПоказателиОтчета.МСФО.Значение + ПоказателиОтчета.ВалютнаяСумма.Значение;
			...
			Если Не ЗначениеЗаполнено(ЗначениеСчет.Родитель) И Не ЗначениеСчет.Забалансовый Тогда
				Для Индекс = 0 По КоличествоПоказателей - 1 Цикл
					Для ПодИндекс = 1 По 6 Цикл
						...
						//ИТОГИ ОСВ
						//Если Не ЗначениеСчет.МСФО И Индекс = (КоличествоПоказателей - 1)
						//Если Индекс = (КоличествоПоказателей - 1) 
						//	И ДанныеОтчета.ПоказателиОтчета.МСФО.Значение Тогда
						//	Значение = 0;
						//КонецЕсли;
...
Процедура ДоработатьКомпоновщикПередВыводом(ВнешниеНаборыДанных) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ДанныеОтчета.РазвернутоеСальдо Цикл
		...
		Если СтрокаТаблицы.Использование И ЗначениеЗаполнено(СтрокаТаблицы.Счет) Тогда
			...
			АК_СтандартныеОтчеты.СкопироватьПараметрСхемыКомпоновкиДанных(СхемаКомпоновкиДанных, "СчетРазвернутоеСальдо" + Индекс, "СчетРазвернутоеСальдо", СтрокаТаблицы.Счет);
			...
			АК_СтандартныеОтчеты.СкопироватьПараметрСхемыКомпоновкиДанных(СхемаКомпоновкиДанных, "СчетаИсключенныеИзЗапросаПоСчетамРазвернутое" + Индекс, "СчетаИсключенныеИзЗапросаПоСчетамРазвернутое", СчетаИсключенныеИзЗапросаПоСчетамРазвернутое);
			
			// Установка параметра "СубконтоДетализацииРазвернутый
			АК_СтандартныеОтчеты.СкопироватьПараметрСхемыКомпоновкиДанных(СхемаКомпоновкиДанных, "СубконтоРазвернутый" + Индекс, "СубконтоРазвернутый", СубконтоРазвернутоеСальдо);
	...
	Для Каждого СтрокаТаблицы Из ДанныеОтчета.Группировка Цикл	
		...
		Если СубконтоДетализации.Количество() > 0 Тогда
			...
			АК_СтандартныеОтчеты.СкопироватьПараметрСхемыКомпоновкиДанных(СхемаКомпоновкиДанных, "СчетДетализации" + Индекс, "СчетДетализации", СтрокаТаблицы.Счет);
			
			// Добавление и установка значения параметра СубконтоДетализации{Индекс}
			АК_СтандартныеОтчеты.СкопироватьПараметрСхемыКомпоновкиДанных(СхемаКомпоновкиДанных, "СубконтоДетализации" + Индекс, "СубконтоДетализации", СубконтоДетализации);
	...
	СписокСчетовПоСубсчетам = АК_СтандартныеОтчеты.ПолучитьСписокСчетовПоСубсчетам(ДанныеОтчета.Группировка);
...
Функция ПолучитьТекстЗаголовка(ОрганизацияВНачале = Истина) Экспорт 
	...
	ЗаголовокОтчета = "Оборотно-сальдовая ведомость" + АК_СтандартныеОтчеты.ПолучитьПредставлениеПериода(ЭтотОбъект);
...
Процедура ПолучитьСтруктуруПоказателейОтчета() Экспорт
	...
	ПоказателиОтчета = АК_СтандартныеОтчеты.ПолучитьСтруктуруПоказателейОтчета(,, Истина, Истина, Ложь, Ложь);
