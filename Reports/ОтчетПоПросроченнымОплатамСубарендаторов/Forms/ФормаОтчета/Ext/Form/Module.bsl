...
Процедура МесяцПриИзменении(Элемент)
	ДатаКакМесяцПодобратьДатуПоТексту(МесяцСтрока, Отчет.НачалоПериода);
	МесяцСтрока = ДатаКакМесяцПредставление(Отчет.НачалоПериода);
КонецПроцедуры
...
Процедура МесяцНачалоВыбораИзСписка(Элемент, СтандартнаяОбработка)
	НачалоВыбораИзСпискаПредставленияПериодаРегистрации(Элемент, СтандартнаяОбработка, Отчет.НачалоПериода, ЭтаФорма);
КонецПроцедуры
...
Процедура НачалоВыбораИзСпискаПредставленияПериодаРегистрации(Элемент, СтандартнаяОбработка, ПериодРегистрации, ЭтаФорма, НачальноеЗначение = Неопределено) Экспорт
	...
	Отчет.НачалоПериода=ПериодРегистрации;
