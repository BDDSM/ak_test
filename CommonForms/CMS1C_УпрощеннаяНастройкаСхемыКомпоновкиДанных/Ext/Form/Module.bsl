	...
	Если ПроверятьНаИзменение Тогда
		...
		Если ЭтоАдресВременногоХранилища(АдресПриемник) Тогда
			...
			Если ТипЗнч(ТекущаяСКД) = Тип("СхемаКомпоновкиДанных") Тогда
				...
				Если CMS1C_ПроцедурыОбменаССайтомСервер.ПолучитьXML(СхемаКомпоновкиДанных) <> CMS1C_ПроцедурыОбменаССайтомСервер.ПолучитьXML(ТекущаяСКД) Тогда
...
Процедура ПоместитьНастройкиВСхемуКомпоновкиДанных(КомпоновщикНастроек, АдресСхемыКомпоновкиДанных)
	...
	CMS1C_ПроцедурыОбменаССайтомСервер.ОчиститьНастройкиКомпоновкиДанных(СхемаКомпоновкиДанных.НастройкиПоУмолчанию);
	CMS1C_ПроцедурыОбменаССайтомСервер.СкопироватьНастройкиКомпоновкиДанных(СхемаКомпоновкиДанных.НастройкиПоУмолчанию, КомпоновщикНастроек.ПолучитьНастройки());
...
Процедура ЗагрузитьСхемуИзФайла(Команда)
	...
	ПредложитьУстановкуРасширенияРаботыСФайлами();
