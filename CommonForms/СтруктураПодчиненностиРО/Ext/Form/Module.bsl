...
&НаСервере
Процедура ВывестиТабличныйДокумент(ТЗЗадания=Неопределено)

	ТаблицаОтчета.Очистить();
	
	Макет = ПолучитьОбщийМакет("СтруктураПодчиненности");
	
	ВывестиРодительскиеЭлементыДерева(ДеревоРодительскиеДокументы.ПолучитьЭлементы(),Макет,1,ТЗЗадания);
	ВывестиТекущийДокумент(Макет,ТЗЗадания);
	ВывестиПодчиненныеЭлементыДерева(ДеревоПодчиненныеДокументы.ПолучитьЭлементы(),Макет,1,ТЗЗадания)
...
//  
&НаСервере
Процедура ВывестиРодительскиеЭлементыДерева(СтрокиДерева,Макет,УровеньРекурсии,ТЗЗадания=Неопределено)
	...
	Пока Счётчик >0 Цикл
		...
		ВывестиДокументИКартинку(ТекущаяСтрокаДерева,Макет,Ложь,Ложь,ТЗЗадания);
...
//
&НаСервере
Процедура ВывестиДокументИКартинку(СтрокаДерева,Макет,ЭтоТекущийДокумент = Ложь,ЭтоПодчиненный = Неопределено,ТЗЗадания=Неопределено)
...
//	ОбластьКартинка.Параметры.Документ = СтрокаДерева.Ссылка;
	...
	//+++АК BELN 2018.04.06 ИП-00018296
	Если ТипЗнч(СтрокаДерева.Ссылка)=Тип("ДокументСсылка.ЗаданиеНаРазборку") Тогда
		Если ТЗЗадания=Неопределено Тогда
			Если СтрокаДерева.Ссылка.Закрыто Тогда
				ОбластьДокумент.Области[?(ЭтоТекущийДокумент,"ТекущийДокумент","Документ")].ЦветФона=WebЦвета.СветлоЗеленый;
			КонецЕсли; 
		Иначе
			СтрЗад=ТЗЗадания.Найти(СтрокаДерева.Ссылка);
			Если СтрЗад=Неопределено Тогда
				Если СтрокаДерева.Ссылка.Закрыто Тогда
					ОбластьДокумент.Области[?(ЭтоТекущийДокумент,"ТекущийДокумент","Документ")].ЦветФона=WebЦвета.СветлоЗеленый;
				КонецЕсли; 
			Иначе	
				Если СтрЗад.ЗаданиеНаРазборкуЗакрыто Тогда
					ОбластьДокумент.Области[?(ЭтоТекущийДокумент,"ТекущийДокумент","Документ")].ЦветФона=WebЦвета.СветлоЗеленый;
				КонецЕсли; 
			КонецЕсли; 
		КонецЕсли; 
...
&НаСервере
Процедура ВывестиТекущийДокумент(Макет,ТЗЗадания=Неопределено)
	Выборка = ПолучитьВыборкуПоРеквизитамДокумента(ДокументСсылка);
	Если Выборка.Следующий() Тогда
		
		ВывестиДокументИКартинку(Выборка,Макет,Истина,,ТЗЗадания);
...
Функция ПолучитьПредставлениеДокументаДляПечати(Выборка)
	...
	Если ЗначениеЗаполнено(Выборка.Магазин) Тогда
		ПредставлениеДокумента = ПредставлениеДокумента + " (" + Выборка.Магазин + ")";
	КонецЕсли;
	//---АК SHEP 20161028
...
//
&НаСервере
Процедура ВывестиПодчиненныеЭлементыДерева(СтрокиДерева,Макет,УровеньРекурсии,ТЗЗадания=Неопределено)
	...
	Для каждого СтрокаДерева Из СтрокиДерева Цикл
		...
		ВывестиДокументИКартинку(СтрокаДерева,Макет,Ложь,Истина,ТЗЗадания);
		
		//Вывод подчиненных элементов дерева
		ВывестиПодчиненныеЭлементыДерева(ПодчиненныеЭлементыДерева,Макет,УровеньРекурсии + 1,ТЗЗадания);
...
Процедура ОбновитьДеревоСтруктурыПодчиненности()
	...
	Если ОсновнойДокументДоступен() Тогда
		//+++АК BELN 2018.04.06 ИП-00018296
		Если ТипЗнч(ДокументСсылка)=Тип("ДокументСсылка.РасходныйОрдерСклад") Тогда
			Запрос = Новый Запрос;
			Запрос.Текст = 
				"ВЫБРАТЬ
				...
				|		ПО РасходныйОрдерСкладТовары.ЗаданиеНаРазборку = втЗадания.ЗаданиеНаРазборку
				|
				|СГРУППИРОВАТЬ ПО
				|	РасходныйОрдерСкладТовары.Ссылка
				|;
				...
				|ИЗ
				|	Документ.РасходныйОрдерСклад.Товары КАК РасходныйОрдерСкладТовары
				|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втЗадания1 КАК втЗадания
				|		ПО РасходныйОрдерСкладТовары.ЗаданиеНаРазборку = втЗадания.ЗаданиеНаРазборку
				|
				|СГРУППИРОВАТЬ ПО
				|	РасходныйОрдерСкладТовары.Ссылка
				...
				|	РасходныйОрдерСкладТовары.КоличествоТТ,
				|	РасходныйОрдерСкладТовары.Отклонение,
				|	РасходныйОрдерСкладТовары.ЗаявкаНаСклад,
				|	РасходныйОрдерСкладТовары.НомерРаспределения,
				|	РасходныйОрдерСкладТовары.ДатаПроизводства,
				|	РасходныйОрдерСкладТовары.ЗаявкаНаПроизводствоПерсональнойУпаковки,
				|	РасходныйОрдерСкладТовары.СтандартнаяЭтикетка,
				|	РасходныйОрдерСкладТовары.ЗаданиеНаРазборку,
				|	РасходныйОрдерСкладТовары.Пол,
				|	РасходныйОрдерСкладТовары.Ссылка.Дата,
				|	РасходныйОрдерСкладТовары.Ссылка.Проведен,
				|	РасходныйОрдерСкладТовары.Ссылка.ПометкаУдаления,
				|	ПРЕДСТАВЛЕНИЕ(РасходныйОрдерСкладТовары.Ссылка.Ссылка) КАК Представление
				|ИЗ
				|	Документ.РасходныйОрдерСклад.Товары КАК РасходныйОрдерСкладТовары
				|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ вт1 КАК вт
				|		ПО РасходныйОрдерСкладТовары.Ссылка = вт.Ссылка
				|ГДЕ
				|	РасходныйОрдерСкладТовары.ЗаданиеНаРазборку <> ЗНАЧЕНИЕ(Документ.ЗаданиеНаРазборку.ПустаяСсылка)
				|
				|СГРУППИРОВАТЬ ПО
				|	РасходныйОрдерСкладТовары.ЗаданиеНаРазборку.Закрыто,
				|	РасходныйОрдерСкладТовары.Ссылка,
				...
				|	РасходныйОрдерСкладТовары.Ссылка.Проведен,
				|	РасходныйОрдерСкладТовары.Ссылка.ПометкаУдаления,
				|	ПРЕДСТАВЛЕНИЕ(РасходныйОрдерСкладТовары.Ссылка.Ссылка)";
			Запрос.УстановитьПараметр("Ссылка", ДокументСсылка);
			
			РезультатЗапроса = Запрос.Выполнить();
			
			ТЗЗадания = РезультатЗапроса.Выгрузить();
			ТЗЗадания.Индексы.Добавить("Ссылка");
		ИначеЕсли ТипЗнч(ДокументСсылка)=Тип("ДокументСсылка.ЗаданиеНаРазборку") Тогда
			Запрос = Новый Запрос;
			Запрос.Текст = 
				"ВЫБРАТЬ
				|	РасходныйОрдерСкладТовары.ЗаданиеНаРазборку
				...
				|	Документ.РасходныйОрдерСклад.Товары КАК РасходныйОрдерСкладТовары
				|ГДЕ
				|	РасходныйОрдерСкладТовары.ЗаданиеНаРазборку = &Ссылка
				|	И РасходныйОрдерСкладТовары.ЗаданиеНаРазборку <> ЗНАЧЕНИЕ(Документ.ЗаданиеНаРазборку.ПустаяСсылка)
				|
				|СГРУППИРОВАТЬ ПО
				|	РасходныйОрдерСкладТовары.ЗаданиеНаРазборку
		...
		Если НЕ ТЗЗадания=Неопределено Тогда
			Запрос = Новый Запрос;
			Запрос.Текст = 
				"ВЫБРАТЬ
				...
				|	ЗаданиеНаРазборкуТовары.Ссылка.Дата,
				|	ЗаданиеНаРазборкуТовары.Ссылка.ПометкаУдаления,
				|	ЗаданиеНаРазборкуТовары.Ссылка.Проведен,
				|	ПРЕДСТАВЛЕНИЕ(ЗаданиеНаРазборкуТовары.Ссылка.Ссылка) КАК Представление
				|ИЗ
				|	Документ.ЗаданиеНаРазборку.Товары КАК ЗаданиеНаРазборкуТовары
				|ГДЕ
				|	ЗаданиеНаРазборкуТовары.Ссылка В(&Ссылка)";
			
			Запрос.УстановитьПараметр("Ссылка", ТЗЗадания.ВыгрузитьКолонку("ЗаданиеНаРазборку"));
			
			РезультатЗапроса = Запрос.Выполнить();
			
			ТЧЗадания = РезультатЗапроса.Выгрузить();
		Иначе	
			ТЧЗадания=Неопределено;
		КонецЕсли; 
		...
		СформироватьДеревьяДокументов(ТЗЗадания,ТЧЗадания);
		ВывестиТабличныйДокумент(ТЗЗадания);
	Иначе
...
&НаСервере
Процедура СформироватьДеревьяДокументов(ТЗЗадания=Неопределено,ТЧЗадания=Неопределено)
	...
	ВывестиРодительскиеДокументы(ДокументСсылка,ДеревоРодительскиеДокументы,ТЗЗадания,ТЧЗадания);
	ВывестиПодчиненныеДокументы(ДокументСсылка,ДеревоПодчиненныеДокументы,ТЗЗадания);
...
Функция ПолучитьВыборкуПоРеквизитамДокумента(ДокументСсылка)
	...
	ЗаменитьТекстЗапроса(ТекстЗапроса, МетаданныеДокумента, "#Магазин", "Магазин");
...
&НаСервере
Процедура ВывестиРодительскиеДокументы(ТекущийДокумент,ДеревоРодитель,ТЗЗадания=Неопределено,ТЧЗадания=Неопределено,Ит=0)
	Если Ит=2 Тогда
		Возврат;	
	...
	Если НЕ ТипЗнч(ТекущийДокумент)=Тип("ДокументСсылка.РасходныйОрдерСклад") И НЕ ТипЗнч(ТекущийДокумент)=Тип("ДокументСсылка.ЗаданиеНаРазборку") Тогда
	...
	Для Каждого ТЧ Из МетаданныеДокумента.ТабличныеЧасти Цикл
		Если НЕ (НЕ ТипЗнч(ТекущийДокумент)=Тип("ДокументСсылка.РасходныйОрдерСклад") И НЕ ТипЗнч(ТекущийДокумент)=Тип("ДокументСсылка.ЗаданиеНаРазборку")) Тогда
			Если НЕ (ТЧ.Имя="Товары" И ТипЗнч(ТекущийДокумент)=Тип("ДокументСсылка.РасходныйОрдерСклад") 
				ИЛИ ТЧ.Имя="Товары" И ТипЗнч(ТекущийДокумент)=Тип("ДокументСсылка.ЗаданиеНаРазборку")) 				
				Тогда
				Продолжить;
			КонецЕсли; 
		КонецЕсли; 
		...
		Если ТипЗнч(ТекущийДокумент)=Тип("ДокументСсылка.РасходныйОрдерСклад") И ТЗЗадания<>Неопределено И ТЧЗадания<>Неопределено  Тогда
			СодержимоеТЧВсе =ТЗЗадания.НайтиСтроки(Новый Структура("Ссылка",ТекущийДокумент));
			Для каждого Эл Из СодержимоеТЧВсе Цикл
				Если ЗначениеЗаполнено(Эл.ЗаданиеНаРазборку) Тогда
					Если СписокРеквизитов.НайтиПоЗначению(Эл.ЗаданиеНаРазборку) = Неопределено Тогда

						СписокРеквизитов.Добавить(Эл.ЗаданиеНаРазборку,Формат(Эл.Дата,"ДФ=yyyyMMddЧЧММсс"));

					КонецЕсли;
				КонецЕсли; 
			КонецЦикла; 
		ИначеЕсли ТипЗнч(ТекущийДокумент)=Тип("ДокументСсылка.ЗаданиеНаРазборку") И ТЗЗадания<>Неопределено И ТЧЗадания<>Неопределено   Тогда
	
			СодержимоеТЧВсе =ТЧЗадания.НайтиСтроки(Новый Структура("Ссылка",ТекущийДокумент));
			Для каждого Эл Из СодержимоеТЧВсе Цикл
				Если ЗначениеЗаполнено(Эл.РасходныйОрдер) Тогда
					Если СписокРеквизитов.НайтиПоЗначению(Эл.РасходныйОрдер) = Неопределено Тогда

						СписокРеквизитов.Добавить(Эл.РасходныйОрдер,Формат(Эл.Дата,"ДФ=yyyyMMddЧЧММсс"));
	...
	Для каждого ЭлементСписка Из СписокРеквизитов Цикл
		...
		Если ТипЗнч(ЭлементСписка.Значение)=Тип("ДокументСсылка.РасходныйОрдерСклад")  И ТЗЗадания<>Неопределено И ТЧЗадания<>Неопределено  Тогда
			СодержимоеТЧВсе =ТЗЗадания.НайтиСтроки(Новый Структура("Ссылка",ЭлементСписка.Значение));
		
			Если СодержимоеТЧВсе.Количество() Тогда
				СтрокаДерева = ДобавитьСтрокуВДерево(СтрокиДерева, СодержимоеТЧВсе[0]);
				Если НЕ ДобавляемыйДокументИмеетсяСредиРодителей(ДеревоРодитель,ЭлементСписка.Значение) Тогда
					ВывестиРодительскиеДокументы(ЭлементСписка.Значение,СтрокаДерева,ТЗЗадания,ТЧЗадания,Ит+1);	
				КонецЕсли;
				
			
			КонецЕсли; 
		ИначеЕсли ТипЗнч(ЭлементСписка.Значение)=Тип("ДокументСсылка.ЗаданиеНаРазборку")  И ТЗЗадания<>Неопределено И ТЧЗадания<>Неопределено  Тогда
	
			СодержимоеТЧВсе =ТЧЗадания.НайтиСтроки(Новый Структура("Ссылка",ЭлементСписка.Значение));
			Если СодержимоеТЧВсе.Количество() Тогда
				СтрокаДерева = ДобавитьСтрокуВДерево(СтрокиДерева, СодержимоеТЧВсе[0]);
				Если НЕ ДобавляемыйДокументИмеетсяСредиРодителей(ДеревоРодитель,ЭлементСписка.Значение) Тогда
					ВывестиРодительскиеДокументы(ЭлементСписка.Значение,СтрокаДерева,ТЗЗадания,ТЧЗадания,Ит+1);	
...
Процедура ДополнитьКэшМетаданных(МетаданныеДокумента, ИмяДокумента)
	...
	Если РеквизитыДокумента = Неопределено Тогда
		...
		РеквизитыДокумента.Вставить("Магазин",         МетаданныеДокумента.Реквизиты.Найти("Магазин") <> Неопределено);
...
&НаСервере
Процедура ВывестиПодчиненныеДокументы(ТекущийДокумент,ДеревоРодитель,ТЗЗадания=Неопределено)
	...
	Для Каждого КлючИЗначение Из КэшПоТипамДокументов Цикл
		...
		|" + ?(мКэшРеквизитовДокумента[КлючИЗначение.Ключ]["Магазин"],        "Магазин", "NULL") + "        КАК Магазин,
		|	Представление
		|ИЗ
