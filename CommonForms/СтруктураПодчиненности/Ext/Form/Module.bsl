...
Процедура ОбновитьДеревоСтруктурыПодчиненности()
	...
	Если ОсновнойДокументДоступен() Тогда
		...
		СформироватьДеревьяДокументов(ТЗЗадания);
...
&НаСервере
Процедура СформироватьДеревьяДокументов(ТЗЗадания=Неопределено)
	...
	ВывестиРодительскиеДокументы(ДокументСсылка,ДеревоРодительскиеДокументы,ТЗЗадания);
...
&НаСервере
Процедура ВывестиРодительскиеДокументы(ТекущийДокумент,ДеревоРодитель,ТЗЗадания=Неопределено)
	...
	Для Каждого ТЧ Из МетаданныеДокумента.ТабличныеЧасти Цикл
		СтрРеквизитов = "";
		//+++АК BELN 2018.06.18 
		Если ТипЗнч(ТекущийДокумент)=Тип("ДокументСсылка.РасходныйОрдерСклад") И ТЧ.Имя="Товары" И ТЗЗадания<>Неопределено Тогда
			СодержимоеТЧВсе =ТЗЗадания.НайтиСтроки(Новый Структура("Ссылка",ТекущийДокумент));
			Если СодержимоеТЧВсе.Количество() Тогда
				СодержимоеТЧ=ТЗЗадания.СкопироватьКолонки();
				Для каждого Эл Из СодержимоеТЧВсе Цикл
					ЗаполнитьЗначенияСвойств(СодержимоеТЧ.Добавить(),Эл);
				КонецЦикла; 
				СодержимоеТЧ.Колонки.Удалить("Ссылка");
				СодержимоеТЧ.Колонки.Удалить("ЗаданиеНаРазборкуЗакрыто");
			Иначе	
	...
	Для каждого ЭлементСписка Из СписокРеквизитов Цикл
		...
		Если Выборка.Следующий() Тогда
			...
			Если НЕ ДобавляемыйДокументИмеетсяСредиРодителей(ДеревоРодитель,ЭлементСписка.Значение) Тогда
				ВывестиРодительскиеДокументы(ЭлементСписка.Значение,СтрокаДерева,ТЗЗадания);	
			КонецЕсли;
		КонецЕсли;
...
Функция ДобавитьСтрокуВДерево(СтрокиДерева, Выборка)
	...
	ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка, "Ссылка, Представление, СуммаДокумента, Валюта, Магазин, Проведен, ПометкаУдаления");
