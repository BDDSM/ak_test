...
Процедура СформироватьДвиженияПоРегиструНДСПредъявленный(СтруктураШапкиДокумента, ТаблицаПредъявленногоНДС, ИдТабЧасти, ТаблицаАвансов = Неопределено, Движения, Отказ) Экспорт
	...
	Для Каждого СтрокаДок из ТаблицаПредъявленногоНДС Цикл
		...
		Если (ТипЗнч(СтрокаДок.СчетУчетаЦенности) = Тип("ПланСчетовСсылка.Финансовый")) и СтрокаДок.СчетУчетаЦенности.Забалансовый Тогда
...
Процедура СформироватьДвиженияПоРегиструНДСНачисленный_ПоступлениеАгентскогоНДС(СтруктураШапкиДокумента,ДвиженияНДСПредъявленный,Движения) Экспорт
	...
	СчетНДС = ПланыСчетов.Финансовый.НДС;
...
Процедура СформироватьДвиженияПоРегиструНДСНачисленный_ОтражениеРеализации(СтруктураШапкиДокумента, ТабЧасть, Движения, Отказ, СчетОтнесенияНДС, ТаблицаОплат = Неопределено, ЭтоВозврат = ложь) Экспорт
	...
	Если ЕстьСчетУчета Тогда
		...
		|ГДЕ
		|	Финансовый.Забалансовый = ИСТИНА";
...
// этих товаров
Процедура ОтразитьВключениеИсключениеНДСВСтоимость(СтруктураШапкиДокумента, ТаблицаСписания, ТаблицаДвиженийПартии, ВключитьВСтоимость, НаборДвиженийФинансовый, Заголовок, Движения, ОтражатьПоПартиям = Истина) Экспорт
	...
	Если ВключитьВСтоимость Тогда
		...
		Для каждого СтрокаТаблицыВключения Из ДанныеДляОбработки Цикл
			...
			СформироватьПроводкиПоВключениюИсключениюНДСИзСтоимости(СтруктураШапкиДокумента,ВключитьВСтоимость, СтрокаТаблицыВключения,КолонкиТаблицыСписания, НаборДвиженийФинансовый, Заголовок);
...
Процедура СформироватьПроводкиПоВключениюИсключениюНДСИзСтоимости(СтруктураШапкиДокумента,ВключитьВСтоимость = Истина, СтрокаСписания, КолонкиСтрокиСписания, НаборДвиженийФинансовый, Заголовок, ЭтоВозврат = Ложь)
	...
	Иначе
		...
		Если (не ВключитьВСтоимость = ЭтоВозврат) Тогда
			...
			Если не СуммаКВключениюВСтоимость = 0 Тогда
				...
				СтрокаСписанияПроводка = НаборДвиженийФинансовый.Добавить();
...
Процедура ОтразитьПредположениеСтавки0(СтруктураШапкиДокумента, ТаблицаСписания, ТаблицаДвиженийПартии, НаборДвиженийФинансовый,Заголовок, Движения) Экспорт
	...
	Для каждого СтрокаСписания Из ДанныеДляОбработки Цикл
		СформироватьПроводкиПоПредположениюСтавки0(СтруктураШапкиДокумента,СтрокаСписания, НаборДвиженийФинансовый,Заголовок);
...
Процедура СформироватьПроводкиПоПредположениюСтавки0(СтруктураШапкиДокумента,СтрокаДвижения, НаборДвиженийФинансовый,Заголовок)
	...
	СтрокаДвиженияПроводка = НаборДвиженийФинансовый.Добавить();
	
	// проводка, отражающая включение НДС в стоимость
	СтрокаДвиженияПроводка.СчетДт = ПланыСчетов.Финансовый.НДСПоТоварамРеализованнымПоСтавке0; 
...
Процедура ОтразитьПринятиеКУчетуОС(СтруктураШапкиДокумента, ТаблицаСписания, ТаблицаДвиженийПокупки, ТаблицаДвиженийПартии, НаборДвиженийФинансовый,Заголовок, ПереданНДСНаСтроительство = Ложь) Экспорт
...
Процедура СформироватьДвиженияПоРегиструНДСНачисленныйПредъявленный_ПринятиеКУчету_ХозСпособ(СтруктураШапкиДокумента, Движения, Отказ, СуммаБезНДС, СуммаНДС) Экспорт
	...
	СтрокаДвижений_НДСНачисленный.ДатаСобытия 			= СтруктураШапкиДокумента.Дата;
	СтрокаДвижений_НДСНачисленный.СчетУчетаНДС			= ПланыСчетов.Финансовый.НДСприСтроительствеОсновныхСредств.Ссылка;
	...
	СтрокаДвижений_НДСПредъявленный.СчетУчетаНДС		= Планысчетов.Финансовый.НДСприСтроительствеОсновныхСредств.Ссылка;
...
Процедура КорректировкаРегистровНДСПоСуммовымРазницам_Реализация(СтруктураШапкиДокумента, ИсходнаяТаблицаСуммовыхРазниц, Движения) Экспорт
	...
	Для каждого СтрокаТаблицы Из ТаблицаСуммовыхРазниц Цикл
		Если СтруктураШапкиДокумента.НачислятьНДСПоОтгрузке 
			...
			СтрокаТаблицы.Документ = СтрокаТаблицы.ДокументРасчетов.ДокументОтгрузки;
		КонецЕсли;
	...
	Если СтруктураШапкиДокумента.УчитыватьНДС Тогда
		Для Каждого СтрокаТаблицы Из ТаблицаВыручки Цикл
			...
			Если ОтражатьНДСКомитента Тогда
				...
				Проводка.СчетКт = ПланыСчетов.Финансовый.НДСНалоговогоАгента;
...
Процедура СформироватьДвиженияНалоговыйАгентПоОплате(СтруктураШапкиДокумента, ТаблицаОплаты, Движения) Экспорт
	...
	СчетУчетаРасчетовПоНДС = ПланыСчетов.Финансовый.РасчетыНДСНалоговогоАгента;
	
	СчетНДС = ПланыСчетов.Финансовый.НДСНалоговогоАгента;
	...
	Для Каждого СтрокаТаблицы Из ТаблицаОплаты Цикл
		...
		Иначе
			...
			НовоеДвижение.ДоговорКонтрагента = СтруктураШапкиДокумента.ДоговорКонтрагента;
			НовоеДвижение.ВидЦенности = СтрокаТаблицы.ВидЦенности;
			НовоеДвижение.Событие = Перечисления.СобытияПоНДСПродажи.ПолученАванс;
...
Процедура СформироватьДвиженияОплатаАгентскогоНДСВБюджет(СтруктураШапкиДокумента, Движения) Экспорт
	...
	Если СтруктураШапкиДокумента.СчетУчетаРасчетовСКонтрагентом <> ПланыСчетов.Финансовый.НДСНалоговогоАгента Тогда
		Возврат;
	КонецЕсли;
...
Процедура СформироватьПроводкиПоРазницамНалоговыйАгент(СтруктураШапкиДокумента, ТаблицаОплат, Движения)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаОплат Цикл
		...
		Проводка.СчетКт = ПланыСчетов.Финансовый.РасчетыНДСНалоговогоАгента;
...
Процедура УпрощенныйНДС_СформироватьДвиженияПоНДСНачисленному(СтруктураШапкиДокумента, ТаблицаНачисленногоНДС, Движения, Отказ, СторноНачисления = Ложь) Экспорт
	...
	Для Каждого СтрокаТаблицы ИЗ ТаблицаНачисленногоНДС Цикл
		...
		Если ЕстьСчетУчета 
			...
			И СтрокаТаблицы.СчетУчета.Забалансовый 
