...
Функция ИДКонфигурации() Экспорт
	...
	ИначеЕсли МетаданныеИмя = "БухгалтерияБюджетногоУчреждения" ИЛИ МетаданныеИмя = "БухгалтерияБюджетногоУчрежденияБазовая" Тогда
		Возврат "ББУ";
	ИначеЕсли МетаданныеИмя = "БухгалтерияГосударственногоУчреждения" Тогда
		Возврат "БГУ";
...
Функция роЗаполнитьСведенияОбОрганизации(Форма, Знач СписокСведений, Дополнения = "", РазделДляВывода = "Титульный") Экспорт
	...
	Если (мТитульный.Области.Найти("ИННУп1") <> Неопределено) Или
		...
		ДанныеПредставителя = ПолучитьСведенияОПредставителе(Форма, Ложь, КодНО);
...
Процедура роПроставитьСведенияОПредставителеОрганизации(Форма, ФлагТолькоФЛ, СхемаПолученияКодИФНС = "0") Экспорт
	...
	СтруктураПредставителя = ПолучитьСведенияОПредставителеОрганизации(Форма.Организация, КодИФНС, Форма.ДатаПодписи);
...
КонецПроцедуры // роПроставитьСведенияОПредставителеОрганизации
...
//
Функция ПолучитьСведенияОПредставителеОрганизации(Организация, КодИФНС, ДатаПодписиОтчета)
...
//
Функция ПолучитьСведенияОПредставителе(Форма, Знач МожетБытьТолькоЮРЛицо = Неопределено, Знач ИФНС = Неопределено) Экспорт
...
Функция ПриЗаписиРегламентированногоОтчета(Форма, КодИФНС = "не применимо", Автосохранение = Ложь, Вариант = Неопределено, Уникальность = Истина) Экспорт
...
Процедура УправлениеВидимостьюЭлементовОтправкиРегламентированногоОтчета(Форма)
	...
	УчетнаяЗаписьОбмена = Неопределено; //ОрганизацияОтчета.УчетнаяЗаписьОбмена;
	...
	Если НЕ ДоступКУчетнойЗаписиЕсть Тогда
...
Функция ПолучитьРезультатОтправкиФСС(ИдентификаторОтправкиНаСервере) Экспорт
	...
	ПолученПротокол = Ложь;
	ПолученаКвитанция = Ложь;
	...
	Пока ПовторятьСоединение Цикл
		...
		РесурсСервера = "get_ack.php?id=" + ИдентификаторОтправкиНаСервере; //квитанция
		ФайлРезультата = ПолучитьРезультатЗапросаGet(АдресСервера, РесурсСервера);
		
		Если ФайлРезультата <> Неопределено Тогда
			// пришла квитанция - отчет сдан
			ПолученаКвитанция = Истина;
			// считываем содержимое квитанции
		...
		// запрос протокола
		АдресСервера = "f4.fss.ru";
		РесурсСервера = "service3.php?id=" + ИдентификаторОтправкиНаСервере; //протокол
		ФайлРезультата = ПолучитьРезультатЗапросаGet(АдресСервера, РесурсСервера);
			
		Если ФайлРезультата <> Неопределено Тогда
			// пришел протокол обработки
			ПолученПротокол = Истина;
			// считываем содержимое протокола
			ОбъектЧтение = Новый ЧтениеТекста(ФайлРезультата);
			Протокол = СокрЛП(ОбъектЧтение.Прочитать());
			ОбъектЧтение.Закрыть();
			
			Если НЕ ПолученаКвитанция Тогда
				// анализируем содержимое протокола
				Если Найти(Протокол, "Отчет не принят")  Тогда
					// отчет не принят
