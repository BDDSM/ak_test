...
Функция ПолучитьХарактеристикуСпособаОтражения(СпособОтражения) Экспорт
	...
	"ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА СпособыОтражения.СчетДт = ЗНАЧЕНИЕ(ПланСчетов.Финансовый.ПустаяСсылка)
	|			ТОГДА ЛОЖЬ
	|		КОГДА СпособыОтражения.СчетКт = ЗНАЧЕНИЕ(ПланСчетов.Финансовый.ПустаяСсылка)
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
...
Процедура НазначитьВидимостьСубконто(ОформлениеСтроки, ДтКт, СведенияОСчете) Экспорт
	...
	Для СчетчикСубконто = 1 По 3 Цикл
		ПоказыватьСубконто = Не (СведенияОСчете.УчетПоПодразделениям и СведенияОСчете["ВидСубконто"+СчетчикСубконто] = ПланыВидовХарактеристик.ВидыСубконто.Подразделения);
