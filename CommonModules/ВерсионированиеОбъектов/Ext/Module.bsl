...
Процедура МеханизмВерсионированияОбъектов_ПриЗаписиОбъекта(Источник, Отказ) Экспорт
	...
	Если ПараметрыСеанса.НеСохранятьВерсииОбъектов Тогда
		Возврат;
	КонецЕсли;	
	
	Если ОбъектВерсионируется(Источник, ЧислоВерсийОбъекта) Тогда
		
		// +++ golv ИП-00018747 22.05.2018		
		...
		ЗаписьXML.Закрыть();
		ХранилищеДанных = Новый ХранилищеЗначения(Поток.ЗакрытьИПолучитьДвоичныеДанные(), Новый СжатиеДанных(9));		
