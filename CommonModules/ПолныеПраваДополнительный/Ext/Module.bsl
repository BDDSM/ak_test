...
Функция ПолучитьСписокПользователейДляАкцептования(ФормаОплаты)Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	               |	ПользователиДляУведомленийПоЗаявкам.Пользователь
	               |ИЗ
	               |	РегистрСведений.ПользователиДляУведомленийПоЗаявкам КАК ПользователиДляУведомленийПоЗаявкам
	               |ГДЕ
	               |	ПользователиДляУведомленийПоЗаявкам.ФормаОплаты = &ФормаОплаты";
...
Функция ПолучитьСписокПользователейДляУведомленияОПлатеже()Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	               ...
	               |	РегистрСведений.ПользователиДляУведомленийПоЗаявкам КАК ПользователиДляУведомленийПоЗаявкам
	               |ГДЕ
	               |	ПользователиДляУведомленийПоЗаявкам.УведомлятьОПлатежеКонтрагенту";
	
	Возврат Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Пользователь");
	
КонецФункции

Функция ПолучитьСписокТехнологовДляУведомленияОПлатеже(Контрагент) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ИЗ
	               |	РегистрСведений.СоответствиеОбъектРоль.СрезПоследних(&ТекДата, ТипРоли = ЗНАЧЕНИЕ(ПланВидовХарактеристик.ТипыРолейПользователя.ТехнологПоКачеству)) КАК СоответствиеОбъектРольСрезПоследних
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.РолиПользователей.СоставРоли КАК РолиПользователейСоставРоли
	               |		ПО СоответствиеОбъектРольСрезПоследних.РольПользователя = РолиПользователейСоставРоли.Ссылка
	               |ГДЕ
	               |	СоответствиеОбъектРольСрезПоследних.Объект = &Объект";
