...
Функция ПолучитьДанныеШтрихкодаОптимиз(ДанныеМобильногоПриложения) Экспорт
	Если ТипЗнч(ДанныеМобильногоПриложения)=Тип("Строка") Тогда
		ШтрихКод=ДанныеМобильногоПриложения;
	ИначеЕсли ДанныеМобильногоПриложения.Свойство("Штрихкод") Тогда
	...
	Если  ПустаяСтрока(ШтрихКод) Тогда
		...
		Для каждого Эл Из МассивБукв Цикл
			...
			//+++АК BELN 2018.12.11 ЗА-00019590 
			СтрОтб2=СтрОтб2+" ИЛИ ШтрихКодыУпаковокПоставщиков.Характеристика.Владелец.Наименование ПОДОБНО """+Эл+"""";
			//---АК BELN  
		КонецЦикла;
		СтрОтб="("+СтрОтб+")";
		СтрОтб1="("+СтрОтб1+")";
		...
		Запрос.Текст = 
			"ВЫБРАТЬ
			|	ШтрихКодыУпаковокПоставщиков.ШК,
			...
			|ГДЕ
			|	ШтрихКодыУпаковокПоставщиков.Характеристика.Владелец.ПометкаУдаления = ЛОЖЬ
			|	И ШтрихКодыУпаковокПоставщиков.Характеристика.ПометкаУдаления = ЛОЖЬ
			|	И "+СтрОтб2;
		РезультатЗапроса = Запрос.Выполнить();
		
		НаборЗаписейШКПоставщиков = РезультатЗапроса.Выгрузить();
		//---АК BELN 2018.12.11 
	...
	Если НЕ ПустаяСтрока(ШтрихКод) Тогда
		...
		НаборЗаписейНумератор.Прочитать();
		//+++АК BELN 2018.12.11 ЗА-00019590
		НаборЗаписейШКПоставщиков= РегистрыСведений.ШтрихКодыУпаковокПоставщиков.СоздатьНаборЗаписей();
		НаборЗаписейШКПоставщиков.Отбор.Характеристика.Установить(СтруктураДанных.Характеристика);
		НаборЗаписейШКПоставщиков.Прочитать();
		//---АК BELN
	КонецЕсли; 
	//
	ЗаписьXML = Новый ЗаписьXML;
	...
	КонецЦикла;  
	//+++АК BELN 2018.12.11 ЗА-00019590
	Для Каждого Запись Из НаборЗаписейШКПоставщиков Цикл
		НЗ = РегистрыСведений.ШтрихКодыУпаковокПоставщиков.СоздатьНаборЗаписей();
		НоваяЗапись = НЗ.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяЗапись, Запись);
		ЗаписатьДанные(ВозвращаемыйСписок, НЗ,,КэшНом);
	КонецЦикла;  
