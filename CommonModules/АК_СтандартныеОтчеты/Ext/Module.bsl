...
Функция ВыбратьСчет(Тип = "Группировка", НачальноеЗначениеВыбора = Неопределено) Экспорт
	...
	ФормаВыбораСчета = ПланыСчетов.Финансовый.ПолучитьФормуВыбора();
	...
	Если Найти(Тип, "Группировка") = 1 Тогда
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ПланСчетов.Финансовый.ВидыСубконто КАК ФинансовыйВидыСубконто
		|		ПО (ФинансовыйВидыСубконто.Ссылка = Финансовый.Ссылка)
		|ГДЕ
		|	(НЕ Финансовый.ЗапретитьИспользоватьВПроводках)
		|
		|СГРУППИРОВАТЬ ПО
		|	Финансовый.Ссылка
		|
		|ИМЕЮЩИЕ
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ФинансовыйВидыСубконто.ВидСубконто) = 0";
...
Функция ПолучитьСтруктуруПоказателейОтчета(Упр = Истина, Корр = Истина, МСФО = Истина, ВалютнаяСумма = Ложь, Количество = Ложь, РазвернутоеСальдо = Ложь) Экспорт
	
	Возврат Новый Структура("Упр, Корр, МСФО, ВалютнаяСумма, Количество, РазвернутоеСальдо", 
							Новый Структура("Значение, Использование, Представление", Истина, Упр,               "Упр"),
							Новый Структура("Значение, Использование, Представление", Ложь,   Корр,              "Корр"),
							Новый Структура("Значение, Использование, Представление", Ложь,   МСФО,              "МСФО (Упр + Корр)"),
...
Процедура ДобавитьОтборДляПоказателяМСФО(ОтчетОбъект) Экспорт
	
	МассивПоказателей = Новый Массив;
	МассивПоказателей.Добавить("Упр");
	МассивПоказателей.Добавить("Корр");
	Если ОтчетОбъект.ДанныеОтчета.ПоказателиОтчета.МСФО.Значение Тогда 
		...
		ГруппаОтборИЛИ.Представление = "###МСФО###";
		ГруппаОтборИЛИ.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли;
		ТиповыеОтчеты.ДобавитьОтбор(ГруппаОтборИЛИ, "СальдоНаНачалоПериода.МСФОНачальныйОстатокДт", 0, ВидСравненияКомпоновкиДанных.НеРавно);
		ТиповыеОтчеты.ДобавитьОтбор(ГруппаОтборИЛИ, "СальдоНаНачалоПериода.МСФОНачальныйОстатокКт", 0, ВидСравненияКомпоновкиДанных.НеРавно);
		ТиповыеОтчеты.ДобавитьОтбор(ГруппаОтборИЛИ, "ОборотыЗаПериод.МСФООборотДт", 0, ВидСравненияКомпоновкиДанных.НеРавно);
		ТиповыеОтчеты.ДобавитьОтбор(ГруппаОтборИЛИ, "ОборотыЗаПериод.МСФООборотКт", 0, ВидСравненияКомпоновкиДанных.НеРавно);
		ТиповыеОтчеты.ДобавитьОтбор(ГруппаОтборИЛИ, "СальдоНаКонецПериода.МСФОКонечныйОстатокДт", 0, ВидСравненияКомпоновкиДанных.НеРавно);
		ТиповыеОтчеты.ДобавитьОтбор(ГруппаОтборИЛИ, "СальдоНаКонецПериода.МСФОКонечныйОстатокКт", 0, ВидСравненияКомпоновкиДанных.НеРавно);
...
Процедура СохранитьНастройкиФормы(ФормаОтчета) Экспорт
	...
	Если ЭлементыФормы.Найти("ФлагВыводитьЕдиницуИзмерения") <> Неопределено Тогда
		НастройкиФормы.Вставить("ВыводитьЕдиницуИзмерения", ЭлементыФормы.ФлагВыводитьЕдиницуИзмерения.Значение);
	КонецЕсли;
...
Процедура ВосстановитьНастройкиФормы(ФормаОтчета, ВсеНастройки = Истина) Экспорт
	...
	Если ВсеНастройки Тогда
		...
		Если ЭлементыФормы.Найти("ФлагВыводитьЕдиницуИзмерения") <> Неопределено Тогда
			Если НастройкиФормы.Свойство("ВыводитьЕдиницуИзмерения") Тогда
				ВыводитьЕдиницуИзмерения = НастройкиФормы.ВыводитьЕдиницуИзмерения;
			Иначе
				НастройкиФормы.Вставить("ВыводитьЕдиницуИзмерения", Ложь);
				ВыводитьЕдиницуИзмерения = Ложь;
			КонецЕсли;
			ЭлементыФормы.ФлагВыводитьЕдиницуИзмерения.Значение = ВыводитьЕдиницуИзмерения;
		КонецЕсли;
...
Процедура ОтборНачалоВыбора(ОтчетОбъект, ФормаОтчета, Элемент, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли ТипЗнч(Элемент.Значение) = Тип("СправочникСсылка.ЗначенияСвойствОбъектов") Тогда
		...
		Если ТипПолеРодитель <> Неопределено Тогда
			...
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли ТипЗнч(Элемент.Значение) = Тип("СправочникСсылка.Субконто") Тогда
		Поле = ФормаОтчета.ЭлементыФормы.Отбор.ТекущиеДанные.ЛевоеЗначение;
		НаименованиеСубконто = ПолучитьСвойствоПоля(ОтчетОбъект.КомпоновщикНастроек, Поле, "Заголовок");
		Запрос = Новый Запрос;
		Запрос.Текст =
		"ВЫБРАТЬ
		|	ВидыСубконто.Ссылка,
		...
		|ГДЕ
		|	ВидыСубконто.Наименование = &Наименование";
		Запрос.УстановитьПараметр("Наименование", НаименованиеСубконто);
		Выборка = Запрос.Выполнить().Выбрать();
		Пока Выборка.Следующий() Цикл
			Если Выборка.ТипЗначения.СодержитТип(Тип("СправочникСсылка.Субконто")) Тогда
				Элемент.ВыборПоВладельцу = Выборка.Ссылка;
				Прервать;
			КонецЕсли;
		КонецЦикла;
	Иначе 
			...
			Для Каждого СтрокаОтбора Из ОтчетОбъект.КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
				Если ТипЗнч(СтрокаОтбора) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
					Если ТипЗнч(СтрокаОтбора.ПравоеЗначение) <> Тип("ПолеКомпоновкиДанных") Тогда
						...
						Если ТипЗначенияПоля = ПланыВидовХарактеристик.ВидыСубконто.Материалы.ТипЗначения Тогда
							СписокПараметров.Вставить("Номенклатура", ЗначениеОтбора);
						ИначеЕсли ТипЗначенияПоля = ПланыВидовХарактеристик.ВидыСубконто.Склады.ТипЗначения Тогда
							СписокПараметров.Вставить("Склад", ЗначениеОтбора);
						ИначеЕсли ТипЗначенияПоля = ПланыВидовХарактеристик.ВидыСубконто.Контрагенты.ТипЗначения Тогда
...
Процедура УсловноеОформлениеПриВыводеСтроки(ОтчетОбъект, Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт 
	...
	Если Текст.Использование Тогда
		Если ТипЗнч(Текст.Значение) = Тип("ПолеКомпоновкиДанных") Тогда
			ПредставлениеПоля = АК_СтандартныеОтчеты.ПолучитьСвойствоПоля(ОтчетОбъект.КомпоновщикНастроек, Текст.Значение);
		Иначе
...
Функция ПолучитьЗапрещенныеПоля(ОтчетОбъект, Режим = "Группировка") Экспорт
	...
	Если ИдентификаторОбъекта = "ОтчетОбъект.АК_ОборотноСальдоваяВедомость" Тогда
		Если Режим = "ДополнительныеПоля" Тогда
			НоваяСтрока = РодителиПоля.Добавить();
		...
		КонецЕсли;		
	ИначеЕсли ИдентификаторОбъекта = "ОтчетОбъект.АК_ОборотноСальдоваяВедомостьПоСчету" Тогда
		...
		ИначеЕсли Режим = "Отбор" ИЛИ Режим = "Сортировка" Тогда
			...
			НоваяСтрока.Поле = "Период";
		КонецЕсли;
	ИначеЕсли ИдентификаторОбъекта = "ОтчетОбъект.АК_ОборотыСчета" Тогда
		Если Режим = "Группировка" Тогда
			Если ЗначениеЗаполнено(ОтчетОбъект.Счет) Тогда 
				КоличествоСубконто = ОтчетОбъект.Счет.ВидыСубконто.Количество();
			Иначе
		...
		ИначеЕсли Режим = "Отбор" ИЛИ Режим = "Сортировка" Тогда
			...
			КонецЦикла;
		КонецЕсли;
	ИначеЕсли ИдентификаторОбъекта = "ОтчетОбъект.АК_АнализСчета" Тогда
		Если Режим = "Группировка" Тогда
			Если ЗначениеЗаполнено(ОтчетОбъект.Счет) Тогда 
				КоличествоСубконто = ОтчетОбъект.Счет.ВидыСубконто.Количество();
			Иначе
		...
		ИначеЕсли Режим = "Отбор" ИЛИ Режим = "Сортировка" Тогда
			...
			КонецЦикла;
		КонецЕсли;
	ИначеЕсли ИдентификаторОбъекта = "ОтчетОбъект.АК_КарточкаСчета" Тогда
		Если Режим = "Отбор" Тогда
			ПоказателиОтчета = ДанныеОтчета.ПоказателиОтчета;
			Если Не ПоказателиОтчета.Упр.Значение Тогда
				НоваяСтрока = РодителиПоля.Добавить();
				НоваяСтрока.Поле = "УпрДт";
				НоваяСтрока = РодителиПоля.Добавить();
				НоваяСтрока.Поле = "УпрКт";
			КонецЕсли;
			Если Не ПоказателиОтчета.МСФО.Значение Тогда
				НоваяСтрока = РодителиПоля.Добавить();
				НоваяСтрока.Поле = "КоррДт";
				НоваяСтрока = РодителиПоля.Добавить();
				НоваяСтрока.Поле = "КоррКт";
			КонецЕсли;
	...
	ИначеЕсли ИдентификаторОбъекта = "ОтчетОбъект.КарточкаСубконто" Тогда
		Если Режим = "Отбор" Тогда
			...
			НоваяСтрока.Поле = "ПериодГруппировки";
		КонецЕсли;
	ИначеЕсли ИдентификаторОбъекта = "ОтчетОбъект.АК_АнализСубконто" Тогда
		Если Режим = "Группировка" Тогда
			Если ЗначениеЗаполнено(ОтчетОбъект.СписокВидовСубконто) Тогда 
				КоличествоСубконто = ОтчетОбъект.СписокВидовСубконто.Количество();
			Иначе
		...
		ИначеЕсли Режим = "Отбор" ИЛИ Режим = "Сортировка" Тогда
			...
			НоваяСтрока.Поле = "Период";
		КонецЕсли;
	ИначеЕсли ИдентификаторОбъекта = "ОтчетОбъект.АК_ОборотыМеждуСубконто" Тогда
		Если Режим = "Группировка" Тогда
			Если ЗначениеЗаполнено(ОтчетОбъект.СписокВидовСубконто) Тогда 
				КоличествоСубконто = ОтчетОбъект.СписокВидовСубконто.Количество();
			Иначе
	...
	ИначеЕсли ИдентификаторОбъекта = "ОтчетОбъект.ОтчетПоПроводкам" Тогда
		Если Режим = "Отбор" Тогда
			...
			Если Не ПоказателиОтчета.Упр.Значение Тогда
				...
				НоваяСтрока.Поле = "УпрКт";
			КонецЕсли;
			Если Не ПоказателиОтчета.Корр.Значение Тогда
				НоваяСтрока = РодителиПоля.Добавить();
				НоваяСтрока.Поле = "КоррДт";
				НоваяСтрока = РодителиПоля.Добавить();
				НоваяСтрока.Поле = "КоррКт";
...
Процедура ДобавитьПоляРесурсовВЗапрещенныеПоля(ОтчетОбъект, ТаблицаПолей)
	...
	Для Каждого Показатель Из ПоказателиОтчета Цикл
		...
		Если Показатель.Значение.Значение Тогда
			Если Показатель.Ключ = "МСФО" Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ДействияФормыДополнительныеВосстановитьЗначения(ОтчетОбъект, ФормаОтчета) Экспорт
	...
	ТекущаяНастройка = ОтчетОбъект.СохраненнаяНастройка;
	...
	ОтчетОбъект.СохраненнаяНастройка = ТекущаяНастройка;
...
Процедура ДействияФормыДополнительныеСохранитьЗначения(ОтчетОбъект, ФормаОтчета) Экспорт
	...
	СохранениеНастроек.ВыбратьНастройкуФормы(ОтчетОбъект.СохраненнаяНастройка, ФормаОтчета, "ОтчетОбъект." + ОтчетОбъект.Метаданные().Имя, Истина);
...
Процедура ЗаполнитьДополнительныеПоляПоУмолчанию(ОтчетОбъект) Экспорт
	...
	//	ТипПоля = АК_СтандартныеОтчеты.ПолучитьСвойствоПоля(КомпоновщикНастроек, Группировка.Поле, "Тип");	
		...
		//	НоваяСтрока.Представление = АК_СтандартныеОтчеты.ПолучитьСвойствоПоля(КомпоновщикНастроек, НоваяСтрока.Поле);
		//	НоваяСтрока = ДанныеОтчета.ДополнительныеПоля.Добавить();
		//	НоваяСтрока.Использование = Ложь;
...
Процедура ВыводЗаголовкаОтчета(ОтчетОбъект, Результат, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ОтчетОбъект.Организация) Тогда
		...
		//Раков П.С. ++
		Если НЕ ЗначениеЗаполнено(ТекстОрганизация) тогда
			ТекстОрганизация =
			?(ЗначениеЗаполнено(ОтчетОбъект.Организация.НаименованиеПолное),ОтчетОбъект.Организация.НаименованиеПолное, 
			?(ЗначениеЗаполнено(ОтчетОбъект.Организация.НаименованиеСокращенное), ОтчетОбъект.Организация.НаименованиеСокращенное, Строка(ОтчетОбъект.Организация))); 
		КонецЕсли;	
		//Раков П.С. --
	...
	Если ИдентификаторОбъекта <>  "ОтчетОбъект.АК_ОборотноСальдоваяВедомостьМСФО" 
		И ИдентификаторОбъекта <> "ОтчетОбъект.АК_КарточкаСчета"
	...
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура")
		И ДополнительныеПараметры.Свойство("ВыводитьОписаниеЕдиницыИзмерения")
		И ДополнительныеПараметры.ВыводитьОписаниеЕдиницыИзмерения Тогда
...
Процедура ВыводПодписейОтчета(ОтчетОбъект, Результат, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ОтчетОбъект.НастройкиФормы.ВыводитьПодписи Тогда
		...
		// Ответственное лицо
		Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура")
			И ДополнительныеПараметры.Свойство("ОтветственноеЛицо")
			И ЗначениеЗаполнено(ДополнительныеПараметры.ОтветственноеЛицо) Тогда
			
			ОбластьПодписи.Параметры.Заполнить(ПолучитьДанныеОтветственногоЛица(ДополнительныеПараметры));		
		КонецЕсли;
...
Функция ПроверитьЗаполнениеОбязательныхРеквизитов(ФормаОтчета) Экспорт
	...
	Если ИдентификаторОбъекта = "ОтчетОбъект.АК_ОборотноСальдоваяВедомостьПоСчету"
		ИЛИ ИдентификаторОбъекта = "ОтчетОбъект.АК_КарточкаСчета" 
		ИЛИ ИдентификаторОбъекта = "ОтчетОбъект.АК_АнализСчета"
		ИЛИ ИдентификаторОбъекта = "ОтчетОбъект.АК_ОборотыСчета" Тогда
	...
	Если ИдентификаторОбъекта = "ОтчетОбъект.АК_ОборотыМеждуСубконто" Тогда
		КоличествоСубконто = 0;
		...
		Если КоличествоСубконто = 0 Тогда
			...
			Если Ответ = КодВозвратаДиалога.Да Тогда
				...
				Если ВвестиЗначение(ФормаОтчета.СписокВидовСубконто[0].Значение, "Выберите вид субконто", Тип("ПланВидовХарактеристикСсылка.ВидыСубконто")) Тогда
					ЗаполнениеКорректно = Истина;
		...
		Если КоличествоКорСубконто = 0 Тогда
			...
			Если Ответ = КодВозвратаДиалога.Да Тогда
				...
				Если ВвестиЗначение(ФормаОтчета.СписокВидовКорСубконто[0].Значение, "Выберите вид субконто", Тип("ПланВидовХарактеристикСсылка.ВидыСубконто")) Тогда
					ЗаполнениеКорректно = Истина;
					ФормаОтчета.ОбработкаИзмененияСоставаСубконто();
					ФормаОтчета.ОбновитьПанельНастроек();
				Иначе
...
Процедура ЗаполнитьДанныеОтчета(ОтчетОбъект) Экспорт
	...
	Если ИдентификаторОбъекта = "ОтчетОбъект.АК_ОборотноСальдоваяВедомостьПоСчету"
		ИЛИ ИдентификаторОбъекта = "ОтчетОбъект.АК_АнализСубконто" 
		ИЛИ ИдентификаторОбъекта = "ОтчетОбъект.АК_ОборотыСчета" 
		ИЛИ ИдентификаторОбъекта = "ОтчетОбъект.АК_ОборотыМеждуСубконто" 
		ИЛИ ИдентификаторОбъекта = "ОтчетОбъект.АК_ОборотноСальдоваяВедомость"
		ИЛИ ИдентификаторОбъекта = "ОтчетОбъект.АК_АнализСчета" Тогда
	...
	Если ИдентификаторОбъекта = "ОтчетОбъект.АК_АнализСчета" Тогда
		Если Не ОтчетОбъект.ДанныеОтчета.Свойство("ГруппировкаКор") Тогда
			...
			МассивТипов = Новый Массив;
			МассивТипов.Добавить(Тип("ПланСчетовСсылка.Финансовый"));
...
Процедура ОбработкаРасшифровкиСтандартногоОтчета(ОтчетОбъект, ФормаОтчета, Расшифровка, СтандартнаяОбработка) Экспорт
	...
	Если ИдентификаторОбъекта = "ОтчетОбъект.АК_КарточкаСчета" 
	...
	Иначе
		...
		ЕстьДиаграмма = Ложь;
		Для Каждого ЭлементСтурктуры Из ФормаОтчета.ДанныеРасшифровки.Настройки.Структура Цикл
			
			Если ТипЗнч(ЭлементСтурктуры) = Тип("ДиаграммаКомпоновкиДанных") Тогда
		...
		Если ПервыйЭлемент <> Неопределено Тогда
			Если ИдентификаторОбъекта = "ОтчетОбъект.АК_ОборотноСальдоваяВедомость" Тогда
				...
				КонецЕсли;
			Иначе
				Если ЕстьПоказатель ИЛИ ЕстьДиаграмма Тогда
					// Если расшифровывается показатель или диаграмма, то необходимо сразу формировать отчет
					Если ИдентификаторОбъекта = "ОтчетОбъект.АК_АнализСубконто" Тогда
					...
					ИначеЕсли ИдентификаторОбъекта = "ОтчетОбъект.АК_ОборотыСчета" Тогда				
						ФормироватьОтчетПоПроводкам = Истина;
					ИначеЕсли ИдентификаторОбъекта = "ОтчетОбъект.АК_СводныеПроводки" Тогда
						ФормироватьОтчетПоПроводкам = Истина;
					КонецЕсли;
				Иначе
					Если ИдентификаторОбъекта = "ОтчетОбъект.АК_СводныеПроводки" Тогда
						ФормироватьОтчетПоПроводкам = Истина;
						УстановитьВсеПоказатели     = Истина;
					Иначе 
						// Создадим и инициализируем обработчик расшифровки
			...
			Если ИдентификаторОбъекта = "ОтчетОбъект.АК_ОборотноСальдоваяВедомость"
				И ФормаРасшифровка.Метаданные().Реквизиты.Найти("ПоСубсчетам") <> Неопределено Тогда
			...
			Для каждого Отбор из МассивПолей Цикл
				Если ТипЗнч(Отбор) = Тип("ЗначениеПоляРасшифровкиКомпоновкиДанных") тогда
					...
					ИначеЕсли Отбор.Поле = "Счет" И (ИдентификаторОбъекта = "ОтчетОбъект.АК_АнализСчета" ИЛИ ИдентификаторОбъекта = "ОтчетОбъект.АК_ОборотыСчета") Тогда
					...
					ИначеЕсли Найти(Отбор.Поле, "Субконто") = 1 И (ИдентификаторОбъекта = "ОтчетОбъект.АК_АнализСчета" ИЛИ ИдентификаторОбъекта = "ОтчетОбъект.АК_ОборотыСчета") Тогда
					...
					ИначеЕсли Отбор.Поле = "Подразделение" И (ИдентификаторОбъекта = "ОтчетОбъект.АК_АнализСчета" ИЛИ ИдентификаторОбъекта = "ОтчетОбъект.АК_ОборотыСчета") Тогда
						ПолеПодразделение = "Подразделение" + БухТипРесурса;
						ТиповыеОтчеты.ДобавитьОтбор(ФормаРасшифровка.КомпоновщикНастроек, ПолеПодразделение, Отбор.Значение, ?(Отбор.Иерархия, ВидСравненияКомпоновкиДанных.ВИерархии, ВидСравненияКомпоновкиДанных.Равно));							
					...
					ИначеЕсли Найти(Отбор.Поле, "Субконто") = 1 тогда
						...
						ИначеЕсли ИдентификаторОбъекта = "ОтчетОбъект.АК_АнализСубконто" И ФормироватьКарточкуСчета Тогда
							ИндексСубконто = Сред(Отбор.Поле, 9, 1); 
							ПозицияСубконто = Найти(Отбор.Поле, "Субконто");
				...
				ИначеЕсли ТипЗнч(Отбор) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных") Тогда
					Если Отбор.Представление = "###ОтборПоОрганизацииСОП###" Тогда
						Для Каждого ЭлементОтбора Из Отбор.Элементы Цикл
							...
							КонецЕсли;
						КонецЦикла;
					ИначеЕсли Отбор.Представление = "###МСФО###" Тогда
					КонецЕсли;
				ИначеЕсли ТипЗнч(Отбор) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
					Если СписокПолейОтборов.Найти(Строка(Отбор.ЛевоеЗначение)) = Неопределено Тогда
						Если Найти(Отбор.ЛевоеЗначение, "Субконто") = 1 И ИдентификаторОбъекта = "ОтчетОбъект.АК_АнализСубконто" И ФормироватьКарточкуСчета Тогда
			...
			Если Период <> Неопределено Тогда
				...
				ФормаРасшифровка.КонецПериода  = АК_СтандартныеОтчеты.КонецПериода(Период, ОтчетОбъект.Периодичность);
			Иначе
			...
			Если УстановитьВсеПоказатели Тогда
				Для Каждого ЭлементСтруктуры Из ФормаРасшифровка.ДанныеОтчета.ПоказателиОтчета Цикл
					Если Не ФормаРасшифровка.ДанныеОтчета.ПоказателиОтчета.МСФО.Использование
						И ОтчетОбъект.ДанныеОтчета.ПоказателиОтчета.МСФО.Значение Тогда 
...
Процедура ОбработкаРасшифровкиОтчетовДляРуководителя(ОтчетОбъект, ФормаОтчета, Расшифровка, СтандартнаяОбработка) Экспорт
	...
	Если ИдентификаторОбъекта = "ОтчетОбъект.ДинамикаЗадолженностиПокупателей"
		...
		Если ИдентификаторОбъекта = "ОтчетОбъект.ДинамикаЗадолженностиПокупателей" Тогда
			...
			|				ТОГДА ИСТИНА
			|			КОГДА ДоговорыКонтрагентов.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.СКомитентомНаЗакупку)
			|				ТОГДА ИСТИНА
			|			ИНАЧЕ ЛОЖЬ
			|		КОНЕЦ
			|
		...
		Иначе
			...
			|				ТОГДА ИСТИНА
			|			КОГДА ДоговорыКонтрагентов.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.СКомитентом)
			|				ТОГДА ИСТИНА
			|			ИНАЧЕ ЛОЖЬ
			|		КОНЕЦ
			|
		...
		|	ФинансовыйОстатки.СуммаРазвернутыйОстатокДтКт КАК Сумма,
		|	""Остаток ДтКт "" + ФинансовыйОстатки.Счет.Код + "" """""" + ФинансовыйОстатки.Счет.Наименование + """""""" КАК СтрокаРасшифровки,
		|	NULL КАК КорСчет,
		|	NULL КАК Субконто1,
		|	NULL КАК Субконто2,
		...
		|ФинансовыйОстатки.Субконто1 КАК Субконто11,
		|ФинансовыйОстатки.Субконто2 КАК Субконто21
		|ИЗ
		...
		|			&ВидыСубконтоКД,
		|			Субконто2 В (ВЫБРАТЬ Ссылка ИЗ ВТ_Договоры)
		|				И Организация = &Организация) КАК ФинансовыйОстатки) КАК ВложенныйЗапрос
		|
		|СГРУППИРОВАТЬ ПО
		|	ВложенныйЗапрос.Знак,
		|	ВложенныйЗапрос.СтрокаРасшифровки,
		...
		ВидыСубконтоКД.Добавить(ПланыВидовХарактеристик.ВидыСубконто.Контрагенты);
		ВидыСубконтоКД.Добавить(ПланыВидовХарактеристик.ВидыСубконто.Договоры);
...
Процедура ПолеРасшифровкаОбработкаРасшифровки(ОтчетОбъект, ФормаОтчета, Расшифровка, СтандартнаяОбработка) Экспорт
	...
	Если КорСчет <> Неопределено И КорСчет <> null Тогда
		ФормаРасшифровка = Отчеты.АК_АнализСчета.ПолучитьФорму();
	...
	Иначе
		ФормаРасшифровка = Отчеты.АК_ОборотноСальдоваяВедомостьПоСчету.ПолучитьФорму();
	КонецЕсли;
	ФормаРасшифровка.РежимРасшифровки = Истина;
	...
	Для Индекс = 1 По Счет.ВидыСубконто.Количество() Цикл
		...
		Если ИдентификаторОбъекта = "ОтчетОбъект.ДинамикаЗадолженностиПокупателей"
			...
			Если Счет.ВидыСубконто[Индекс - 1].ВидСубконто.ТипЗначения.СодержитТип(Тип("СправочникСсылка.ДоговорыКонтрагентов")) Тогда
				...
				СписокОтбора.Добавить(Перечисления.ВидыДоговоровКонтрагентов.СКомитентомНаЗакупку);
		...
		ИначеЕсли ИдентификаторОбъекта = "ОтчетОбъект.ДинамикаЗадолженностиПоставщикам"
			...
			Если Счет.ВидыСубконто[Индекс - 1].ВидСубконто.ТипЗначения.СодержитТип(Тип("СправочникСсылка.ДоговорыКонтрагентов")) Тогда
				...
				СписокОтбора.Добавить(Перечисления.ВидыДоговоровКонтрагентов.СКомиссионеромНаЗакупку);
				ТиповыеОтчеты.ДобавитьОтбор(ФормаРасшифровка.КомпоновщикНастроек, "Субконто" + Индекс + ".ВидДоговора", СписокОтбора, ВидСравненияКомпоновкиДанных.ВСписке);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура ДобавитьДиаграммуВАК_СтандартныеОтчеты(ОтчетОбъект) Экспорт
	...
	МассивПоказателей = Новый Массив;
	МассивПоказателей.Добавить("СальдоНаНачалоПериода.УпрНачальный" + ТекстРазвернутый + "ОстатокДт");
	МассивПоказателей.Добавить("СальдоНаНачалоПериода.УпрНачальный" + ТекстРазвернутый + "ОстатокКт");
	МассивПоказателей.Добавить("ОборотыЗаПериод.УпрОборотДт");
	МассивПоказателей.Добавить("ОборотыЗаПериод.УпрОборотКт");
	МассивПоказателей.Добавить("СальдоНаКонецПериода.УпрКонечный" + ТекстРазвернутый + "ОстатокДт");
	МассивПоказателей.Добавить("СальдоНаКонецПериода.УпрКонечный" + ТекстРазвернутый + "ОстатокКт");
	
	Если ПолеСерииДиаграммы <> Неопределено И ОтчетОбъект.ДанныеОтчета.ПоказателиОтчета.Упр.Значение Тогда
		Для Каждого ПолеРесурсДиаграммы Из МассивПоказателей Цикл
			...
			ТиповыеОтчеты.УстановитьПараметрВывода(Диаграмма, "ТипДиаграммы"                     , АК_СтандартныеОтчеты.ПолучитьПредставлениеДиаграммы(ОтчетОбъект.ДанныеОтчета.ПредставлениеДиаграммы));
			ТиповыеОтчеты.УстановитьПараметрВывода(Диаграмма, "ТипДиаграммы.ВидПодписей"         , АК_СтандартныеОтчеты.ПолучитьВидПодписиДиаграммы(ОтчетОбъект.ДанныеОтчета.ВидПодписиДиаграммы));
...
Процедура ОбработкаРезультатаОтчетаСДиаграммой(ОтчетОбъект, Результат) Экспорт
	...
	МассивЗаголовков.Добавить("Упр Дт (нач. сальдо)");
	МассивЗаголовков.Добавить("Упр Кт (нач. сальдо)");
	МассивЗаголовков.Добавить("Упр Дт (оборот)");
	МассивЗаголовков.Добавить("Упр Кт (оборот)");
	МассивЗаголовков.Добавить("Упр Дт (кон. сальдо)");
	МассивЗаголовков.Добавить("Упр Кт (кон. сальдо)");
...
Функция ПолучитьПредставлениеНастройкиДляИстории(ОтчетОбъект) Экспорт 
	...
	Исключение
		...
		Если ИдентификаторОбъекта = "ОтчетОбъект.АК_АнализСубконто" 
			ИЛИ ИдентификаторОбъекта = "ОтчетОбъект.АК_КарточкаСубконто" Тогда
		...
		Если ИдентификаторОбъекта = "ОтчетОбъект.АК_ОтчетПоПроводкам"
			ИЛИ ИдентификаторОбъекта = "ОтчетОбъект.АК_СводныеПроводки" Тогда 
			Префикс = "Проводки ";
		КонецЕсли;
		...
		СоответствиеПоказателей.Вставить("Упр"              , "Упр");
		СоответствиеПоказателей.Вставить("Корр"             , "Корр");
		СоответствиеПоказателей.Вставить("МСФО"             , "МСФО.");
		...
		Если ИдентификаторОбъекта = "ОтчетОбъект.АК_АнализСубконто" 
			ИЛИ ИдентификаторОбъекта = "ОтчетОбъект.АК_ОборотноСальдоваяВедомостьПоСчету" 
			ИЛИ ИдентификаторОбъекта = "ОтчетОбъект.АК_ОборотыМеждуСубконто" 
			ИЛИ ИдентификаторОбъекта = "ОтчетОбъект.АК_АнализСчета" 
			ИЛИ ИдентификаторОбъекта = "ОтчетОбъект.АК_ОборотыСчета" Тогда
		...
		ИначеЕсли ИдентификаторОбъекта = "ОтчетОбъект.АК_КарточкаСчета" 
...
Процедура ДействияФормыДополнительныеИстория(ФормаОтчета, Кнопка) Экспорт 
	...
	АК_СтандартныеОтчеты.ВосстановитьНастройкиФормы(ФормаОтчета, Ложь);
	ФормаОтчета.ОбновитьПанельНастроек();
...
Функция ПолучитьДенежныеСредства(Период, Организация)
	...
	|	ПланСчетов.Финансовый КАК Финансовый
	|ГДЕ
	|	(Финансовый.Ссылка В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Финансовый.Касса))
	|				И (НЕ Финансовый.Ссылка В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Финансовый.ДенежныеДокументы)))
	|				И (НЕ Финансовый.Ссылка В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Финансовый.ДенежныеДокументыВал)))
	|			ИЛИ (Финансовый.Ссылка В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Финансовый.РасчетныеСчета))
	|				ИЛИ Финансовый.Ссылка В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Финансовый.ВалютныеСчета))
	|				ИЛИ Финансовый.Ссылка В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Финансовый.СпециальныеСчета))
	|				ИЛИ Финансовый.Ссылка В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Финансовый.ПереводыВПути_))
	|					И (НЕ Финансовый.Ссылка В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Финансовый.ПриобретениеИностраннойВалюты)))
	|					И (НЕ Финансовый.Ссылка В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Финансовый.ПродажиПоПлатежнымКартам)))
	|					И (НЕ Финансовый.Ссылка В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Финансовый.РеализацияИностраннойВалюты)))))
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Счет
	|;
	...
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ФинансовыйОстатки.Счет КАК Счет,
	|	ФинансовыйОстатки.СуммаОстатокДт КАК Остаток
	|ИЗ
	|	РегистрБухгалтерии.Финансовый.Остатки(
	|			&ГраницаПериод,
	|			Счет В
	...
	|			,
	|			Организация = &Организация) КАК ФинансовыйОстатки
	|ИТОГИ
	|	СУММА(Остаток)
	|ПО
	|	ОБЩИЕ";
...
Функция ПолучитьПродажи(НачалоПериода, КонецПериода, Организация)
	...
	|ИЗ
	|	РегистрБухгалтерии.Финансовый.Обороты(&НачалоПериода, &КонецПериода, , Счет В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Финансовый.Выручка)), , Организация = &Организация, , ) КАК ФинансовыйОбороты";
...
Функция ПолучитьПоступлениеДенежныхСредств(НачалоПериода, КонецПериода, Организация)
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
	               ...
	               |ПОМЕСТИТЬ СчетаКД
	               |ИЗ
	               |	ПланСчетов.Финансовый.ВидыСубконто КАК СчетаКонтрагентов
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ (ВЫБРАТЬ РАЗЛИЧНЫЕ
	               |			ФинансовыйВидыСубконто.Ссылка КАК Ссылка
	               |		ИЗ
	               |			ПланСчетов.Финансовый.ВидыСубконто КАК ФинансовыйВидыСубконто
	               |		ГДЕ
	               |			ФинансовыйВидыСубконто.ВидСубконто = ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконто.Договоры)) КАК СчетаДоговоров
	               |		ПО СчетаКонтрагентов.Ссылка = СчетаДоговоров.Ссылка
	               |ГДЕ
	               |	СчетаКонтрагентов.ВидСубконто = ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконто.Контрагенты)
	               |
	               |ИНДЕКСИРОВАТЬ ПО
	               |	Счет
	               |;
	               ...
	               |						ИЗ
	               |							СчетаКД КАК СчетаКД))
	               |				И (НЕ КорСчет В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Финансовый.РасчетыСРозничнымиПокупателями)))
	               |				И (НЕ КорСчет В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Финансовый.Выручка)))
	               |				И (НЕ КорСчет = ЗНАЧЕНИЕ(ПланСчетов.Финансовый.Вспомогательный))
	               ...
	               |			,
	               |			Организация = &Организация,
	               |			КорСчет В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Финансовый.РасчетыСРозничнымиПокупателями))
	               |				ИЛИ КорСчет В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Финансовый.Выручка)),
	...
	СчетаДС.Добавить(ПланыСчетов.Финансовый.КассаОрганизации);
	СчетаДС.Добавить(ПланыСчетов.Финансовый.ОперационнаяКасса);
	СчетаДС.Добавить(ПланыСчетов.Финансовый.КассаПлатежногоАгента);
	СчетаДС.Добавить(ПланыСчетов.Финансовый.КассаОрганизацииВал);
	СчетаДС.Добавить(ПланыСчетов.Финансовый.РасчетныеСчета);
	СчетаДС.Добавить(ПланыСчетов.Финансовый.ВалютныеСчета);
	СчетаДС.Добавить(ПланыСчетов.Финансовый.СпециальныеСчета);
	СчетаДС.Добавить(ПланыСчетов.Финансовый.ПереводыВПути);
	СчетаДС.Добавить(ПланыСчетов.Финансовый.ПереводыВПутиВал);
	СчетаДС.Добавить(ПланыСчетов.Финансовый.РасчетыСПодотчетнымиЛицами_);
...
Функция ПолучитьВыборкуПоСчету(ВидОстатков, НачалоПериода, КонецПериода, Организация, Счета = Неопределено, ВидыСубконто = Неопределено)
	...
	Если ВидОстатков = 0 Тогда
		СтрокаПоля = "ФинансовыйОстаткиИОбороты.СуммаНачальныйОстатокДт КАК СуммаНачальныйОстатокДт,
		|	ФинансовыйОстаткиИОбороты.СуммаНачальныйОстатокКт КАК СуммаНачальныйОстатокКт,
		|	ФинансовыйОстаткиИОбороты.СуммаКонечныйОстатокДт КАК СуммаКонечныйОстатокДт,
		|	ФинансовыйОстаткиИОбороты.СуммаКонечныйОстатокКт КАК СуммаКонечныйОстатокКт";	
	...
	ИначеЕсли ВидОстатков = 1 Тогда 
		СтрокаПоля = "ФинансовыйОстаткиИОбороты.СуммаНачальныйРазвернутыйОстатокДт КАК СуммаНачальныйРазвернутыйОстатокДт,
		|	ФинансовыйОстаткиИОбороты.СуммаНачальныйРазвернутыйОстатокКт КАК СуммаНачальныйРазвернутыйОстатокКт,
		|	ФинансовыйОстаткиИОбороты.СуммаКонечныйРазвернутыйОстатокДт КАК СуммаКонечныйРазвернутыйОстатокДт,
		|	ФинансовыйОстаткиИОбороты.СуммаКонечныйРазвернутыйОстатокКт КАК СуммаКонечныйРазвернутыйОстатокКт";	
	...
	ИначеЕсли ВидОстатков = 2 Тогда
		...
		|	ФинансовыйОстаткиИОбороты.СуммаКонечныйРазвернутыйОстатокДт КАК СуммаКонечныйРазвернутыйОстатокДт,
		|	ФинансовыйОстаткиИОбороты.СуммаКонечныйРазвернутыйОстатокКт КАК СуммаКонечныйРазвернутыйОстатокКт,
		|	ФинансовыйОстаткиИОбороты.СуммаНачальныйОстатокДт КАК СуммаНачальныйОстатокДт,
		|	ФинансовыйОстаткиИОбороты.СуммаНачальныйОстатокКт КАК СуммаНачальныйОстатокКт,
		|	ФинансовыйОстаткиИОбороты.СуммаКонечныйОстатокДт КАК СуммаКонечныйОстатокДт,
		|	ФинансовыйОстаткиИОбороты.СуммаКонечныйОстатокКт КАК СуммаКонечныйОстатокКт";	
	...
	Иначе
		Для Индекс = 1 По ВидыСубконто.Количество() Цикл
			СтрокаСубконто = СтрокаСубконто + "ФинансовыйОстаткиИОбороты.Субконто" + Индекс + ", ";	
		КонецЦикла;
	КонецЕсли;
	...
	+ СтрокаПоля + 
	" ИЗ
	|	РегистрБухгалтерии.Финансовый.ОстаткиИОбороты(" + СтрокаУсловие + ") КАК ФинансовыйОстаткиИОбороты
	|ИТОГИ " 
	+ СтрокаИтог + 
	"
	|ПО
...
//процедура формирует подписи отчета
Процедура СформироватьИВывестиПодписиОтчета(ОтчетОбъект, ДокументРезультат, ВысотаЗаголовка, ПоказыватьПодписи) Экспорт
	
	ОбластьПодписи = ОтчетОбъект.СформироватьПодписи();
	ДокументРезультат.Вывести(ОбластьПодписи, 1);
	
	ДокументРезультат.Области.Подписи.Видимость = ПоказыватьПодписи;
...
// процедура обрабатывает вывод подписей
Процедура ОбработатьВыводПодписей(Форма, ОтчетОбъект, ПоказыватьПодписи) Экспорт

	// Перезаполнять подписи можно только у "чистого" отчета
	Если Форма.ЭлементыФормы.ДокументРезультат.ВысотаТаблицы = 0 Тогда

		ОбластьПодписей = ОтчетОбъект.СформироватьПодписи();
		Форма.ЭлементыФормы.ДокументРезультат.Вывести(ОбластьПодписей);
		
	КонецЕсли;
	
	Если Форма.ЭлементыФормы.ДокументРезультат.Области.Найти("Подписи") <> Неопределено Тогда
		Форма.ЭлементыФормы.ДокументРезультат.Области.Подписи.Видимость = ПоказыватьПодписи;
	КонецЕсли;

	Форма.ЭлементыФормы.КоманднаяПанельФормы.Кнопки.Подписи.Пометка = ПоказыватьПодписи;
	Форма.ЭлементыФормы.КоманднаяПанельФормы.Кнопки.Подменю.Кнопки.Подписи.Пометка = ПоказыватьПодписи;
...
Процедура ОтборОбработатьВыборЗначения(Элемент, СтандартнаяОбработка, Знач Организация, СписокПараметров = Неопределено, ОграничениеТипаЭлемента = Неопределено)
	...
	//ИначеЕсли ОграничениеТипаЭлемента = ПланыВидовХарактеристик.ВидыСубконто.Партии.ТипЗначения 
	...
	//ИначеЕсли ОграничениеТипаЭлемента = ПланыВидовХарактеристик.ВидыСубконто.Документы.ТипЗначения
