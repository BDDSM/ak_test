	...
	Если Выборка.Следующий() Тогда
		...
		Если НачалоМесяца(Константы.ДатаКонтроляПеременнойЧастиПоСчетчикам.Получить()) >= НачалоМесяца(ПериодАренды)
			...
			Если СуммаОплатИЗаявок  > Лимит   Тогда
				//+++АК losa 13.12.2018 ИП-00020636
				...
				ОбщегоНазначения.СообщитьОбОшибке(ЦФОИМенеджерПоАренде(ТорговаяТочка)+Символы.ПС+
				 									"По торговой точке <" + ТорговаяТочка + "> превышен лимит оплаты по аренде за " +
													Формат(ПериодАренды, "ДФ='ММММ гггг'") + " !" + Символы.ПС +
													"Сумма произведенных и запланированных оплат с учетом документа составляет: " +
													Формат(СуммаОплатИЗаявок , "ЧДЦ=2") + " руб. Лимит оплаты: " + Формат(Лимит, "ЧДЦ=2; ЧН=") + " руб." , Отказ);
				//---АК losa 13.12.2018 ИП-00020636	
...
Процедура ПроверитьЛимитыПоАрендеЗаявкаНаУслугиМатериалы(ДокументСсылка, Отказ) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Сумма > Выборка.Лимит Тогда                                                                                                                                                                                                  
			...
			ОбщегоНазначения.СообщитьОбОшибке(ЦФОИМенеджерПоАренде(Выборка.ТорговаяТочка)+Символы.ПС+
												"По торговой точке <" + Выборка.ТорговаяТочка +
												"> превышен лимит оплаты по аренде (" + ТекстТипАренднойПлаты + ") за " +
												Формат(ДокументСсылка.НачалоПериодаАренды, "ДФ='ММММ гггг'") + "!" + Символы.ПС +
												"Сумма начисленных оплат с учетом документа составляет: " + Формат(Выборка.Сумма, "ЧДЦ=2") +
												" руб. Лимит оплаты: " + Формат(Выборка.Лимит, "ЧДЦ=2; ЧН=") + " руб." , Отказ);
...
Процедура ПроверитьЛимитыПоАрендеДокументыУчетаДоговоров(ТорговаяТочка, Дата, Отказ) Экспорт
	...
	Если СуммаПоДоговорам > Лимит  Тогда
		...
		ОбщегоНазначения.СообщитьОбОшибке(ЦФОИМенеджерПоАренде(ТорговаяТочка)+Символы.ПС+"По торговой точке <" + ТорговаяТочка + "> превышен лимит оплаты по аренде!" + Символы.ПС + "Сумма оплат по действующим договорам с учетом документа составляет: " + Формат(СуммаПоДоговорам, "ЧДЦ=2") + " руб. Лимит оплаты: " + Формат(Лимит, "ЧДЦ=2; ЧН=") + " руб." , Отказ);
		//---АК losa 13.12.2018 ИП-00020636
	КонецЕсли;
...
//+++АК losa 13.12.2018 ИП-00020636
Функция ЦФОИМенеджерПоАренде(ТорговаяТочка)
	Результат = "";
	...
	Запрос.Текст = 
		...
		|	ЦФОСтруктурныхЕдиницСрезПоследних.ЦФО.Наименование КАК НаименованиеЦФО
		|ИЗ
		|	РегистрСведений.ЦФОСтруктурныхЕдиниц.СрезПоследних(&ДатаЗапроса, СтруктурнаяЕдиница = &СтруктурнаяЕдиница) КАК ЦФОСтруктурныхЕдиницСрезПоследних";
	...
	Если Выборка.Следующий() Тогда
		Результат = Результат + СокрЛП(Выборка.НаименованиеЦФО);
	КонецЕсли;
	...
	Запрос.Текст = 
		...
		|	СоответствиеОбъектРольСрезПоследних.РольПользователя.Наименование КАК НаименованиеРоли
		|ИЗ
	...
	Запрос.УстановитьПараметр("ТипРоли", ПредопределенноеЗначение("ПланВидовХарактеристик.ТипыРолейПользователя.МенеджерПоАренде"));
	...
	Пока Выборка.Следующий() Цикл
		...
		Результат = Результат + Выборка.НаименованиеРоли;
	КонецЦикла;
