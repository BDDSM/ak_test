...
Функция СчитатьДанныеПоШтрихКоду(Знач ШтрихКод) Экспорт
	...
	НайденШК=Ложь;
	Если НЕ СтрДлина(ШтрихКод)>=24 Тогда
    ...
    Иначе
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	РегистрСведений.ШтрихКодыУпаковокПоставщиков КАК ШтрихКодыУпаковокПоставщиков
		               |ГДЕ
		               |	ШтрихКодыУпаковокПоставщиков.ШК подобно &ШК";
					   
		Запрос.УстановитьПараметр("ШК", "%"+Сред(ШтрихКод,3,14)+"%");
		...
		Если Выборка.Следующий() Тогда
			...
			СтруктураВозврат.Характеристика = Выборка.Характеристика;
			СтруктураВозврат.Количество = Выборка.КоличествоВУпаковке;
			СтруктураВозврат.Вставить("ДатаПроизводства", Дата(2000+Число(Сред(ШтрихКод,19,2)),Число(Сред(ШтрихКод,21,2)),Число(Сред(ШтрихКод,23,2))));
			НайденШК=Истина;
		КонецЕсли; 
	КонецЕсли; 
	...
	Если НЕ НайденШК Тогда//Результат.Пустой() Тогда
	//	Выборка = Результат.Выбрать();
	//	Выборка.Следующий();
	//	СтруктураВозврат.Номенклатура = Выборка.Номенклатура;
	//	СтруктураВозврат.Характеристика = Выборка.Характеристика;
