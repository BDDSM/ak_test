...
Функция ОтключитьСоединенияИБ(ПараметрыАдминистрированияИБ) Экспорт
	...
	Если ОпределитьЭтаИнформационнаяБазаФайловая() Тогда
		...
		ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Предупреждение, , , Сообщение);
#Если Клиент Тогда
...
#КонецЕсли
	...
	Попытка
		...
		Для Каждого Соединение Из Соединения.Соединения Цикл
			...
			ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация, , , СтрСообщения);
...
Функция РазорватьСоединение(ПараметрыСоединенияССервером1СПредприятие,
	...
	Если ОпределитьЭтаИнформационнаяБазаФайловая() Тогда
		ВызватьИсключение НСтр("ru = 'Невозможно получить список активных соединений в Файловом варианте системы'");
	КонецЕсли;
...
#Если Клиент Тогда
		...
		КонецЕсли; 
#Иначе
		ВызватьИсключение Сообщение + Символы.ПС + ИнформацияОбОшибке().Описание;
#КонецЕсли
		Результат = Ложь;
	КонецПопытки;
	Возврат Результат;
...
Функция ПолучитьАктивныеСоединенияИБ(НастройкаБлокировки, Знач ВсеКромеТекущего = Истина) 
	...
	Для Каждого Кластер Из ServerAgent.GetClusters() Цикл
		...
		ServerAgent.Authenticate(
			Кластер, 
			НастройкаБлокировки.ИмяАдминистратораКластера, 
