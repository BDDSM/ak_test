...
//АК БЕЛН 25.03.2016++
Функция ПолучитьПапкуКонтрагента(Наименование)  Экспорт
	Наим=СтрЗаменить(Наименование,"""","");
	...
	Наим=СтрЗаменить(Наим,"<","");
	Наим=СтрЗаменить(Наим,"?","");
	ПутьКПапке=ОбщегоНазначения.ПолучитьЗначениеКонстанты("КаталогХраненияФайловКонтрагентов");
	Папка="";	
	Если ЗначениеЗаполнено(ПутьКПапке) Тогда
		Кат=Новый Файл(ПутьКПапке+"\"+Наим);
...
Процедура ПроверитьСтатьюДДССчетУчетаРасчетовОбъектаСВопросом(Объект) Экспорт
	
	СтруктураРеквизитовОбъекта = ЗаполнениеДокументов.ПолучитьСтруктуруСтатьяДДССчетУчетаРасчетов(Объект);
	
	Если СтруктураРеквизитовОбъекта.СтатьяДДС <> Объект.СтатьяДДС
		ИЛИ СтруктураРеквизитовОбъекта.СчетУчета <> ОБъект.СчетУчетаРасчетовСКонтрагентом Тогда
			Если СтруктураРеквизитовОбъекта.СтатьяДДС <> Объект.СтатьяДДС И ЗначениеЗаполнено(СтруктураРеквизитовОбъекта.СтатьяДДС) Тогда
				Ответ = Вопрос("Статья ДДС объекта отличается от статьи ДДС, полученной по правилам определения статей ДДС." 
							+ Символы.ПС + "-Статья ДДС объекта: " + Строка(Объект.СтатьяДДС)
							+ Символы.ПС + "-Статья ДДС полученная: " + Строка(СтруктураРеквизитовОбъекта.СтатьяДДС)
							+ Символы.ПС + "Заменить статью в документе?", РежимДиалогаВопрос.ДаНет);
							
				Если Ответ = КодВозвратаДиалога.Да Тогда
					Объект.СтатьяДДС = СтруктураРеквизитовОбъекта.СтатьяДДС;
				КонецЕсли;
			КонецЕсли;
			
			Если СтруктураРеквизитовОбъекта.СчетУчета <> Объект.СчетУчетаРасчетовСКонтрагентом И ЗначениеЗаполнено(СтруктураРеквизитовОбъекта.СчетУчета) Тогда
				Ответ = Вопрос("Счет учета расчетов с контрагентом у документа отличается от счета учета, полученного по правилам определения счетов учета." 
							+ Символы.ПС + "-Счет учета объекта: " + Строка(Объект.СчетУчетаРасчетовСКонтрагентом)
							+ Символы.ПС + "-Счет учета полученный: " + Строка(СтруктураРеквизитовОбъекта.СчетУчета)
							+ Символы.ПС + "Заменить счет учета расчетов с контрагентом в документе?", РежимДиалогаВопрос.ДаНет);
							
				Если Ответ = КодВозвратаДиалога.Да Тогда
					Объект.СчетУчетаРасчетовСКонтрагентом = СтруктураРеквизитовОбъекта.СчетУчета;
				КонецЕсли;
			КонецЕсли;
