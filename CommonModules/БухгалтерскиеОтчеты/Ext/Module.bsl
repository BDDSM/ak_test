...
Процедура ОбработатьВыборДляСтрокиОтбораБухОтчетов(СтрокаОтбора, Элемент, СтандартнаяОбработка, ОбъектОтчет) Экспорт
	...
	Попытка
		Если СтрокаОтбора.ТипЗначения.Типы().Количество() = 1
			...
			Если ОбъектОтчет.ИмяРегистраБухгалтерии = "Финансовый" Тогда
...
Процедура ВывестиПоказателиОборотноСальдовойВедомостиПоСчету(Выборка, СтруктураПараметров, Знач Счет, 
	...
	Для Каждого ИмяПоказателя Из СтруктураПараметров.МассивПоказателей Цикл
		...
		Если Выборка.Уровень() = 1 Тогда
			ВыводимаяОбластьРаскраски = ДокументРезультат.Присоединить(Область);
			ВыводимаяОбластьРаскраски.ЦветФона = Новый Цвет(244, 236, 197);
		ИначеЕсли Выборка.Уровень() = 2 Тогда
			ВыводимаяОбластьРаскраски = ДокументРезультат.Присоединить(Область);
			ВыводимаяОбластьРаскраски.ЦветФона = Новый Цвет(248, 242, 216);
		ИначеЕсли Выборка.Уровень() = 3 Тогда
			ВыводимаяОбластьРаскраски = ДокументРезультат.Присоединить(Область);
			ВыводимаяОбластьРаскраски.ЦветФона = Новый Цвет(251, 249, 236);
		Иначе
			ДокументРезультат.Присоединить(Область);			
		КонецЕсли;
		//ДокументРезультат.Присоединить(Область);
...
Процедура ВывестиГруппировкуОборотноСальдовойВедомостиПоСчету(ОбъектОтчета, Выборка, Знач ИндексТекущейГруппировки, СтруктураПараметров) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Уровень() = 1 Тогда
			ВыводимаяОбластьРаскраски = ДокументРезультат.Вывести(ВыводимаяОбласть, Выборка.Уровень());
			ВыводимаяОбластьРаскраски.ЦветФона = Новый Цвет(244, 236, 197);
		ИначеЕсли Выборка.Уровень() = 2 Тогда
			ВыводимаяОбластьРаскраски = ДокументРезультат.Вывести(ВыводимаяОбласть, Выборка.Уровень());
			ВыводимаяОбластьРаскраски.ЦветФона = Новый Цвет(248, 242, 216);
		...
		Если СтруктураПараметров.Свойство("СоответствиеСтруктурныхЕдиниц")
		   И Лев(ИмяГруппировки, 8) = "Субконто"
		   И ТипЗнч(Выборка[ИмяГруппировки]) = Тип("СправочникСсылка.СтруктурныеЕдиницы")
		   И СтруктураПараметров.СоответствиеСтруктурныхЕдиниц[Выборка[ИмяГруппировки]] <> Неопределено Тогда
			ДокументРезультат.Область(ДокументРезультат.ВысотаТаблицы, 2, ДокументРезультат.ВысотаТаблицы, 8).Шрифт = СтруктураПараметров.ШрифтЖирный;
		КонецЕсли;
		//---АК ILIK
