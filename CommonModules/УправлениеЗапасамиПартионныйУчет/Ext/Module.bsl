	...
	Для Каждого СтрокаПартии Из СтрокаДокумента.СписанныеПартии Цикл
		...
		Если КомиссионныйТовар(СтрокаПартии.СчетУчета) 
			...
			Если (ТипЗнч(СтрокаПартии.Партия) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг"))
				ИЛИ (ТипЗнч(СтрокаПартии.Партия) = Тип("ДокументСсылка.Партия")) Тогда
				...
				Если НЕ ЗначениеЗаполнено(СтруктураПараметров.ДоговорКонтрагента) Тогда
					...
					Иначе
						...
						СуммаПоступления = МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(СтрокаПартии.СуммаСписания, мВалютаРегламентированногоУчета,ВалютаРасчетовСКомитентом
																				,1,СтрокаПартии.Партия.КурсВзаиморасчетов
																				,1,СтрокаПартии.Партия.КратностьВзаиморасчетов);
					КонецЕсли;
				Иначе
