...
Функция ЭтоИмпортныйТовар(СчетУчета, НомерГТД, СтранаПроисхождения) Экспорт
	...
	ИначеЕсли СчетУчета.ПринадлежитЭлементу(ПланыСчетов.Финансовый.ТоварыНаСкладах) или
		 СчетУчета = ПланыСчетов.Финансовый.ТоварыНаСкладах или
		 
		 СчетУчета.ПринадлежитЭлементу(ПланыСчетов.Финансовый.ТоварыВРозничнойТорговле) или
		 СчетУчета = ПланыСчетов.Финансовый.ТоварыВРозничнойТорговле или
		 
		 СчетУчета.ПринадлежитЭлементу(ПланыСчетов.Финансовый.ТоварыПринятыеНаКомиссию) или
		 СчетУчета = ПланыСчетов.Финансовый.ТоварыПринятыеНаКомиссию или
		 
		 СчетУчета.ПринадлежитЭлементу(ПланыСчетов.Финансовый.ТоварыВРозничнойТорговлеВПродажныхЦенахАТТ) или
		 СчетУчета = ПланыСчетов.Финансовый.ТоварыВРозничнойТорговлеВПродажныхЦенахАТТ или
		 
		 СчетУчета.ПринадлежитЭлементу(ПланыСчетов.Финансовый.ПокупныеИзделия) или
		 СчетУчета = ПланыСчетов.Финансовый.ПокупныеИзделия Тогда
...
Процедура ПроведениеДокументаСписанияПоСчетуГТД(Документ, ТаблицаПоТоварам, СодержаниеПроводки = "Списаны МПЗ") Экспорт
	...
	Проводки = Документ.Движения.Финансовый;
	...
	|	ФинансовыйОстатки.Субконто2 КАК НомерГТД,
	|	ФинансовыйОстатки.Субконто3 КАК СтранаПроисхождения,
	|	ФинансовыйОстатки.КоличествоОстатокДт КАК Количество
	|ИЗ
	...
	Запрос.УстановитьПараметр("СчетУчетаНомеровГТД", ПланыСчетов.Финансовый.ГТД);
	...
	ВидыСубконто = ПланыСчетов.Финансовый.ГТД.ВидыСубконто.ВыгрузитьКолонку("ВидСубконто");
	...
	Для каждого ТоварПоНомеруГТД из ТоварыПоНомерамГТД Цикл
		...
		Если КоличествоВПроводке > 0 Тогда
			...
			Проводка.СчетКт = ПланыСчетов.Финансовый.ГТД;
