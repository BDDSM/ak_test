...
Функция ВерсииАдресныхОбъектов() Экспорт
	...
	//	Результат.Добавить(ДатаВерсии, "AL");
	//	Результат.Добавить(ДатаВерсии, "SO");
	//КонецЕсли;
...
Функция ЗапросАдресаПоИндексуКЛАДР(Знач ВыбиратьСтрок, Знач СкрыватьНеактуальные) Экспорт
	...
	Запрос = Новый Запрос("
		...
		|	ВЫБОР КОГДА НЕ Улицы.Код ЕСТЬ NULL ТОГДА Улицы.Код
		|		ИНАЧЕ НаселенныеПункты.Код
		|	КОНЕЦ КАК Код,
		|	
		...
		|	
		|ГДЕ
		|	(НЕ Улицы.Код ЕСТЬ NULL ИЛИ НЕ НаселенныеПункты.Код ЕСТЬ NULL) И ПочтовыеИндексы.Индекс = &ПочтовыйИндекс
