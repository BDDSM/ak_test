...
Функция ЭтоСтараяВерсияОтчета(ОтчетОбъект) Экспорт
	...
	Возврат ТиповыеОтчетыПереопределяемыйИнфоком.ЭтоСтараяВерсияОтчетаПереопределяемая(ОтчетОбъект);
...
#Если Клиент Тогда
	...
	функция ОбработкаРасшифровкиТиповогоОтчета(Расшифровка, СтандартнаяОбработка, ОтчетОбъект, ФормаОтчета, ДополнительныеРасшифровки = Неопределено) Экспорт
		...
		ИначеЕсли Команда = "РасшифроватьПоле" Тогда
			...
			НовыйОтчет = ТиповыеОтчетыПереопределяемыйИнфоком.ПолучитьОтчетДляРасшифровкиПереопределяемая(ОтчетОбъект);
	...
	Процедура НарисоватьСтандартныйПериод(ОтчетОбъект, ФормаОтчета, ЗначенияНастроек, Параметры, Лево, ЕстьПериодНаГорПанели)
		...
		Если ЗначениеПараметраПериод <> Неопределено Тогда
			...
			//Начало Инфоком
			ЗначениеПараметраПериодДоступное = КомпоновщикНастроек.Настройки.ПараметрыДанных.ДоступныеПараметры.Элементы.Найти("Период");
			Если ЗначениеПараметраПериодДоступное = Неопределено Тогда
				Возврат;
			КонецЕсли;
			
			Если Не ЗначениеПараметраПериодДоступное.Видимость Тогда
				Возврат;
			КонецЕсли;
			//Конец Инфоком
	...
	Процедура ПерерисоватьПанельНастроек(ОтчетОбъект, ФормаОтчета, ЗначенияНастроек = Неопределено, ПереноситьПараметры = истина) Экспорт
		...
		Если ОтчетОбъект.КомпоновщикНастроек.Настройки.ПараметрыДанных.ДоступныеПараметры <> Неопределено Тогда
			...
			Для каждого Параметр Из ОтчетОбъект.КомпоновщикНастроек.Настройки.ПараметрыДанных.ДоступныеПараметры.Элементы Цикл
				...
				//+++ZEZA
				Если СокрЛП(ВРЕГ(Параметр.Параметр)) = "ПЕРИОД" Тогда
					Продолжить;
				ИначеЕсли СокрЛП(ВРЕГ(Параметр.Параметр)) = "НАЧАЛОПЕРИОДА" Тогда
					Продолжить;
				ИначеЕсли СокрЛП(ВРЕГ(Параметр.Параметр)) = "КОНЕЦПЕРИОДА" Тогда
					Продолжить;	
				КонецЕсли;	
				//---ZEZA
		...
		Если ФормаОтчета.ЭлементыФормы.Найти("Отбор") <> Неопределено тогда
			...
			Если ФормаОтчета.ЭлементыФормы.Отбор.Ширина <> ШиринаПанели-31 тогда
				//???АК ФормаОтчета.ЭлементыФормы.Отбор.Ширина = ШиринаПанели-31;
			КонецЕсли;
		КонецЕсли;
		...
		Если ФормаОтчета.ЭлементыФормы.Найти("Сортировка") <> Неопределено тогда
			...
			Если ФормаОтчета.ЭлементыФормы.Сортировка.Ширина <> ШиринаПанели-70 тогда
				//???АК ФормаОтчета.ЭлементыФормы.Сортировка.Ширина  = ШиринаПанели-70;
		...
		Если Параметры.Свойство("Параметры") тогда
			...
			Если ФормаОтчета.ЭлементыФормы.ПараметрыДанных.Ширина <> ШиринаПанели-31 тогда
				//???АК ФормаОтчета.ЭлементыФормы.ПараметрыДанных.Ширина = ШиринаПанели-31;
			КонецЕсли;
		...
		Если Страница <> Неопределено и Страница.Видимость тогда
			...
			Если ЗначенияНастроек.Свойство("Показатели")  тогда
				...
				Если ФормаОтчета.ЭлементыФормы.Показатели.Ширина <> ШиринаПанели-31 тогда
					//???АК ФормаОтчета.ЭлементыФормы.Показатели.Ширина = ШиринаПанели-31;
				КонецЕсли;
		...
		ТиповыеОтчетыПереопределяемыйИнфоком.ПослеВыводаПанелиПользователя(ОтчетОбъект, ФормаОтчета, ДеревоНастроекСтандартныхСтраниц, ЗначенияНастроек);
	...
	Функция ПолучитьЗначенияНастроекПанелиПользователя(ОтчетОбъект, ФормаОтчета) Экспорт
		...
		ТиповыеОтчетыПереопределяемыйИнфоком.ДополнитьЗначенияНастроекПанелиПользователя(ЗначенияНастроек, ОтчетОбъект, ФормаОтчета);
...
Функция ПолучитьОписаниеНастроекОтчета(КомпоновщикНастроек)
	...
	Если ЗначениеПараметраНачалоПериода <> Неопределено 
		...
		И ЗначениеПараметраПериод = Неопределено Тогда
...
Функция ПолучитьТаблицуДоступныхВариантов(НастраиваемыйОбъект, Пользователь = Неопределено, СПомеченнымиНаУдаление = Ложь, ТипНастройки = Неопределено, СписокДоступныхНастроек = Неопределено) Экспорт
	...
	Возврат ТиповыеОтчетыПереопределяемыйИнфоком.ПолучитьТаблицуДоступныхВариантовПереопределяемая(НастраиваемыйОбъект, Пользователь, СПомеченнымиНаУдаление, ТипНастройки, СписокДоступныхНастроек);
...
Функция ПолучитьЗаголовокОтчета(ОтчетОбъект, ФормаОтчета = Неопределено) Экспорт
	...
	Возврат ТиповыеОтчетыПереопределяемыйИнфоком.ПолучитьЗаголовокОтчетаПереопределяемая(ОтчетОбъект, ФормаОтчета);
...
Функция ПолучитьЗначенияНастроекПанелиПользователяПоУмолчанию(ОтчетОбъект, ФормаОтчета = Неопределено) Экспорт
	...
	ТиповыеОтчетыПереопределяемыйИнфоком.ДополнитьЗначенияНастроекПанелиПользователяПоумолчанию(ЗначенияНастроек, ОтчетОбъект);
...
Функция ПолучитьПараметрыПанелиПользователяПоУмолчанию(ОтчетОбъект, ФормаОтчета = Неопределено) Экспорт
	...
	Возврат ТиповыеОтчетыПереопределяемыйИнфоком.ПолучитьПараметрыПанелиПользователяПоУмолчаниюПереопределяемая(ОтчетОбъект, ФормаОтчета);
...
Функция ПолучитьИдентификаторОбъекта(ОтчетОбъект) Экспорт
	...
	Возврат ТиповыеОтчетыПереопределяемыйИнфоком.ПолучитьИдентификаторОбъектаПереопределяемая(ОтчетОбъект);
	
КонецФункции

Функция ПолучитьСхемуКомпоновкиОбъекта(ОтчетОбъект) Экспорт
	
	Возврат ТиповыеОтчетыПереопределяемыйИнфоком.ПолучитьСхемуКомпоновкиОбъектаПереопределяемая(ОтчетОбъект);
