...
Функция ПолучитьВидОбъектаДоступа(ОбъектДоступа) Экспорт
	...
	ИначеЕсли ТипЗнч(ОбъектДоступа) = Тип("СправочникСсылка.ГруппыДоступаСтруктурныхЕдиниц") Тогда
		Возврат Перечисления.ВидыОбъектовДоступа.СтруктурныеЕдиницы;		
...
Функция ПолучитьОбщуюДатуЗапретаДляПользователя() Экспорт
	
	Запрос = Новый Запрос();
	Запрос.Текст = "ВЫБРАТЬ
	               |	ГраницыЗапретаИзмененияДанных.ГраницаЗапретаИзменений,
	               |	ВЫБОР
	               |		КОГДА ГраницыЗапретаИзмененияДанных.Пользователь = &Пользователь
	               |			ТОГДА 1
	               |		КОГДА ГраницыЗапретаИзмененияДанных.Пользователь = ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)
	               |			ТОГДА 0
	               |		ИНАЧЕ -1
	               |	КОНЕЦ КАК Порядок
	               |ИЗ
...
Функция ДокументВЗакрытомПериодеНДС(ДокументОбъект) Экспорт
	
	Результат = Ложь;
	
	СоответствиеГраницЗапрета = ПолныеПрава.ПолучитьГраницыЗапретаИзмененияДанныхНДС().Получить();
...
//Функция возвращает дату запрета
Функция ПолучитьТекущуюДатуЗапрета(ДокументОбъект) Экспорт
	...
	ДатаЗапрета = СоответствиеГраницЗапрета[ДокументОбъект.Организация];
		
	// Если дата запрета по организации не определена, то используется основная дата запрета для пользователя
	Если НЕ ЗначениеЗаполнено(ДатаЗапрета) Тогда
		ДатаЗапрета = СоответствиеГраницЗапрета[Справочники.Организации.ПустаяСсылка()];       
	КонецЕсли;
	
	// Если основная дата запрета для пользователя не определена, то используется общая дата запрета
	Если НЕ ЗначениеЗаполнено(ДатаЗапрета) Тогда
		ДатаЗапрета = СоответствиеГраницЗапрета["ОбщаяДатаЗапретаРедактирования"];
	КонецЕсли;
...
Процедура ПроверкаПериодаЗаписейРегистров(НаборЗаписей, Отказ, ВидРегистра = "РегистрНакопления") Экспорт
	...
	ГраницаПериодаБух = Константы.АК_ДатаЗапретаРедактированияБух.Получить();
    ...
    Если ПроверятьСуществующиеЗаписиПоРегистратору Тогда
		...
		//Дополнительная проверка для записей МСФО
		//ПроверкаСуществующихЗаписейРегистра(НаборЗаписей, СтруктураПараметров, Отказ, Истина);
		//
		//Если Отказ Тогда
		//	Сообщить("Редактирование данных периода МСФО запрещено");
		//	Возврат;
		//КонецЕсли;	
	...
	Если НаборЗаписей.Количество() > 0 И НЕ Отказ Тогда				
		...
		Если ЕстьОрганизация Тогда
			Для Каждого Запись ИЗ НаборЗаписей Цикл
				...
				//ГраницаПериодаМСФО = СоответствиеГраницЗапретаМСФО[Запись.Организация];
				...
				//Если НЕ ЗначениеЗаполнено(ГраницаПериодаМСФО) Тогда
				//	ГраницаПериодаМСФО = СоответствиеГраницЗапретаМСФО[Справочники.Организации.ПустаяСсылка()];       
				//КонецЕсли;
				...
				//Если НЕ ЗначениеЗаполнено(ГраницаПериодаМСФО) Тогда
				//	ГраницаПериодаМСФО = СоответствиеГраницЗапретаМСФО["ОбщаяДатаЗапретаРедактирования"];
				//КонецЕсли;

				Если ЗначениеЗаполнено(ГраницаПериода) Тогда
					
					Если ТолькоМСФО(Запись) Тогда
						Если Запись[ПолеПериод] <= ГраницаПериодаБух Тогда	
							Отказ = Истина;
							Сообщить("Дата корректировки МСФО: " +Запись[ПолеПериод] + ". Граница запрета МСФО: " + ГраницаПериодаБух);
							Возврат;
						Иначе
							//Не выполняем проверорк по основной дате если только МСФО
							Продолжить;
						КонецЕсли;
					КонецЕсли;
		...
		Иначе
			...
			Если ЗначениеЗаполнено(ГраницаПериода) Тогда
				Для Каждого Запись ИЗ НаборЗаписей Цикл
					...
					//Если движения только по ресурсу суммаМСФО - разрешаем движения в любом периоде. 	
					Если ТолькоМСФО(Запись) Тогда
						Если Запись[ПолеПериод] <= ГраницаПериодаБух Тогда	
							Отказ = Истина;
							Сообщить("Дата корректировки МСФО: " +Запись[ПолеПериод] + ". Граница запрета МСФО: " + ГраницаПериодаБух);
...
Функция ТолькоМСФО(Запись)
	
	Если ТипЗнч(Запись) <> Тип("РегистрБухгалтерииЗапись.Финансовый") Тогда
		Возврат Ложь;
	КонецЕсли;	
	...
	Если Запись.ВалютнаяСуммаКт <> 0 И Запись.ВалютнаяСуммаКт <> null Тогда
		Возврат Ложь;
	КонецЕсли;	

	Если Запись.ВалютнаяСуммаДт <> 0 И Запись.ВалютнаяСуммаДт <> null Тогда
		Возврат Ложь;
	КонецЕсли;	

	Если Запись.КоличествоКт <> 0 И Запись.КоличествоКт  <> null Тогда
		Возврат Ложь;
	КонецЕсли;	

	Если Запись.КоличествоДт <> 0 И Запись.КоличествоДт  <> null Тогда
		Возврат Ложь;
	КонецЕсли;	
	
	Если Запись.СуммаМСФО = 0 Тогда
		Возврат Ложь;		
	КонецЕсли;	
...
Процедура ПроверкаСуществующихЗаписейРегистра(НаборЗаписей, СтруктураПараметров, Отказ, МСФО=Ложь)
	Перем ИмяПоляПериода;
	
	//Возврат; // минеев выключил, все блокирует
				// включил обратно, видимо в момент выключил индекс на регистре бухгалтерии сдох
	ЭтоФинансовый = (ТипЗнч(НаборЗаписей.ЭтотОбъект) = Тип("РегистрБухгалтерииНаборЗаписей.Финансовый"));
	//Если МСФО И НЕ ЭтоФинансовый Тогда
	//	Возврат;
	//КонецЕсли;	
	...
	//Разрешаем движения если заполнен только ресурс МСФО
	
	//Если ЭтоФинансовый Тогда
	//	Если НЕ МСФО Тогда
	//		УсловиеМСФО = " И (Сумма <> 0 ИЛИ КоличествоДт <> 0 ИЛИ КоличествоКт <> 0 ИЛИ ВалютнаяСуммаДт <> 0 ИЛИ ВалютнаяСуммаКт <> 0) ";	
	//	Иначе
	//		УсловиеМСФО = " И СуммаМСФО <> 0 ";
	//	КонецЕсли;	
	//Иначе
	//	УсловиеМСФО = "";
	//КонецЕсли;	
	...
	Запрос = Новый Запрос;
	ВложенныйЗапрос = "";
	Если НЕ ЭтоФинансовый И СтруктураПараметров.ЕстьОрганизация Тогда
		Индекс = 1;
	...
	Если ЭтоФинансовый Тогда
	   ...
	   |	Набор.ВалютнаяСуммаДт КАК ВалютнаяСуммаДт,
	   |	Набор.ВалютнаяСуммаКт КАК ВалютнаяСуммаКт,
	   |	Набор.КоличествоДт КАК КоличествоДт,
	   |	Набор.КоличествоКт КАК КоличествоКт,
	   |	Набор.Сумма КАК Сумма,
	   |	Набор.СуммаМСФО КАК СуммаМСФО
	   |ПОМЕСТИТЬ втЗаписиРаньшеМаксимальнойДатыЗапрета
	   |ИЗ
	   |	РегистрБухгалтерии.Финансовый КАК Набор
	   |ГДЕ
	   |	Набор.Регистратор = &Регистратор
	   |	И Набор.Период <= &ДатаЗапретаМаксимальная
	   |;
	   |
	   |////////////////////////////////////////////////////////////////////////////////
	   |ВЫБРАТЬ
	   |	МИНИМУМ(втЗаписиРаньшеМаксимальнойДатыЗапрета.Период) КАК Период
	   |ПОМЕСТИТЬ втПериодФинансовый
	   |ИЗ
	   |	втЗаписиРаньшеМаксимальнойДатыЗапрета КАК втЗаписиРаньшеМаксимальнойДатыЗапрета
	   |ГДЕ
	   |	(втЗаписиРаньшеМаксимальнойДатыЗапрета.ВалютнаяСуммаДт <> 0
	   |			ИЛИ втЗаписиРаньшеМаксимальнойДатыЗапрета.ВалютнаяСуммаКт <> 0
	   |			ИЛИ втЗаписиРаньшеМаксимальнойДатыЗапрета.КоличествоДт <> 0
	   |			ИЛИ втЗаписиРаньшеМаксимальнойДатыЗапрета.КоличествоКт <> 0
	   |			ИЛИ втЗаписиРаньшеМаксимальнойДатыЗапрета.Сумма <> 0)
	   |;
	   |
	   |////////////////////////////////////////////////////////////////////////////////
	   |ВЫБРАТЬ
	   |	МИНИМУМ(втЗаписиРаньшеМаксимальнойДатыЗапрета.Период) КАК Период
	   |ПОМЕСТИТЬ втПериодМСФО
	   |ИЗ
	   |	втЗаписиРаньшеМаксимальнойДатыЗапрета КАК втЗаписиРаньшеМаксимальнойДатыЗапрета
	   |ГДЕ
	   |	втЗаписиРаньшеМаксимальнойДатыЗапрета.СуммаМСФО <> 0
	   |;
	   |
	   |////////////////////////////////////////////////////////////////////////////////
	   |ВЫБРАТЬ
	   |	Истина КАК ЭтоФинансовый
	   |ИЗ
	   |	втПериодФинансовый КАК втПериодФинансовый
	   |ГДЕ
	   //+++АК ILIK 2018.08.16 ИП-00019570
	   //|	втПериодФинансовый.Период < &ПериодФинансовый
	   |	втПериодФинансовый.Период <= &ПериодФинансовый
	   //---АК ILIK
	   |
	   |ОБЪЕДИНИТЬ ВСЕ
	   ...
	   |	Ложь
	   |ИЗ
	   |	втПериодМСФО КАК втПериодМСФО
	   |ГДЕ
	   //+++АК ILIK 2018.08.16 ИП-00019570
	   //|	втПериодМСФО.Период < &ПериодМСФО";
	   |	втПериодМСФО.Период <= &ПериодМСФО";
	   //---АК ILIK
	   
		ГраницаПериодаМСФО = КонецДня(АК_УчетМСФОПривилегированный.ПолучитьДатуЗапретаБух());
		Если ЗначениеЗаполнено(ГраницаПериодаМСФО) Тогда			
       		ГраницаОтбор = ?(ГраницаПериода>ГраницаПериодаМСФО,ГраницаПериода,ГраницаПериодаМСФО);
		Иначе 
			ГраницаОтбор = ГраницаПериода;
		КонецЕсли;	
		Запрос.УстановитьПараметр("ДатаЗапретаМаксимальная", ?(ГраницаОтбор=Неопределено, NULL, ГраницаОтбор));
		Запрос.УстановитьПараметр("ПериодМСФО", ?(ГраницаПериодаМСФО=Неопределено, NULL, ГраницаПериодаМСФО));
		Запрос.УстановитьПараметр("ПериодФинансовый", ?(ГраницаПериода=Неопределено, NULL, ГраницаПериода));
	   ...
	   РезультатЗапроса = Запрос.Выполнить();
		...
		Иначе 	
			...
			Если РезультатЗапроса.Выгрузить()[0].ЭтоФинансовый = Ложь Тогда
				Сообщить("Документ выполняет движения в закрытом периоде МСФО");
			КонецЕсли;
		КонецЕсли;	
	...
	Иначе 
		...
		|МИНИМУМ(" + ИмяПоляПериода + ") КАК Период  
		|ИЗ " + ИмяРегистра + " КАК Набор
...
Процедура ПередЗаписьюДокументаПроверкаДоступностиПериода(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	Если ТипЗнч(Источник) = Тип("ДокументОбъект.ЗаявкаНаРасходованиеСредств") ИЛИ ТипЗнч(Источник) = Тип("ДокументОбъект.ПоступлениеТоваровУслуг") 
		//+++АК BELN 2018.10.11 ИП-00020132
		 ИЛИ ТипЗнч(Источник) = Тип("ДокументОбъект.ПриходныйОрдерСклад")
		//---АК BELN 2018.10.11 
		Тогда
		Если Источник.НеПроверятьДатуЗапретаРедактирования Тогда
			Возврат;
		КонецЕсли;	
	КонецЕсли;	
	
	//+++АК POZM 2017.11.22 ИП-00017139
	Если Источник.ДополнительныеСвойства.Свойство("ПроводитьВЗакрытомПериоде") Тогда
		Если Источник.ДополнительныеСвойства.ПроводитьВЗакрытомПериоде Тогда
			Возврат;
		КонецЕсли;	
	КонецЕсли;	
	//---АК POZM 
