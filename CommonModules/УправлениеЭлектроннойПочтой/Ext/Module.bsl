...
Функция ПолучитьИнтернетПочтовыйПрофиль(УчетнаяЗапись) Экспорт
	...
	Если УчетнаяЗапись.ОтправкаЧерезСерверItStuff Тогда
...
//+++АК SHEP 2018.02.05 ИП-00017780
// Формирует почтовый профиль без аутентификации по имени отправителя
//
// Параметры
//  ИмяОтправителя - Строка
//
// Возвращаемое значение:
//   ИПП - ИнтернетПочтовыйПрофиль
//
Функция ПолучитьИнтернетПочтовыйПрофильБезАутентификации(ИмяОтправителя) Экспорт
...
Процедура ПослатьОтРассылкиСОбратнымАдресом(АдресаПолучателей, ТекстПисьма, ОтКого, Тема, ИмяОтправителя = "", ТипТекстаСообщения = Неопределено, АдресФайла = "") Экспорт
	
	УчётнаяЗапись = ОбщиеПроцедуры.ПолучитьУчетнуюЗаписьДляРассылки();
	Профиль = ПолучитьИнтернетПочтовыйПрофиль(УчётнаяЗапись);
	
	Письмо = Новый ИнтернетПочтовоеСообщение;
	Письмо.ИмяОтправителя  	= "" + СокрЛП(УчётнаяЗапись) + "";
	Письмо.Отправитель     	= "" + СокрЛП(УчётнаяЗапись) + "";
	...
	Письмо.Получатели.Добавить(АдресаПолучателей);
	Письмо.Тема				= Тема;
	Письмо.Тексты.Добавить(ТекстПисьма, ?(ТипТекстаСообщения = Неопределено, ТипТекстаПочтовогоСообщения.HTML, ТипТекстаПочтовогоСообщения));
...
#Если Клиент Тогда
	...
	Процедура НаписатьПисьмоЗаявке(Объект) Экспорт
		...
		Для Каждого СтрокаТЧ Из Объект.ТорговыеТочки Цикл
			Акцептант = Акцептант + Строка(СтрокаТЧ.Управляющий) + "; ";	
		КонецЦикла;
		СтруктураНовогоПисьма = Новый Структура;
		...
		СтруктураНовогоПисьма.Вставить("Тема"			, "Создана новая заявка № " + Строка(Объект.Номер) + " от " +
		Формат(Объект.Дата, "ДФ=dd.MM.yyyy") + " и требует АКЦЕПТОВАНИЯ!");
		СтруктураНовогоПисьма.Вставить("ЗаявкаКандидата", Объект.Ответственный);
		СтруктураНовогоПисьма.Вставить("Тело"			, "Создана новая заявка № " + Строка(Объект.Номер) + " от " +
		Формат(Объект.Дата, "ДФ=dd.MM.yyyy") + Символы.ПС + Символы.ПС + 
		"Вид оплаты: " + Объект.ФормаОплаты + Символы.ПС + 
		"Ответственный: " + Объект.Ответственный + Символы.ПС +
		"Получатель: " + Объект.Контрагент + Символы.ПС +
		"Акцептант: " + Акцептант);
		...
		//	Если Пользователь.Роли.Содержит(Метаданные.Роли.ПравоАкцептованияВсехЗаявокНаРасходованиеСредств) Тогда
		//		МассивПользователей.Добавить(Справочники.Пользователи.НайтиПоНаименованию(Пользователь.Имя));	
		//	КонецЕсли;
		//КонецЦикла;
		
		МассивПользователей = ПолныеПраваДополнительный.ПолучитьСписокПользователейДляАкцептования(Объект.ФормаОплаты);
		СпрПользователи = Справочники.Пользователи;
		мПользовательМашкина 	= СпрПользователи.НайтиПоНаименованию("Машкина Олеся");
		мПользовательНесифорова = СпрПользователи.НайтиПоНаименованию("Несифорова Алена");
		ТекПользовательМашкина 	= (ПараметрыСеанса.ТекущийПользователь = мПользовательМашкина); 
		Для Каждого СтрокаТЧ Из Объект.ТорговыеТочки Цикл
			Если НЕ МассивПользователей.Найти(СтрокаТЧ.Управляющий) = Неопределено Тогда
				Продолжить;	
			КонецЕсли;
			
			//
			Если НЕ (ТекПользовательМашкина
				И СтрокаТЧ.Управляющий = мПользовательНесифорова) Тогда
				МассивПользователей.Добавить(СтрокаТЧ.Управляющий);
			Иначе
				МассивПользователей.Добавить(мПользовательМашкина);
			КонецЕсли;
		КонецЦикла;
		...
		//Для каждого СтрокаСпискаОбъектов Из СписокОбъектов Цикл
		//	Для каждого Адрес Из СписокКому Цикл
		//		
		//	КонецЦикла;	
		//	Адрес = "finmanager@izbenka.msk.ru";
		//	Если НЕ ПустаяСтрока(Адрес) Тогда
		//		СписокКому.Добавить(Адрес, Адрес);
		//		//СписокКому.Добавить("dira@automacon.ru", "dira@automacon.ru");
		//	КонецЕсли;
		//КонецЦикла;
		...
		СтруктураВозврата = НаписатьПисьмо(глЗначениеПеременной("глТекущийПользователь"), СтруктураНовогоПисьма,,, , "Отправить",,, Ложь);
		
		// отправка							
		СоответствиеПисем = Новый Соответствие();
		СоответствиеПисем.Вставить(СтруктураВозврата.ПисьмоСсылка); 
		ТекстОшибок = "";
		
		УправлениеЭлектроннойПочтой.ОтправитьПисьма(СоответствиеПисем, "", глЗначениеПеременной("глТекущийПользователь"), Ложь, ТекстОшибок);
	...
	Процедура НаписатьПисьмоЗаявкеОтветственному(Объект) Экспорт
		...
		СтруктураНовогоПисьма = Новый Структура;
		СтруктураНовогоПисьма.Вставить("Основание", Объект);
		СтруктураНовогоПисьма.Вставить("Тема", "Заявка № " + Строка(Объект.Номер) + " от " + Формат(Объект.Дата, "ДФ=dd.MM.yyyy") + " ОПЛАЧЕНА");
		СтруктураНовогоПисьма.Вставить("ЗаявкаКандидата", Объект.Ответственный);
		СтруктураНовогоПисьма.Вставить("Тело", "Заявка № " + Строка(Объект.Номер) + " от " + Формат(Объект.Дата, "ДФ=dd.MM.yyyy") + " на сумму " + Формат(Объект.СуммаДокумента, "ЧДЦ=2; ЧГ=0") + " ОПЛАЧЕНА" + Символы.ПС+ Символы.ПС + "Ответственный: " + Объект.Ответственный + Символы.ПС + "Контрагент: " + Объект.Контрагент);
		...
		Для каждого СтрокаСпискаОбъектов Из СписокОбъектов Цикл
			...
			Запрос.УстановитьПараметр("Объект", СтрокаСпискаОбъектов.Значение.Ответственный);
			Запрос.УстановитьПараметр("Тип", Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты);
		...
		СтруктураВозврата = НаписатьПисьмо(глЗначениеПеременной("глТекущийПользователь"),СтруктураНовогоПисьма,,, (НЕ ПустаяСтрока(Адрес)), "Отправить",,, Ложь);
		
		СоответствиеПисем = Новый Соответствие();
		
		СсылкаНаПисьмо = СтруктураВозврата.ПисьмоСсылка;
		//СтруктураПисем.("ПисьмоСсылка", СсылкаНаПисьмо);
		...
		Если ТекстОшибок = "" Тогда
			ПолныеПрава.ОтметитьОтправкуУведомленияВЗаявкеНаРасходование(Объект);	
		КонецЕсли;
	...
	Процедура НаписатьПисьмоЗаявкеОшибка(Объект) Экспорт
		...
		СтруктураНовогоПисьма = Новый Структура;
		СтруктураНовогоПисьма.Вставить("Основание", Объект);
		СтруктураНовогоПисьма.Вставить("Тема", "В заявке № " + Строка(Объект.Номер) + " от " + Формат(Объект.Дата, "ДФ=dd.MM.yyyy") + " ОШИБКА");
		СтруктураНовогоПисьма.Вставить("ЗаявкаКандидата", Объект.Ответственный);
		СтруктураНовогоПисьма.Вставить("Тело", "В заявка № " + Строка(Объект.Номер) + " от " + Формат(Объект.Дата, "ДФ=dd.MM.yyyy") + " ОШИБКА" + Символы.ПС+ Символы.ПС + "Ответственный: " + Объект.Ответственный
		+ Символы.ПС+" Форма оплаты: " + Строка(Объект.ФормаОплаты) + Символы.ПС+ " Получатель: " + Объект.Контрагент);
		...
		СтруктураВозврата = НаписатьПисьмо(глЗначениеПеременной("глТекущийПользователь"),СтруктураНовогоПисьма,,, (НЕ ПустаяСтрока(Адрес)), "Отправить",,, Истина);
		
		СоответствиеПисем = Новый Соответствие();
		
		СсылкаНаПисьмо = СтруктураВозврата.ПисьмоСсылка;
		
		//ФормаПисьма = СсылкаНаПисьмо.ПолучитьФорму("ФормаДокумента");
		//СтруктураПисем.("ПисьмоСсылка", СсылкаНаПисьмо);
		//СоответствиеПисем.Вставить(СсылкаНаПисьмо); 
		// ФормаПисьма.Открыть();
	...
	Процедура НаписатьПисьмоПоОплатеПоставщику(ТекстСообщения, Контрагент) Экспорт
		...
		СтруктураНовогоПисьма.Вставить("ВидТекста", Перечисления.ВидыТекстовЭлектронныхПисем.Текст);
		//СтруктураНовогоПисьма.Вставить("Основание", Объект);
		СтруктураНовогоПисьма.Вставить("Тема", "Платежи поставщикам от " + Формат(ТекущаяДата(), "ДФ=dd.MM.yyyy"));
		//СтруктураНовогоПисьма.Вставить("ЗаявкаКандидата", Объект.Ответственный);
		...
		МассивПользователей = ПолныеПраваДополнительный.ПолучитьСписокТехнологовДляУведомленияОПлатеже(Контрагент);
				...
				Если ТипЗнч(ДеревоУчетныхЗаписейИПисем) = Тип("ДеревоЗначений") Тогда
					...
					Если СтрокаДереваУчетнойЗаписи <> Неопределено Тогда
						...
						Для каждого СтрокаПисьма Из СтрокаДереваУчетнойЗаписи.Строки Цикл
							...
							// Kasper(
							ПочтовоеСообщение.ОбратныйАдрес.Очистить();
							Если ЗначениеЗаполнено(Письмо.АК_ОбратныйАдрес) Тогда
								ПочтовоеСообщение.ОбратныйАдрес.Добавить(Письмо.АК_ОбратныйАдрес);
							КонецЕсли;	
							// Kasper)
							...
							Пока Выборка.Следующий() Цикл
								...
								//+++АК sole 2018.06.14 ИП-00018850
								оВложения = Выборка.Ссылка.ПолучитьОбъект();
								ДвоичныеДанные = оВложения.Хранилище.Получить();
								
								Если ТипЗнч(ДвоичныеДанные) = Тип("ДвоичныеДанные") Тогда
									ПочтовоеСообщение.Вложения.Добавить(ДвоичныеДанные, ИмяФайла);
								КонецЕсли; 
								//+++АК sole 2018.06.14 ИП-00018850
							КонецЦикла; 
...
Функция НаписатьПисьмо(ТекущийПользователь, СтруктураНовогоПисьма = Неопределено, ПеренестиВложенияИзОснования = Ложь, Копирование = Ложь,
	...
	Если ЕстьДокументОснование Тогда
		...
		ИначеЕсли ТипЗнч(Основание) = Тип("ДокументОбъект.ЗаявкаНаРасходованиеСредств") Тогда
			Письмо.ОснованиеПисьма = Основание;
		...
		Если Письмо.УчетнаяЗапись.ИспользоватьКлассификациюПисемПоПредметам Тогда
			...
			Иначе
				Если Письмо.УчетнаяЗапись.ИспользоватьКлассификациюПисемПоПредметам 
	...
	#Если Клиент Тогда
		...
		Если ТипЗнч(Основание) = Тип("ДокументСсылка.ЭлектронноеПисьмо")
			...
			"ВЫБРАТЬ РАЗРЕШЕННЫЕ
			|	ВложенияЭлектронныхПисем.ИмяФайла КАК ИмяФайла,
			|	ВложенияЭлектронныхПисем.Ссылка КАК ВложенияЭлектронныхПисемСсылка,
			|	ВложенияЭлектронныхПисем.Наименование КАК Наименование,
			...
			Если НЕ РезультатЗапроса.Пустой() Тогда
				...
				Пока Выборка.Следующий() Цикл
					//+++АК sole 2018.06.14 ИП-00018850
					оВложение = Выборка.ВложенияЭлектронныхПисемСсылка.ПолучитьОбъект();
					//+++АК sole 2018.06.14 ИП-00018850
					Если ПустаяСтрока(Выборка.ИДФайлаПочтовогоПисьма) Тогда
						Если ПеренестиВложенияИзОснования Тогда
							...
							//+++АК sole 2018.06.14 ИП-00018850
							СтрокаТЗ.Данные       = оВложение.Хранилище;
							//---АК sole 2018.06.14 ИП-00018850
						КонецЕсли; 
					Иначе
...
Функция ПропарситьHTMLТекстКартинкаВ_ИД(ДокументCOMОбъект, Письмо = Неопределено, СоответствиеПисемИТекстов) Экспорт
	...
	Для а = 0 По ТегиКартинок.length - 1 Цикл
		...
		ИмяФайла = Строка(ТегиКартинок.item(а).href);
		ИД = "";
		Если НЕ ПустаяСтрока(ИмяФайла) И ТипЗнч(ТаблицаСуществующихФайлов) = Тип("ТаблицаЗначений") Тогда
			
			ИмяФайла = СтрЗаменить(Сред(ИмяФайла,9,СтрДлина(ИмяФайла)-8),"/","\");
			ТаблицаСоответствийФайловЭлектронныхПисем = глЗначениеПеременной("ТаблицаСоответствийФайловЭлектронныхПисем");
			НайденнаяСтрока = ТаблицаСоответствийФайловЭлектронныхПисем.Найти(ИмяФайла,"ПутьКФайлу");
			
			Если НайденнаяСтрока <> Неопределено Тогда
				
				СтрокаТаблицы = ТаблицаСуществующихФайлов.Найти(НайденнаяСтрока.ИД, "ИДФайлаПочтовогоПисьма");
...
Функция ПропарситьHTMLИДВ_ТекстКартинки(СоответствиеТекстовЭлектронныхПисем, ТекущийПользователь, Письмо, ТекстПисьма) Экспорт
	...
	Для каждого СтрокаТаблицы Из ТаблицаДанных Цикл
		...
		ТаблицаСоответствийФайловЭлектронныхПисем = глЗначениеПеременной("ТаблицаСоответствийФайловЭлектронныхПисем");
		НайденнаяСтрока = ТаблицаСоответствийФайловЭлектронныхПисем.Найти(СтрокаТаблицы.ИДФайлаПочтовогоПисьма,"ИД");
		
		ПутьКФайлу = Неопределено;
		
		Если НайденнаяСтрока <> Неопределено Тогда
			
			ПроверяемыйФайл = Новый Файл(НайденнаяСтрока.ПутьКФайлу);
			Если ПроверяемыйФайл.Существует() Тогда
				ПутьКФайлу = НайденнаяСтрока.ПутьКФайлу;
			КонецЕсли;
		...
		Если ПутьКФайлу = Неопределено Тогда
			...
			Попытка
				//+++АК sole 2018.06.14 ИП-00018850
				оВложения = СтрокаТаблицы.ВложенияЭлектронныхПисемСсылка.ПолучитьОбъект();
				оВложения.Хранилище.Получить().Записать(ПутьКФайлу);
				//---АК sole 2018.06.14 ИП-00018850
				
				НоваяСтрока = ТаблицаСоответствийФайловЭлектронныхПисем.Добавить();
				НоваяСтрока.ИД = СтрокаТаблицы.ИДФайлаПочтовогоПисьма;
...
Функция ПолучитьДоступныеУчетныеЗаписи(Пользователь) Экспорт
	...
	Если ПравоДоступа("Чтение", Метаданные.Справочники.УчетныеЗаписиЭлектроннойПочты) Тогда
		...
		Если ЗначениеЗаполнено(Пользователь) И НЕ РольПолныеПрава Тогда
			...
			|ГДЕ
			|	(НЕ ЕСТЬNULL(УчетныеЗаписиЭлектроннойПочтыДоступКУчетнойЗаписи.Ссылка.ПометкаУдаления, ЛОЖЬ))
			|
		...
		Иначе
			Запрос.Текст = Запрос.Текст + "
			|ГДЕ
			|	(НЕ ЕСТЬNULL(УчетныеЗаписиЭлектроннойПочты.ПометкаУдаления, ЛОЖЬ))
			|
			|УПОРЯДОЧИТЬ ПО УчетныеЗаписиЭлектроннойПочты.Наименование
			|";
		КонецЕсли;
...
Процедура УстановитьСвойстваОбъектаОснования(ОснованиеОбъектСсылка, Ответ, Переадресация, СтатусПисьма, УчетнаяЗапись, мЭтоНовый) Экспорт
	...
	Если ТипЗнч(ОснованиеОбъектСсылка) = Тип("ДокументСсылка.ЭлектронноеПисьмо") Тогда
		мОбъектОснование = ОснованиеОбъектСсылка.ПолучитьОбъект();
	Иначе
		мОбъектОснование = ОснованиеОбъектСсылка;
	КонецЕсли;
...
Функция EmailValid(Адрес) Экспорт
	...
	ПравильнаяСтрока = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789@_.;-";
	...
	Для н = 1 По СтрДлина(Адрес) Цикл
		Если Найти(ПравильнаяСтрока, Сред(Адрес, н, 1)) = 0 Тогда
			Возврат ЛОЖЬ;
		КонецЕсли;	
	КонецЦикла;	
...
//+++ АК Pans 20140428 ИП-00014932
Процедура РассылкаУведомленийОбОплатеПремий() Экспорт
	
	ОТЧисло = Новый ОписаниеТипов("Число");
	
	ТЗПисемУтверждающим = Новый ТаблицаЗначений;
	ТЗПисемУтверждающим.Колонки.Добавить("Утверждающий");
	ТЗПисемУтверждающим.Колонки.Добавить("Период");
	ТЗПисемУтверждающим.Колонки.Добавить("Сумма", ОТЧисло);
	ТЗПисемУтверждающим.Колонки.Добавить("ФизЛица");
	
	// делаем выборку неразосланных
	Запрос1 = Новый Запрос;
	Запрос1.Текст = 
	"ВЫБРАТЬ
	|	ВыплатыПоПремиям1.Период,
	|	ВыплатыПоПремиям1.ФизЛицо,
	|	СУММА(ВыплатыПоПремиям1.Сумма) КАК Сумма,
	|	МАКСИМУМ(ВыплатыПоПремиям1.ДатаВыплаты) КАК ДатаВыплаты
	|ИЗ
	|	РегистрСведений.ВыплатыПоПремиям1 КАК ВыплатыПоПремиям1
	|ГДЕ
	|	ВыплатыПоПремиям1.Разослано = ЛОЖЬ
	|
	|СГРУППИРОВАТЬ ПО
	|	ВыплатыПоПремиям1.ФизЛицо,
	|	ВыплатыПоПремиям1.Период";
	...
	Пока Выб1.Следующий() Цикл
		// получим Утверждающих из РН "Премии" для сочетания периода и ФЛ
		МСУтверждающих = РегистрыНакопления.Премии.ПолучитьУтверждающихПоПремии(Выб1.Период, Выб1.ФизЛицо);
		Для каждого Утверждающий Из МСУтверждающих Цикл
			СО1 = Новый Структура;
			СО1.Вставить("Утверждающий", Утверждающий);
			СО1.Вставить("Период", Выб1.Период);
			
			СтрокиТЗУ = ТЗПисемУтверждающим.НайтиСтроки(СО1);
			Если СтрокиТЗУ.Количество() = 0 Тогда
				СтрокаТЗУ = ТЗПисемУтверждающим.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаТЗУ, СО1);
				СтрокаТЗУ.Утверждающий = Утверждающий;
				СтрокаТЗУ.ФизЛица = Новый Массив;
			Иначе
				СтрокаТЗУ = СтрокиТЗУ[0];
			КонецЕсли;
			СтрокаТЗУ.ФизЛица.Добавить(Выб1.ФизЛицо);
			СтрокаТЗУ.Сумма = СтрокаТЗУ.Сумма + Выб1.Сумма;
		КонецЦикла;
		// а для ФЛ отправляем письмо с указанием периода и суммы
		ОтправитьПисьмоОПремии(Выб1.ФизЛицо, Выб1.Период, Выб1.Сумма, Выб1.ДатаВыплаты);
		
		// Устанавливаем реквизит "Разослано"
		НЗ1 = РегистрыСведений.ВыплатыПоПремиям1.СоздатьНаборЗаписей();
		НЗ1.Отбор.Период.Установить(Выб1.Период);
		НЗ1.Отбор.ФизЛицо.Установить(Выб1.ФизЛицо);
		НЗ1.Прочитать();
		
		Для каждого Запись1 Из НЗ1 Цикл
			Запись1.Разослано = Истина;
			Запись1.ДатаРассылкиФЛ = ТекущаяДатаСеанса();
			Запись1.РассылкаУтверждающим = 1;
		КонецЦикла;
		...
		//НЗ1.Записать();
		//
		//ТЗ1.ЗаполнитьЗначения(Ложь, "Разослано");
		//			
		//НЗ1 = РегистрыСведений.АК_ВыплатыПоПремиям1.СоздатьНаборЗаписей();
		//		
		//НЗ1.Загрузить(ТЗ1);
		//НЗ1.Записать(Ложь);
	...
	// делаем рассылку утверждающим
	Для каждого СтрокаТЗУ Из ТЗПисемУтверждающим Цикл
		ОтправитьПисьмоОПремии(СтрокаТЗУ.Утверждающий.ФизЛицо, СтрокаТЗУ.Период, СтрокаТЗУ.Сумма,,СтрокаТЗУ.ФизЛица);
	КонецЦикла;
	
КонецПроцедуры

Процедура ОтправитьПисьмоОПремии(ФизЛицо, Период, Сумма, ДатаВыплаты, ФЛУтверждающего = Неопределено) Экспорт
	
	АдресЭППолучателя = ПолучитьEMailФизЛица(ФизЛицо);
	Если ОбщегоНазначения.ЭтоКопияБазы() Тогда
		АдресЭППолучателя = "pans@automacon.ru"; // для целей тестирования
	КонецЕсли;
	
	
	Если НЕ ЗначениеЗаполнено(АдресЭППолучателя) Тогда
		Возврат;
	КонецЕсли;
	
	Если ФЛУтверждающего <> Неопределено И ФЛУтверждающего.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Почта.Подключиться(Профиль);
	Письмо.Тема = "Оплата премии";
	Получатель = Письмо.Получатели.Добавить();
	Получатель.Адрес = АдресЭППолучателя;
	
	// временная переадресация адресов ВСЕХ на адрес Рустема и мой
	//Если Найти(НРег(Получатель.Адрес), "krivenko") > 0 И Найти(НРег(Получатель.Адрес), "andy") > 0 Тогда
	//Получатель.Адрес = "abdr@automacon.ru";
	Получатель2 = Письмо.СлепыеКопии.Добавить();
	Получатель2.Адрес = "pans@automacon.ru";
	//КонецЕсли;
	
	Текст1 = Письмо.Тексты.Добавить();
	Если ФЛУтверждающего = Неопределено Тогда
		Текст1.Текст     = "Здравствуйте, " + ФизЛицо + "! Вам произведена выплата премии за "
		+ Формат(Период, "ДФ='MMMM yyyy'") + " г. в сумме " + Сумма
		+ ". Дата документа оплаты: " + Формат(ДатаВыплаты, "ДЛФ=Д");
	Иначе
		Текст1.Текст     = "Здравствуйте, " + ФизЛицо + "! Произведена выплата утвержденных Вами премий за "
		+ Формат(Период, "ДФ='MMMM yyyy'") + " г. на общую сумму " + Сумма + " руб. след. работникам: ";
		Для каждого Работник1 Из ФЛУтверждающего Цикл
			Текст1.Текст = Текст1.Текст + Символы.ПС;
			Текст1.Текст = Текст1.Текст + Работник1;
		КонецЦикла;
	КонецЕсли;
	...
	Попытка
		Почта.Послать(Письмо);
		ЗаписьЖурналаРегистрации("Отправка письма о премии", УровеньЖурналаРегистрации.Предупреждение,,, Строка(ФизЛицо));
	Исключение
		ТекстОшибки = "Не удалось отправить письмо. Ошибка: " + ОписаниеОшибки();
		ЗаписьЖурналаРегистрации("Отправка письма о премии", УровеньЖурналаРегистрации.Ошибка,,, Строка(ФизЛицо) + ". " + ТекстОшибки);
...
Функция ПолучитьEMailФизЛица(ФизЛицо) Экспорт
...
Функция ЭлектронныеАдресаФизическихЛиц(ФизическиеЛица) Экспорт
	...
	Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	КонтактнаяИнформация_АдресЭлПочты.Объект КАК ФизЛицо,
		|	ВЫРАЗИТЬ(КонтактнаяИнформация_АдресЭлПочты.Объект КАК Справочник.ФизическиеЛица).Наименование КАК НаименованиеПолучателя,
		|	ВЫРАЗИТЬ(КонтактнаяИнформация_АдресЭлПочты.Представление КАК СТРОКА(250)) КАК АдресЭлектроннойПочты
		|ИЗ
		|	РегистрСведений.КонтактнаяИнформация КАК КонтактнаяИнформация_АдресЭлПочты
		|ГДЕ
		|	КонтактнаяИнформация_АдресЭлПочты.Объект В(&ФизическиеЛица)
		|	И НЕ ВЫРАЗИТЬ(КонтактнаяИнформация_АдресЭлПочты.Объект КАК Справочник.ФизическиеЛица).ПометкаУдаления
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Выборка = РезультатЗапроса.Выбрать();
		Пока Выборка.Следующий() Цикл
			ЭлектронныйАдресФизическогоЛица = Новый Структура("ФизЛицо, Адрес, Представление"
				, Выборка.ФизЛицо, Выборка.АдресЭлектроннойПочты, Выборка.НаименованиеПолучателя);
			Результат.Добавить(ЭлектронныйАдресФизическогоЛица);
		КонецЦикла;
...
Функция СформироватьПисьмоСОтчетом(Отчет, Тема, Получатели, ПолучателиКопий = Неопределено, Знач Тело = "", ПоместитьОтчетВТелоПисьма = Истина) Экспорт
	...
	Если ПоместитьОтчетВТелоПисьма Тогда
		Тело = Тело + ?(ПустаяСтрока(Тело), "", Символы.ПС) + ПоместитьОтчетВТелоПисьма(Отчет);
	Иначе
		// Вложения надо доделать
	КонецЕсли;
...
//+++АК LATV 2018.11.30 ИП-00020296
Функция ПоместитьОтчетВТелоПисьма(Отчет) Экспорт

	ИмяФайлаHTML = ПолучитьИмяВременногоФайла("html");
	Отчет.Записать(ИмяФайлаHTML, ТипФайлаТабличногоДокумента.HTML4);
...
//+++АК LATV 2018.11.30 ИП-00020296
Процедура СформироватьИОтправитьПисьма(ПисьмаДляОтправки) Экспорт
...
Функция ОтправитьЗаявкуНаПодтверждение(Тема, Таблица, НомерЗаявки, СписокКому, GUID_Заявки, ID_MESSAGE, TYPE_MESSAGE, Адрес, СсылкаАкцептант, УчетнаяЗапись = Неопределено, Объект_Ссылка, РедактироватьКомментарий = Ложь) Экспорт 
	...
	Если РедактироватьКомментарий Тогда
		БлокКомментария = "%0D%0AМежду фигурными скобками Вы можете ввести комментарий%0D%0A{%0D%0A%0D%0A}";
		пТекстСсылкиНаКнопкуПодтвердить 			= пТекстСсылкиНаКнопкуПодтвердить + БлокКомментария;
		пТекстСсылкиНаКнопкуОтменитьПодтверждение 	= пТекстСсылкиНаКнопкуОтменитьПодтверждение + БлокКомментария;
	КонецЕсли;
	//---АК sole 2018.10.02 ИП-00019785.02
...
//---АК ZHAS 16-09-17 ИП-00015200.02.000.00001
Функция ОтправитьЗаявкуНаПодтверждениеЗаданиеТехнолога(Тема, Таблица, НомерЗаявки, СписокКому, GUID_Заявки, ID_MESSAGE, TYPE_MESSAGE, Адрес, СсылкаАкцептант, УчетнаяЗапись = Неопределено, Объект_Ссылка) Экспорт 
	...
	Таблица = СтрЗаменить(Таблица, ". ", "<br>");
	Таблица = СтрЗаменить(Таблица, "! ", "<br>");
	Таблица = СтрЗаменить(Таблица, "!. ", "<br>");
	...
	ТекстСообщения 									= Письмо.Тексты.Добавить();
	ТекстСообщения.Текст    						= ТекстСообщения.Текст + Таблица;
	пТекстСсылкиНаКнопкуПлюс1День 					= "www.vkusvill.ru";
	пТекстСсылкиНаКнопкуПлюс3Дня 					= "www.vkusvill.ru";
	пТекстСсылкиНаКнопкуНеЗакрыватьАвтоматически 	= "www.vkusvill.ru";//
	
	пТекстСсылкиНаКнопкуПлюс1День 					= "mailto:vkusvill.333@gmail.com?reply-to=vkusvill.333@gmail.com&subject=Акцепт заявки №" + НомерЗаявки + "&body=GUID%20" + GUID_Заявки + "%20ID_MESSAGE%20" + ID_MESSAGE   + "%20TYPE_MESSAGE%20" + TYPE_MESSAGE  + "%20ПлюсОдинДень";
	пТекстСсылкиНаКнопкуПлюс3Дня 					= "mailto:vkusvill.333@gmail.com?reply-to=vkusvill.333@gmail.com&subject=Акцепт заявки №" + НомерЗаявки + "&body=GUID%20" + GUID_Заявки + "%20ID_MESSAGE%20" + ID_MESSAGE   + "%20TYPE_MESSAGE%20" + TYPE_MESSAGE  + "%20ПлюсТриДня";
	
	пТекстСсылкиНаКнопкуНеЗакрыватьАвтоматически 	= "mailto:vkusvill.333@gmail.com?reply-to=vkusvill.333@gmail.com&subject=Акцепт заявки №" + НомерЗаявки + "&body=GUID%20" + GUID_Заявки + "%20ID_MESSAGE%20" + ID_MESSAGE   + "%20TYPE_MESSAGE%20" + TYPE_MESSAGE  + "%20НеЗакрыватьАвтоматически";//
	пТекстКнопкиПодтвердитьНеЗакрыватьАвтоматически = "<br><br><a href=""" + пТекстСсылкиНаКнопкуНеЗакрыватьАвтоматически + """><b>НеЗакрыватьАвтоматически</b></a><br><br>";
	
	пТекстСсылкиНаКнопкуПлюс1День 					= "<br><a href=""" + пТекстСсылкиНаКнопкуПлюс1День + """><b>ПлюсОдинДень</b></a><br><br>";
	пТекстСсылкиНаКнопкуПлюс3Дня 					= "<br><a href=""" + пТекстСсылкиНаКнопкуПлюс3Дня + """><b>ПлюсТриДня</b></a><br><br>";
	ТекстСообщения.Текст 							= ТекстСообщения.Текст + пТекстСсылкиНаКнопкуПлюс1День + пТекстСсылкиНаКнопкуПлюс3Дня + пТекстКнопкиПодтвердитьНеЗакрыватьАвтоматически;
	ТекстСообщения.ТипТекста 						= ТипТекстаПочтовогоСообщения.HTML;
	Почта.Послать(Письмо);
	Почта.Отключиться();
	Запись 											= РегистрыСведений.АкцептЗаявок.СоздатьМенеджерЗаписи();
...
Функция АкцептЗаявкоАктивирован() Экспорт 	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
...
Функция АкцептЗаявкоРежимТеста() Экспорт 	
	...
	|	РегистрСведений.ПараметрыДляПодстановкиВЗаявки КАК ПараметрыДляПодстановкиВЗаявки
	|ГДЕ
	|	ПараметрыДляПодстановкиВЗаявки.Индекс = 111";
	Возврат НЕ Запрос.Выполнить().Пустой();
КонецФункции

Процедура КонтрольАкцептРассылки(Источник, Параметры = Неопределено) Экспорт
	Попытка	
		Таблица			= ЗначениеИзСтрокиВнутр(Источник);
		Тема 			= "КонтрольАкцептРассылки";
		НомерЗаявки 	= "999";
