...
//	УказаниеКППплательщикаОбязательно = ВБюджет;
	...
	ЗначенияРеквизитов.Вставить("КПППлательщика", 
		...
		КонецЕсли;
		Если ТипЗнч(ВладелецСчетаПолучателя) = Тип("СправочникСсылка.ФизическиеЛица") Тогда
			СтрокаРеквизитов = "ИНН";
		Иначе
			СтрокаРеквизитов = "ИНН,КПП";
		КонецЕсли;
			
		РеквизитыВладельцаСчетаПолучателя = 
			ОбщегоНазначения.ПолучитьЗначенияРеквизитов(ВладелецСчетаПолучателя, СтрокаРеквизитов);
	...
	//		УказаниеКППполучателяОбязательно = ВБюджет;
		...
		Если Найти(СтрокаРеквизитов,"КПП")>0 И 
			УказаниеКППполучателяОбязательно Тогда

			КПППолучателя = ?(НЕ ПустаяСтрока(РеквизитыВладельцаСчетаПолучателя.КПП), РеквизитыВладельцаСчетаПолучателя.КПП, "0");
			...
			Если ЗначениеЗаполнено(СчетПолучателя) Тогда
				КППИзБанковскогоСчёта = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СчетПолучателя, "КПППолучателя"); 			
		...
		Иначе
			КПППолучателя = "";
		КонецЕсли;
		ЗначенияРеквизитов.Вставить("КПППолучателя", КПППолучателя);
...
Функция ПолучитьЧастьЗапросаДляВыбораСодержанияУслуг(Знач ТабличнаяЧасть) Экспорт
	...
	|		КОГДА НЕ (" + ТабличнаяЧасть + "Номенклатура.Наименование ПОДОБНО """") ТОГДА
	|			ВЫРАЗИТЬ(" + ТабличнаяЧасть + "Номенклатура.Наименование КАК Строка (1000))
	|		ИНАЧЕ
