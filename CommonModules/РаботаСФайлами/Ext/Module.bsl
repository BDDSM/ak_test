...
Процедура ОткрытьФайлДополнительнойИнформации(ИмяКаталога, ИмяФайла) Экспорт
	...
	Иначе
		//--- AK suvv 16.04.2018 ИП-00018410
		...
		Для каждого СсылкаФайл из ВыделенныеСтроки Цикл
			...
			Если ДанныеВХранилище = Неопределено Тогда
				...
				Если Файлы.Количество() = 0 Тогда
					Продолжить;
				КонецЕсли;	
				СохранитьФайл(СтруктураПараметров, СсылкаФайл.ИмяФайла, Новый ХранилищеЗначения(Новый ДвоичныеДанные(Файлы[0].ПолноеИмя)), СпособПерезаписи);
			Иначе
...
Процедура ОткрытьФайлы(ОбъектФайла, ВыделенныеСтроки = Неопределено, ВопросОПерезаписи = Истина) Экспорт
	...
	Иначе
		...
		Для каждого Элем из ВыделенныеСтроки Цикл
			...
			СсылкаФайл = Элем.ПолучитьОбъект();
			//---АК sole 2018.06.09 ИП-00018850
			...
			Если ДанныеВХранилище = Неопределено Тогда
				Расширение = РаботаСФайлами.ПолучитьРасширениеФайла(СсылкаФайл.ИмяФайла);
				Файлы = НайтиФайлы(КаталогСохранения, Строка(СсылкаФайл.Ссылка.УникальныйИдентификатор()) + "." + Расширение, Истина);
				Если Файлы.Количество() = 0 Тогда
					Продолжить;
				КонецЕсли;	
				ОткрытьФайлИзХранилища(СсылкаФайл.ИмяФайла, Новый ХранилищеЗначения(Новый ДвоичныеДанные(Файлы[0].ПолноеИмя)), СпособПерезаписи, ВопросОПерезаписи);
			Иначе	
...
Процедура ОткрытьФормуСпискаФайловИИзображений(СтруктураДляСпискаИзображений, СтруктураДляСпискаДополнительныхФайлов, ОбязательныеОтборы, ФормаВладелец, ИмяСправочника = "ХранилищеДополнительнойИнформации") Экспорт
	...
	Если СтруктураДляСпискаИзображений.Свойство("ЭДО") = Истина Тогда
		ФормаФайлов.ЭДО = Истина;
	КонецЕсли;	
