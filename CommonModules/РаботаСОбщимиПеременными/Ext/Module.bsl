...
Функция ПолучитьЗначениеПеременной(Имя) Экспорт
	...
	//+++AK VERN 2016-09-06 Обход проблемы с правами
...
//					ОбщегоНазначения.СообщитьОбОшибке("Ошибка загрузки внешней компоненты для склонения ФИО! Функции склонения будут недоступны!");
		...
		//+++АК
		//korn 2015.10.05 
		ИначеЕсли ИмяПараметраВР = ВРег("глАК_ЗагруженаКомпонентаROMAsterisk") Тогда
			Если НЕ ПолучитьИзКэшаКонфигурации(ИмяПараметраВР, НайденноеЗначение, Кэш, ПоместитьВКэш) Тогда
				НайденноеЗначение = ПараметрыСеанса.АК_ЗагруженаКомпонентаROMAsterisk;
			КонецЕсли; 
		ИначеЕсли ИмяПараметраВР = ВРег("глАК_ВнутреннийНомер") Тогда
			Если НЕ ПолучитьИзКэшаКонфигурации(ИмяПараметраВР, НайденноеЗначение, Кэш, ПоместитьВКэш) Тогда
				НайденноеЗначение = ПараметрыСеанса.АК_ВнутреннийНомер;
			КонецЕсли; 
		ИначеЕсли ИмяПараметраВР = ВРег("глАК_TrunkName") Тогда
			Если НЕ ПолучитьИзКэшаКонфигурации(ИмяПараметраВР, НайденноеЗначение, Кэш, ПоместитьВКэш) Тогда
				НайденноеЗначение = ПараметрыСеанса.АК_TrunkName;
			КонецЕсли;
		//---АК
		//++АК luza 2018.06.07
		ИначеЕсли ИмяПараметраВР = ВРег("глДопПараметрыПользователя") Тогда
			Если НЕ ПолучитьИзКэшаКонфигурации(ИмяПараметраВР, НайденноеЗначение, Кэш, ПоместитьВКэш) Тогда
				НайденноеЗначение = Неопределено;
			КонецЕсли;		
		//-- АК luza 2018.06.07	
		...
		ИначеЕсли ИмяПараметраВР = ВРег("глСоответствиеТекстовЭлектронныхПисем")  
			...
			ИЛИ ИмяПараметраВР = ВРег("ЗначенияДополнительныхПравПользователя") Тогда
		...
		ИначеЕсли ВРег(ИмяПараметраВР) = ВРег("глОбработкаАвтоПолученияОтправкиЭлектронныхПисем") Тогда	
		...
		ИначеЕсли ВРег(ИмяПараметраВР) = ВРег("глОбработкаАвтоОбменДанными") Тогда
			НайденноеЗначение = ПроцедурыОбменаДанными.ВыполнитьИнициализациюАвтообменаПриНачалеРаботы();
			
		ИначеЕсли ВРег(ИмяПараметраВР) = ВРег("КэшСтратегииАвтонумерации") Тогда	
			НайденноеЗначение = Новый Соответствие;		
		#КонецЕсли
		
		ИначеЕсли ВРег(ИмяПараметраВР) = ВРег("ОсновнаяОрганизация") Тогда	
		...
		ИначеЕсли ИмяПараметраВР = ВРег("ДублироватьСообщенияВЖурналРегистрации") Тогда
			Если НЕ ПолучитьИзКэшаКонфигурации(ИмяПараметраВР, НайденноеЗначение, Кэш, ПоместитьВКэш) Тогда
				НайденноеЗначение = Ложь;
			КонецЕсли;
			
		ИначеЕсли ИмяПараметраВР = ВРег("РазрешитьВыводитьСообщенияВПанельКомментариев") Тогда
			Если НЕ ПолучитьИзКэшаКонфигурации(ИмяПараметраВР, НайденноеЗначение, Кэш, ПоместитьВКэш) Тогда
				НайденноеЗначение = Истина;
			КонецЕсли;
		...
		ИначеЕсли ИмяПараметраВР = ВРег("ИспользоватьВерсионированиеОбъектов") Тогда
			Если НЕ ПолучитьИзКэшаКонфигурации(ИмяПараметраВР, НайденноеЗначение, Кэш, ПоместитьВКэш) Тогда
 				НайденноеЗначение = Константы.ИспользоватьВерсионированиеОбъектов.Получить();
			КонецЕсли;
			
		ИначеЕсли ИмяПараметраВР = ВРег("ИспользоватьОграниченияПравДоступаНаУровнеЗаписей") Тогда
			Если НЕ ПолучитьИзКэшаКонфигурации(ИмяПараметраВР, НайденноеЗначение, Кэш, ПоместитьВКэш) Тогда
				НайденноеЗначение = Константы.ИспользоватьОграниченияПравДоступаНаУровнеЗаписей.Получить();
			КонецЕсли;
		...
		ИначеЕсли ВРег(ИмяПараметраВР) = ВРег("ФормаОповещенияОЗадачах") Тогда	
			НайденноеЗначение = Задачи.ЗадачаИсполнителя.ПолучитьФорму("ФормаОповещенияОЗадачах", ,"Оригинал");
		Иначе
...
Функция ПоказыватьВалюту()
	...
	Возврат ОпределитьНаличиеВалютныхПоказателей();
