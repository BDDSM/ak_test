...
Процедура ЗаполнитьРасходПоОснованию(ДокументОбъект, Основание, ПользовательДокумента) Экспорт
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ЗаявкаНаРасходованиеСредств") Тогда
		...
		Если Основание.ФормаОплаты=Перечисления.ФормыОплаты.Наличные Тогда
...
Процедура ЗаполнитьПриходПоОснованию(ДокументОбъект, Основание, ПользовательДокумента) Экспорт
	...
	ИначеЕсли ТипЗнч(Основание)=Тип("ДокументСсылка.ПланируемоеПоступлениеДенежныхСредств") Тогда
		...
		Если (НЕ Основание.ФормаОплаты=Перечисления.ФормыОплаты.Безналичные) 
...
Функция ЕстьРасчетыСКонтрагентами(ВидОперации=Неопределено) Экспорт
	...
	Если ВидОперации=Неопределено Тогда 
		Возврат Ложь;
	Иначе
		Возврат  ВидОперации = Перечисления.ВидыОперацийППИсходящее.ОплатаПоставщику
		...
		ИЛИ ВидОперации = Перечисления.ВидыОперацийППИсходящее.ВозвратПокупателю
		...
		ИЛИ ВидОперации = Перечисления.ВидыОперацийППИсходящее.РасчетыПоКредитамИЗаймам
...
//
// Возвращаемое значение:
//   <ПланСчетовСсылка.Финансовый>	– счет учета денежных средств, соответствующий банковскому счету
//
Функция ОпределитьСчетУчетаДенежныхСредств(БанковскийСчет, ВалютаРеглУчета, РасчетыВВалюте = Ложь) Экспорт
	
	Если НЕ ЗначениеЗаполнено(БанковскийСчет) Тогда
		РасчетыВВалюте = Ложь;
		Возврат ПланыСчетов.Финансовый.РасчетныеСчета;
	КонецЕсли;
	...
	Если ВидБанковскогоСчета = "Расчетный" Тогда
		Если РасчетыВВалюте Тогда
			СчетБУ = ПланыСчетов.Финансовый.ВалютныеСчета;
		Иначе
			СчетБУ = ПланыСчетов.Финансовый.РасчетныеСчета;
		КонецЕсли;
		
	ИначеЕсли ВидБанковскогоСчета = "Депозитный" Тогда
		
		Если РасчетыВВалюте Тогда
			СчетБУ = ПланыСчетов.Финансовый.ДепозитныеСчетаВал;
		Иначе
			СчетБУ = ПланыСчетов.Финансовый.ДепозитныеСчета;
		КонецЕсли;
		
	ИначеЕсли ВидБанковскогоСчета = "Аккредитивный" Тогда
		
		Если РасчетыВВалюте Тогда
			СчетБУ = ПланыСчетов.Финансовый.АккредитивыВал;
		Иначе
			СчетБУ = ПланыСчетов.Финансовый.Аккредитивы;
		КонецЕсли;
		
	Иначе
		
		Если РасчетыВВалюте Тогда
			СчетБУ = ПланыСчетов.Финансовый.ПрочиеСпециальныеСчетаВал;
		Иначе
			СчетБУ = ПланыСчетов.Финансовый.ПрочиеСпециальныеСчета;
		КонецЕсли;
...
Процедура ОпределитьСчетаУчетаПлатежаПоУмолчанию(ДокументОбъект,СтрокаПлатеж,ПерезаписыватьЗначения)
	...
	ИначеЕсли  ПерезаписыватьЗначения Тогда
		СтрокаПлатеж.СчетУчетаРасчетовСКонтрагентом = ПланыСчетов.Финансовый.ПустаяСсылка();
		СтрокаПлатеж.СчетУчетаРасчетовПоАвансам     = ПланыСчетов.Финансовый.ПустаяСсылка();
	КонецЕсли; 	
...
Процедура ЗаполнитьРеквизитыРасчетногоДокумента(ДокументОбъект, ТекПользователь, ВалютаРегламентированногоУчета = Неопределено, 
	...
	Если МетаданныеДокумента.Реквизиты.Найти("ФормаОплаты") <> Неопределено 
		И (НЕ ЗначениеЗаполнено(ДокументОбъект.ФормаОплаты) ИЛИ ДокументОбъект.ФормаОплаты=Перечисления.ФормыОплаты.Наличные) Тогда
		...
		Если НЕ ЗначениеЗаполнено(ДокументОбъект.ФормаОплаты) Тогда
			ДокументОбъект.ФормаОплаты=Перечисления.ФормыОплаты.Наличные;
		КонецЕсли;
	...
	Если МетаданныеДокумента.Реквизиты.Найти("Статус") <> Неопределено И (НЕ ЗначениеЗаполнено(ДокументОбъект.Состояние))Тогда
		ДокументОбъект.Статус = Перечисления.СтатусыЗаявокНаРасходованиеСредств.Новая;
	КонецЕсли;
...
Функция ПроверитьСогласованиеЗаявок(ПроверяемыеЗаявки, Отказ = Ложь, Заголовок = "") Экспорт
	...
	Для Каждого ТекЗаявка ИЗ МассивЗаявок Цикл
		...
		Если НЕ Выборка.НайтиСледующий(Новый Структура("Заявка", ТекЗаявка)) Тогда
			...
			Продолжить;
		КонецЕсли;
		Если Выборка.Состояние <> Перечисления.СтатусыЗаявокНаРасходованиеСредств.Акцептована Тогда
			ЗаявкаСогласована = Ложь;
			ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru = '""" + СокрЛП(ТекЗаявка) + """ не акцептована. Ввод платежа по заявке невозможен'"), Отказ, Заголовок);
			Продолжить;
		КонецЕсли;
	КонецЦикла;
...
Функция РазрешеноИзменениеЗаявки(ДокументСсылка, ТекстСообщенияПользователю = "") Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Выборка.Состояние) ИЛИ Выборка.Состояние = Перечисления.СтатусыЗаявокНаРасходованиеСредств.Новая Тогда
		Возврат Истина;
	//Заявка уже согласована к оплате
	ИначеЕсли Выборка.Состояние = Перечисления.СтатусыЗаявокНаРасходованиеСредств.Акцептована Тогда
		ТекстСообщенияПользователю = НСтр("ru = 'Акцептованная заявка не может быть изменена.'");
		Возврат Ложь;
	Иначе
		...
		Если Выборка.Пользователь = ТекущийПользователь И 
			(Выборка.Состояние = Перечисления.СтатусыЗаявокНаРасходованиеСредств.Новая ИЛИ //Отклонен
			Выборка.Состояние = Перечисления.СтатусыЗаявокНаРасходованиеСредств.Отложен) Тогда //Отложен
			Возврат Истина;
		//Заявка перешла на согласование текущему пользователю
		ИначеЕсли Выборка.Состояние = Перечисления.СтатусыЗаявокНаРасходованиеСредств.Акцептована И 
...
//+++АК MIND 2017.10.26 доработал процедуру получения расчетного счета с учетом настройки внутригрупповой для контрагента
//+++АК SUVV 2018.02.28 ИП-00017941
//Функция ПолучитьСчетРасчетовСКонтрагентом(СтатьяДДС, Контрагент = Неопределено, ТипОперации = "") Экспорт
Функция ПолучитьСчетРасчетовСКонтрагентом(ВыбДата, СтатьяДДС, Контрагент = Неопределено, ТипОперации = "") Экспорт
//---АК SUVV
Если ЗначениеЗаполнено(Контрагент)
	И ТипЗнч(Контрагент) = Тип("СправочникСсылка.Контрагенты")
		И Контрагент.ИспользоватьВГСчета Тогда
		МассивСчетов = Новый Массив();
		Если ТипОперации = "Реализация" Тогда
			МассивСчетов.Добавить("62.ВГ");
		Иначе	
			МассивСчетов.Добавить("60.ВГ");
		КонецЕсли;	
	Иначе	
		Запрос = Новый Запрос;
		//+++АК SUVV 2018.02.20 ИП-00017941
		//Запрос.Текст = "ВЫБРАТЬ
		//               |	СоответствияСтатейСчетов.Счет.Код КАК Код
		//               |ИЗ
		...
		МассивСчетов = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Код");
	КонецЕсли;	
