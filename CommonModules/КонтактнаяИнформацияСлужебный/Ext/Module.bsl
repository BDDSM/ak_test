...
//
Функция ПредставлениеНаселенногоПункта(ОбъектАдреса, ТолькоУровниКЛАДР = Истина) Экспорт
...
Функция ОшибкиЗаполненияАдреса(Знач XDTOАдрес, Знач ВидИнформации = Неопределено, Знач РезультатГруппами = Ложь) Экспорт
	...
	Если Не XDTOКонтактнаяИнформацияЗаполнена(АдресРФ) Тогда
		// Адрес пуст
		Если ФлагиПроверки.Свойство("ОбязательноеЗаполнение") И ФлагиПроверки.ОбязательноеЗаполнение Тогда
...
Функция СписокРеквизитовНаселенныйПунктКЛАДР(Код = Неопределено)
	...
	Результат.Вставить("Округ",          ЭлементАдреснойСтруктуры(НСтр("ru = 'Округ'"), "Округ",,, Истина) );
	...
	Результат.Вставить("ВнутригРайон",          ЭлементАдреснойСтруктуры(НСтр("ru = 'Внутр. р-н.'"), "ВнутригРайон",,, Истина));
	...
	Результат.Округ.Вставить("Подсказка",          НСтр("ru = 'Округ адреса (только просмотр)'"));
	Результат.Район.Вставить("Подсказка",           НСтр("ru = 'Район адреса'"));
	Результат.Город.Вставить("Подсказка",           НСтр("ru = 'Город адреса'"));
	Результат.ВнутригРайон.Вставить("Подсказка",          НСтр("ru = 'Внутригородской район адреса (только просмотр)'"));
...
Функция ЭлементАдреснойСтруктуры(Заголовок, ПутьXPath, Значение = Неопределено, Предопределенный = Ложь, ТолькоПросмотр = Ложь)
	СтруктураАдреса = Новый Структура();
	СтруктураАдреса.Вставить("Заголовок", Заголовок);
	СтруктураАдреса.Вставить("ПутьXPath", ПутьXPath);
	СтруктураАдреса.Вставить("Значение", Значение);
	СтруктураАдреса.Вставить("Предопределенный", Предопределенный);
	СтруктураАдреса.Вставить("Наименование");
	СтруктураАдреса.Вставить("Сокращение");
	СтруктураАдреса.Вставить("КодКлассификатора");
	СтруктураАдреса.Вставить("ТолькоПросмотр", ТолькоПросмотр);
...
Функция ДесериализацияАдресаПоПредставлениюКЛАДР(Знач Представление)
	...
	НаименованиеРоссии = СокрЛП(Справочники.КлассификаторСтранМира.Россия.Наименование);
	
	Классификатор = Справочники.КлассификаторСтранМира.ТаблицаКлассификатора();
