...
Функция ПроверкаПрограмногоВыводаПозицииИзАссортиментаАвто(ТорговаяТочка, Номенклатура, Интерактивно = Ложь, ТекстСообщенния = "") Экспорт
	...
	Если ЗначениеЗаполнено(НомерТТСтрока) И ЗначениеЗаполнено(ID_tovСтрока) Тогда
		...
		Если ЗначениеЗаполнено(СтрокаУведомления) Тогда
			...
			Если ТекстСообщенния <> "БезУведомлений" Тогда
...
// Формирует комментарий для записей Товарного ассортимента без контроля по автоформированию
//
// Параметры:
//  ТекущийКомментарий  - <Тип.Строка> - Текущий комментарий записи
//  ИмяПроцедуры  - <Тип.Строка> - Имя процедуры с помощь которой товар был выведен из ассортимента в обход запрета 
//
// Возвращаемое значение:
//   <Тип.Строка>   - Новый комментарий
//
Функция ПолучитьКомментарийОбходаКонтроляАвтовывода(ТекущийКомментарий, ИмяПроцедуры = "") Экспорт //+++АК mika 2018.12.14 ИП-00020392.02
	
	ТэгБезАвто = Найти(ТекущийКомментарий, "#БезАвто");
	Если ТэгБезАвто > 0 Тогда
		ТекущийКомментарий = СокрЛП(Сред(ТекущийКомментарий, Найти(ТекущийКомментарий, "#БезАвто")+ 9, СтрДлина(ТекущийКомментарий)));
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ИмяПроцедуры) Тогда
		ТекущийКомментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1 #БезАвто %2", ИмяПроцедуры, ТекущийКомментарий);
	КонецЕсли;
