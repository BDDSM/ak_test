...
Процедура ПредставлениеИсточникаЗагрузкиОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Оповещение = Новый ОписаниеОповещения("ЗавершениеИзмененияИсточникаЗагрузки", ЭтаФорма);
...
Процедура АдресЗагрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	АдресныйКлассификаторКлиент.ВыбратьКаталог(ЭтаФорма, "АдресЗагрузки", 
...
Процедура КонтрольРасширенияРаботыСФайлами(Знач ТекстПредложения, Знач КодИсточника, Знач АдресИсточника)
	...
	Оповещение = Новый ОписаниеОповещения("КонтрольРасширенияРаботыСФайламиЗавершение", ЭтаФорма, Новый Структура);
...
Процедура ЗагрузитьКлассификаторИзКаталогаИТС(Знач КорневойКаталог)
	...
	Если ЕстьОшибки Тогда
		...
		Сообщить(НСтр("ru = 'В указанном каталоге 1С:ИТС не найдены файлы для загрузки'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗагрузитьКлассификаторССайта(Знач Аутентификация = Неопределено)
	...
	Если ПустаяСтрока(Аутентификация.Логин) Тогда
		...
		Оповещение = Новый ОписаниеОповещения("ЗагрузитьКлассификаторССайтаЗапросАутентификации", ЭтаФорма, Новый Структура);
		Оповещение.ДополнительныеПараметры.Вставить("КодыРегионов", КодыРегионов);
		
		ОткрытьФорму("РегистрСведений.АдресныйКлассификатор.Форма.АвторизацияНаПользовательскомСайте", , ЭтаФорма, , , ,Оповещение);
		Возврат;
	КонецЕсли;
...
Процедура ЗагрузитьКлассификаторССайтаКЛАДР()
	...
	Если Не Результат.Статус Тогда
		...
		Если Найти(Результат.СообщениеОбОшибке, "401 Unauthorized") > 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Не удалось получить доступ к сайту 1C для загрузки файлов КЛАДР, возможно неверное имя пользователя или пароль.'"));
		Иначе
...
Процедура Подключаемый_ОжиданиеДлительнойОперации()
	...
	ИначеЕсли Состояние.Завершено Тогда
		...
		Оповестить("ЗагруженАдресныйКлассификатор", , ЭтаФорма);
