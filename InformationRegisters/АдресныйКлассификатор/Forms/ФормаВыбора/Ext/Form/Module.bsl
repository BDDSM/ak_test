...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Если адрес содержит дополнительную территорию которая была получена из ФИАС
	Если Параметры.Свойство("ДополнительнаяТерритория") И ЗначениеЗаполнено(Параметры.ДополнительнаяТерритория) Тогда
		Элементы.ДополнительнаяТерритория.Видимость = Истина;
		ДополнительнаяТерритория = Параметры.ДополнительнаяТерритория;
	КонецЕсли;
...
Процедура ОсуществитьВыбор()
	...
	ИначеЕсли ТекущиеДанные.ПризнакАктуальности<>0 Тогда
		...
		Оповещение = Новый ОписаниеОповещения("ОсуществитьВыборЗавершение", ЭтаФорма, ДополнительныеПараметры);
