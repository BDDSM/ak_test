...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если СвязанныеТоварыМассивСтруктур.Количество() > 0 Тогда
		...
		// Если есть номенклатура для переноса и пользователь выбрал не переносить
		// то отменим перенос и текущей номенклатуры
		Если НЕ ПеренестиСвязанныеТовары Тогда
			Отказ = Истина;
		КонецЕсли;
