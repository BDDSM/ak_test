...
Процедура ПередЗаписью(Отказ, Замещение)
	...
	Если СтруктураОтбор.Количество() > 0 Тогда
		Запрос = Новый Запрос();
		ТекстЗапроса = "ВЫБРАТЬ * ИЗ РегистрСведений." + ЭтотОбъект.Метаданные().Имя + " КАК Таб
					|ГДЕ
					|	ИСТИНА";
...
Процедура ПриЗаписи(Отказ, Замещение)
	...
	Если НЕ ЭтотОбъект.ОбменДанными.Загрузка И ТабДвижения.Количество() > 0 Тогда
	//---АК mika
		Запрос = Новый Запрос;   
