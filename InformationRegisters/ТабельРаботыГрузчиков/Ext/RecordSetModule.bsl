...
Процедура ПриЗаписи(Отказ, Замещение)
	...
	Если ТабДвижения.Количество() > 0 Тогда
		...
		|ВЫБРАТЬ
		|	ТабельРаботыПекарей.Период,
		|	ТабельРаботыПекарей.Сотрудник,
		|	""Пекари""
		|ИЗ
		|	ВТ_Данные КАК ВТ_Данные
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ТабельРаботыПекарей КАК ТабельРаботыПекарей
		|		ПО ВТ_Данные.Период = ТабельРаботыПекарей.Период
		|			И ВТ_Данные.Сотрудник = ТабельРаботыПекарей.Сотрудник
		|			И (НЕ ТабельРаботыПекарей.Отсутствие В (&ВидыОтсутствия))
		|
		|ОБЪЕДИНИТЬ ВСЕ
		//---АК mika ИП-00018000
		|
